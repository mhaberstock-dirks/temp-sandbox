  ...
  CREATE OR REPLACE EDITIONABLE TRIGGER "DB_LVS_LAM" 
  after insert or update or delete on lvs_lam
  for each row
declare
...
begin
  ...
  if db_p_trace.get_trigger_aktiv('lvs_lam') = 0
  then
    return;
  end if;
  ...
  if inserting
  then
    v_pk_values := :new.firma_nr|| ';' || :new.lam_id|| ';' || :new.sid;
    v_command := 'INSERT';
  ...
  then
    v_pk_values := :old.firma_nr|| ';' || :old.lam_id|| ';' || :old.sid;
    v_command := 'DELETE';
    -- v_info := 'vorgang_id=' || :old.vorgang_id || ';';
  end if;

  -- Autonome Transaktion
  db_p_trace.c_db_act_log('lvs_lam',
                        'FIRMA_NR;LAM_ID;SID',
                        v_pk_values,
                        v_command,
                        v_info);
...
/
ALTER TRIGGER "DB_LVS_LAM" ENABLE;


-- sqlcl_snapshot {"hash":"d16c159dd01558c1121c17926f45f6e87db9be8c","type":"TRIGGER","name":"DB_LVS_LAM","schemaName":"DIRKSPZM32","sxml":""}