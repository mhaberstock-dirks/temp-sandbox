create or replace package body dirkspzm32.isi_migration is
  ...
  *******************************************************************************/
    ...
    function c_update_action_info (
        in_action_old_name in varchar2,
        in_action_new_name in varchar2
    ) return varchar2 is
        ...
        from
            sec_action_info
        where
            name = in_action_old_name;

    begin
        open c_action_info_new;
        fetch c_action_info_new into v_action_info_new;
        v_found_new := c_action_info_new%found;
        open c_action_info_old;
        fetch c_action_info_old into v_action_info_old;
  ...
  *******************************************************************************/
    ...
    begin
        ...
        update isi_firma t
        set
            t.lte_barcode_type = 'STD'
        where
            t.lte_barcode_type = 'Standard';

        update isi_firma t
        set
            t.lhm_barcode_type = 'STD'
        where
            t.lhm_barcode_type = 'Standard';
    ...
    -- Anpassung  LTE_SEC_ACTION_INFO     old           new
        ...
        set
            l.lang_key = 'DE'
        where
            l.lang_key = 'lang_name_german';

        update isi_language l
        set
            l.lang_key = 'EN'
        where
            l.lang_key = 'lang_name_english';

        update isi_language l
        set
            l.lang_key = 'NL'
        where
            l.lang_key = 'lang_name_dutch';
...
-- sqlcl_snapshot {"hash":"ae70322a4b86d758cd00c39e132d17945d97c688","type":"PACKAGE_BODY","name":"ISI_MIGRATION","schemaName":"DIRKSPZM32","sxml":""}