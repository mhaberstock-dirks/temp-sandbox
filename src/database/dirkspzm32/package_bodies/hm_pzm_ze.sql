create or replace package body dirkspzm32.hm_pzm_ze is
...
*/
  ...
  -- Konstanten
        ...
        cursor c_zetag (
            cp_pers_nr pzm_zeiterfassung.ze_pers_nr%type
        ) is
        select
            ze_id,
            ze_ist_start,
  ...
  close c_ZETag_All;
  */
  -- Alle Aggregationen aus pzm_zeiterfassung in einem Schritt
        select
      -- erste nicht-null Kostenstelle (geordnet)
            min(ze_kst_id) keep(dense_rank first order by
                case
                    when ze_kst_id is not null then
                        0
                    else
                        1
                end, ze_calc_ist_start, ze_id),
      -- std des Halbtagsurlaub-Eintrags
            max(
                case
                    when ze_status = status_abwesend
                         and ze_aa_status = status_urlaub_halb then
                        ze_std
                end
            )
        into
        ...
        v_kst_id := nvl(v_kst_id_ze, v_kst_id);

  -- UPDATE direkt über PK — kein Cursor nötig
        update pzm_ze_tagessatz
        set
            ts_day_kst_id = v_kst_id
        where
                ts_pers_nr = p_pers_nr
            and ts_datum = v_schicht_datum;

        v_found := false;
        v_kenz_urlaub := false;
        open c_zetag(p_pers_nr);
        loop
    -- Eintraege aus der Zeiterfassung holen
            fetch c_zetag into
                v_id,
      ...
      -- Pausenzeiten unter Beruecksichtigung der gestempelten Pausen berechnen
        ...
        --v_DayPauseStd := 0;                                                                   -- Pausezeit dann 0
            ...
            -- Abweseneheitsdifferenz bei Gleitzeit hinten dran schreiben
                            if not pzm_p_schicht_tag.c_schicht_tag_fehlzeit_luecken_pruefen(p_pers_nr, v_schicht_datum, v_daysakurzname
...
-- sqlcl_snapshot {"hash":"d701f6c66326456adf02c3cfb97e20bead3f6bfd","type":"PACKAGE_BODY","name":"HM_PZM_ZE","schemaName":"DIRKSPZM32","sxml":""}