...
package body DIRKSPZM32.MELD is
  ...
  procedure c_alle_meldungen_gehen(in_sid in varchar2,
                                   ...
                                   in_engine_id in number default null) is
  begin
  ...
  begin
    meldung_buchen(in_sid, in_firma_nr, in_gruppe, in_md_ix, in_bereich, in_status, in_hilfstext, in_engine_id, null, v_md_id);
    commit;
  end;
  ...
  procedure c_meldung_buchen_47x(in_sid in varchar2,
                                 ...
                                 in_typ in varchar2) is
    v_md_id number;

  begin
    meldung_buchen_47x(in_sid, in_firma_nr, in_gruppe, in_md_ix, in_bereich, in_status, in_hilfstext, in_engine_id, null, in_param_1, in_param_2, in_typ, v_md_id);
    commit;
  end;
  ...
  procedure c_meldung_buchen_ausloes(in_sid in varchar2,
                                     ...
                                     out_md_id out number) is
  begin
    meldung_buchen(in_sid, in_firma_nr, in_gruppe, in_md_ix, in_bereich, in_status, in_hilfstext, in_engine_id, in_md_ausloes_md_id, out_md_id);
    commit;
  end;
  ...
  begin
    out_md_id := null;
    if in_status in ('K','KG')
    then
      insert into meldung_daten
      values (in_sid,
              in_firma_nr,
              in_engine_id,
              null, /* md_id */
      ...
      returning md_id into out_md_id;

    else
      update meldung_daten t
         set t.md_geht = sysdate,
             t.md_status = 'KG'
       where sid = in_sid
         and firma_nr = in_firma_nr
         and engine_id = in_engine_id
  ...
  procedure meldung_statistik_gen(in_von_datum     in meldung_daten.md_kommt%type,
                                  ...
                                  is
  begin
    meldung_statistik_gen_31(in_von_datum,
                             in_bis_datum,
                             null);
  end;
  ...
  procedure meldung_statistik_gen_31(in_von_datum     in meldung_daten.md_kommt%type,
    ...
    CURSOR c_bereiche is
      select *
        from meldung_cfg t
      order by t.engine_id,
               t.nr;

    CURSOR c_meldungen is
      select *
        from meldung_daten t
       where t.md_bereich = v_bereich.name
    ...
    CURSOR c_meldungen_texte is
      select t.*
        from meldung_texte t
       where t.mt_gruppe = v_meldungen.md_gruppe
  ...
  begin
    ...
    */
    delete meldung_statistik t
     where t.ms_begin >= trunc(in_von_datum)
       and t.ms_begin <= trunc(in_bis_datum) + 1;

    if in_sprach_id is not null
    then
      v_mt_sprache := in_sprach_id;
    else
      if isi_p_base.get_isi_firma('01',
                                  1,
                                  v_firma)
      then
    ...
    LOOP   -- Über alle Bereiche
      ...
      LOOP
        ...
        or v_stat_daten.ms_begin < v_meldungen.md_kommt
        then
          v_stat_daten.ms_begin := v_meldungen.md_kommt;
        end if;

        if v_stat_daten.ms_ende is NULL
        or v_stat_daten.ms_ende < v_meldungen.md_geht
        then
        ...
        if v_meldungen_texte.mt_fehlertext not like nvl(v_stat_daten.ms_texte, 'XXXX')
        then
          if v_stat_daten.ms_texte is NULL
          then
            v_stat_daten.ms_texte := v_meldungen_texte.mt_fehlertext;
          else
            v_stat_daten.ms_texte := substr(v_stat_daten.ms_texte || ', ' || v_meldungen_texte.mt_fehlertext, 1, 4000);
          end if;
        end if;
      end LOOP;
      CLOSE c_meldungen;
      if v_stat_daten.sid is not NULL
      then
        v_stat_daten.ms_minuten := round((v_stat_daten.ms_ende - v_stat_daten.ms_begin) * 1440, 2);
        begin
          insert into meldung_statistik
