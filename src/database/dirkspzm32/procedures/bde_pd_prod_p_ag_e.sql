...
(in_sid         in isi_sid.sid%type,
 ...
 in_ls_login_id in isi_user.login_id%type) is
  ...
  SELECT sum(menge_a),
    ...
    WHERE pd_p_all.sid = in_sid and
          pd_p_all.res_id = in_res_id and
          pd_p_all.vorg_typ = 'PA' and
          pd_p_all.leitzahl = v_res_zus.leitzahl and
          pd_p_all.fa_ag = v_res_zus.fa_ag and
          nvl(pd_p_all.fa_upos, 0) = nvl(v_res_zus.fa_upos, 0) and
          pd_p_all.res_id = in_res_id and
          pd_p_all.prod_beginn >= v_res_zus.akt_aufgabe_seit and
          pd_p_all.prod_beginn <= v_sysdate
    GROUP BY pd_p_all.leitzahl;

  -- Lesen des aktuellen PA Eintrags
  CURSOR c_bde_pd_prod IS
  SELECT nvl(menge_a, 0),
         nvl(menge_a, 0) + nvl(menge_b, 0)
    FROM bde_pd_prod pd_p
    WHERE pd_p.sid = in_sid and
          pd_p.firma_nr = in_firma_nr and
          pd_p.leitzahl = v_res_zus.leitzahl and
          pd_p.fa_ag = v_res_zus.fa_ag and
          nvl(pd_p.fa_upos, 0) = nvl(v_res_zus.fa_upos, 0) and
          pd_p.res_id = in_res_id and
          pd_p.vorg_typ = 'PA' and
          pd_p.prod_ende is NULL;

  -- Lesen der aktuellen Transporteinheit, Prüfen ob Material auf der Palette
  CURSOR c_lte IS
  SELECT *
    FROM lvs_lte
    WHERE lte_id = v_res_zus.lte_id;
...
begin
  ...
  if not v_found then
    ...
    if in_leitzahl = v_res_zus.leitzahl and
       in_fa_ag = v_res_zus.fa_ag and
       v_res_zus.akt_aufgabe = 'P' and
       nvl(in_fa_upos,0) = nvl(v_res_zus.fa_upos, 0) then
      if nvl(in_menge_a, 0) = 0 and
         nvl(in_menge_b, 0) = 0 and
         nvl(in_schrott, 0) = 0 then
        OPEN c_bde_pd_prod_all;
        FETCH c_bde_pd_prod_all into v_menge_a, v_menge_b, v_schrott;
      ...
      FETCH c_bde_pd_prod into v_menge, v_menge_ab;
  ...
  then
    if ((nvl(v_fa_auftrag.ag_ist_mg, 0) + nvl(v_menge_a, 0) - v_menge >= nvl(v_fa_auftrag.ag_soll_mg, 0)
    or  ((nvl(v_fa_auftrag.ag_ist_mg, 0) + nvl(v_fa_auftrag.ag_ist_mg_b, 0)+ nvl(v_menge_a, 0)  + nvl(v_menge_b, 0) - v_menge_ab >= nvl(v_fa_auftrag.ag_soll_mg, 0)
  ...
  commit;
  select sum(t.schrott) into v_schrott
    from bde_pd_prod t
  ...
  select sum(t.schrott + t.menge_a + t.menge_b) into v_ruest
    from bde_pd_prod t
   where t.leitzahl = in_leitzahl
     and t.fa_ag = in_fa_ag
     and t.fa_upos = in_fa_upos
  ...
  v_prod_zeit := bde_funktionen.get_fa_zeiten_upos(in_sid, in_firma_nr, in_leitzahl, in_fa_ag, in_fa_upos, NULL);
  ...
  v_prod_std_erf := bde_funktionen.get_ma_erf_zeiten(v_res_zus.sid, v_res_zus.firma_nr, v_res_zus.res_id, v_res_zus.akt_aufgabe_seit, sysdate) * 60;
  ...
  then
    ...
    update bde_pd_kopf_ma t
       set t.pd_kopf_ende = sysdate
     where t.sid       = in_sid
       and t.firma_nr  = in_firma_nr
       and t.res_id    = in_res_id
       and t.pd_kopf_ende is NULL
       and t.pers_nr not in (select k.pers_nr
                               from BDE_PD_KOPF k
                              where k.res_id = in_res_id
