create or replace package body dirkspzm32.bde_tms_push is
...
*/
    ...
    procedure c_create_tms_push_transport (
        in_sid      in isi_sid.sid%type,
        in_firma_nr in isi_firma.firma_nr%type,
        in_ice      in varchar2
    ) is
    begin
        bde_tms_push.c_crt_tms_push_transport_db31(
            in_sid      => in_sid,
  ...
  --      Zus. wird der FA-AG ggf. fertig gemeldet und an der Reasource abgemeldet.
    ...
    --                            wenn Menge nicht auf bereits reservierte LAM's dazu gebucht werden sollen  
        ...
        where
            ...
            and ( t.order_pos_auf_id is null
                  or v_chk_on_res_lam = 'T' )
        order by
            decode(t.order_pos_auf_id, null, 0, v_bde_fa.auf_id, 1,
                   2);
        ...
        from
            lvs_charge t
        where
            t.charge_id = v_lam_lte_lgr.charge_id;
    ...
    begin
        v_chk_on_res_lam := isi_allg.get_firma_cfg_param(
            in_sid                => in_sid,
            in_firma_nr           => in_firma_nr,
            in_kategorie          => 'CFG',
        ...
        v_bde_fa := null;
        v_result := c.c_true;
        if not bde_p_base.get_fa_ag(in_sid, in_firma_nr, in_leitzahl, in_fa_ag, in_fa_upos,
                                    v_bde_fa) then
      ...
      --                            wenn Menge nicht auf bereits reservierte LAM's dazu gebucht werden sollen 
          ...
          -- AG 20170308 Neue LAM dafür anlegen mit Menge 0
                    ...
                    if
                        v_lam_lte_lgr.hersteller_kuerzel_liste is not null
                        and v_lam_lte_lgr.hersteller_kuerzel_liste != ';'
                        and isi_p_base.get_artikel_ctrl_typ(v_lam_lte_lgr.sid,
                                                            v_lam_lte_lgr.artikel_id,
                                                            substr(v_lam_lte_lgr.hersteller_kuerzel_liste,
                                                                   1,
                                                                   length(v_lam_lte_lgr.hersteller_kuerzel_liste) - 1),
                                                            v_art_ctrl)
                    then
                    ...
                    v_lhm_id := lvs_p_lte.lvs_lte_lhm_next_id_v35(v_lam_lte_lgr.sid, v_lam_lte_lgr.firma_nr, c.basis_lhm, v_charge.charge_bez
                    , v_lam_lte_lgr.artikel_id,
          ...
          --                           verwendet. Sonst werden die Daten aus der LHM CFG geladen und eingetragen
                    ...
                    if ( v_found ) then
                        ...
                        close c_lhm_cfg;
                        v_lhm_vol := nvl(v_lhm_cfg.lhm_vol_hoehe, 0) * nvl(v_lhm_cfg.lhm_vol_breite, 0) * nvl(v_lhm_cfg.lhm_vol_tiefe
                        , 0) / 1000000000;
                    ...
                    insert into lvs_lam values ( v_lam_lte_lgr.sid,                 -- SID                 VARCHAR2(2) not null,
        ...
        if in_menge != 0 then
            ...
            loop
                ...
                or v_menge_rest = 0;
                if
                    v_lam.lgr_platz is not null
                    and in_menge > 0
                then
                    if
                        bde_p_base.get_fa_ag(in_sid, in_firma_nr, in_leitzahl, in_fa_ag, in_fa_upos,
                                             v_bde_fa)
                    ...
                    then
                        ...
                        begin
                            bde_p_base.c_bde_pd_set_deaktiv_ag(in_sid, in_firma_nr, in_res_id, null, in_login_id);
                        exception
                            when others then
                                null; -- Wenn fehler, dann bleit der Auftrag angemeldet
                        end;
                    ...
                    insert into lvs_lam_bh t values ( v_lam.sid,                                        -- SID               VARCHAR2(2) not null,
                                                      ...
                                                      nvl(v_lam_lte_lgr.bh_lam_bh_kg, v_lam.lam_kg) / nvl(v_lam_lte_lgr.bh_menge, v_lam.menge
                    ...
                    if v_menge_rest > v_lam.menge then
                    ...
                    insert into lvs_lam_bh t values ( v_lam.sid,                   -- SID               VARCHAR2(2) not null,
                                                      ...
                                                      v_lam.lam_kg / v_lam.menge * ( v_menge ),                 -- LAM_BH_KG         NUMBER,
                    ...
                    if v_lam.order_pos_auf_id is not null then
                        update lvs_lam t
                        set
                            t.res_menge = t.res_menge + v_menge
                        where
            ...
            -- FA grade rücken
                        if v_lam.res_menge + v_menge = 0                           -- Reservierung ist raus
                         then
                            v_result_res := lvs_ausl.lvs_lte_res_rueck(in_sid, in_firma_nr, v_bde_fa.leitzahl, v_bde_fa.auf_id, v_lam.lte_id
                            ,
                        ...
                        if
                            bde_p_base.get_fa_by_auf_id(in_sid,                    -- FA Einlesen
                             in_firma_nr, v_lam.order_pos_auf_id, v_bde_fa)
                            and v_bde_fa.ma_reserviert = c.c_true
                        ...
                        then
                            ...
                            set
                                t.ma_res_menge = t.ma_res_menge + v_menge
                            where
                                    t.sid = in_sid
                                and t.firma_nr = in_firma_nr
                                and t.leitzahl = v_bde_fa.leitzahl
                            ...
                            elsif v_bde_fa.ma_res_charge_id is not null then
                            ...
                            pps_p_bde.create_bde_fa_dispo(in_sid,                     -- Disponieren nach FA-Vorgabe
                             ...
                             in_firma_nr, v_dispo_charge_rein, v_bde_fa.ma_hersteller_kuerzel_liste, v_bde_fa.leitzahl,
                                                          v_bde_fa.fa_ag, v_bde_fa.fa_upos, 'F',                       -- Nachreservieren was geht
                                                           -1, c.c_true);                 -- in_ice -> express-Platz
                            v_bde_faktor_reduzieren := v_bde_fa.ag_soll_mg;
                            if
                                v_bde_fa.ma_res_menge = v_bde_fa.ag_soll_mg          -- War komplett reserviert,
                                and bde_p_base.get_fa_by_auf_id(in_sid, in_firma_nr, v_lam.order_pos_auf_id, v_bde_fa)
                            ...
                            then
                                ...
                                if v_bde_faktor_reduzieren != 0 then
                                    v_bde_faktor_reduzieren := v_bde_fa.ma_res_menge / v_bde_faktor_reduzieren;
                                end if;
                                update bde_fa_auftrag t      -- Genau diesen reduzieren
                                set
                                    t.ag_soll_mg = t.ma_res_menge,
                                    t.freig_status = decode(t.ma_res_menge_komm, t.ma_res_menge, 'F', t.freig_status)
                                where
                                        t.sid = in_sid
                                    and t.firma_nr = in_firma_nr
                                    and t.leitzahl = v_bde_fa.leitzahl
                                ...
                                update bde_fa_auftrag t      -- Wegen der Fehlmenge auch den Rest reduzieren
                                set
                                    t.ag_soll_mg = round((t.ag_soll_mg * v_bde_faktor_reduzieren), 0)
                                where
                                        t.sid = in_sid
                                    and t.firma_nr = in_firma_nr
                                    and t.leitzahl = v_bde_fa.leitzahl
    ...
    begin
        v_bde_fa := null;
        if bde_p_base.get_fa_ag(in_sid, in_firma_nr, in_leitzahl, in_fa_ag, in_fa_upos,
                                v_bde_fa) then
            v_in_menge := in_menge;
            open c_lam;
            loop
                ...
                exit when c_lam%notfound
                or v_in_menge = 0;
                if v_lam.res_menge + v_in_menge >= 0 then
                ...
                update lvs_lam t
                set
                    t.res_menge = t.res_menge + v_b_menge
                where
                    t.lam_id = v_lam.lam_id;

                if
                    v_lam.res_menge + v_b_menge = 0  -- Nicht mehr reserviert
                    and v_in_menge = 0
                then
                    v_result_res := lvs_ausl.lvs_lte_res_rueck(in_sid, in_firma_nr, v_bde_fa.leitzahl, v_bde_fa.auf_id, v_lam.lte_id,
                                                               v_bde_fa.leitzahl, v_bde_fa.auf_id, c.c_true);
            ...
            update bde_fa_auftrag t                                   -- Reservierte Menge korrigieren
            set
                t.ma_res_menge = t.ma_res_menge + in_menge
            where
                    t.sid = in_sid
                and t.firma_nr = in_firma_nr
                and t.leitzahl = v_bde_fa.leitzahl
    ...
    procedure c_crt_tms_push_transport_db31 (
        in_sid      in isi_sid.sid%type,
        in_firma_nr in isi_firma.firma_nr%type,
        in_ice      in varchar2,
        in_leitzahl in bde_fa_auftrag.leitzahl%type
    ...
    begin
        bde_tms_push.crt_tms_push_transport_db31(
            in_sid      => in_sid,
            in_firma_nr => in_firma_nr,
            in_ice      => in_ice,
    ...
    procedure crt_tms_push_transport_db31 (
        in_sid      in isi_sid.sid%type,
        in_firma_nr in isi_firma.firma_nr%type,
        in_ice      in varchar2,
        in_leitzahl in bde_fa_auftrag.leitzahl%type
  ...
  -- AG 2017.10.10 BugFix
        ...
        select
            ...
            lvs_ausl.lvs_lte_platz_bewerten(in_sid,
                                            ...
                                            lgr.lgr_platz_gruppe,
                                            lgr.lgr_typ,
                                            t.artikel_id)   ausl_sort2
        from
            lvs_lam     t,                                     -- In der LAM ist die resservierng
        ...
        where
            ...
            and not exists (
                select
                    lgr_grp.lgr_platz -- Keine LTE transportieren die auf einem Maschinenplat steht
                from
                    lvs_lgr      lgr,
                    isi_resource r,
                    lvs_lgr      lgr_grp
                where
                    ( lgr.lgr_platz = r.lager_roh
    ...
    begin
        ...
        v_bde_push_q_chk := isi_allg.get_firma_cfg_param(in_sid, in_firma_nr, 'AUSL_BDE_PUSH', 'TRANSPORT', 'BDE_AUSL_QPLATZ_PUSH_SET'
        ,
                                                         'BDE', 'CFG', 'F', 'BOOLEAN');

        v_check_z_durchl := isi_allg.get_firma_cfg_param(in_sid, in_firma_nr, 'AUSL_BDE_PUSH', 'TRANSPORT', 'BDE_AUSL_CHK_Z_DURCHL',
                                                         'BDE', 'CFG', 'T', 'BOOLEAN');
        ...
        loop
            ...
            or v_bde_fak.soll_betriebsart = 'EXPRESS' then
      ...
      --and v_bde_fa.ma_res_menge > 0                   -- Es wurde bereits etwas Reaserviert (Fehlmenge)
             then
                if v_bde_fa.ma_hersteller_kuerzel_liste is not null then
                    v_dispo_charge_rein := 'H';
                elsif v_bde_fa.ma_res_charge_id is not null then
                    v_dispo_charge_rein := c.c_true;
                else
                ...
                pps_p_bde.create_bde_fa_dispo(in_sid, in_firma_nr, v_dispo_charge_rein, v_bde_fa.ma_hersteller_kuerzel_liste, v_bde_fa.leitzahl
                ,
                                              v_bde_fa.fa_ag, v_bde_fa.fa_upos, 'F',                       -- War nicht komplett reserviert, dann Nachreservieren was geht
                                               -1, c.c_false);                -- in ICE
            end if;
          ...
          -- CMe 20220706: Prüfung der Transporte und Transportziele
                    if ( v_check_z_durchl = 'T' ) then
                        ...
                        loop
                            ...
                            if ( v_chk_sum_target > 0 )
                            or ( v_chk_sum_trans_to_target > 0 ) then
               ...
               -- AG 2017.10.10 BugFix
                            ...
                            if not lvs_p_base.get_lvs_lgr_ort_ue_platz(v_lgr_grp.sid, v_lgr_grp.firma_nr, v_lte.lgr_ort, v_lgr_grp.lgr_ort
                            , v_lgr_grp.lgr_platz,      -- in_lgr_platz => nicht nötig - Standard lesen
                                                                       v_lte.lte_name, v_lvs_lgr_ort_ue_platz) then
                                ...
                                close c_transport_count;
                                if not lvs_p_base.get_lgr_platz(v_lgr_platz, v_lgr_ue) then
                                    v_lgr_ue := null;
              ...
              -- erst mal prüfen, ob der LTE_NAME in der liste der LTE_Namen_liste auftaucht
                            if
                                v_lgr_grp.wa_typ = v_wa_type
                                and (
                                    ( ( v_lgr_grp.lte_namen like '%'
                                                                 || nvl(v_lte.basis_lte_name, ';')
                                                                 || ';'
                                                                 || '%' )
                                    or ( v_lgr_grp.lte_namen like '%'
                                                                  || v_lte.lte_name
                                                                  || ';'
                                                                  || '%' ) )
                                    and (
                                        ( nvl(v_lgr_grp.lgr_akt_te, 0) + nvl(v_lgr_grp.lgr_dispo_einl_te, 0) + 1 <= v_lgr_grp.lgr_max_te
                                        )
                          -- AG 2017.10.10 BugFix
                          ...
                          --            das korigiert sich dann wieder selbstaendig.
                            ...
                            or (
                                ...
                                and ( nvl(v_lgr_grp.lte_namen, v_lvs_lgr_ort_ue_platz.lte_name_ziel || ';') like '%'
                                                                                                                 || v_lvs_lgr_ort_ue_platz.lte_name_ziel
                                                                                                                 || ';'
                                                                                                                 || '%' )
                                ...
                                begin
                                    v_result := lvs_transport.lvs_transp_lte_353(in_sid, in_firma_nr, 'BDE', null, 'F',
                                                                                 'A', -1, v_bde_fa.auf_id, null, c.lgr_transp_std_prio_ms
                                                                                 ,
                                                                                 null, null, null, v_lte.lgr_platz, v_lgr_grp.lgr_platz
                                                                                 ,
                                                                                 v_lte.lte_id, null, 'F', null, null,
                                                                                 null, null, null, null, null,
                                                                                 null, null, null, v_transp_grp, v_transp_id);
                    ...
                    raise_isi_error(c.fmid_lager_platz_fehlt,
                                    lc.ec_p1(lc.o_tp1_lagerplatz_fehlt, v_res.lager_roh));
                end if;
            else
                raise_isi_error(c.fmid_ziel_existiert_nicht,
                                lc.ec_p1(lc.o_tp1_resource_fehlt, v_bde_fa.res_id));
            end if;
...
-- sqlcl_snapshot {"hash":"2b9c59d02099ee9944649a0892b005cfa2bd1eb4","type":"PACKAGE_BODY","name":"BDE_TMS_PUSH","schemaName":"DIRKSPZM32","sxml":""}