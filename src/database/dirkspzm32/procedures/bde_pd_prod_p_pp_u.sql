...
(in_sid         in isi_sid.sid%type,
  ...
  */
  ...
  CURSOR c_bde_pd_prod  IS
    SELECT *
      FROM bde_pd_prod pa
      WHERE pa.sid = v_prod_pp.sid and
            pa.vorg_typ = 'PA' and
            pa.firma_nr = v_prod_pp.firma_nr and
            pa.leitzahl = v_prod_pp.leitzahl and
            pa.fa_ag = v_prod_pp.fa_ag and
            nvl(pa.fa_upos, 0) = nvl(v_prod_pp.fa_upos, 0) and
            pa.res_id = v_prod_pp.res_id and
            pa.prod_beginn <= v_prod_pp.prod_beginn and
            ((pa.prod_ende is NULL) or
             (pa.prod_ende >= v_prod_pp.prod_beginn))
     order by pa.prod_beginn desc;
  ...
  if v_found then
    ...
    if v_diff_mg != 0 then
      ...
      if v_found then                       -- Daten gefunden !!
        v_err_nr := 10;
        v_err_text := 'Fehler beim Eintragen der Produktionsmeldung:' || v_prod_pp.leitzahl || '/' || v_prod_pp.fa_ag || '/' || v_prod_pp.fa_upos ;
        ...
        update bde_pd_prod pa
          set pa.menge_a = nvl(v_prod_pa.menge_a, 0) + v_diff_mg
          where pa.sid = v_prod_pp.sid and
                pa.vorg_typ = 'PA' and
                pa.firma_nr = v_prod_pp.firma_nr and
                pa.leitzahl = v_prod_pp.leitzahl and
                ...
                -- pa.prod_beginn = v_prod_pp.prod_beginn and       -- 26.02.2017   3.5.10.1    (-AG-)
                pa.prod_beginn <= v_prod_pp.prod_beginn and       -- 13.12.2018  Ticket W20310-185 (-SKo-)
                ((pa.prod_ende is NULL) or
                 (pa.prod_ende >= v_prod_pp.prod_beginn));
        if sql%rowcount != 1 then
          v_err_nr := 20;
          v_err_text := 'Fehler beim Eintragen der Produktionsmeldung: ' || v_prod_pp.leitzahl || '/' || v_prod_pp.fa_ag || '/' || v_prod_pp.fa_upos || ' Eintrag nicht eindeutig.';
          raise v_error;
        end if;
        -- Update des Aktuelle Zustands des Arbeitsgangs
        update bde_fa_auftrag fa
           set fa.ag_ist_mg = nvl(fa.ag_ist_mg, 0) + v_diff_mg       -- Mengen addieren
           where fa.sid = v_prod_pp.sid and
                 fa.leitzahl = v_prod_pp.leitzahl and
                 fa.fa_ag = v_prod_pp.fa_ag and
                 nvl(fa.fa_upos, 0) = nvl(v_prod_pp.fa_upos, 0);
      end if;

      update lvs_lam_bh lbh
         set lbh.menge = in_menge
       where lbh.sid = v_prod_pp.sid and
             lbh.firma_nr = v_prod_pp.firma_nr and
             lbh.lam_id = v_prod_pp.lam_id and
             lbh.buch_datum = v_prod_pp.prod_ende;
    end if;
