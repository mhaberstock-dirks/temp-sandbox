...
package body DIRKSPZM32.sls_terminal is
  ...
  function get_next_auftrag(in_sid               in  isi_sid.sid%type,
    ...
    CURSOR c_transport is
      select *
       ...
       where t.sid              = in_sid
         ...
         and ((t.status = 'F' and t.res_id is NULL) or
                                  t.res_id = in_res_id)
       order by abs(nvl(t.res_id, -1) - in_res_id) asc,
    ...
    CURSOR c_lgr_ort is
      select min(ort.stapler_ls_id) min_id,
  ...
  begin
    ...
    LOOP
      ...
      OPEN c_lgr_ort;
      FETCH c_lgr_ort into v_min_id, v_max_id;
      CLOSE c_lgr_ort;
      if not v_found
      ...
      v_stapler_ls_id := nvl(v_fahrzeuge.stapler_ls_id, nvl(v_max_id, v_min_id));
      if v_stapler_ls_id is NULL
      or v_stapler_ls_id = nvl(v_min_id, nvl(v_max_id, v_stapler_ls_id))
      or v_stapler_ls_id = nvl(v_max_id, nvl(v_min_id, v_stapler_ls_id))
      then

        if v_found
        then
          v_fahrzeuge_IDs := ';' || to_char(in_res_id) || ';';
          v_return := Lvs_Platz.Lvs_c_Transp_Check_Zugriff(v_transport.sid, v_transport.firma_nr,
                                                           v_transport.modul_erzeuger,
                                                           v_transport.modul_bearbeiter,
                                                           in_freifahren, in_user_id,
  ...
  begin
    ...
    v_result := lvs_p_lte.lvs_lte_insert_v358 (in_sid,                  -- SID der Maschine
                                               ...
                                               null,
                                               NULL,
                                               'KOMM',                     -- Charge nicht bekannt
                                               NULL,
                                               NULL,                      -- Packschema nicht bekannt
  ...
  begin
    ...
    then
      ...
      if v_found
      then
        if v_transport.status != 'F'
        then
          v_ret := 'Transport dieser Palette bereits vergeben.';
          return v_ret;
        end if;
        if v_soll_lte.lgr_platz != v_scann_lte.lgr_platz
        then
        ...
        or v_soll_lgr.lgr_verwendung = c.LGR_TYP_EP
        then
          if  v_soll_lte.res_string = v_scann_lte.res_string
  ...
  procedure c_log_transport_check(in_sid in isi_transport_log.sid%type,
                                  ...
                                  in_check_passed in isi_transport_log.check_passed%type) is
    v_transport isi_transport%rowtype;
  begin
    ...
    if in_check_passed = c.C_TRUE
       and in_check_typ in ('waren', 'we_waren', 'wa_waren')
    then
      update isi_transport t
         set t.check_ware_login_id = in_login_id
       where t.sid = in_sid
         and t.transp_id = in_transp_id;
    elsif in_check_passed = c.C_TRUE
          and in_check_typ in ('einl_lgr', 'einl_lgr_z', 'ausl_lgr', 'ausl_lgr_z')
    then
      update isi_transport t
         set t.check_platz_z_login_id = in_login_id
       where t.sid = in_sid
         and t.transp_id = in_transp_id;
    elsif in_check_typ in ('ausl_lgr_q')
    then
      ...
      update isi_transport t
         set t.check_platz_q_login_id = in_login_id
       where t.sid = in_sid
         and t.transp_id = in_transp_id;
    end if;
  ...
  procedure transp_check_ausl_ware(in_sid in isi_transport_log.sid%type,
                                   ...
                                   out_lgr_platz_quelle out lvs_lam.lgr_platz%type) is
    v_transport isi_transport%rowtype;
    ...
    cursor c_komm_transp_lam is
      select *
        from lvs_lam t
       where t.res_ziel_lte_id = v_transport.lte_id;
  ...
  begin
    ...
    if not lvs_p_base.get_lam_by_lhm_id(in_sid, in_firma_nr, in_old_id, v_old_lam)
    then
      -- in_old_id ist keine LHM ID also prüfen, ob es eine LTE ID ist
      if not lvs_p_base.get_lte(in_old_id, v_lte)
      then
      ...
      if not lvs_p_base.get_lhm(in_old_id, v_old_lhm)
      then
        raise_isi_error(20, 'Die Daten für LHM ID ' || nvl(to_char(in_old_id), '(null)') ||
                            ' konnten nicht gefunden werden.');
      end if;

      v_old_lte_id := nvl(v_old_lhm.komm_quell_lte_id, v_old_lam.lte_id);
      if v_old_lte_id is null
      then
    ...
    loop
      ...
      if not lvs_p_base.get_lhm(v_transp_lam.lhm_id, v_transp_lhm)
      then
        v_transp_lhm.komm_quell_lte_id := null;
      end if;

      -- Prüfen, ob eine LAM des aktuellen Transports auf der alten LTE steht oder stand
      if nvl(v_transp_lhm.komm_quell_lte_id, v_transp_lam.lte_id) = v_old_lte_id
      then
        -- Die alte LTE ID stimmt mit der neuen LHM ID überein, also ok und fertig
        if v_transp_lam.check_ware_transp_id is null
        then
          update lvs_lam t
             set t.check_ware_transp_id = v_transport.transp_id
           where t.sid = v_transp_lam.sid
        ...
        if out_lgr_platz_quelle is null
        then
...
*/
    ...
    loop
      ...
      v_check_passed := v_check_passed and v_transp_lam.check_ware_transp_id is not null;
    end loop;
    close c_transp_lam;
