...
begin
  ...
  if v_pzm_konten.buch_einheit != :old.einheit
  then
    raise_application_error(-20000, 'Die Buchungseinheit etnspricht nicht der Buchungseinheit in den Kontoeinstellungen.');
  end if;


  if :old.bus = 2 or :old.bus = 3        -- normaler Abgang / Zugang storniert
  then
    v_pzm_konten.saldo := v_pzm_konten.saldo + :old.wert;
  elsif :old.bus = 1 or :old.bus = 4           -- normaler Zugang / Abgang storniert
  then
    v_pzm_konten.saldo := v_pzm_konten.saldo - :old.wert;
  end if;
