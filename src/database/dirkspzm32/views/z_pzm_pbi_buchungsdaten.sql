  ...
  select b.bu_id,
       ...
       case when nvl(pb.pb_extern, 'F') = 'T'
            then nvl(pb.pb_bemerkungen, ze.ze_pb_id)
            else to_char(ze.ze_pb_id)
            end
            firma,
       ...
       case when nvl(pb.pb_extern, 'F') = 'T' and ts.ts_ueb_ok_datum is not null
            then round(ze.ze_std / (ts.ts_day_anw_std) * (ts.ts_day_arb_std + ts.ts_day_ueb_std + ts.ts_day_flex_std), 3)
            else round(ze.ze_std / (ts.ts_day_anw_std) * ts.ts_day_arb_std, 3)
            end wert,
       ze.ze_kst_id kostenstelle,
       (select la.lz_lohnart from pzm_lohnarten la where 'ARBSTD' = la.lz_operator) status,
       'Anwesende Zeit ' || ze.ze_std || ' ' || ts.ts_day_arb_std || ' P ' || ts.ts_day_pause_std status_text,
       'ZE' status_text2,
       NULL sap_zuordnung,
...
select b.bu_id,
       ...
       case when nvl(pb.pb_extern, 'F') = 'T'
            then nvl(pb.pb_bemerkungen, loa.zeaw_pb_id)
            else to_char(loa.zeaw_pb_id)
            end
            firma,
       get_pers_pb_id(p.pers_nr) pd_mandant,
       a.abt_pb_id mandant,
       ...
       case when pb.pb_extern = 'T'
            ...
            then 'UNB'
            else substr(loa_cfg.lz_operator, 1, 50)
            end status_text2,
       NULL sap_zuordnung,
       '00:00' start_zeit,
       '00:00' ende_zeit,
   ...
   from pzm_personal p,
        ...
        pzm_produktionsbereiche_bu bu,
        pzm_prod_business_units_bereiche b
  where p.pers_nr = loa.zeaw_pers_nr
    ...
    and (pb.pb_extern != 'T' or (    (   loa_cfg.lz_konto_name_kurz is NULL
                                      or (    loa_cfg.lz_konto_name_kurz is not NULL
...
select b.bu_id,
       ...
       loa_ex.datum,
       case when nvl(pb.pb_extern, 'F') = 'T'
            then nvl(pb.pb_bemerkungen, pb.pb_id)
            else to_char(pb.pb_id)
            end
            firma,
       get_pers_pb_id(p.pers_nr) pd_mandant,
       a.abt_pb_id mandant,
   ...
   from pzm_personal p,
        ...
        pzm_produktionsbereiche_bu bu,
        pzm_prod_business_units_bereiche b
  where p.pers_nr = loa_ex.pers_nr
    and p.pers_abt_id = a.abt_id
    and a.abt_pb_id = bu.pb_id(+)
    and bu.bu_id = b.bu_id(+)
...
select b.bu_id,
       ...
       am.beginn datum,
       case when nvl(pb.pb_extern, 'F') = 'T'
            then nvl(pb.pb_bemerkungen, get_pers_pb_id(p.pers_nr))
            else to_char(get_pers_pb_id(p.pers_nr))
            end
            firma,
       get_pers_pb_id(p.pers_nr) pd_mandant,
       a.abt_pb_id mandant,
       ...
       'von ' || to_char(am.beginn, 'dd.mm.yyyy') || ' bis ' || to_char(max(am.ende), 'dd.mm.yyyy') || ' ' || substr(aa.aa_kurzname, 1, 50) status_text2,
       NULL sap_zuordnung,
       '00:00' start_zeit,
       '00:00' ende_zeit,
       nvl(max(am.aend_datum), max(am.erz_datum)) last_change_date
   from pzm_personal p,
...
select b.bu_id,
       ...
       get_pers_kst_id(p.pers_nr) kostenstelle,
       to_char(nvl(aa.aa_id, 0)) status,
       substr(aa.aa_name || ' ' || aan.au_bemerkung, 1, 50) status_text,
       'von ' || to_char(aan.au_beginn, 'dd.mm.yyyy') || ' bis ' || to_char(max(aan.au_ende), 'dd.mm.yyyy') || ' ' ||
       ...
       '00:00' start_zeit,
       '00:00' ende_zeit,
       nvl(max(aan.au_datum), max(aan.au_ende)) last_change_date
   from pzm_personal p,
        ...
        pzm_produktionsbereiche_bu bu,
        pzm_prod_business_units_bereiche b
  where p.pers_nr = aan.au_pers_nr
    and p.pers_abt_id = a.abt_id
    and a.abt_pb_id = bu.pb_id(+)
    and bu.bu_id = b.bu_id(+)
