...
package body DIRKSPZM32.isi_message_board is
  ...
  --------------------------------------------------------------------------------
  --******************************************************************************
    procedure send_message(in_sender_name                 in     varchar2,
  ...
  begin
    ...
    v_protokoll := v_protokoll || '<sender name="' || nvl(in_sender_name, '') ||
                                  '" module_name="' || nvl(in_sender_module_name, '') ||
                                  '" application_handle="' || nvl(in_sender_application_handle, 0) ||
                                  '" database_username="' || nvl(v_database_username, '') ||
                                  '"/> ';
    v_protokoll := v_protokoll || '<recipient name="' || nvl(in_recipient_name, '') ||
                                  '" module_name="' || nvl(in_recipient_module_name, '') ||
                                  '"/>';
    v_protokoll := v_protokoll || '<response_required>' || nvl(v_tf_txt, '') || '</response_required> ';
    v_protokoll := v_protokoll || '<message_type>' || nvl(to_char(in_message_type), '0') || '</message_type> ';
    v_protokoll := v_protokoll || ' <message_command>' || nvl(to_char(in_message_command), '0') || '</message_command> ';
    v_protokoll := v_protokoll || ' <data_type>' || nvl(to_char(in_data_type), '0') || '</data_type> ';
    v_protokoll := v_protokoll || ' <data>' || nvl(in_data, '') || '</data> ';
    v_protokoll := v_protokoll || ' </isi_message> ';
    dbms_alert.signal('isi_message', v_protokoll);
    commit;
  end;

  procedure c_send_raw_message(in_raw_data in varchar2) is
    pragma autonomous_transaction;
  begin
  ...
  procedure c_send_raw_message(in_signal in varchar2,
                                 in_raw_data in varchar2) is
   pragma autonomous_transaction;
  begin
