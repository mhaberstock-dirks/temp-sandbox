...
package body DIRKSPZM32.tms_p_delivery is
  ...
  */
   ...
   * private functions
   ******************************************************************************************************/
  function get_deliv_order_li_nr(in_sid        in isi_order_pos.sid%type,
  ...
  begin
    ...
    if not v_found
    then
      select seq_tms_order_li_nr.nextval into v_li_nr from dual;
    end if;

    return (v_li_nr);
  end;

  function get_deliv_order_li_pos_nr(in_sid        in isi_order_pos.sid%type,
                                     in_firma_nr   in isi_order_pos.firma_nr%type,
                                     in_vorgang_id in isi_order_pos.vorgang_id%type,
  ...
  function get_deliv_sped_adress_id(in_sid        in isi_order_kopf.sid%type,
    ...
    cursor c_order_kopf is
      select t.sp_adress_id
        from isi_order_kopf t
       where t.vorgang_id = in_vorgang_id
         and t.vorgang_typ = 'WAE'
         and t.firma_nr = in_firma_nr
         and t.sid = in_sid
       order by t.transport_gruppe;
  begin
  ...
  function get_deliv_transport_group(in_sid        in isi_order_kopf.sid%type,
                                     in_firma_nr   in isi_order_kopf.firma_nr%type,
                                     in_vorgang_id in isi_order_kopf.vorgang_id%type,
  ...
  begin
    v_vorgang_id := tms_p_delivery.c_create_new_delivery_ord_3511 (in_sid,
                                                                   in_firma_nr,
                                                                   in_auftrag_nr,
                                                                   in_pos_nr,
                                                                   in_upos_nr,
  ...
  begin
    ...
    select seq_tms_order_vorgang_id.nextval into v_vorgang_id from dual;
  ...
  begin
    ...
    c_add_pos_to_delivery_order(in_sid, in_firma_nr, v_vorgang_id,
                                in_auftrag_nr, in_pos_nr, in_upos_nr,
                                in_login_id, in_sp_adress_id);
    return (v_vorgang_id);
  exception
    -- Im Fehlerfall is der Fehlertext bereits gesetzt.
    when v_error then  -- Update 2011 show Exception Source Line
  ...
  begin
    tms_p_delivery.c_add_pos_to_delivery_ord_3511(in_sid,
                                                  in_firma_nr,
                                                  in_vorgang_id,
                                                  in_auftrag_nr,
                                                  in_pos_nr,
  ...
  procedure c_add_pos_to_delivery_ord_3511 (in_sid        in isi_order_pos.sid%type,
    ...
    cursor c_order_kopf is
      select ok.*
       ...
       where ok.vorgang_id = in_vorgang_id
    ...
    cursor c_order_pos is
      select op.*
       ...
       where op.vorgang_id = in_vorgang_id
    ...
    cursor c_order_vorgang_status is
      select nvl(stradd_distinct(ok.status), 'N')
        from isi_order_kopf ok
       where ok.vorgang_id = in_vorgang_id
         and ok.vorgang_typ = 'WAE'; -- delivery immer WAE
  ...
  begin
    ...
    if not tms_p_base.get_kunden_auftr_pos_by_uix(in_auftrag_nr, in_pos_nr, in_upos_nr, in_firma_nr, in_sid, v_kunden_auftr_pos)
    then
      raise_isi_error(10, lc.ec_p3('O_TP3_CUST_ORDER_LINE_NA', in_auftrag_nr, in_pos_nr, in_upos_nr));
    end if;

    v_order_li_nr := v_kunden_auftr_pos.li_nr;
    if v_order_li_nr is null
    then
    ...
    then
      ...
      if v_vorgang_status <> 'N'
      then
        raise_isi_error(20, lc.ec_p1('O_TP1_DELIV_ORDER_STATUS_ERR', in_vorgang_id));
      end if;
      ...
      v_order_kopf.transport_gruppe := get_deliv_transport_group(v_kunden_auftr_pos.sid,
                                                                 v_kunden_auftr_pos.firma_nr,
                                                                 in_vorgang_id,
    ...
    update tms_kunden_auftr_pos t
       set t.status = 'B', -- dieser Kundenauftrag ist jetzt in Bearbeitung
    ...
    if v_kunden_auftr_pos.besteller = 'WAWI' then
       ...
       set pk.status = 'B', -- dieser Kundenauftrag ist jetzt in Bearbeitung
           pk.lieferschein_nr = v_order_kopf.li_nr
     where pk.id = in_auftrag_nr
  ...
  procedure c_mov_pos_to_delivery_order(in_sid            in isi_order_pos.sid%type,
    ...
    CURSOR c_order_pos is
      select *
        from isi_order_pos t
       where t.vorgang_id = v_vorgang_id
     order by t.li_nr, t.li_pos_nr;
  begin
    reset_isi_error();

    if in_new_vorgang_id = in_old_vorgang_id
    then
      return;
    end if;

    update isi_order_kopf t
       set t.vorgang_id = in_new_vorgang_id
     where t.vorgang_id = in_old_vorgang_id
    ...
    LOOP
      ...
      LOOP
        ...
        update isi_order_pos t
           set t.vorgang_pos = v_vorgang_pos
         where t.auf_id = v_order_pos.auf_id;
  ...
  begin
    ...
    if not v_found
    then
      raise_isi_error(10, lc.ec_p2('O_TP2_ORDER_KOPF_LI_DATA_NA', in_vorgang_id, in_li_nr));
    end if;

    if nvl(v_ok_status, 'N') != 'N'
    then
      raise_isi_error(20, lc.ec_p2('O_TP3_ORDER_KOPF_LI_ACTIVE', in_vorgang_id, in_li_nr));
    end if;

    open c_order_pos;
    fetch c_order_pos into v_op_status, v_op_auf_id_extern;
    v_found := c_order_pos%found;
    close c_order_pos;

    if not v_found
    then
      raise_isi_error(30, lc.ec_p3('O_TP3_ORDER_POS_LI_DATA_NA', in_vorgang_id, in_li_nr, in_li_pos_nr));
    end if;

    if v_op_status <> 'N'
    then
      raise_isi_error(40, lc.ec_p3('O_TP3_ORDER_POS_LI_ACTIVE', in_vorgang_id, in_li_nr, in_li_pos_nr));
    end if;
    ...
    delete isi_order_kopf ok
     where ok.vorgang_id = in_vorgang_id
       ...
       and ok.firma_nr = in_firma_nr
       and ok.sid = in_sid
       and ok.vorgang_id not in ( select op.vorgang_id
                                    from isi_order_pos op
                                   where op.li_nr = ok.li_nr
                                     and op.vorgang_typ = 'WAE'
                                     and op.vorgang_id = in_vorgang_id
                                     and op.firma_nr = in_firma_nr
                                     and op.sid = in_sid);
    ...
    update tms_kunden_auftr_pos t
       set t.status = 'N', -- dieser Kundenauftrag ist jetzt wieder neu
           t.aend_login_id = in_login_id,
           t.aend_datum = sysdate
     where t.kunden_auftr_pos_id = v_op_auf_id_extern
       and t.firma_nr = in_firma_nr
       and t.sid = in_sid
       and t.status = 'B';
