...
declare
  ...
  CURSOR c_lgr is                        -- Lesen des Lagerplatz
    select *
    from lvs_lgr lgr
    where lgr.lgr_platz = v_lam_akt.lgr_platz;
...
begin
  ...
  if not lvs_p_base.get_lte(v_lam_akt.lte_id, v_lte)
  then
    v_lte := NULL;
  end if;

  if :new.bus  != c.LAM_BH_BUS_INV                    -- Keine Inventurbuchung
  ...
  if :new.bus != 1 then                  -- Inventur
    ...
    if :old.bus != 5                             -- Sperren
    and :new.bus = 5
    and nvl(:new.menge, 0) > 0
    then
      s_schnittstelle.write_host_bew(NULL, v_lam_akt, :new.lam_bh_id, :new.bus, :new.vorg_typ, NULL, 'UE', v_lgr, NULL, :new.ls_login_id, nvl(:new.menge, 0));
    end if;
    :new.menge := nvl(:new.menge, 0);
    if (lvs_ausl.lvs_lam_bh_update_mg(v_old, :new.menge) != 1) then
      return;
    end if;
  ---------------------------------------------------------------------------------------------------------------------------------
    ...
    then
      ...
      v_diff_kg := nvl(:new.lam_bh_kg, 0) - nvl(:old.lam_bh_kg, 0);               -- Gewicht rechnen
      v_diff_mg := nvl(:new.menge, 0) - nvl(:old.menge, 0);
  ---------------------------------------------------------------------------------------------------------------------------------
    ...
    then
      v_diff_kg := nvl(:old.lam_bh_kg, 0) - nvl(:new.lam_bh_kg, 0);               -- Gewicht rechnen
      v_diff_mg := nvl(:old.menge, 0) - nvl(:new.menge, 0);
    else
      v_diff_kg := nvl(:old.lam_bh_kg, 0) - nvl(:new.lam_bh_kg, 0);               -- Gewicht rechnen
    end if;
   else
    v_diff_kg := nvl(:new.lam_bh_kg, 0) - nvl(:old.lam_bh_kg, 0);               -- Gewicht rechnen
    v_diff_mg := nvl(:new.menge, 0) - nvl(:old.menge, 0);
    s_schnittstelle.write_host_bew(NULL, v_lam_akt, :new.lam_bh_id, :new.bus, :new.vorg_typ, NULL, 'UE', v_lgr, NULL, :new.ls_login_id, v_diff_mg);
  end if;
  -- Update dokumentieren
  if :new.bus != c.LAM_BH_BUS_WKE_KONSI
  then
    :new.last_change_date := sysdate;
    :new.last_change_login_id := :new.ls_login_id;
    :new.change_menge := nvl(:new.menge, 0) + v_diff_mg;
  end if;

  if ((:new.bus = 24                   -- Entnahme KONSI
  or   :new.bus = 22)                  -- Zugang KONSI
  and
      abs(nvl(:old.menge, 0)) != v_lam_akt.menge -- Entnahme wird in + und - gebucht (+)= Entnahme KONSI (-)= Zurück ins KONSI mit Menge
     )
  or (:new.bus = 23                    -- Abgang KONSI
  ...
  then
    v_err_text :=
    'Fehler: KONSI-Buchung nicht möglich. ' ||
    'Lagerbestand für LHM ' || v_lam_akt.lhm_id ||
  ...
  UPDATE lvs_lhm
      SET lhm_akt_kg = nvl(lhm_akt_kg, 0)  + nvl(v_diff_kg, 0),
          lgr_platz = v_lam_akt.lgr_platz
      WHERE lhm_id = :new.lhm_id;

  UPDATE lvs_lte
      SET lte_akt_kg = nvl(lte_akt_kg, 0) + nvl(v_diff_kg, 0)
      WHERE lte_id = :new.lte_id;

  -- -AG- Bug-Fix 2015.05.07
  UPDATE lvs_lgr
    SET lgr_akt_kg = nvl(lgr_akt_kg, 0) + nvl(v_diff_kg, 0)
    WHERE lgr_platz = v_lgr.lgr_platz;

  -- Nur wenn bei einem Zugang der einzige LAM abgezogen wird
  -- und es sich um einen echten Lagerplatz hanndelt
  if  v_lam_akt.lgr_platz is NULL
