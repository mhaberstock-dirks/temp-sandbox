...
function DIRKSPZM32.bde_pd_prod_p_ag_b_f (in_sid in isi_sid.sid%type,
  ...
  select l.*
    from lvs_lam l
    where l.lam_id = v_lam_stl.stl_lam_id;
...
begin
  ...
  v_check_funktion := isi_allg.c_get_firma_cfg_param(in_sid,
                                                     in_firma_nr,
                                                     'BDE_FA_AN',           -- in_kategorie             in isi_firma_cfg.kategorie%type,
  ...
  then
    ...
    then
      ...
      update bde_fa_auftrag fa
         set fa.mde_ist_mg       = nvl(fa.mde_ist_mg, 0) + nvl(fa.mde_ist_mg_t, 0)
       where fa.sid = in_sid
  ...
  then
    ...
    LOOP
      ...
      if v_lam.lam_id = v_lam_stl.stl_lam_id
      then
    ...
    delete isi_resource_lam_akt lam_akt
     where lam_akt.sid = in_sid
       and lam_akt.res_id = in_res_id
       and not exists (select lam_akt2.artikel_id
                         from isi_resource_lam_akt lam_akt2,
                              bde_fa_auftrag fa_leit
                        where lam_akt2.sid = in_sid
  ...
  update bde_fa_auftrag fa
     set fa.anz_res          = nvl(fa.anz_res, 0) + v_anz_res,
         fa.freig_status     = 'AP',
  ...
  update bde_pd_prod pd
     set pd.prod_ende = v_sysdate
   where pd.res_id = in_res_id
     and pd.vorg_typ = 'PA'
     --and pd.sid = in_sid -WK-
     --and pd.firma_nr = in_firma_nr -WK-
     and pd.prod_beginn <= v_sysdate
     and ((pd.prod_ende is NULL) or (pd.prod_ende >= v_sysdate));
