create or replace function dirkspzm32.get_pause_time_day (
...
begin
  ...
  -- Wenn Schitdaten gefunden
    if v_found then
        if in_day_pause_std = 0 then
            if v_pzm_schichtarten.calc_basis = 'GLEITZ' then
                if in_anw_std = 0 then
                    return ( 0 );
                end if;
        ...
        -- Wenn die Pause1 im Zeitraum ist
                ...
                or ( v_ende between in_beginn and in_ende ) then
          -- Ueberschneidungen Pause Zeitraum dann Pausenzeit reduzieren
                    if v_beginn < in_beginn then
                        v_beginn := in_beginn;
                    end if;
                    if v_ende > in_ende then
                        v_ende := in_ende;
                    end if;
                    if v_pzm_schichtarten.sa_pause1_unbez = 1 then
        ...
        -- Wenn die Pause2 im Zeitraum ist
          ...
          -- Ueberschneidungen Pause Zeitraum dann Pausenzeit reduzieren
                    ...
                    if v_pzm_schichtarten.sa_pause2_unbez = 1 then
                        time_std := time_std + ( ( v_ende - v_beginn ) * 24 );
                    end if;
    ...
    -- für Gleitzeit und Wechselschicht
        if
            ...
            and time_std = 0
        then
            v_pause_in_arbeitszeit := nvl(
                pzm_p_base.get_allg_parameter_mandant(in_pb_id, 'PAUSE_IN_ARBEITSZEIT'),
                'T'
            );
            if v_pause_in_arbeitszeit != 'T' -- Berechnung nach Anwesenheit und nicht nach Arbeitszeit (Euscher)
             then
                if
                    nvl(v_pzm_schichtarten.gleitz_pause1_dauer_min, 0) >= 0
          ...
          -- Stufe 1
                    ...
                    nvl(v_pzm_schichtarten.gleitz_pause2_dauer_min, 0) >= 0
          ...
          -- Stufe 2
                    ...
                    nvl(v_pzm_schichtarten.gleitz_pause3_dauer_min, 0) >= 0
          ...
          -- regulär
                ...
                if in_anw_std > v_pzm_schichtarten.gleitz_pause1_arb_std then
          -- Stufe 1
                ...
                if in_anw_std > v_pzm_schichtarten.gleitz_pause2_arb_std then
          -- Stufe 2
                    if in_anw_std < v_pzm_schichtarten.gleitz_pause2_arb_std - time_std + ( nvl(v_pzm_schichtarten.gleitz_pause2_dauer_min
                    , 0) / 60 ) then
                ...
                if in_anw_std > v_pzm_schichtarten.gleitz_pause3_arb_std then
          -- Stufe 3
                    if in_anw_std < v_pzm_schichtarten.gleitz_pause3_arb_std - time_std + ( nvl(v_pzm_schichtarten.gleitz_pause3_dauer_min
                    , 0) / 60 ) then
          ...
          -- regulär
            ...
            if v_pause_unbezahlt then
        -- unbazahlte Pause
                if in_day_pause_std > time_std then
...
-- sqlcl_snapshot {"hash":"1b211d97b7714a4b78cb950667d2f62f8a2b7fd7","type":"FUNCTION","name":"GET_PAUSE_TIME_DAY","schemaName":"DIRKSPZM32","sxml":""}