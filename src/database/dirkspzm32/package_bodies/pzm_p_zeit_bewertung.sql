...
package body DIRKSPZM32.pzm_p_zeit_bewertung is
   ...
   */
  function time_of_day(in_timestamp in date) return number is
  begin
   ...
   */
  function is_abwesenheit_status(in_status in number) return boolean is
  begin
  ...
  begin
    if in_raster_minuten = 0 or in_raster_minuten is null then
  ...
  begin
    ...
    v_config.raster_minuten := nvl(
      to_number(pzm_p_base.get_allg_parameter_mandant(in_pers_nr, 'SCHICHT_FESTZ_RASTER_MIN')),
      c_default_raster
    );
    ...
    if v_kappung = 'F' and nvl(v_schicht_modell.kappung_schicht_ende, 'F') = 'T' then
      v_kappung := 'T';
    end if;
  ...
  begin
    ...
    if in_ze_status = pzm_p_zeiterfassung.STATUS_ANWESEND then
      if in_is_erster_eintrag and in_config.beginn_gutschr_min > 0 then
    ...
    if not in_config.gleitz_runden or v_raster_min < 1 then
      return;
    end if;

    -- Startzeit-Rundung
    if v_rec_start is not null then
      v_minuten_offset := round((v_rec_start - trunc(v_rec_start, 'HH24')) * 24 * 60, 3);
    ...
    if v_rec_end is not null then
      v_minuten_offset := round((v_rec_end - trunc(v_rec_end, 'HH24')) * 24 * 60, 3);
      v_raster_count := v_minuten_offset / v_raster_min;
    ...
    pzm_p_log.debug(
      'Gleitzeit bewertet: ' ||
      'Rec=' || to_char(in_rec_start, 'HH24:MI') || '-' || to_char(in_rec_end, 'HH24:MI') ||
      ' -> Calc=' || to_char(io_calc_start, 'HH24:MI') || '-' || to_char(io_calc_end, 'HH24:MI') ||
      ' (Raster=' || v_raster_min || 'min, Runden=' || 
        case when in_config.gleitz_runden then 'T' else 'F' end || ')',
  ...
  begin
    ...
    if in_rec_start <= trunc(in_rec_start) and in_rec_end <= trunc(in_rec_end) then
      return;
    end if;

    -- Gutschrift anwenden (nur fuer ersten Anwesend-Eintrag)
    if in_is_erster_eintrag 
       and in_ze_status = pzm_p_zeiterfassung.STATUS_ANWESEND
       and in_config.beginn_gutschr_min > 0 then
      v_rec_start := v_rec_start - (in_config.beginn_gutschr_min / 60 / 24);
    end if;
    ...
    pzm_p_log.debug(
      'Festschicht Kommen bewertet: ' ||
      'Rec=' || to_char(v_rec_start, 'HH24:MI') ||
      ', Schicht=' || to_char(in_schicht.sa_beginn, 'HH24:MI') ||
      ' -> Calc=' || to_char(io_calc_start, 'HH24:MI') ||
      ' (Delta=' || round(v_beginn_delta * 24 * 60) || 'min)',
      pzm_p_log.CAT_ZEITERFASSUNG,
  ...
  begin
    ...
    if in_rec_start = in_rec_end and io_calc_end is null then
    ...
    if in_ze_status = pzm_p_zeiterfassung.STATUS_ANWESEND
       and in_config.ende_gutschr_min > 0 then
      v_rec_end := v_rec_end + (in_config.ende_gutschr_min / 60 / 24);
    end if;
    ...
    pzm_p_log.debug(
      'Festschicht Gehen bewertet: ' ||
      'Rec=' || to_char(v_rec_end, 'HH24:MI') ||
      ', Schicht=' || to_char(v_sa_ende2, 'HH24:MI') ||
      ' -> Calc=' || to_char(io_calc_end, 'HH24:MI') ||
      ' (Delta=' || round(v_ende_delta * 24 * 60) || 'min)',
      pzm_p_log.CAT_ZEITERFASSUNG,
  ...
  begin
    ...
    if io_calc_end > in_schicht.sa_ende_effektiv
       and in_ist_ende > in_schicht.sa_ende_effektiv
       and in_ze_status = pzm_p_zeiterfassung.STATUS_ANWESEND then

      pzm_p_log.debug(
        'Kappung angewendet: ' || to_char(io_calc_end, 'HH24:MI') ||
  ...
  begin
    ...
    if not pzm_p_base.get_schicht_modell(in_pers_nr, v_schicht_modell) then
      pzm_p_log.warning(
        'Kein Schichtmodell fuer PersNr=' || in_pers_nr || ' - keine Bewertung moeglich',
        pzm_p_log.CAT_ZEITERFASSUNG,
        'bewerte_ist_zeiten'
      );
    ...
    if v_rec_start is null and v_calc_start is null then
    ...
    if v_rec_start is null and v_calc_start is not null then
      v_rec_start := v_calc_start;
    end if;
    if v_rec_end is null and v_calc_end is not null then
    ...
    if v_rec_start is not null and v_rec_end is null then
    ...
    if not v_sa_found then
      pzm_p_log.error(
        'Keine Schichtdaten fuer PersNr=' || in_pers_nr ||
        ', Zeiten=' || to_char(v_rec_start, 'YYYY-MM-DD HH24:MI') ||
    ...
    if not pzm_p_base.get_schichtart_by_uix(v_sa_kurzname, v_schichtart) then
      pzm_p_log.error(
        'Keine Schichtart fuer SA_Kurzname=' || v_sa_kurzname,
        pzm_p_log.CAT_ZEITERFASSUNG,
        'bewerte_ist_zeiten'
      );
      v_result.calc_ist_start := v_calc_start;
    ...
    if trunc(v_calc_start) = v_calc_start and trunc(v_calc_end) = v_calc_end then
    ...
    if v_config.calc_basis = CALC_BASIS_GLEITZEIT then
      -- GLEITZEIT-Bewertung
      if v_calc_start is null or v_calc_end is null
         or (v_calc_end = v_schicht.sa_ende_effektiv and not v_config.kappung_schicht_ende) then
      ...
      if v_calc_end is null
         or (v_calc_end = v_schicht.sa_ende_effektiv 
             and not v_config.kappung_schicht_ende
             and v_rec_end != v_rec_start) then
        bewerte_gehen_festschicht(
          in_config     => v_config,
          in_schicht    => v_schicht,
          in_rec_start  => v_rec_start,
    ...
    if v_calc_start is not null and v_calc_end is not null then
