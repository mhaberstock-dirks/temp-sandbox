create or replace package body dirkspzm32.s_schnittstelle is
  ...
  --******************************************************************************
    ...
    -------------------------------------------------------------------------------------------------------
        ...
        from
            isi_artikel art
        where
                art.sid = nvl(in_lam.sid, in_order_pos.sid)
        ...
        from
            isi_arbeitsplatz ap
        where
            ap.arbeitsplatz_id = in_order_pos.arbeitsplatz_id;
    ...
    begin
        ...
        if v_sid.sid_schnittstelle is null then
            ...
            or in_order_pos.besteller != 'WAWI' then
                return;
            end if;
        end if;

        if
            in_order_pos.vorgang_typ = 'LIF'
        ...
        or in_order_pos.besteller is null then
      ...
      -- Bei Umlagerungen auf den gleichen Lagerort kann diese Buchrung Ignoriert werden
            if
                ( in_lam_bh_bus = c.lam_bh_bus_uml
            ...
            then
                if
                    ( in_lam_bh_bus != c.lam_bh_bus_up          -- Prüfen, ob HOST die LTE_IDs verwaltet (benötigt)
                    or in_lam.lte_id is null                      -- Dann auch bei gleichem Lagerort buchen
                    or isi_allg.c_get_firma_cfg_param(in_lam.sid, in_lam.firma_nr, 'CFG',                  -- in_kategorie             in isi_firma_cfg.kategorie%type,
            ...
            if in_order_pos.vorgang_typ is null then
                if in_tabelle = 'S_FA' then
                    open c_resource;
                    fetch c_resource into v_res;
                    close c_resource;
                end if;

                if ( in_lam_bh_bus = c.lam_bh_bus_uml ) then
                ...
                if ( in_lam_bh_bus = c.lam_bh_bus_up ) then
                    v_vorgang_typ := 'KOM';
                end if;
        -- Hier jetzt KONSI Zugang
          ...
          -- bei Konsi-Buchungen immer die Lieferscheinnr des Lieferanten verwenden
            ...
            -- Im SAP kommt diese Buchung als Wareneingean Extern 'WEE'
                        if v_sid.sid_schnittstelle = 'EUS_SAP' then
                            v_vorgang_typ := 'WEE';
                        elsif v_sid.sid_schnittstelle = 'ERP_STD' -- Standard-Schnittstelle
                         then
                            if
                                nvl(in_tabelle, 'xx') != 'S_FA'      -- keine fertigung
        ...
        -- Hier jetzt KONSI Abgang
                if in_lam_bh_bus = c.lam_bh_bus_abg_konsi then
                    v_vorgang_typ := 'KWA';
                end if;
        -- Hier jetzt Entnahme KONSI
                if in_lam_bh_bus = c.lam_bh_bus_wke_konsi then
                    v_vorgang_typ := 'WKE';
          -- bei Konsi-Buchungen immer die Lieferscheinnr des Lieferanten verwenden
                    v_ext_liefs_nr := in_lam.li_nr_lief;
                end if;
      ...
      -- AG 17.06.2015 Im Standard soll die Lieferscheinnummer des Lieferanten in der Schnittstelle eingeragen werden
            if
                in_lam_bh_bus = c.lam_bh_bus_zug
      ...
      -- Lesen der User-Daten zum übermitteln der Personalnummer zu Hostsystemen
            if not isi_allg.get_user_by_login_id(v_sid.sid, in_login_id, v_user) then
                v_user.pers_nr := null;
  ...
  --******************************************************************************
    ...
    --v_eus_sap_rcv_auftr  s_eus_sap_rcv_auftr%rowtype;
        ...
        from
            isi_arbeitsplatz a
        where
            a.arbeitsplatz_id = io_bew.arbeitsplatz_id;
    ...
    */
        ...
        select
            t.*
        from
            s_eus_sap_send_bew t
        where
                t.firma_nr = io_bew.firma_nr
            and t.herkunft = io_bew.herkunft
          -- Bei Euscher SAP muss die Tabelle in der Rueckmeldung S_AUFTR heissen
            and nvl(t.tabelle, 'keine') = nvl(
                decode(io_bew.tabelle, 'S_AUF', 'S_AUFTR', io_bew.tabelle),
                'keine'
            )
            and nvl(t.auf_id, -1) = nvl(io_bew.auf_id, -1)
            and t.aktion = v_aktion
    ...
    */
        ...
        select
            t.*
        from
            s_sas_sap_send_bew t
        where
                t.slb_buchnr = v_sap_bus
        ...
        select
            max(to_date(t.b_datum, 'dd.mm.yyyy hh24:mi:ss'))
        from
            s_sqd_sap_send_bew t
        where
                t.firma_nr = io_bew.firma_nr
            and t.herkunft = io_bew.herkunft
        ...
        from
            s_qs_babtec_wee t
        where
                t.bestellnummer = v_order_pos.vorgang_id
    ...
    */
      ...
      -- Dann sind keine Daten in der S_RCV_AUFTR sondern dann nur in der ISI-Order
            ...
            insert into s_smi_send_bew values ( io_bew.firma_nr,             -- FIRMA_NR        NUMBER(3),
      ...
      -- Bei SAP soll nur Fertig gemeldet werden
            ...
            v_aktion := io_bew.aktion;
            if not lvs_p_base.get_lam(v_sid.sid, io_bew.firma_nr, io_bew.lam_id, v_lam) then
                v_lam := null;
            end if;

            if not isi_allg.get_artikel_by_artikel_nr(io_bew.artikel, v_artikel) then
                v_artikel := null;
            end if;
      ...
      -- Dann sind keine Daten in der S_RCV_AUFTR sondern dann nur in der ISI-Order
            ...
            if ( io_bew.aktion = 'KOM' ) then
                open c_get_komm_quelle;
                fetch c_get_komm_quelle into
                    v_q_lte_id,
                    v_q_lte_r_menge;
            ...
            if isi_allg.c_get_firma_cfg_param(v_sid.sid, io_bew.firma_nr, 'CFG',                 -- in_kategorie             in isi_firma_cfg.kategorie%type,
        ...
        -- Relevant. Diese sollen immer geschickt werden
                if
                    0 = nvl(v_s_erp_bew.menge, 0) + nvl(io_bew.menge, 0)
             ...
             -- Bei Euscher SAP muss die Tabelle in der Rueckmeldung S_AUFTR heissen
                ...
                then
                    ...
                    set
                        t.menge = nvl(t.menge, 0) + nvl(io_bew.menge, 0),
             ...
             -- Bei Euscher SAP muss die Tabelle in der Rueckmeldung S_AUFTR heissen
               ...
               -- Bei Euscher SAP muss die Tabelle in der Rueckmeldung S_AUFTR heissen
                    ...
                    if v_s_erp_bew.aktion = 'KOM' then
                        update s_erp_send_bew t
                        set
                            t.menge = nvl(t.menge, 0) + nvl(io_bew.menge, 0),
                            t.menge_b = nvl(t.menge_b, 0) + nvl(io_bew.menge_b, 0),
        ...
        -- -AG- 20200923 - Weitere Paramert in die Schnittstelle geschrieben (LAM_SEL und NR_PRUEFUNG)
                insert into s_erp_send_bew values ( io_bew.firma_nr,             -- FIRMA_NR        NUMBER(3),
                                                    io_bew.herkunft,             -- HERKUNFT        VARCHAR2(3),
                                                    v_bew_tabelle,               -- TABELLE         VARCHAR2(5),
                                                    io_bew.auf_id,               -- AUF_ID          NUMBER,
                                                    'N',                         -- STATUS          VARCHAR2(3),
    ...
    ---------------------------------------------------------------------------------------------
      ...
      -- -AG- 05.01.2009 Im SAP Kommission nicht uebermitteln
            ...
            if v_found then
                if v_aktion = 'BLF' then
                    return;
                end if;
            end if;
            if lvs_p_base.get_lam(v_sid.sid, io_bew.firma_nr, io_bew.lam_id, v_lam) then
                if
                    v_lam.owner_address_id is not null            -- KONSI Bestand
      ...
      -- Bestellung fertig kommt immer nur aus SAP und wird nicht zurückgemeldet
            if v_aktion = 'BEF' then
                return;
            end if;
            if v_aktion in ( 'WAS', 'WAI', 'WEI', 'WAE', 'WEE',
                             'INV', 'WUI', 'KWE', 'KWA', 'LIF' ) then
                if
                    ( v_aktion = 'WEE'
                    or v_aktion = 'KWE'
                    or v_aktion = 'LIF' )
                ...
                then
                    ...
                    fetch c_auftr into v_auftr;
                    close c_auftr;
                    if io_bew.aktion != 'LIF' then
                        if
                            isi_p_order_base.get_order_be_pos(v_sid.sid, v_auftr.auftrag, v_auftr.auftrag, v_auftr.pos_nr, v_order_pos
                            )
                            ...
                            if not v_found then
                                insert into s_qs_babtec_wee (
                                    ...
                                    fehler_text
                                           ...
                                           io_bew.lte_nr,
                                           nvl(v_artikel.preis_standard, v_artikel.preis_gleitend),
                                ...
                                set
                                    t.liefermengeist = nvl(t.liefermengeist, 0) + nvl(io_bew.menge, 0)
                                where
                                        t.bestellnummer = v_order_pos.vorgang_id
                                    and t.bestellposition = v_order_pos.pos_nr
                                    and t.artikel = v_artikel.artikel
                        ...
                        begin
                            if isi_p_order_base.get_order_pos(v_sid.sid, io_bew.order_pos_auf_id, v_order_pos) then
                ...
                if v_found then
                    ...
                    then
                        v_found := false;
                        isi_p_log.db_act_log('s_eus_sap_send_bew', 'insert into s_eus_sap_send_bew', 'insert into s_eus_sap_send_bew'
                        , 'insert', 'insert into wegen gesperrter Ware.');
                    else
            -- Bei SAP führen 0 Mengen buchungen zum Fehler daher nicht schreiben
            -- IST_BESTAND ist nur bei Bestandsabgleich und Gezählter Inventur
            -- Relevant. Diese sollen immer geschickt werden
                        if
                            0 = nvl(v_s_eus_sap_bew.menge, 0) + nvl(io_bew.menge, 0)
        ...
        -- -AG- 2010.05.05 Erweiterung Proj 5356-2010
                ...
                or (
                    ...
                    if v_lam.labor_status != c.lab_stat_f then
                        insert into s_eus_sap_send_bew     -- Aus dem gesperretn Bestand auf dem Host in den freien buchen
                         values ( io_bew.firma_nr,             -- FIRMA_NR        NUMBER(3),
                                                                io_bew.herkunft,             -- HERKUNFT        VARCHAR2(3),
                                                                v_bew_tabelle,               -- TABELLE         VARCHAR2(5),
                                                                io_bew.auf_id,               -- AUF_ID          NUMBER,
                                                                'N',                         -- STATUS          VARCHAR2(3),
                ...
                insert into s_eus_sap_send_bew values ( io_bew.firma_nr,             -- FIRMA_NR        NUMBER(3),
                                                        io_bew.herkunft,             -- HERKUNFT        VARCHAR2(3),
                                                        v_bew_tabelle,               -- TABELLE         VARCHAR2(5),
                                                        io_bew.auf_id,               -- AUF_ID          NUMBER,
                                                        'N',                         -- STATUS          VARCHAR2(3),
                                                        decode(v_aktion, 'WAK', 'WAI',        -- eus_sap kennt keine Kommissionierung Direkt
                    ...
                    if v_lam.labor_status != c.lab_stat_f then
                        ...
                        insert into s_eus_sap_send_bew     -- Jetzt wirder in den Sperrbestand
                         ...
                         values ( io_bew.firma_nr,             -- FIRMA_NR        NUMBER(3),
                                                                ...
                                                                io_bew.lte_nr,               -- LTE_NR          VARCHAR2(20),
                                                                io_bew.lhm_nr,               -- LHM_NR          VARCHAR2(20),
                                                                decode(v_aktion, 'WUI', io_bew.zlagerort, io_bew.lagerort),    -- LAGERORT        VARCHAR2(10),
                                                                null,                        -- ZLAGERORT       VARCHAR2(10),
                                                                io_bew.menge,                -- MENGE           NUMBER(12,3),
                                                                io_bew.menge_b,              -- MENGE_B         NUMBER(12,3),
                                                                io_bew.schrott,              -- SCHROTT         NUMBER(12,3),
    ...
    ---------------------------------------------------------------------------------------------
        ...
        -- (LAM_AG != NULL) übertragen werden !!!
            ...
            if v_aktion != 'WUI' then
                ...
                or v_aktion = 'LHM'             -- Melden an SAP z.B. Menden
                 then
                    if not bde_p_base.get_fa_ag(v_sid.sid, io_bew.firma_nr, io_bew.leitzahl, io_bew.fa_ag, io_bew.fa_upos,
                                                v_bde_fa_ag) then
                        v_bde_fa_ag := null;
                        v_bde_fa_ag.abnr := 0;
                    end if;
                ...
                if v_aktion = 'TF'
                or v_aktion = 'F' then
                    if isi_p_base.get_resource_zust_akt(v_sid.sid, io_bew.res_id, v_res_zust_akt) then
      ...
      -- Bestellung fertig kommt immer nur aus SAP und wird nicht zurückgemeldet
            ...
            if v_aktion = 'WEI' then
                v_nve := '740485532'
                         || substr(io_bew.lhm_nr, 8, 8);
        ...
        -- lagerort mapping für HU-Management
                if v_bde_fa_ag.abnr != 0          -- Mit Kundenrelevanz
                 then
                    if io_bew.lagerort = '1121' then
                        io_bew.lagerort := '1021';
                    elsif io_bew.lagerort = '1122' then
                        io_bew.lagerort := '1022';
                    elsif io_bew.lagerort = '1123' then
                        io_bew.lagerort := '1023';
                    elsif io_bew.lagerort = '1124' then
                        io_bew.lagerort := '1024';
                    end if;

                    if io_bew.zlagerort = '1121' then
                        io_bew.zlagerort := '1021';
                    elsif io_bew.zlagerort = '1122' then
                        io_bew.zlagerort := '1022';
                    elsif io_bew.zlagerort = '1123' then
                        io_bew.zlagerort := '1023';
                    elsif io_bew.zlagerort = '1124' then
          ...
          -- Relevant. Diese sollen immer geschickt werden
                    if
                        0 = nvl(v_s_sqd_sap_bew.menge, 0) + nvl(io_bew.menge, 0)
        ...
        -- Bei SAP führen 0 Mengen buchungen zum Fehler daher nicht schreiben
                ...
                if
                    io_bew.aktion = 'SG'  -- Beim Rüsten nur Rüsten start und Rüsten Ende senden
                    and io_bew.ma_s_grund != 209
                ...
                if
                    ( io_bew.aktion = 'SE'  -- Beim Rüsten nur Rüsten start und Rüsten Ende senden
                    or io_bew.aktion = 'SB' )
                    and io_bew.ma_status = 'R'
                then
                    return;
                ...
                then
                    ...
                    set
                        fa.ruest_zeit_ist = nvl(fa.ruest_zeit_ist, 0) + 1 / 60,
              ...
              -- Ab hier Lagerort Mapping
                            if v_s_sqd_sap_bew.lagerort = '1121' then
                                v_s_sqd_sap_bew.lagerort := '1021';
                            elsif v_s_sqd_sap_bew.lagerort = '1122' then
                                v_s_sqd_sap_bew.lagerort := '1022';
                            elsif v_s_sqd_sap_bew.lagerort = '1123' then
                                v_s_sqd_sap_bew.lagerort := '1023';
                            elsif v_s_sqd_sap_bew.lagerort = '1124' then
                                v_s_sqd_sap_bew.lagerort := '1024';
                            end if;

                            if v_s_sqd_sap_bew.zlagerort = '1121' then
                                v_s_sqd_sap_bew.zlagerort := '1021';
                            elsif v_s_sqd_sap_bew.zlagerort = '1122' then
                                v_s_sqd_sap_bew.zlagerort := '1022';
                            elsif v_s_sqd_sap_bew.zlagerort = '1123' then
                                v_s_sqd_sap_bew.zlagerort := '1023';
                            elsif v_s_sqd_sap_bew.zlagerort = '1124' then
                                v_s_sqd_sap_bew.zlagerort := '1024';
                            end if;

              -- Artikel passend lesen
                            if not isi_allg.get_artikel_by_artikel_nr(v_s_sqd_sap_bew.artikel, v_artikel) then
                                v_artikel := null;
                            end if;
              ...
              -- In Tabelle schreiben (WUI)
                            ...
                            insert into s_sqd_sap_send_bew values ( io_bew.firma_nr,             -- FIRMA_NR        NUMBER(3),
                                                                    io_bew.herkunft,             -- HERKUNFT        VARCHAR2(3),
                                                                    v_bew_tabelle,               -- TABELLE         VARCHAR2(5),
                                                                    io_bew.auf_id,               -- AUF_ID          NUMBER,
                                                                    'N',                         -- STATUS          VARCHAR2(3),
          ...
          -- Alle offenen Einträge schicken (XPE und XPS sind hier die nicht gesendeten Rückmeldungen Produktion)
            ...
            -- werden, da SAP sonst nicht buchen kann
                        ...
                        loop
                            ...
                            begin -- Bei Fehler, diese markieren und weiter machen
                                ...
                                v_nve := lvs_p_lte_lhm.lvs_lte_lhm_pruefziffer_mod10(v_nve);
                                if v_aktion <> 'LHM'
                                or v_nve = v_s_sqd_sap_bew_pe.lhm_nr then
                                    v_nve := v_s_sqd_sap_bew_pe.lhm_nr;
                                    if io_bew.ma_s_grund > 200 then
                                        select
                                            seq_s_send_bew_id.nextval
                                        into v_bew_id
                                        from
                                        ...
                                        set
                                            t.status = 'NW',            -- Alle offenen Einträge schicken
                            ...
                            exception
                                when others then
                                    update s_sqd_sap_send_bew t        -- Bei Auftragsende oder Palettenwechsel
                                    set
                                        t.status = 'ERR'            -- Merken hier ist ein Fehler
                                    where
                                            t.status = 'W'
                                        and t.leitzahl = io_bew.leitzahl
                                        and t.fa_ag = io_bew.fa_ag
            ...
            -- -AG- BugFix: Nur in diesen Fällen den Wareneingang buchen
                        if v_aktion = 'PE'
                        or v_aktion = 'LTE' then
              -- Wareneingann jetzt übertragen
                            update s_sqd_sap_send_bew t        -- Bei Auftragsende oder Palettenwechsel
                            set
                                t.status = 'N',             -- Alle offenen Einträge schicken
              ...
              -- Wareneingann jetzt übertragen
                    ...
                    or (
                        v_aktion != 'WEI'             -- HU nur senden, wenn Wareneingang
                        and v_aktion != 'WUI'
                    )            --    oder Umlagen
                     then
                        io_bew.lte_nr := null;
                ...
                if v_aktion = 'WUI'                  -- Umlagerung Intern dann karton zur Übergabe aktivieren (WEI)
                 ...
                 then
                    if isi_p_base.get_resource_zust_akt(v_sid.sid, io_bew.res_id, v_res_zust_akt) then
                        ...
                        if v_res_zust_akt.status_id > 200 then
                            select
                                seq_s_send_bew_id.nextval
                            into v_bew_id
                            from
        ...
        -- lagerort mapping für HU-Management
          ...
          -- -AG- 2011.06.08Pruefen ob fue AG 20 (= Ruesten) schon ein eintrag gebucht wenn nicht dann ergebnis NULL
                ...
                if
                    v_aktion = 'PE' -- Darf nur gesendet werden, wenn Auftragsmenge erreicht
                ...
                if
                    v_aktion = 'PE' -- Produktionsende währen Störung
                ...
                then
                    ...
                    close c_s_sqd_sap_send_bew_tc;
                    v_max_date_tc := nvl(v_max_date_tc, io_bew.b_datum);
                    if v_max_date_tc >= io_bew.b_datum then
                        select
                            seq_s_send_bew_id.nextval
                        into v_bew_id
                        from
                ...
                v_bew_id := io_bew.bew_id;
                v_max_date_tc := io_bew.b_datum;
                if v_aktion in ( 'SG', 'PS', 'SB', 'PE', 'SE' ) then
                    open c_s_sqd_sap_send_bew_tc;
                    fetch c_s_sqd_sap_send_bew_tc into v_max_date_tc;
                    close c_s_sqd_sap_send_bew_tc;
                    v_max_date_tc := nvl(v_max_date_tc, io_bew.b_datum);
                ...
                insert into s_sqd_sap_send_bew values ( io_bew.firma_nr,             -- FIRMA_NR        NUMBER(3),
                                                        ...
                                                        io_bew.auf_id,               -- AUF_ID          NUMBER,
                                                        v_s_sqd_sap_bew.status,      -- STATUS          VARCHAR2(3),
                                                        decode(v_aktion, 'WAK', 'WAI',        -- sqd_sap kennt keine Kommissionierung Direkt
        ...
        if v_sid.sid_schnittstelle = 'NAV_RRK' then
            if io_bew.aktion = 'WAI' then
        ...
        if v_sid.sid_schnittstelle = 'ACHILLES115' then
            ...
            or io_bew.aktion = 'WAK' then
                v_aktion := 'WAE';
            end if;

            if io_bew.aktion = 'WEI'
            or io_bew.aktion = 'WEK' then
                v_aktion := 'WEE';
            end if;

            v_bestand := io_bew.ist_bestand;
            if io_bew.aktion = 'INV' then
                if io_bew.menge < 0 then
                    v_bestand := 0;
                else
            ...
            insert into s_ach_send_bew values ( io_bew.firma_nr,             -- FIRMA_NR        NUMBER(3),
                                                io_bew.herkunft,             -- HERKUNFT        VARCHAR2(3),
                                                v_bew_tabelle,               -- TABELLE         VARCHAR2(5),
                                                io_bew.auf_id,               -- AUF_ID          NUMBER,
        ...
        if v_sid.sid_schnittstelle = 'HUF' then
            if io_bew.aktion = 'L' then
                if io_bew.tabelle = 'S_AUF' then
                    delete s_rcv_auftr s
                    where
                        s.auf_id = io_bew.auf_id;
        ...
        then
          ...
          return;
            ...
            fetch c_arbeitsplatz into v_arbeitsplatz;
            close c_arbeitsplatz;
            insert into s_huf_send_bew t values ( v_aktion,                 -- BEWEGUNGSART NUMBER(2) not null,
                                                  ...
                                                  nvl(v_arbeitsplatz.orts_kz, 'KEINE'),            -- USERABT      VARCHAR2(40),
                                                  1 );                      -- UPDNR        NUMBER default 1 not null
        end if;

        if v_sid.sid_schnittstelle = 'ESSEX' then
            ...
            if (
                ...
                if
                    io_bew.aktion = 'WAI'                         -- Warenabgang Intern
            ...
            if io_bew.lte_nr like 'LTE_VL%' then
                v_status := '0';           -- Palletierlager

                select
                    nvl(t.res_ziel_lte_id, t.lte_id)
                into io_bew.lte_nr
                from
                    lvs_lam t
                where
                    t.lhm_id = io_bew.lhm_nr;
            ...
            insert into s_essex_send_bew t values ( io_bew.send_id,      -- SEND_ID     NUMBER not null,
        ...
        if v_sid.sid_schnittstelle = 'SAS_SAP' then
            ...
            close c_scanner;
            if
                io_bew.lagerort != 'WBBL' -- Nur auf diesen Lagerorten Buchen bei sasol
      ...
      -- Hier kommt immer nur ein Scann je Palette
            ...
            close c_bde_fa_kopf;
            v_aktion := null;
            if io_bew.aktion = 'WEI' then
                v_aktion := 'Z_SCANS_WE_PRODUKTION';
                v_sap_bus := '1';
                if v_bde_fa_kopf.erzeuger = 'HOST' then
                    v_aktion := 'Z_SCANS_WE_PRAUF';
                end if;
            elsif io_bew.aktion = 'WAI' then
      ...
      -- Z_SCANS_VERBRAUCH_PRODUKTION (BUS 4)
            ...
            if v_aktion is not null then
                ...
                if v_found then
                    ...
                    set
                        t.slb_menge = t.slb_menge + v_menge,
                        t.slb_last_scan = io_bew.b_datum
                    where
                        t.send_id = v_s_sas_sap_bew.send_id;

                    io_bew.send_id := v_s_sas_sap_bew.send_id;
                else
                    select
                        seq_sas_sap_job_id.nextval
                    into io_bew.send_id
                    from
                        dual;

                    insert into s_sas_sap_send_bew values ( io_bew.send_id,             -- JOB_ID             NUMBER not null,
  ...
  --******************************************************************************
    ...
    --v_result    number;
        ...
        from
            s_send_bew bew
        where
                bew.lam_bh_id = v_lam_bh.lam_bh_id
    ...
    -- Jetzt erst mal die Lagerdaten lesen. Wenn etwas fehlt sofort abbrechen !!!
        ...
        if v_found then
            ...
            if not v_found then
                v_err_nr := 10;
                v_err_text := 'Fehler: Lagerort '''
                              || nvl(
                    to_char(v_lgr.lgr_ort),
                    '<NULL>'
                )
                              || ''' fehlt.';
    ...
    -- Jetzt alle Materialbestände dieser Palette lesen
      ...
      -- Jetzt die passende Zugansbuchung lesen
        ...
        -- Ist in dieser Buchung ist noch kein Lagerplatz erfasst dann eintragen
                if v_lam_bh.lgr_platz is null then
                    update lvs_lam_bh bh
                    set
                        bh.lgr_platz = in_lte.lgr_platz,
                ...
                if v_lam_bh.abnr_extern is null then
                    v_tabelle := null;
                else
          -- Bei Euscher SAP muss die Tabelle in der Rueckmeldung S_AUFTR heissen
                    if v_sid.sid_schnittstelle = 'EUS_SAP' then
                ...
                v_err_nr := 30;
                v_err_text := 'Fehler: Lagerzugangsbuchung für lam_ID:  '''
                              || nvl(
                    to_char(v_lam.lam_id),
                    '<NULL>'
                )
                              || ''' fehlt.';
      ...
      -- Eintrag gefunden und noch nicht gesendet (bew.status is NULL)
        ...
        -- Evtl. muss noch ein ISI-Ordereintrag aktuallisiert werden
                if v_found then
                    if
                        v_order_pos.leitzahl > 0
                        and v_order_pos.fa_ag > 0
                    then
                        update bde_fa_auftrag fa
                        set
                            fa.ag_ist_mg = nvl(fa.ag_ist_mg, 0) + v_lam.menge
                        where
                                fa.sid = in_lte.sid
  ...
  --******************************************************************************
    ...
    -- -AG- 20190917 - Einbau mitarbeiteranmeldung an Maschine zu FA mit Arbeitszeiterfassung
        ...
        from
            lvs_lgr lgr
        where
                lgr.lgr_platz = in_lam.lgr_platz
            and lgr.sid = in_lam.sid;
    ...
    begin
        ...
        if in_lam_bh_bus is null then
            if not isi_p_base.get_resource_zust_akt(in_sid, in_fa_auftrag.res_id, v_res_zus) then
                v_res_zus := null;
                v_res_zus.status_id := 0;
            end if;

            if isi_allg.c_get_firma_cfg_param(in_sid, in_firma_nr, 'BDE_FA_AB',           -- in_kategorie             in isi_firma_cfg.kategorie%type,
  ...
  --******************************************************************************
    ...
    begin
        insert into s_mfr_rcv_auftr values ( in_firma,            -- FIRMA_NR        NUMBER(2) not null,
  ...
  --******************************************************************************
    ...
    begin
        insert into s_mfr_send_offline values ( in_firma,            -- FIRMA_NR   NUMBER(2) not null,
...
-- sqlcl_snapshot {"hash":"37582f755eb61da95757ddea7919298a0e3614ec","type":"PACKAGE_BODY","name":"S_SCHNITTSTELLE","schemaName":"DIRKSPZM32","sxml":""}