create or replace 
procedure DIRKSPZM32.aps_c_look_for_relation(in_auftrag_nr in aps_order_materialrelation.auftrag_nr%type,
  ...
  CURSOR c_get_aps_order_pos_x is
    select max(op.hersteller_kuerzel_liste) hersteller_kuerzel_liste
      from aps_order_materialrelation mr,
           aps_order_auftr_pos op,
           aps_plan_ergebnis pe
     where mr.auftrag_nr = in_auftrag_nr
       and mr.pos_nr <= 4
  ...
  CURSOR c_check_bestand is
    select lam.hersteller_kuerzel_liste, 
           apmr.*
      from aps_order_materialrelation apmr,
           lvs_lam lam,
           lvs_lgr lgr,
           lvs_lte lte
     where apmr.auftrag_nr = v_aps.auftrag_nr
       ...
       and not exists (select * from aps_order_materialrelation apmrx where apmrx.child_id = apmr.child_id and apmrx.materialrelation_type = apmr.materialrelation_type and apmrx.aps_plan_status = in_aps_plan_status)
       and (   (nvl(lam.hersteller_kuerzel_liste, 'Keiner') = nvl(nvl(v_hersteller_liste_akt, lam.hersteller_kuerzel_liste), 'Keiner') and apmr.pos_nr <= 4)
            or (nvl(lam.hersteller_kuerzel_liste, 'Keiner') = nvl(nvl(v_hersteller_pos_5, lam.hersteller_kuerzel_liste), 'Keiner') and apmr.pos_nr = 5)
           )
  ...
  CURSOR c_get_alternativ_lte is
    select --+rule
      ...
      from aps_order_auftr_pos ap,
           ...
           lvs_lgr lgr,
           lvs_lte lte
     where ap.artikel_id = v_aps.artikel_id
  ...
  CURSOR c_get_fa_tf is
    ...
    order by op.auftrag_nr, op.pos_nr, op.upos_nr;
  v_bde_fa_plan                               c_get_fa_tf%rowtype;

  CURSOR c_herst_art is
    select stradd_distinct(ah.herstellerkuerzel) || ';' herstellerkuerzel_liste,
           count(ah.herstellerkuerzel) anz
      from isi_artikel_hersteller ah
     where ah.artikel_id = in_artikel_id;
...
begin
  if nvl(in_aps_plan_status, 'X') != 'PS'
  then
    return;
  end if;
  ...
  LOOP
    ...
    update aps_order_materialrelation apmr
       set apmr.materialrelation_type = 8,
           apmr.child_id = v_fa_bestand.lte_id,
           apmr.child_artikel_id = v_fa_bestand.artikel_id,
           apmr.materialrelation_info = 'Valide Lagerbestand aus aktueller Produktion Fertigungsauftrag: ' || v_fa_bestand.leitzahl || ' LTE_ID: ' || v_fa_bestand.lte_id ||  ' übernommen',
    ...
    update aps_order_auftr_pos ap
       set ap.hersteller_kuerzel_liste = substr(v_fa_bestand.hersteller_kuerzel_liste, 1, 2)
     where ap.aps_plan_status = in_aps_plan_status
       and ap.auftrag_nr = v_fa_bestand.auftrag_nr
       and (ap.pos_nr = v_fa_bestand.pos_nr  or v_reifen_artikel_p_4_5_gleich = 'T');
  ...
  */
  ...
  if b_reifen_artikel_p_4_5_gleich
  then
  ...
  then
    ...
    then -- Nein dan suchen
      ...
      then
        ...
        v_hersteller_liste_akt := aps_look_for_hersteller(in_auftrag_nr,
                                                          in_pos_nr,
                                                          in_aps_plan_status);
      end if;
    ...
    then
      update aps_order_auftr_pos ap
         set ap.hersteller_kuerzel_liste = substr(v_hersteller_liste_akt, 1, 2)
       where ap.aps_plan_status = in_aps_plan_status
         and ap.auftrag_nr = in_auftrag_nr
         and (ap.pos_nr = in_pos_nr or v_reifen_artikel_p_4_5_gleich = 'T');   -- Pos 5 oder alle wenn Reifen POS4 und 5 gleicher Artikel
      --update Z_DIR_AUSL_ROBER ar
  ...
  if v_found -- Aus Auftrag kann eine relation übernommen werden
  and v_hersteller_liste_plan is not NULL
  then
    update aps_order_auftr_pos ap
       set ap.hersteller_kuerzel_liste = substr(v_hersteller_liste_plan, 1, 2)
     where ap.aps_plan_status = in_aps_plan_status
       and ap.auftrag_nr = in_auftrag_nr
       and (ap.pos_nr <= 4 or v_reifen_artikel_p_4_5_gleich = 'T');   -- Pos 5 oder alle wenn Reifen POS4 und 5 gleicher Artikel
    --update Z_DIR_AUSL_ROBER ar
  ...
  if v_hersteller_liste_akt is not NULL
  then
    update aps_order_auftr_pos ap
       set ap.hersteller_kuerzel_liste = substr(v_hersteller_liste_akt, 1, 2)
     where ap.aps_plan_status = in_aps_plan_status
    ...
    then
      update aps_order_materialrelation apmr
         set apmr.materialrelation_type = v_aps_mr.materialrelation_type,
             ...
             apmr.materialrelation_info = 'Valide aus der Vorplanung mit Lagerbestand aus ' || v_aps_mr.child_id ||  ' übernommen P4_5 Gleich ' || v_reifen_artikel_p_4_5_gleich || ' - ' || v_artikel_p6 || ' Hersteller=<' || v_hersteller_liste_akt ||'> Hersteller LAM=' || v_aps_mr.hersteller_kuerzel_liste,
             apmr.materialrelation_valide = 'T'
       where apmr.aps_plan_status = in_aps_plan_status
         and apmr.auftrag_nr = v_aps_mr.auftrag_nr
         and apmr.pos_nr = v_aps_mr.pos_nr
         ...
         and apmr.aps_plan_status = in_aps_plan_status;
      update aps_order_auftr_pos ap
         set ap.hersteller_kuerzel_liste = v_aps_mr.hersteller_kuerzel_liste
       where ap.aps_plan_status = in_aps_plan_status
         and ap.auftrag_nr = v_aps_mr.auftrag_nr
         and (ap.pos_nr = v_aps_mr.pos_nr or (ap.pos_nr <= 4 and v_aps_mr.pos_nr <= 4) or (v_reifen_artikel_p_4_5_gleich = 'T'));
      --update Z_DIR_AUSL_ROBER ar
      ...
      then
        update aps_order_materialrelation apmr
           set apmr.materialrelation_type = v_aps_mr.materialrelation_type,
               apmr.child_id = v_aps_mr.child_id,
               apmr.child_artikel_id = nvl(v_aps_mr.child_artikel_id, v_aps.artikel_id),
               apmr.materialrelation_info = 'Valide alternative aus der Vorplanung mit Lagerbestand aus ' || v_aps_mr.child_id ||  ' übernommen. P4_5 Gleich ' || v_reifen_artikel_p_4_5_gleich || ' - ' || v_artikel_p6 || ' Hersteller=<' || v_hersteller_liste_akt ||'> Hersteller LAM=' || v_aps_mr.hersteller_kuerzel_liste,
               apmr.materialrelation_valide = 'T'
         where apmr.auftrag_nr = v_aps.auftrag_nr
           and apmr.pos_nr = v_aps.pos_nr
           and apmr.upos_nr = v_aps.upos_nr
        ...
        update aps_order_auftr_pos ap
         ...
         where ap.aps_plan_status = in_aps_plan_status
           ...
           and (ap.pos_nr = v_aps.pos_nr or (ap.pos_nr <= 4 and v_aps.pos_nr <= 4) or (v_reifen_artikel_p_4_5_gleich = 'T'));   -- Pos 5 oder alle wenn Reifen POS4 und 5 gleicher Artikel);
        --update Z_DIR_AUSL_ROBER ar
        --   set ar.hersteller = v_aps_mr.hersteller_kuerzel_liste
        -- where ar.auftrag = v_aps.auftrag_nr
        ...
        update aps_order_materialrelation apmr
           set apmr.child_artikel_id = v_aps.artikel_id,
...
-- sqlcl_snapshot {"hash":"b899c14fb21fa5e2a0cdb98bebca901a9503eda3","type":"PROCEDURE","name":"APS_C_LOOK_FOR_RELATION","schemaName":"DIRKSPZM32","sxml":""}