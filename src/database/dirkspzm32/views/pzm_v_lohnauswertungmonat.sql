create or replace force editionable view dirkspzm32.pzm_v_lohnauswertungmonat (
    ...
    with a as (
        select
            ...
            case
                when pb.pb_extern = 'T' then
                    null
                else
                    ts.ts_aa_id
            end                                                       aa_id,  -- (1) pb_extern aus JOIN
            round(
                nvl(sa.sa_std_pro_tag, sm.d_arb_std_pro_tag),
                3
            )                                                         ts_day_soll_std,
    ...
    -- (4) nur "genehmigte" Ueberstunden ausgeben
            case
                when ts.ts_ueb_ok_datum is null then
                    0
                when ts.ts_ueb_storno_datum is null then
                    nvl(ts.ts_day_ueb_std, 0)
                else
                    0
            end                                                       ts_day_ueb_std,
    ...
    -- Gesamte Arbeitsstunden inkl. genehmigter Ueber- und Flexstunden
            nvl(ts.ts_day_arb_std, 0) + nvl(ts.ts_day_korr_std, 0) +
            case
                    when ts.ts_ueb_ok_datum is null then
                        0
                    when ts.ts_ueb_storno_datum is null then
                        nvl(ts.ts_day_ueb_std, 0) + nvl(ts.ts_day_flex_std, 0)
                    else
                        0
            end
            ts_ges_arb_std,
    -- (2) vier ZE-Scans ¿ ein JOIN + konditionaler LISTAGG
            listagg(
                case
                    when ze.ze_aa_status is null
                         or pb.pb_extern = 'F' then
                            case
                                when ze.ze_ist_start is null then
                                    ' '
                                else
            ...
            listagg(
                case
                    when ze.ze_aa_status is null
                         or pb.pb_extern = 'F' then
                            case
                                when ze.ze_calc_ist_start is null then
                                    ' '
                                else
            ...
            listagg(
                case
                    when ze.ze_aa_status is null
                         or pb.pb_extern = 'F' then
                            case
                                when ze.ze_aa_status is null then
                                    ' '
                                else
                                    nvl(aa.aa_kurzname,
                                        to_char(ze.ze_aa_status))
                            end
                end,
                cr_lf()) within group(
            order by
    ...
    -- loa_list_cr bleibt als Subquery (andere Quelle: pzm_ze_loa_ausw)
        ...
        group by
            ...
            ts.ts_aa_id,
            ts.ts_sa_kurzname,
            sm.d_arb_std_pro_tag,
            sa.sa_std_pro_tag,
            ts.ts_day_ist_start,
            ts.ts_day_ist_ende,
            ts.ts_day_wert_start,
            ts.ts_day_wert_ende,
            ts.ts_day_arb_std,
            ts.ts_day_abw_std,
            ts.ts_day_pause_bez_std,
            ts.ts_day_pause_std,
            ts.ts_day_ueb_std,
            ts.ts_day_flex_std,
            ts.ts_ueb_ok_datum,
            ts.ts_ueb_storno_datum,
            ts.ts_day_korr_std
        order by
            ts.ts_datum
    )
    select
     ...
     --ShiftDayOfWeek muss in Dot.Net abgeleitet werden wegen aktiver Spracheinstellung des Frontend (Deutsch/Englisch,Polnisch, etc)
        ...
        case
            when a.ts_ges_arb_std = 0 then
                null
            else
                case
                    when lz.lz_operator in ( 'U', 'K', 'SU' ) then
                            null      -- Nicht berechnen bei Urlaub, Krankheit und SonderUrlaub 
                    else
                        a.ts_ges_arb_std - a.ts_day_soll_std
                end
