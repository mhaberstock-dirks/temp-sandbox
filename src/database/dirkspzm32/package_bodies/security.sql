create or replace package body dirkspzm32.security is
  ...
  *******************************************************************************/
    ...
    begin
        ...
        if in_section_name is null
           or v_section_id is null then
  ...
  *******************************************************************************/
    ...
    begin
        ...
        open c_module_info;
        fetch c_module_info into v_module_info;
        if c_module_info%found then
            ...
            select
                seq_sec_module_id.nextval
            into out_module_id
            from
                dual;

            if out_module_id < 900 then
            ...
            insert into sec_module_info values ( out_module_id,
                                                 in_module_name,
                                                 out_module_caption,
  ...
  *                             ^
  *******************************************************************************/
    procedure c_check_section_exeistence (
    ...
    begin
        c_check_section_existence(in_module_id, in_section_name, io_section_description, out_section_caption, out_section_help_file,
                                  out_section_id);
    end;
  /*******************************************************************************
  ...
  *******************************************************************************/
    procedure c_check_section_existence (
        in_module_id           in number,
        in_section_name        in varchar2,
        io_section_description in out varchar2,
        out_section_caption    out varchar2,
    ...
    begin
        ...
        open c_section_info;
        fetch c_section_info into v_section_info;
        if c_section_info%found then
  ...
  *******************************************************************************/
    procedure c_check_action_existence (
    ...
    --end if;
        ...
        open c_action_info;
        fetch c_action_info into v_action_info;
        ...
        if v_found then
            ...
            out_action_id := null;
            if io_action_caption is not null then
                select
                    seq_sec_action_id.nextval
                into out_action_id
                from
                    dual;

                if out_action_id < 900 then
                    raise_application_error(-20001, 'Applikationen der Version 3.3 dürfen keine neuen Actions mit ID < 900 eintragen.'
                    );
                end if;
                insert into sec_action_info values ( out_action_id,
                                                     in_section_id,
                                                     in_action_name,
                                                     io_action_caption,
  ...
  -------------------------------------------------------------------------------------------------------
    ...
    begin
        ...
        if not v_found then
            v_err_nr := 10;
            v_err_text := 'Fehler: Firma mit SID: '
                          || in_sid
                          || ' Firma Nr.: '
                          || to_char(in_firma_nr)
                          || ' fehlt.';
  ...
  * procedure c_check_module_existense (...)
  *******************************************************************************/
    procedure c_check_module_existense_34 (
        in_module_name         in varchar2,
    ...
    begin
        ...
        -- erst mal schauen, ob die ID bereits vergeben ist
            ...
            insert into sec_module_info values ( in_module_id,
                                                 in_module_name,
                                                 in_module_caption,
  ...
  *******************************************************************************/
    procedure c_check_section_existence_34 (
        in_mod_id              in number,
        in_section_name        in varchar2,
    ...
    begin
      ...
      --then
            update sec_section_info t
            set
                t.name = in_section_name,
                t.caption = in_section_caption,
                t.description = in_section_description,
                t.lang_cn_caption = in_lang_cn_caption,
                t.lang_cn_description = in_lang_cn_description
            where
                t.section_id = v_section_info.section_id;
  ...
  *******************************************************************************/
    procedure c_check_action_existence_34 (
        in_mod_id              in number,
        in_action_name         in varchar2,
        ...
        where
            ( t.caption = in_action_caption
              or t.name = in_action_name )
            and t.section_id = si.section_id
            and ( si.name = in_category
                  or ( si.mod_id = in_mod_id
                       and si.lang_cn_caption like '_MOD_%' ) );
    ...
    */
      ...
      --then
            ...
            set
                ...
                t.lang_cn_description = in_lang_cn_description
            where
                t.action_id = v_action_info.action_id;
        ...
        when others then
            ...
            raise_application_error(-20001,
                ...
                '(NULL)'
                                    ...
                                    || ' ActionName: '
                                    || nvl(in_action_name, '(NULL)')
                                    || ' Category: '
                                    || nvl(in_category, '(KEINE)'),
                                    true);
  ...
  *******************************************************************************/
    ...
    begin
        ...
        loop
            fetch c_enabled_modules into v_module_id;
            exit when c_enabled_modules%notfound;
            if out_cs_module_id_list is null then
                out_cs_module_id_list := to_char(v_module_id);
            else
                out_cs_module_id_list := out_cs_module_id_list
                                         || in_separator
                                         || to_char(v_module_id);
            end if;
  ...
  *******************************************************************************/
    ...
    begin
        ...
        loop
            fetch c_enabled_sections into v_section_id;
            exit when c_enabled_sections%notfound;
            if out_cs_section_id_list is null then
                out_cs_section_id_list := to_char(v_section_id);
            else
                out_cs_section_id_list := out_cs_section_id_list
                                          || in_separator
  ...
  *******************************************************************************/
    ...
    begin
        ...
        loop
            fetch c_enabled_actions into v_action_id;
            exit when c_enabled_actions%notfound;
            if out_cs_action_id_list is null then
                out_cs_action_id_list := to_char(v_action_id);
            else
                out_cs_action_id_list := out_cs_action_id_list
                                         || in_separator
                                         || to_char(v_action_id);
            end if;
  ...
  *******************************************************************************/
    function is_user_valid (
        ...
        from
            isi_user
        where
                lower(username) = lower(in_username)
    ...
    begin
        ...
        if c_isi_user%found then
            v_result := true;
            out_sid := v_user.sid;
            out_login_id := v_user.login_id;
            out_firma_nr := v_user.firma_nr;
            if v_user.pers_nr is not null then
  ...
  *******************************************************************************/
    function is_user_valid (
        ...
        from
            ...
            sec_user_groups
        where
                sec_groups.sid = sec_user_groups.sid
    ...
    begin
        open c_isi_user;
        fetch c_isi_user into out_user_daten;
        v_result := c_isi_user%found;
        close c_isi_user;
        if v_result then
            if out_user_daten.security_level is null then
                open c_group_security_level;
                fetch c_group_security_level into out_user_daten.security_level;
                if c_group_security_level%notfound then
  ...
  *******************************************************************************/
    function is_transponder_valid (
        in_transponder_key in varchar2,
        out_user_daten     out isi_user%rowtype
    ) return boolean is
        ...
        from
            isi_user
        where
            transponder = in_transponder_key;
  ...
  *******************************************************************************/
    ...
    procedure alles_freigeben_fuer_gruppe (
        in_sid      in varchar2,
    ...
    procedure c_delete_action_info (
        in_action_id in sec_action_info.action_id%type
    ) is
    begin
        delete from sec_action_info t
        where
            t.action_id = in_action_id;

        delete from sec_group_actions t
        where
            t.action_id = in_action_id;
...
-- sqlcl_snapshot {"hash":"2377395fd3e1bbb9fdbfea528efb3da0435980c4","type":"PACKAGE_BODY","name":"SECURITY","schemaName":"DIRKSPZM32","sxml":""}