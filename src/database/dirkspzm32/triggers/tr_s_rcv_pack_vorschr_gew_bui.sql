create or replace editionable trigger dirkspzm32.tr_s_rcv_pack_vorschr_gew_bui before
    insert or update on dirkspzm32.s_rcv_pack_vorschr_gew
    for each row
declare
    v_found            boolean;
...
begin
    ...
    if not v_found then
        insert into isi_pack_vorschr_gew values ( :new.sid,
                                                  :new.firma_nr,
                                                  (
                                                      select
                                                          a.artikel_id
...
-- sqlcl_snapshot {"hash":"12eddb9c55ad9cce0ced3aeb7502f569d1468b36","type":"TRIGGER","name":"TR_S_RCV_PACK_VORSCHR_GEW_BUI","schemaName":"DIRKSPZM32","sxml":""}