...
select 1 union_order,
       ...
       nvl(stradd_distinct(to_char(loa_exp.kst_id)), to_char(p.pers_kst_id)) kst_id,
       null sa_kurzname,
       null aa_id,
  ...
  from pzm_ze_loa_exp_host loa_exp
       ...
       left join pzm_personal p
         on p.pers_nr = loa_exp.pers_nr
 group by
       loa_exp.pers_nr,
       trunc(loa_exp.datum),
...
-- sqlcl_snapshot {"hash":"8209d45d4b1e32dcb250aade137331ab66f258ba","type":"VIEW","name":"TEST_PZM_V_ZE_TS_LOA_MONAT_ERP_STAT","schemaName":"DIRKSPZM32","sxml":""}