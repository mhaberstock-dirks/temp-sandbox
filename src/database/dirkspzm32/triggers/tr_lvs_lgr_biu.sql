create or replace editionable trigger dirkspzm32.tr_lvs_lgr_biu before
    update or insert on dirkspzm32.lvs_lgr
    for each row
declare
  /*
  ...
  */
  ...
  -- local variables here
    ...
    from
        lvs_lgr_grp t
    where
            t.lgr_gruppe_id = :new.lgr_gruppe_id
        and t.sid = :new.sid
    ...
    from
        ...
        lvs_fahrzeuge        f
    where
            fg.lgr_gruppe_id = :new.lgr_gruppe_id
...
begin
  ...
  -- -WK- 2007.11.16: Bei Höhenänderungen die freie Höhe automatisch mit verändern
    if inserting then
        ...
        if :new.lgr_typ = c.kanal1 then
            :new.gruppe := :new.lgr_ort
                           || '.'
                           || :new.lgr_dim_r
                           || '.'
                           || :new.lgr_dim_p
        ...
        :new.lgr_dim_platz := ( ( ( :new.lgr_dim_g * 10000 + :new.lgr_dim_r ) * 10000 + :new.lgr_dim_p ) * 10000 + :new.lgr_dim_e ) * 10000
        + :new.lgr_dim_t;

    end if;

    if updating then
        if :new.lgr_order_res_te < 0 then
            :new.lgr_order_res_te := 0;
        end if;

        if
            :old.lgr_gruppe_id != :new.lgr_gruppe_id
        ...
        then
            ...
            loop
                ...
                set
                    t.res_id = v_res_id
                where
                        v_transport.transp_id = t.transp_id
        ...
        or :new.lgr_ort != :old.lgr_ort then
            :new.lgr_dim_platz := ( ( ( :new.lgr_dim_g * 10000 + :new.lgr_dim_r ) * 10000 + :new.lgr_dim_p ) * 10000 + :new.lgr_dim_e
            ) * 10000 + :new.lgr_dim_t;
    ...
    v_max_lte := 0;
    if :new.lgr_gruppe_id is not null then
        open c_lgr_grp;
        ...
        if inserting then
            ...
            or :new.lte_namen_cfg = c.euro || c.te_trenner then
                v_max_lte := nvl(:new.lgr_max_te,
                                 0);
            end if;
        elsif
            updating
      ...
      -- Feld ist Relevant für die Einlagerplatzsuche
            ...
            if :new.lgr_order_res_te > :new.lgr_akt_te then
                :new.lgr_order_res_te := :new.lgr_akt_te;
            end if;
      -- Beim Update hat sich die Gruppe nicht veraendert
            if :new.lgr_gruppe_id = :old.lgr_gruppe_id then
                if (
                    :new.lgr_typ != c.seg1
                    and :new.lgr_typ != c.seg_duedo1
                )
                or :new.lte_namen_cfg = c.euro || c.te_trenner then
                    v_max_lte := nvl(:new.lgr_max_te,
                                     0) - nvl(:old.lgr_max_te,
                                              0);

                end if;
            else
                if
                    :new.lgr_gruppe_id is not null
                    and ( (
                        :new.lgr_typ != c.seg1
                        and :new.lgr_typ != c.seg_duedo1
                    )
                ...
                if :old.lgr_gruppe_id is not null then
                    ...
                    set
                        grp.lgr_gruppe_max_lte = grp.lgr_gruppe_max_lte - nvl(:old.lgr_max_te,
                                                                              0)
                    where
        ...
        if v_found then
            if nvl(v_max_lte, 0) != 0 then
                ...
                where
                        grp.lgr_ort = :new.lgr_ort
                    and grp.lgr_gruppe_id = :new.lgr_gruppe_id;

            end if;
        else
            insert into lvs_lgr_grp values ( :new.sid,                           -- SID                  VARCHAR2(2),
                                             :new.firma_nr,                      -- FIRMA_NR             NUMBER(2),
                                             :new.lgr_gruppe_id,                 -- LGR_GRUPPE_ID        NUMBER not null,
                                             ' Grp: ' || to_char(:new.lgr_gruppe_id), -- LGR_GRUPPE_NAME      VARCHAR2(30),
...
-- sqlcl_snapshot {"hash":"b9b09274063d84cbd5aaaae7416bd4a06758e507","type":"TRIGGER","name":"TR_LVS_LGR_BIU","schemaName":"DIRKSPZM32","sxml":""}