...
package body DIRKSPZM32.db_p_trace is
  ...
  ***************************************************************************************************************************/
  ...
  begin
    select job
      into v_job
      from user_jobs
     where what like '%c_tabelle_db_trace_verklein_in%';
    return v_job;
  ...
  ***************************************************************************************************************************/
  function get_trigger_name(in_tabellenname in varchar2) return varchar2 is
    -------------------------------------------------------------------------------------------------------
    -- Deklarationen
  ...
  begin
    select trigger_name
      into v_tabellenname
      from user_triggers
     where upper(trigger_name) like 'DB_%'
  ...
  ***************************************************************************************************************************/
  function get_trigger_aktiv(in_tabellenname in varchar2) return integer is
    -------------------------------------------------------------------------------------------------------
    -- Deklarationen
  ...
  begin
    select triggeraktiv
      into v_triggeraktiv
      from db_trace_cfg
     where tabellenname = in_tabellenname;
  ...
  ***************************************************************************************************************************/
  function get_pk_schluessel(in_tabellenname in varchar2) return varchar2 is
    -------------------------------------------------------------------------------------------------------
    -- Deklarationen
  ...
  begin
    select listagg(cols.column_name,
                   ';') within group(order by cols.column_name)
      into v_pk_felder
     ...
     where 1 = 1
       ...
       and cons.owner = (select sys_context('USERENV',
                                            'CURRENT_SCHEMA')
                           from dual)
  ...
  ***************************************************************************************************************************/
  function get_spalten_namen(in_sid          in isi_firma.sid%type,
                             in_firma_nr     in isi_firma.firma_nr%type,
                             in_login_id     in isi_user.login_id%type,
                             in_tabellenname in varchar2) return varchar2 is
    -------------------------------------------------------------------------------------------------------
    -- Deklarationen
  ...
  ***************************************************************************************************************************/
  function ersetze_zeichen(in_pk_liste in varchar2,
                           in_art      in varchar2) return varchar2 is
    -------------------------------------------------------------------------------------------------------
    -- Deklarationen
  ...
  begin
    ...
    if length(trim(in_art)) > 0
    then
      v_pk_felder := ':' || trim(in_art) || '.' || v_pk_felder;
      if instr(in_pk_liste,
               ';') > 0
      then
  ...
  Rückgabewert:
     -
  **************************************************************************************************************************/
  procedure c_tabelle_db_trace_verklein_in(in_sid         in isi_firma.sid%type,
                                           in_firma_nr    in isi_firma.firma_nr%type,
                                           in_login_id    in isi_user.login_id%type,
                                           in_ab_tage     in integer) is
  -------------------------------------------------------------------------------------------------------
    -- Deklarationen
  ...
  begin
    isi_p_log.c_isi_system_meldung(in_sid,
                               in_firma_nr,
                               'Tabelle DB_TRACE verkleinern',
                               'ORA-DB',
                               'db_p_trace.c_tabelle_db_trace_verklein_in',
                               NULL,
                               NULL,
                               in_login_id,
                               NULL,
                               NULL,
                               'Start Tabelle DB_TRACE verkleinern',
                               'IL');

    c_tabelle_db_trace_verkleinern(in_sid,
                                   in_firma_nr,
                                   in_login_id,
                                   in_ab_tage,
                                   v_meldung);
    ...
    isi_p_log.c_isi_system_meldung(in_sid,
                               ...
                               NULL,
                               NULL,
                               'Ende Tabelle DB_TRACE verkleinern',
                               'IL');
  commit;
  ...
  **************************************************************************************************************************/
  procedure c_tabelle_db_trace_verkleinern(in_sid         in isi_firma.sid%type,
                                           in_firma_nr    in isi_firma.firma_nr%type,
                                           in_login_id    in isi_user.login_id%type,
                                           in_ab_tage     in integer,
                                           out_meldung    out varchar2) is
    -------------------------------------------------------------------------------------------------------
    -- Deklarationen
    v_ab_tage             integer;
    v_ab_datum            date;
    ...
    -------------------------------------------------------------------------------------------------------
  begin
    if in_ab_tage < 0 or in_ab_tage is NULL
    then
      v_ab_tage := 30;
    else
    ...
    fetch curs_tabelle_act_cfg
      into rec_act_cfg;

    WHILE curs_tabelle_act_cfg%found
    LOOP
      if (v_ab_tage <= rec_act_cfg.triggeraktiv)
      then
        v_ab_tage := v_ab_tage;
      else
      ...
      delete from db_trace
       where act_table = rec_act_cfg.tabellenname
    ...
    if (length(trim(v_liste_tabellennamen)) > 0)
    then
      out_meldung := 'Die Einträge der Tabellen ' || v_liste_tabellennamen ||
    ...
    isi_p_log.c_isi_system_meldung(in_sid,
                                   in_firma_nr,
                                   'TRACE Tabelle DB_TRACE verkleinern',
                                   'ORA-DB',
                                   'db_p_trace.c_tabelle_db_trace_verkleinern',
                                   NULL,
                                   NULL,
                                   in_login_id,
                                   NULL,
    ...
    */
    ...
    v_liste_tabellennamen := rtrim(v_liste_tabellennamen,
                                   ',');
    delete from db_trace
     where act_table not in v_liste_tabellennamen
       and log_date < v_ab_datum;
    ...
    isi_p_log.c_isi_system_meldung(in_sid,
                                   ...
                                   NULL,
                                   NULL,
                                   'Es wurden '||SQL%ROWCOUNT||' Datensätze gelöscht von Tabellen die nicht in der Tabelle db_trace_cfg vorhanden sind.',
                                   'IL');
  ...
  exception
    when others then
      if curs_tabelle_act_cfg%isopen
      then
        CLOSE curs_tabelle_act_cfg;
        ...
        then
          out_meldung := 'Fehler ' || sqlerrm(sqlcode) || '. ' ||
                         out_meldung;
          isi_p_log.c_isi_system_meldung(in_sid,
                                     in_firma_nr,
                                     'TRACE Tabelle DB_TRACE verkleinern',
  ...
  Rückgabewert:
     out_meldung = Eine Meldung wird zurück gegeben
  **************************************************************************************************************************/
  procedure c_trigger_deaktivieren(in_sid         in isi_firma.sid%type,
                                   in_firma_nr    in isi_firma.firma_nr%type,
                                   in_login_id    in isi_user.login_id%type,
    ...
    -------------------------------------------------------------------------------------------------------
  begin
    if in_tabellename is not NULL or length(trim(in_tabellename)) > 0
    then
      update db_trace_cfg t
         set t.triggeraktiv = 0
       where t.tabellenname = in_tabellename;
    else
      update db_trace_cfg t set t.triggeraktiv = 0;
    end if;
    COMMIT;
    ...
    isi_p_log.c_isi_system_meldung(in_sid,
                                   ...
                                   NULL,
                                   NULL,
                                   'gestartet ' || to_char(sysdate, 'yyyy-mm-dd hh24') || ' für Tabelle ' || nvl(in_tabellename, 'ALL'),
                                   'IL');
  ...
  **************************************************************************************************************************/
  procedure c_trigger_aktivieren(in_sid         in isi_firma.sid%type,
                                 in_firma_nr    in isi_firma.firma_nr%type,
                                 in_login_id    in isi_user.login_id%type,
                                 in_tabellename in varchar2,
                                 in_wert        in integer,
    ...
    -------------------------------------------------------------------------------------------------------
  begin
    if in_wert <= 0 or in_wert is NULL
    then
      v_wert := 30;
    else
      v_wert := in_wert;
    ...
    then
      update db_trace_cfg t
         set t.triggeraktiv = v_wert
       where t.tabellenname = in_tabellename;
    else
      update db_trace_cfg t set t.triggeraktiv = v_wert;
    end if;
    COMMIT;
  ...
  exception
    when others then
      ...
      then
        ...
        isi_p_log.c_isi_system_meldung(in_sid,
                                       in_firma_nr,
                                       'TRACE  Trigger Aktivieren',
                                       'ORA-DB',
                                       'db_p_trace.c_trigger_aktivieren',
                                       NULL,
                                       NULL,
                                       in_login_id,
                                       NULL,
  ...
  **************************************************************************************************************************/
  procedure c_trigger_loeschen_alle(in_sid         in isi_firma.sid%type,
                                    in_firma_nr    in isi_firma.firma_nr%type,
                                    in_login_id    in isi_user.login_id%type,
                                    out_meldung out varchar2) is
    -------------------------------------------------------------------------------------------------------
  ...
  begin
    ...
    fetch curs_user_triggers
      into rec_user_triggers;

    WHILE curs_user_triggers%found
    LOOP
    ...
    isi_p_log.c_isi_system_meldung(in_sid,
                                     in_firma_nr,
                                     'TRACE  Alle Trigger löschen',
                                     'ORA-DB',
                                     'db_p_trace.c_trigger_loeschen_alle',
                                     NULL,
                                     NULL,
                                     in_login_id,
                                     NULL,
  ...
  Rückgabewert:
     out_meldung = Info- oder Fehlermeldung wird zurück gegeben
  **************************************************************************************************************************/
  procedure c_trigger_loeschen(in_sid         in isi_firma.sid%type,
                               in_firma_nr    in isi_firma.firma_nr%type,
                               in_login_id    in isi_user.login_id%type,
                               in_tabellename in varchar2,
                               out_meldung    out varchar2) is
  ...
  begin
    ...
    then
      ...
      if v_trigger_name is not NULL
      then
        execute immediate 'DROP TRIGGER ' || v_trigger_name;
        COMMIT;
        out_meldung := 'Trigger ' || v_trigger_name || ' gelöscht.';
      else
        out_meldung := 'Trigger ' || v_trigger_name || ' existiert nicht.';
      end if;
     ...
     isi_p_log.c_isi_system_meldung(in_sid,
                                    in_firma_nr,
                                    'TRACE Trigger löschen',
                                    'ORA-DB',
                                    'db_p_trace.c_trigger_loeschen',
                                    NULL,
                                    NULL,
                                    in_login_id,
                                    NULL,
  ...
  Rückgabewert:
     ...
     !!!!! Achtung !!!!!                            !!!!! Achtung !!!!!
  **************************************************************************************************************************/
  procedure c_trigger_von_tmpl_erstellen(in_sid         in isi_firma.sid%type,
                                         in_firma_nr    in isi_firma.firma_nr%type,
                                         in_login_id    in isi_user.login_id%type,
                                         in_tabellename in varchar2,
                                         out_meldung    out varchar2) is
  ...
  begin
    ...
    v_pk_new   := ersetze_zeichen(v_pk_liste,
                                  'new');
    v_pk_old   := ersetze_zeichen(v_pk_liste,
                                  'old');
    v_text := '
          create or replace trigger ' || v_trigger_name || '
            after insert or update or delete on ' || in_tabellename || '
            for each row
          declare
          ...
          begin
            ...
            if db_p_trace.get_trigger_aktiv(''' || in_tabellename || ''') = 0
            then
              return;
            end if;
            ...
            then
              v_pk_values := ' || v_pk_old || ';
              v_command := ''DELETE'';
              -- v_info := ''vorgang_id='' || :old.vorgang_id || '';'';
            end if;

            -- Autonome Transaktion
            db_p_trace.c_db_act_log(''' || in_tabellename ||''',''' || v_pk_liste || ''',v_pk_values,v_command,v_info);
    ...
    then
      ...
      out_meldung := 'Trigger ' || v_trigger_name || ' erfolgreich angelegt';
    else
      out_meldung := 'Trigger ' || v_trigger_name || ' existiert bereits';
    end if;

    isi_p_log.c_isi_system_meldung(in_sid,
                               in_firma_nr,
                               'Trigger von Tempalte erstellen',
                               'ORA-DB',
                               'db_p_trace.c_trigger_von_tmpl_erstellen',
                               NULL,
                               NULL,
                               in_login_id,
                               NULL,
  ...
  Rückgabewert:
     out_meldung : Info- oder Fehlermeldung wird zurück gegeben
  **************************************************************************************************************************/
  procedure c_trigger_von_tmpl_alle_tbl(in_sid         in isi_firma.sid%type,
                                        in_firma_nr    in isi_firma.firma_nr%type,
                                        in_login_id    in isi_user.login_id%type,
                                        out_meldung    out varchar2) is
    -------------------------------------------------------------------------------------------------------
    ...
    cursor curs_db_trace_cfg is
      select tabellenname from db_trace_cfg;
    rec_db_trace_cfg curs_db_trace_cfg%rowtype;
    ---------------------------------------------------------------------
  begin
    v_anzahl := 0;
    OPEN curs_db_trace_cfg;
    fetch curs_db_trace_cfg
      into rec_db_trace_cfg;

    WHILE curs_db_trace_cfg%found
    LOOP
      c_trigger_von_tmpl_erstellen(in_sid,
                                   in_firma_nr,
                                   in_login_id,
                                   rec_db_trace_cfg.tabellenname,
                                   out_meldung);
      v_anzahl := v_anzahl + 1;
      fetch curs_db_trace_cfg
        into rec_db_trace_cfg;
    ...
    isi_p_log.c_isi_system_meldung(in_sid,
                           in_firma_nr,
                           'Alle Trigger von Tempalte erstellen',
                           'ORA-DB',
                           'db_p_trace.c_trigger_von_tmpl_alle_tbl',
                           NULL,
                           NULL,
                           in_login_id,
                           NULL,
  ...
  Rückgabewert:
     out_meldung : Info- oder Fehlermeldung wird zurück gegeben
  **************************************************************************************************************************/
  procedure db_act_log_out_meld(in_act_table     in db_trace.act_table%type,
  ...
  begin
    ...
    v_bg_job_id := sys_context('USERENV',
                               'BG_JOB_ID');

    -- SESSIONID: The auditing session identifier.
    v_audsid := sys_context('USERENV',
                            'SESSIONID');
    ...
    v_client_info := sys_context('USERENV',
                                 'CLIENT_INFO');
    ...
    then
      ...
      delete db_trace t where t.db_act_log_id = v_found_act_log_id;
    end if;
  ...
  Rückgabewert:
     -
  **************************************************************************************************************************/
  procedure db_act_log(in_act_table     in db_trace.act_table%type,
                       in_act_pk_cols   in db_trace.act_pk_cols%type,
                       in_act_pk_values in db_trace.act_pk_values%type,
                       in_act_command   in db_trace.act_command%type,
                       in_act_info      in db_trace.act_info%type) is
    -------------------------------------------------------------------------------------------------------
    -- Deklarationen
    v_meldung varchar2(4048);
    -------------------------------------------------------------------------------------------------------
  begin
    db_act_log_out_meld(in_act_table,
                        in_act_pk_cols,
                        in_act_pk_values,
                        in_act_command,
                        in_act_info,
  ...
  **************************************************************************************************************************/
  procedure c_db_act_log_out_meld(in_act_table     in db_trace.act_table%type,
                                  in_act_pk_cols   in db_trace.act_pk_cols%type,
                                  in_act_pk_values in db_trace.act_pk_values%type,
                                  in_act_command   in db_trace.act_command%type,
                                  in_act_info      in db_trace.act_info%type,
  ...
  Rückgabewert:
     -
  **************************************************************************************************************************/
  procedure c_db_act_log(in_act_table     in db_trace.act_table%type,
                         in_act_pk_cols   in db_trace.act_pk_cols%type,
                         in_act_pk_values in db_trace.act_pk_values%type,
                         in_act_command   in db_trace.act_command%type,
                         in_act_info      in db_trace.act_info%type) is
  ...
  Rückgabewert
       out_meldung = Info- oder Fehlermeldung wird zurück gegeben
  **************************************************************************************************************************/
  procedure c_job_erstellen(in_sid         in isi_firma.sid%type,
                            in_firma_nr    in isi_firma.firma_nr%type,
                            in_login_id    in isi_user.login_id%type,
  ...
  begin
    ...
    then
      ...
      if in_startzeit is NULL or length(trim(in_startzeit)) = 0 or
         length(trim(in_startzeit)) <> 5
      then
        out_meldung := 'Falsche Zeitangabe. Zeit wird auf 23:59 gesetzt';
        v_startzeit := '23:59:59';
        isi_p_log.c_isi_system_meldung(in_sid,
                 in_firma_nr,
                 'DB Job erstellen zum verkleinern',
                 'ORA-DB',
                 'db_p_trace.c_job_erstellen',
                 NULL,
                 NULL,
                 in_login_id,
                 NULL,
      ...
      -- Job existiert nicht
      v_startdatum := sysdate || ' ' || v_startzeit;
      v_datum      := to_date(v_startdatum,
                              'dd.mm.yy hh24:mi:ss');
      v_what       := 'begin
                         -- Prozedur: c_tabelle_db_trace_verklein_in wird aufgerufen
                         db_p_trace.c_tabelle_db_trace_verklein_in(in_sid => '''||in_sid||''',
      ...
      COMMIT;
      out_meldung := 'Job mit der Nr.: ' || v_job || ' wurde erstellt.';
    else
      out_meldung := 'Job mit der Nr.: ' || v_job || ' existiert bereits.';
    end if;
    isi_p_log.c_isi_system_meldung(in_sid,
         in_firma_nr,
         'DB Job erstellen zum verkleinern',
  ...
  Rückgabewert:
    out_meldung : Eine Meldung wird zurück gegeben
  **************************************************************************************************************************/
  procedure c_job_loeschen(in_sid         in isi_firma.sid%type,
                           in_firma_nr    in isi_firma.firma_nr%type,
                           in_login_id    in isi_user.login_id%type,
                           out_meldung    out varchar2) is
    -------------------------------------------------------------------------------------------------------
  ...
  begin
    ...
    then
      -- Job existiert nicht
      out_meldung := 'Job ' || v_job || ' existiert nicht.';
    else
      --user_jobs.drop_job(v_job);
      dbms_job.remove(v_job);
      COMMIT;
      out_meldung := 'Job ' || v_job || ' wurde erfolgreich gelöscht.';
    end if;
    isi_p_log.c_isi_system_meldung(in_sid,
             in_firma_nr,
             'DB Job löschen zum verkleinern',
             'ORA-DB',
             'db_p_trace.c_job_loeschen',
             NULL,
             NULL,
             in_login_id,
             NULL,
  ...
  **************************************************************************************************************************/
  procedure c_db_trace_deaktivieren(in_sid         in isi_firma.sid%type,
                                    in_firma_nr    in isi_firma.firma_nr%type,
                                    in_login_id    in isi_user.login_id%type,
                                    out_meldung    out varchar2) is
    -------------------------------------------------------------------------------------------------------
