create or replace function dirkspzm32.bde_pd_prod_r_ag_b_f
/*
...
begin
    ...
    open c_res_zust_akt;
    fetch c_res_zust_akt into v_res_zus;
  ...
  -- Ist der letzte Produktionsauftrag nicht abgemeldet
    if v_res_zus.akt_aufgabe = 'P' then
        ...
        raise v_error;
        bde_pd_prod_r_ag_e(in_sid, in_firma_nr, v_res_zus.leitzahl, v_res_zus.fa_ag, v_res_zus.fa_upos,
                           in_res_id, v_sysdate, null, null, null,
                           in_ls_login_id);
    ...
    if v_res_zus.status_id = 0 then
        ...
        if v_s_cfg.res_st_id is not null then
      -- Werte setzen, evtl Unbegründete Störung (res_st_id is NULL)
            res_status.res_status_beg(in_sid, in_firma_nr, in_res_id, in_ls_login_id, v_s_cfg.res_st_id,
                                      v_s_cfg.res_typ, null, -- res_st_ug_id
                                       null, -- fehler_res_id
                                       v_sysdate);
    ...
    if nvl(v_fa_akt.anz_res, 0) = 0 then
        v_fa_akt.freig_status := 'AR';
    end if;

    update bde_fa_auftrag fa
    set
        fa.freig_status = v_fa_akt.freig_status,
...
-- sqlcl_snapshot {"hash":"1c27bf018b899f586f13bbedf8308d3602622acd","type":"FUNCTION","name":"BDE_PD_PROD_R_AG_B_F","schemaName":"DIRKSPZM32","sxml":""}