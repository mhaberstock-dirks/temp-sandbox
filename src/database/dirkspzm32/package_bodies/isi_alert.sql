...
procedure c_insert_mail(in_mail_address in isi_mail_queue.mail_address%type,
                         ...
                         in_send_modul   in isi_mail_queue.send_modul%type) is
begin
  -- Eine mail in die Tabelle isi_mail_queue schreiben !
  insert into isi_mail_queue(mail_address,    betreff_text, inhalt_text, send_modul)
                      values (in_mail_address, in_betreff,   in_inhalt,   in_send_modul);
  commit;
  v_err_nr := 0;

end;

procedure min_date_from_resource_status(in_res_id             in   isi_resource.res_id%type,
                                        ...
                                        out_min_res_id        out isi_resource_zust_akt.res_id%type) is
  ...
  cursor c_get_min_status_date is
      ...
      where
        ((r.res_id = in_res_id) or
        (r.parent_res_id = in_res_id) or
        (r.linie_res_id = in_res_id)) and
        r.res_id = r_akt.res_id and
        r_akt.status_id <> 0;
...
procedure min_date_from_meldung_status(in_res_id             in   isi_resource.res_id%type,
                                        in_filter             in   isi_alert_cfg.alert_filter%type,
                                        out_seconds           out  number,
                                        out_min_date          out date,
                                        out_min_res_status_id out isi_resource_zust_akt.status_id%type,
    ...
    select x.*
      from (
          ...
          from
            ...
            meldung_texte mt
          where
            ((r.res_id = in_res_id) or                   -- konfigurierte Resource Direkt
            (r.parent_res_id = in_res_id) or             -- konfigurierte Resource is parent
            (r.linie_res_id = in_res_id)) and            -- konfigurierte Resource gehört zu Linie
            (mc.res_id = r.res_id) and                    -- Meldung_cfg benötigt die Resource sonst funktioniert es nicht.
            (md.md_status = 'K') and
            (md.md_bereich = mc.name) and
            (md.md_index = mt.mt_index) and
            (mc.fehler_text_gruppe = mt.mt_gruppe) and
            ((mt.filter_mask like '%'||in_filter||'%')or (in_filter is  null)) -- wenn Filter gesetzt filtern
         order by md.md_kommt, md.md_id) x
       where rownum = 1;                                  -- wir brauchen nur den ältesten Datensatz

begin
  out_min_date := null;
  out_seconds := null;
  open c_get_min_meldung_date;
  fetch c_get_min_meldung_date
      into out_seconds,
           out_min_date,
           out_min_status_text,
           out_min_resource_name;
...
procedure c_service_res_meld_alert is
  ...
  cursor c_isi_alert_cfg_res is
      ...
      from
        isi_alert_cfg t
      where
        t.alert_typ = 'R' or t.alert_typ = 'M'; -- Resourcen Alert

begin
  v_send_modul := 'c_service_resource_alert';
  open c_isi_alert_cfg_res;
  loop
     fetch c_isi_alert_cfg_res
       into
         v_alert_typ,
         v_alert_name,
         v_res_id,
     ...
     if  v_Alert_Aktiv  = 'T' then
       if v_alert_typ = 'R' then --Resourcen Alert
          min_date_from_resource_status(v_res_id,
                                      v_alert_filter,
                                      v_seconds,
                                      v_min_date,
                                      v_min_res_status_id,
                                      ...
                                      v_min_status_text,
                                      v_min_res_id);
        if (v_min_date is not null) and
           (v_seconds > v_alert_intervall) and
           (v_alert_status <> 'T') then
          -- Alert Start Eintragen Meldung gekommen !!!
          v_betreff := v_alert_template_meld_kommt;
          v_inhalt  := v_min_resource_name || ' ' || v_min_status_text;

          c_insert_mail(v_alert_mail_address,
                         v_betreff,
                         v_inhalt,
                         v_send_modul);
          update isi_alert_cfg t
            set t.alert_status = 'T', t.last_alert_date = V_min_date
          where t.alert_cfg_name = v_alert_name;
          commit;
        end if;
        if (v_alert_status = 'T') and (v_min_date is null) then
          ...
          c_insert_mail(v_alert_mail_address,
                         ...
                         v_send_modul);
          update isi_alert_cfg t
            set t.alert_status = 'F', t.alert_datum = V_min_date
          where t.alert_cfg_name = v_alert_name;
          commit;
        end if;
       end if; -- Resourcen Alert
       if v_alert_typ = 'M' then --Meldungen Alert
         min_date_from_meldung_status(v_res_id,
                                      v_alert_filter,
                                      v_seconds,
                                      v_min_date,
                                      v_min_res_status_id,
        ...
        if (v_min_date is not null) and
          ...
          c_insert_mail(v_alert_mail_address,
                         ...
                         v_send_modul);
          update isi_alert_cfg t
            set t.alert_status = 'T', t.alert_datum = V_min_date
          where t.alert_cfg_name = v_alert_name;
          commit;
        end if;
        if (v_alert_status = 'T') and (v_min_date is null) then
