...
package body DIRKSPZM32.z_pzm_startup_import is
    ...
    LOOP
      ...
      then
        ...
        update pzm_konten_bh k_bh
         ...
         where k_bh.pers_nr = v_pzm_u_f_imp.pers_nr
      ...
      exception
        when others then
          dbms_output.put_line('Fehler bei der Übernahme von pers_nr ' || to_char(v_pzm_u_f_imp.pers_nr) 
  ...
  procedure pzm_stempelzeiten_import is
    ...
    CURSOR c_z_stempel_zeiten is
      select * 
        from z_pzm_pers_stempelzeiten t
       where nvl(t.gt_ut, '0') = '0'
       order by t.pers_nr, t.kommt;
  begin
    delete pzm_zeiterfassung t
      where exists (select x.pers_nr from z_pzm_pers_stempelzeiten x where x.pers_nr = t.ze_pers_nr);
    delete pzm_ze_tagessatz t
      where exists (select x.pers_nr from z_pzm_pers_stempelzeiten x where x.pers_nr = t.ts_pers_nr);
    update pzm_ze_loa_ausw t
       set t.aa_id = NULL
     where t.aa_id is not NULL
       and exists (select x.pers_nr from z_pzm_pers_stempelzeiten x where x.pers_nr = t.zeaw_pers_nr);
    delete pzm_ze_loa_ausw t
      where exists (select x.pers_nr from z_pzm_pers_stempelzeiten x where x.pers_nr = t.zeaw_pers_nr);
    delete pzm_abwesenheits_antr t
     where t.au_status = 0
       and exists (select x.pers_nr from z_pzm_pers_stempelzeiten x where x.pers_nr = t.au_pers_nr);
    update pzm_abwesenheits_antr t
       set t.au_status = 0
      where exists (select x.pers_nr from z_pzm_pers_stempelzeiten x where x.pers_nr = t.au_pers_nr);
    delete pzm_abwesenheits_antr t
      where exists (select x.pers_nr from z_pzm_pers_stempelzeiten x where x.pers_nr = t.au_pers_nr);
    delete pzm_abwesenheitsmeldungen t
      where exists (select x.pers_nr from z_pzm_pers_stempelzeiten x where x.pers_nr = t.pers_nr);
    commit;
    ...
    LOOP
      ...
      if pzm_p_base.get_personal(v_stempel_zeiten.pers_nr, v_personal)
      then
        /*
        ...
        */
        ...
        if v_stempel_zeiten.gt_ut in ('UT', 'GT', 'UH', '310')
        then
          v_aa_id := 39;
        elsif v_stempel_zeiten.gt_ut in ('FU', 'SUR') 
        then
          v_aa_id := 22;
        elsif v_stempel_zeiten.gt_ut in ('KKR') 
        then
          v_aa_id := 6;
        elsif v_stempel_zeiten.gt_ut in ('KPDL') 
        then
          v_aa_id := 63;
        elsif v_stempel_zeiten.gt_ut in ('362') 
        then
          v_aa_id := 49;
        elsif v_stempel_zeiten.gt_ut in ('361') 
        then
          v_aa_id := 24;
        elsif v_stempel_zeiten.gt_ut in ('364') 
        then
          v_aa_id := 13;
        elsif v_stempel_zeiten.gt_ut in ('UU') 
        then
          v_aa_id := 23;
        elsif v_stempel_zeiten.gt_ut in ('KeAU') 
        then
          v_aa_id := 40;
        elsif v_stempel_zeiten.gt_ut in ('KB') 
        then
          v_aa_id := 35;
        elsif v_stempel_zeiten.gt_ut in ('KO', 'KL') 
        then
          v_aa_id := 5;
        elsif v_stempel_zeiten.gt_ut in ('KM', 'KL', 'KPDL') 
        then
          v_aa_id := 2;
        end if;
    ...
    LOOP
      ...
      then
        ...
        then
          ...
          if v_ende_date is not NULL
          then
            -- Call the procedure
            LOOP
              -- Call the procedure
              update_pers_ze_tag(v_pers_nr,
                                 v_start_date,
                                 p_result => v_result,
        ...
        if to_date(v_stempel_zeiten.geht, 'dd.mm.yyyy hh24:mi') - to_date(v_stempel_zeiten.kommt, 'dd.mm.yyyy hh24:mi') > 0
        then
          update_personal_ze_status_date(p_pers_nr => v_stempel_zeiten.pers_nr,
                                         p_status => 2,
  ...
  begin
    ...
    LOOP
      ...
      update pzm_abt_leitung t
         set t.abt_l_pers_nr = v_pzm_pers_nr.pers_nr_neu
       where t.abt_l_pers_nr = v_pzm_pers_nr.pers_nr;
      update pzm_abwesenheits_antr t
         set t.au_pers_nr = v_pzm_pers_nr.pers_nr_neu
       where t.au_pers_nr = v_pzm_pers_nr.pers_nr;
      update pzm_abwesenheits_antr t
         set t.au_pruef_pers_nr = v_pzm_pers_nr.pers_nr_neu
       where t.au_pruef_pers_nr = v_pzm_pers_nr.pers_nr;
      ...
      update pzm_azubi_daten t
         set t.ad_pers_nr = v_pzm_pers_nr.pers_nr_neu
       where t.ad_pers_nr = v_pzm_pers_nr.pers_nr;
      update pzm_konten t
         set t.pers_nr = v_pzm_pers_nr.pers_nr_neu
       where t.pers_nr = v_pzm_pers_nr.pers_nr;
      update pzm_konten_bh t
      ...
      update pzm_ze_bde_zeiten t
         set t.ze_bde_pers_nr = v_pzm_pers_nr.pers_nr_neu
       where t.ze_bde_pers_nr = v_pzm_pers_nr.pers_nr;
      update pzm_zeiterfassung t
         set t.ze_pers_nr = v_pzm_pers_nr.pers_nr_neu
       where t.ze_pers_nr = v_pzm_pers_nr.pers_nr;
      update pzm_zeiterfassung t
         set t.ze_korr_pers_nr = v_pzm_pers_nr.pers_nr_neu
       where t.ze_korr_pers_nr = v_pzm_pers_nr.pers_nr;
      update pzm_ze_loa_13w_schnitt t
         set t.pers_nr = v_pzm_pers_nr.pers_nr_neu
       where t.pers_nr = v_pzm_pers_nr.pers_nr;
      update pzm_ze_loa_ausw t
         set t.zeaw_pers_nr = v_pzm_pers_nr.pers_nr_neu
       where t.zeaw_pers_nr = v_pzm_pers_nr.pers_nr;
      update pzm_ze_loa_ausw t
         set t.zeaw_korr_pers_nr = v_pzm_pers_nr.pers_nr_neu
       where t.zeaw_korr_pers_nr = v_pzm_pers_nr.pers_nr;
      ...
      update pzm_ze_tagessatz t
         set t.ts_pers_nr= v_pzm_pers_nr.pers_nr_neu
       where t.ts_pers_nr = v_pzm_pers_nr.pers_nr;
