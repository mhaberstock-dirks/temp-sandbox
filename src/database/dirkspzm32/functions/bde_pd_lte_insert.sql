create or replace function dirkspzm32.bde_pd_lte_insert
/*
...
*/ (
  ...
  -------------------------------------------------------------------------------------------------------
    ...
    from
        lvs_lte res_lte
    where
            res_lte.lte_akt_lhm = 0
        and res_lte.sid = v_res.sid
        and res_lte.firma_nr = v_res.firma_nr
        and res_lte.lgr_platz = v_res.lager_fertig;
...
begin
    ...
    if not isi_p_base.get_isi_firma(in_sid, v_res.firma_nr, v_frima) then
        v_err_nr := 20;
        v_err_text := 'Fehler Firmenstammdaten für Firma NR.: '
                      || to_char(v_res.firma_nr)
                      || ' fehlt.';
        raise v_error;
    else
        open c_res_zus_akt;
        fetch c_res_zus_akt into
            v_artikel_id,
            v_charge,
            v_charge_id,
            v_leitzahl,
  ...
  -- In jedem Fall eine neue Palette anlegen
    ...
    v_hersteller := substr(v_fa_auftrag.ma_hersteller_kuerzel_liste,
                           1,
                           length(v_fa_auftrag.ma_hersteller_kuerzel_liste) - 1);

    if isi_p_base.get_artikel_ctrl_typ(in_sid, v_fa_auftrag.ab_artikel_id, v_hersteller, v_art_ctrl) then
    ...
    v_result := lvs_p_lte.lvs_lte_insert_v359(in_sid,                  -- SID der Maschine
     ...
     in_ls_login_id,           -- Login ID aktuelle User
                                              ...
                                              v_charge_id,              -- Charge ID
                                               ...
                                               v_fa_auftrag.packschema_kopf_id, null,                    -- Auto Depal ist unbekannt
    ...
    set
        lte_id = v_result                            -- Ergebnis im aktuellen Maschinenzustand SPEICHERN
    where
            sid = in_sid
...
-- sqlcl_snapshot {"hash":"73b3251f63518cdb1363df239ed251cbe9915797","type":"FUNCTION","name":"BDE_PD_LTE_INSERT","schemaName":"DIRKSPZM32","sxml":""}