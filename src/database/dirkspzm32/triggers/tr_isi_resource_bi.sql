...
begin
  ...
  then
    if :new.sid is null
    then
      select sid into :new.sid from isi_sid s where s.sid_my_sid = 1;
    end if;

    if :new.variante is null
    then
      :new.variante := 1;
    end if;
    ...
    fetch c_res_zust_akt
      into v_res_zust_akt;
    if c_res_zust_akt%notfound
    then
      insert into isi_resource_zust_akt
      values
        (:new.sid,
         :new.res_id,
         null,
    ...
    fetch c_res_variante
      into v_res_variante;
    if c_res_variante%notfound
    then
      -- variante nur hinzufügen, wenn sie nicht vorhanden ist
      insert into isi_resource_var
      values
        (:new.sid,
         :new.res_id,
