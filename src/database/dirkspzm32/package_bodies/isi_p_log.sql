create or replace package body dirkspzm32.isi_p_log is
  -- Funktions und Prozedur-Deklaration
    ...
    procedure isi_system_meldung (
        ...
        from
            isi_log l
        where
            ( nvl(l.log_error_code, -1) = nvl(in_meldung_nr, -1)
    ...
    begin
        ...
        close c_isi_log;
        v_mc_typ := 0;
        if in_log_type = 'I' then
            v_mc_typ := c.msg_mc_info;
            v_log_status := null;
        elsif in_log_type = 'W' then
            v_mc_typ := c.msg_mc_warning;
            v_log_status := null;
        elsif in_log_type = 'E' then
        ...
        if
            v_found
            and in_log_type not like '%L'
        then
            update isi_log l
            set
                l.log_msg_count = l.log_msg_count + 1
            where
                ( nvl(l.log_error_code, -1) = nvl(in_meldung_nr, -1)
                  or l.log_details = in_details )
                and l.log_time > systimestamp - 1 / 1440
            ...
            if in_log_type not like '%L' then
            ...
            select
                seq_isi_log_id.nextval
            into v_log_id
            from
                dual;

            insert into isi_log values ( systimestamp,
                                         in_category,
                                         in_meldung_nr,
                                         in_details,
                                         in_log_type,
...
-- sqlcl_snapshot {"hash":"8f69868ae8a397e4017c2a223c133e82f5523fac","type":"PACKAGE_BODY","name":"ISI_P_LOG","schemaName":"DIRKSPZM32","sxml":""}