  ...
  CREATE OR REPLACE EDITIONABLE TRIGGER "TR_PZM_PERSONAL_AIU" 
AFTER INSERT OR UPDATE ON PZM_PERSONAL FOR EACH ROW
declare
...
begin
  ...
  then
    ...
    begin
      if :new.pers_urlaub_anspr_wert is not NULL and v_found
      then
        v_date := :new.pers_eintrittsdatum;
        v_jahresurlaub_diff := to_number(to_char(:new.pers_eintrittsdatum, 'MM'));
        if to_number(to_char(:new.pers_eintrittsdatum, 'DD')) > 15
        then
          v_jahresurlaub_diff := v_jahresurlaub_diff + 1;
        elsif to_number(to_char(:new.pers_eintrittsdatum, 'DD')) <= 15
          and to_number(to_char(:new.pers_eintrittsdatum, 'DD')) > 1
        then
          v_jahresurlaub_diff := v_jahresurlaub_diff + 0.5;
        end if;
        -- (30 / 12 * (12 - 7.5 + 1)  
        v_jahresurlaub_diff := round(:new.pers_urlaub_anspr_wert / 12 * (12 - v_jahresurlaub_diff) + 1, 0);
        
        if upper(v_uk_konto.buch_einheit) = 'HH24'
        then
        ...
        update pzm_konten_bh t
           set t.zk_start = :new.pers_eintrittsdatum,   --ToDo: BWe an HJG
               t.zk_aa_id = null
         where t.sid = '01'
           and t.firma_nr = 1
...
/
ALTER TRIGGER "TR_PZM_PERSONAL_AIU" ENABLE;


-- sqlcl_snapshot {"hash":"b58e3a2ecb29ea1a04649c11ed8ebb07777ade42","type":"TRIGGER","name":"TR_PZM_PERSONAL_AIU","schemaName":"DIRKSPZM32","sxml":""}