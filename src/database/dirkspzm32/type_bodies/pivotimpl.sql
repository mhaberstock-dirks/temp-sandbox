create or replace type body dirkspzm32.pivotimpl as
...
--
        ...
        open rc for replace('select distinct '
                            || t_fmt
                            || '
	                      from( '
                            || p_stmt
        ...
        rtype.setinfo(null, null, null, null, null,
                      atyp, dbms_types.typecode_object, 0);
...
--
        type_code := sctx.ret_type.getinfo(prec, scale, len, csid, csfrm,
                                           schema_name, type_name, version, attr_count);

        for i in numcols - 1..attr_count loop
            type_code := sctx.ret_type.getattreleminfo(i, prec, scale, len, csid,
                                                       csfrm, attr_type, attr_name);
        ...
        for i in 1..attr_count loop
            ...
            case type_code
                ...
                when dbms_types.typecode_number then
                    dbms_sql.define_column(sctx.cur, i, to_number(null));
                when dbms_types.typecode_date then
                    dbms_sql.define_column(sctx.cur, i, to_date(null));
                when dbms_types.typecode_timestamp then
                    dbms_sql.define_column(sctx.cur, i, to_timestamp(null));
                when dbms_types.typecode_timestamp_tz then
                    dbms_sql.define_column(sctx.cur, i, to_timestamp_tz(null));
...
--
        ...
        type_code := self.ret_type.getinfo(prec, scale, len, csid, csfrm,
                                           schema_name, type_name, version, attr_count);
        ...
        outset.piecewise();
        for i in 1..attr_count loop
            type_code := self.ret_type.getattreleminfo(i, prec, scale, len, csid,
                                                       csfrm, attr_type, attr_name);
...
-- sqlcl_snapshot {"hash":"45856f8f0702b8c4f0d6992827a3be027a50ac3c","type":"TYPE_BODY","name":"PIVOTIMPL","schemaName":"DIRKSPZM32","sxml":""}