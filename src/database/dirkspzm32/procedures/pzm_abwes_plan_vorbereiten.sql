create or replace 
procedure DIRKSPZM32.pzm_abwes_plan_vorbereiten(in_start_date in date,
                                                       in_end_date in date,
...
begin
  ...
  loop
    ...
    loop
      ...
      delete pzm_abwes_plan t where t.pers_nr = v_pers_nr and t.abwes_plan_tag = v_current_date;
      commit;
      ...
      if v_found
      then
        if v_feiertag = 'F' and v_schicht_vorh and nvl(v_anw_std, 0) = 0
        then
          insert into pzm_abwes_plan values (
            v_pers_nr,
            v_current_date,
            v_feiertag,
            v_wochentag,
        ...
        fetch c_am into v_aa_id, v_aa_kurzname, v_aa_farbe;
        v_found := c_am%found;
        close c_am;
        ...
        then
          ...
          fetch c_ua into v_aa_id, v_aa_kurzname, v_aa_farbe;
          v_found := c_ua%found;
          close c_ua;
...
-- sqlcl_snapshot {"hash":"4a1d97f815f4062c17e5713944adf69ae58e2aff","type":"PROCEDURE","name":"PZM_ABWES_PLAN_VORBEREITEN","schemaName":"DIRKSPZM32","sxml":""}