...
function DIRKSPZM32.get_pers_pause_std(in_pers_nr in pzm_personal.pers_nr%type,
  ...
  cursor c_ze_pausen is
    select t.*
     ...
     order by t.ze_calc_ist_start, t.ze_id;
...
begin
  ...
  then
    ...
    then
      ...
      if nvl(v_tagessatz.ts_day_pause_std, 0) > 0
      then
        open c_ze_pausen;
        loop
          ...
          if v_unb_pause_diff > 0
          then
            if  /* Kollision 1 */ (v_ze_pausen.ze_calc_ist_start <= in_von_zeit
                                   and v_ze_pausen.ze_calc_ist_ende > in_von_zeit)
                or
                /* Kollision 2 */ (v_ze_pausen.ze_calc_ist_start < in_bis_zeit
                                   and v_ze_pausen.ze_calc_ist_ende >= in_bis_zeit)
                or
                /* Kollision 3 */ (v_ze_pausen.ze_calc_ist_start <= in_von_zeit
                                   and v_ze_pausen.ze_calc_ist_ende >= in_bis_zeit)
                or
                /* Kollision 4 */ (v_ze_pausen.ze_calc_ist_start >= in_von_zeit
                                   and v_ze_pausen.ze_calc_ist_ende <= in_bis_zeit)
            then
              -- nur bei Überlappungen (Kollisionen) wird die Pause berechnet
              v_result := v_result + v_unb_pause_diff;
              if v_ze_pausen.ze_calc_ist_start < in_von_zeit
              then
                -- dise Pause ist zum Teil nicht in dem abgefragten Zeitraum
                v_result := v_result - (in_von_zeit - v_ze_pausen.ze_calc_ist_start) * 24;
              elsif v_ze_pausen.ze_calc_ist_ende > in_bis_zeit
              then
                -- dise Pause ist zum Teil nicht in dem abgefragten Zeitraum
  ...
  if nvl(v_result, 0) < nvl(v_unb_pause_diff, 0)
  then
