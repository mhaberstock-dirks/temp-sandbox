create or replace package body dirkspzm32.term_transp is
...
procedure check_man_einl_prio_sat1 (
  ...
  -------------------------------------------------------------------------------------------------------
    ...
    function get_global_transp_cfg (
        in_sid        in isi_firma_cfg.sid%type,
  ...
  -------------------------------------------------------------------------------------------------------
    function get_next_transp_list_csv (
        ...
        select
            ...
            (
                ...
                from
                    isi_transport t1
                where
                        t1.lgr_platz_quelle = t.lgr_platz_quelle
                    and t1.transp_typ = 'E'
                    and ( ( t1.status = 'F'
                            and t1.res_id is null )
                          or t1.res_id = in_res_id )
            )                                                                 we_anz,
            isi_allg.get_firma_cfg_param(t.sid, t.firma_nr, 'CFG', t.lgr_platz_quelle, 'we_anz_prio_high',
                                         'transport', 'CFG', null, 'INTEGER') we_anz_prio_high
        from
            isi_transport t,
            lvs_lgr_ort   loz,
            lvs_lgr_ort   loq
        where
            ...
            and ( t.transp_typ != 'A'
                ...
                from
                    isi_transport t2
                where
                        nvl(t2.lkw_nr, t2.vorgang_id) = nvl(t.lkw_nr, t.vorgang_id)
        ...
        order by
            ...
            decode(we_anz - we_anz_prio_high,
                   null,
                   1, -- keine Relevanz
                   abs(we_anz - we_anz_prio_high),
                   0, -- (positiv = 0 = dringend)
                   1), -- dringende Einlagerungen
            decode(urgent,
                   abs(urgent),
                   1,
                   0), -- positive Werte sind Transporte die später fertig sein dürfen
    ...
    -- -AG- 20190913 Einbau mechfach LSL-ID je Stapler
        ...
        select
            ';'
            || stradd_distinct(lsid.stapler_ls_id)
            || ';'
        from
            lvs_fahrzeuge_ls_id lsid
        where
                lsid.sid = v_fahrzeug.sid
    ...
    begin
        ...
        v_transp_dauer := to_number ( get_global_transp_cfg(in_sid, in_firma_nr, 'STD_TRANSPORT_ZEIT_MIN', '15', 'INTEGER') );

        v_calc_transp_ende := sysdate + ( v_transp_dauer + 5 ) / 1440;
        v_transp_counter := 0;

    -- Leitsystem des aktuellen Staplers holen
        if not lvs_p_base.get_fahrzeug(in_res_id, v_fahrzeug) then
            v_ls_ids := null;
        else
            open c_get_ls_id_s;
            ...
            if
                v_fahrzeug.stapler_ls_id is not null
                and v_ls_ids is not null
            then
                v_ls_ids := v_ls_ids
                            || v_fahrzeug.stapler_ls_id
                            || ';';
            elsif v_fahrzeug.stapler_ls_id is not null then
        ...
        loop
            ...
            if v_ls_ids is null -- dieses Fahrzeug darf alles anfahren
               or v_ls_ids like '%;'
               || to_char(v_transp_ls_id_q)
               || ';%'  -- dieses Fahrzeug darf die Transportquelle anfahren
                  or v_ls_ids like '%;'
                                   || to_char(v_transp_ls_id_z)
                                   || ';%' -- dieses Fahrzeug darf das Transportziel anfahren
                                    then
  ...
  -------------------------------------------------------------------------------------------------------
    ...
    -- lets see if the transport still exists
        if not lvs_p_base.get_transport(in_sid, in_transp_id, v_transport) then
            raise_isi_error(10,
                            lc.ec_p1(lc.o_tp1_transp_id_nf, in_transp_id));
        end if;

    -- Zugriffsmöglichkeit auf den Transport prüfen
        v_ret_val := lvs_transport.lvs_transp_check_zugriff(in_sid,
                                                            in_firma_nr,
      ...
      -- CMe 20220324 (): Transport LTE darf geändert werden
            if isi_allg.get_firma_cfg_param(v_transport.sid, v_transport.firma_nr, 'CFG', null, 'TMS_CHG_TRANSPORT_LTE',
                                            'LVS', 'CFG', c.c_false, 'BOOLEAN') = c.c_true then
        -- CMe 20220324 (): Prüfen ob Transport LTE geändert wurde, wenn ja andere Message senden
                if
                    ( lvs_p_base.get_transport(in_sid, in_transp_id, v_transport_n) )
                    and ( v_transport_n.lte_id != v_transport.lte_id )
                then
                    raise_isi_error(20,
                                    lc.ec(lc.o_txt_transp_lte_changed));
                else
                    raise_isi_error(20,
                                    lc.ec('O_TXT_RELOC_TRANSP_CREATED'));
                end if;

            else
        -- wegen neuem Transport muss die Liste nochmal geholt werden.
                raise_isi_error(20,
                                lc.ec('O_TXT_RELOC_TRANSP_CREATED')); -- 'TODO: new const -> Freifahrtransport wurde angelegt');
            end if;

        elsif v_ret_val = -1 then
            raise_isi_error(30,
                            lc.ec('O_TXT_TU_BLOCKED_BY_OTHER_TU')); --'TODO: new const -> LTE muss erst freigefahren werden');
        end if;

        v_error_code := lvs_transport.lvs_transp_beginnen(in_sid, in_firma_nr, in_login_id, in_transp_id, v_transport.lte_id,
                                                          in_res_id);

        if v_error_code != 0 then
            raise_isi_error(
                abs(v_error_code),
                lc.ec_p2(lc.c_tp2_transp_res_assign_fail, in_transp_id, in_res_id)
            );
        end if;
  ...
  -------------------------------------------------------------------------------------------------------
    ...
    begin
        ...
        set
            ...
            t.status = 'F'
        where
                t.transp_id = nvl(in_transp_id, t.transp_id)
            and t.sid = in_sid
  ...
  -------------------------------------------------------------------------------------------------------
    ...
    -- if returns multilang text instead off storage_cell  Bugfix 17.12.2010 -BW-
        ...
        if v_pos > 1 then
            out_new_storage_cell := substr(out_new_storage_cell,
                                           v_pos + 2,
                                           length(out_new_storage_cell) - v_pos - 2);
        end if;

    -- the transport was modified when another destination was found, so we have to commit here
        commit; -- for modified transports

        if out_new_storage_cell is null then
      -- the result is an error constant
            raise_isi_error(20,
                            lc.ec_p1(lc.o_tp1_lgr_f_lte_n_gefunden, v_transport.lte_id));
        end if;
  ...
  -------------------------------------------------------------------------------------------------------
    ...
    -- try to find a new storage cell
        out_new_storage_cell := lvs_p_lte.lvs_suche_neuen_platz_v349(v_transport, in_login_id, null);
    -- if returns multilang text instead off storage_cell  Bugfix 17.12.2010 -BW-
        v_pos := instr(out_new_storage_cell, '@[', 1, 1);   -- LC.CPTRENNER
        if v_pos > 1 then
            out_new_storage_cell := substr(out_new_storage_cell,
  ...
  -------------------------------------------------------------------------------------------------------
    ...
    -- do we have a valid transp unit?
        if not lvs_p_base.get_lte(in_diff_transp_unit_id, v_diff_lte) then
            raise_isi_error(10,
                            lc.ec_p1(lc.o_tp1_lte_id_fehlt, in_diff_transp_unit_id));
        end if;

    -- do we have an active (waiting) transport for the transp unit?
        if not lvs_p_base.get_transport_by_lte_id(in_diff_transp_unit_id, v_diff_transport) then
            raise_isi_error(20,
                            lc.ec_p1(lc.o_tp1_transp_data_by_tu_nf, in_diff_transp_unit_id));
        end if;

    -- now check if we can use this transport
        if
            v_diff_transport.status != 'F'
            and v_diff_transport.status != 'B' -- Auch begonnen können genommen werden
        then
            raise_isi_error(30,
                            lc.ec_p1('O_TP1_TRANSP_NOT_EXECUTABLE', in_transp_id)); -- 'TODO: new const');
        end if;

    -- now get data for current transport
        if not lvs_p_base.get_transport(in_sid, in_transp_id, v_curr_transport) then
            raise_isi_error(40,
                            lc.ec_p1(lc.o_tp1_transp_data_nf, in_transp_id));
        end if;
    ...
    -- check for outgoing goods
        if v_diff_transport.transp_typ = 'A' then
      -- Read lgr_platz_ziel
            if lvs_p_base.get_lgr_platz(v_diff_transport.lgr_platz_ziel, v_lgr_ziel) then
        -- OK when not a WA or WA not a LDPR
        -- -AG- 14.01.2010 BugFix Nur am Auslagerpunkt prüfrn (Verladung auf den LKW)
                if
                    v_lgr_ziel.lgr_verwendung = c.lgr_typ_wa
                    ...
                    and isi_allg.c_get_firma_cfg_param(v_diff_transport.sid, -- -AG- 20211206 Pruefung jetzt über Firma_CFG abschaltbar 
                     v_diff_transport.firma_nr, 'CFG', -- Kategorie 
                     null,  -- Kategorie IDX
                     'TMS_AUSL_CHECK_TOUR', -- Parameter Name 
                                                       'TMS',              -- Modul-Name 
                                                        'CFG', c.c_true, 'BOOLEAN') = c.c_true
                then
          -- change is only allowed for the same load/tour number
                    if nvl(v_diff_transport.lkw_nr, v_diff_transport.vorgang_id) != nvl(v_curr_transport.lkw_nr, v_curr_transport.vorgang_id
                        ...
                        raise_isi_error(60,
                                        lc.ec_p1('O_TP1_TRANSP_N_CHG_DIFF_TOUR',
          ...
          -- check the required order for the transport units
                    if v_curr_transport.transport_gruppe < v_diff_transport.transport_gruppe then
                        raise_isi_error(70,
                                        lc.ec_p2('O_TP2_TRANSP_N_CHG_ORDER', v_curr_transport.transport_gruppe, v_diff_transport.transport_gruppe
                                        )); --'TODO: new const -> wrong order of transport units');
                    end if;
    ...
    -- now release the transport and commit this process
        c_release_assigned_transp(v_curr_transport.sid, null,  -- release all for this Res_ID
         in_res_id);

    -- assign the new transport
        v_error_code := lvs_transport.lvs_transp_beginnen(v_diff_transport.sid, v_diff_transport.firma_nr, in_login_id, v_diff_transport.transp_id
        , v_diff_lte.lte_id,
                                                          in_res_id);
        ...
        if v_error_code != 0 then
            raise_isi_error(
                abs(v_error_code),
                lc.ec_p2(lc.c_tp2_transp_res_assign_fail, v_diff_transport.transp_id, in_res_id)
            );
        end if;
    ...
    function get_loading_transp_list_csv (
        ...
        from
            isi_transport t
        where
                t.vorgang_id = v_loading_point.order_vorgang_id
    ...
    begin
        ...
        v_result := 'F';
        out_transp_id_list_csv := '';
        if not tms_p_base.get_loading_point(in_loading_point, in_firma_nr, in_sid, v_loading_point) then
            raise_isi_error(10,
                            lc.ec_p1('O_TP1_TMS_LP_NA', in_loading_point));
        end if;
        ...
        loop
            ...
            exit when c_transport%notfound;
            if out_transp_id_list_csv is not null then
                out_transp_id_list_csv := out_transp_id_list_csv || ';';
            end if;
    ...
    -- für diesen kanal un von dieser Quelle eine höhere PRIO bekommen (In einem Rutsch)
        if
            in_transport.transp_typ = 'E' -- Jetzt erst mal nur einlagern
      ...
      -- manuell geführtes Satelitenlager
            ...
            or v_lgr_ziel.lgr_typ = c.kanal1 then
        ...
        -- Ohne Freifaheren die nicht vom gleichen Typ sind
                ...
                if in_transport.prio > v_max_prio then
                    v_max_prio := in_transport.prio;
                elsif in_transport.prio <= v_max_prio then
...
-- sqlcl_snapshot {"hash":"24988cb0caa7fee3aac1eaf1679b9d63e710717e","type":"PACKAGE_BODY","name":"TERM_TRANSP","schemaName":"DIRKSPZM32","sxml":""}