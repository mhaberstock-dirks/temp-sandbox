...
package body DIRKSPZM32.pps_p_bde is
  ...
  begin
    pps_p_bde.crt_bde_fa_auft_a_plan_db31_f(in_sid => in_sid,
  ...
  begin
    pps_p_bde.create_bde_fa_dispo_db31 (in_sid,
                                        in_firma_nr,
                                        in_dispo_charge_rein,
                                        in_hersteller_liste,
                                        in_leitzahl,
  ...
  procedure create_bde_fa_dispo_db31 (in_sid               in isi_sid.sid%type,
    ...
    -- fuer Auslagerungen identisch gehalten werden
      select lte.lte_id,
             ...
             lvs_ausl.lvs_lte_platz_bewerten(in_sid,
              ...
              pps_p_bde.chk_lte_on_res_lgr_grp(in_sid,
                                               in_firma_nr,
                                               lte.lte_id,
                                               v_bde_fa.auf_id) sort3
        from lvs_lte     lte,
             lvs_lam     lam,
             lvs_lgr     lgr,
             isi_artikel a,
       ...
       order by comp_w_fixed_id_in_params(v_bde_fak.prod_params, lte.lte_id) desc,
                decode (nvl((select count(x.lam_id) 
                               from lvs_lam x
                               join bde_fa_auftrag y on y.auf_id = x.order_pos_auf_id
                              where (y.abnr = v_bde_fa.abnr or y.leitzahl = v_bde_fa.leitzahl
                                and x.lte_id = lte.lte_id)),0), 0, 0, 1) desc, -- Bevorzugung LTE's die für die gleich abnr oder Leitzahl reserviert sind
                decode(min(lam.order_pos_auf_id), v_bde_fa.auf_id, 0, 1),
   ...
   --                          oder zum gleichen Batch gehören
    ...
    cursor c_check_anz_lte_res is
      select count(lam.lam_id) anz_res
        from lvs_lam lam
            ...
            then
              ...
              then
                ...
                if (v_found) and
                   ...
                   (v_lte_dispo_ziele = v_lgr_gruppe_fa) and
                   (not v_check_t_ziel_seq) 
                then
                ...
                if v_lte_lams.order_pos_auf_id = v_bde_fa.auf_id then
                  if v_delta_menge > 0 then
                    if v_lte_lams.res_menge <= v_delta_menge then
                      ...
                      pps_p_bde.create_new_lam_f_rest(in_sid => in_sid,
                                                      in_firma_nr => in_firma_nr,
                                                      in_lam => v_lte_lams,
                                                      in_user_id => in_user_id);
                    end if;
  ...
  */
  ...
  cursor c_chk_in_trans is
    select t.transp_id,
     ...
     where t.lte_id = v_lvs_lte.lte_id;
     
  begin
    v_result := 1;
    v_faktor := 100;
    v_found  := lvs_p_base.get_lte(in_lte_id,
                                   v_lvs_lte);
    ...
    then
      ...
      open c_chk_in_trans;
      fetch c_chk_in_trans into v_transport_id,
                                v_lgr_platz_ziel;
      ...
      if (v_found)
      then
        if (v_fa_lgr_platz.lgr_platz_gruppe <> v_lgr_platz_ziel)
        then
      ...
      if (v_fa_lgr_platz.lgr_typ ='DURCHL1')
      then
        v_result := v_result + (100/v_faktor);
      else
        v_lte_platz_grp :=  pps_p_bde.get_lte_dipo_ziele(in_sid => v_lvs_lte.sid,
                                                         in_firma_nr => v_lvs_lte.firma_nr,
                                                         in_lte_id => v_lvs_lte.lte_id);
        
        if (v_lte_platz_grp is not null)
        then
          if (v_lte_platz_grp != v_fa_lgr_platz.lgr_platz_gruppe)
          then
  ...
  */
  ...
  cursor c_get_seperated_params is
    select regexp_substr(in_params, '[^;]+', 1, level) as transport_id
     from dual
  connect by regexp_substr(in_params, '[^;]+', 1, level) is not null;
  
  begin
    v_result := 0;
    v_fixed_lte_id := null;
    
    v_chk_compare_allowed := isi_allg.get_firma_cfg_param(in_sid => '01',
                                                          in_firma_nr => 1,
                                                          in_kategorie => 'CFG',
    ...
    if (v_chk_compare_allowed = 'T')
    then
      open c_get_seperated_params;
      loop
        fetch c_get_seperated_params into v_fixed_lte_param;
        exit when c_get_seperated_params%notfound;
        if (instr(v_fixed_lte_param, 'FIX-LTE-ID') > 0)
        then
          -- Wert entweder 'FIX-LTE-ID={LTE_ID}' oder 'FIX-LTE-ID=LTE_ID'
          if (instr(v_fixed_lte_param, '{') > 0)
          then
            v_fixed_lte_param := substr(v_fixed_lte_param, instr(v_fixed_lte_param, '{') + 1);
            v_fixed_lte_id := substr(v_fixed_lte_param, 0, length(v_fixed_lte_param) - 1);
          else
            v_fixed_lte_id := substr(v_fixed_lte_param, instr(v_fixed_lte_param, '=') + 1);
          end if;
      ...
      if (v_fixed_lte_id is not null) and
         (v_fixed_lte_id = in_lte_id)
      then
        v_result := 1;
      end if;
    end if;
  ...
  */
  ...
  begin
    ...
    if (in_fixed_id > 0) and
       (in_fixed_lte_id is not null)
    then
      v_bde_fa_plan.prod_params := v_bde_fa_plan.prod_params || 'FIX-LTE-ID=' || in_fixed_lte_id;
    end if;
