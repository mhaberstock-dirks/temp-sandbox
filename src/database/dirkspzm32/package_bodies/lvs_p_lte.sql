create or replace package body dirkspzm32.lvs_p_lte is
  /*
 ...
 *******************************************************************************/
    ...
    begin
        lvs_lte_get_druck_daten(in_lhm_id, in_kunden_nr, v_lhm_sid, v_lhm_firma_nr, v_rave_datei,
                                v_rave_report_name, v_job_daten_typ, v_job_daten, v_anzahl_drucke);

        print_engine.insert_new_job(v_lhm_sid, v_lhm_firma_nr, v_rave_datei, v_rave_report_name, v_job_daten_typ,
                                    v_job_daten, in_drucker_name, v_anzahl_drucke, v_job_nr);

        update lvs_lhm l
        set
            l.lhm_eti_druck_status = 'D'
        where
                l.sid = v_lhm_sid
 ...
 *******************************************************************************/
    ...
    begin
        lvs_lhm_drucken(in_lhm_id, in_kunden_nr, in_drucker_name);
        commit;
    end;
    ...
    begin
        ...
        if v_start_pos != 0 then
            ...
            loop
                ...
                v_lfdn_str := v_lfdn_str
                              || substr(in_barcode, v_start_pos, 1);
            end loop;
  ...
  -- AG 03.03.2017 Multiuserfaehig machen auch bei schnell-Drehenden Daten
    function get_datum_lid (
        in_key in lvs_lte_datum_lid.lte_barcode_key%type
    ) return number is
    ...
    begin
        ...
        if v_lid is null then
            begin
                v_lid := 1;
                insert into lvs_lte_datum_lid t values ( in_key,
                                                         v_lid + 1 );
  ...
  -------------------------------------------------------------------------
    function format_barcode (
        ...
        from
            lvs_lte_datum_lid t
        where
            t.lte_barcode_key = v_barcode_test;
    ...
    begin
        ...
        v_ende_pos := 1;
        v_ende_pos := nvl(
            instr(in_format, '_', v_start_pos, 1),
            1
        );
        ...
        or instr(in_format, 'PDMMYY', 1, 1) > 0 then
            v_mit_spez_barc := true;
        end if;

        if instr(in_format, 'CCC', 1, 1) > 0 then
            v_mit_spez_barc := true;
            v_mit_charge := true;
        end if;

        if v_mit_spez_barc then
        ...
        then
            ...
            then
                ...
                or ( v_start_pos > 0 ) then
                                ...
                                isi_utils.spez_barcode_gen(v_artikel.artikel,      -- in_artikel              in  isi_artikel.artikel%type,
                                 ...
                                 v_artikel.ean,          -- in_ean                  in  varchar2,
                                                           v_typ,                  -- in_typ                  in  varchar2,
                                                            in_h_tag, v_barcode_spez, v_barcode_begin);
                    ...
                    if v_start_pos <= in_laenge then
                        v_barcode_ende := substr(
                            nvl(in_format, ''),
                            v_start_pos,
                        ...
                        if v_mit_spez_barc then
                            ...
                            isi_utils.spez_barcode_gen(v_artikel.artikel,      -- in_artikel              in  isi_artikel.artikel%type,
                             ...
                             v_artikel.ean,          -- in_ean                  in  varchar2,
                                                       v_typ,                  -- in_typ                  in  varchar2,
                                                        in_h_tag, v_barcode_spez, v_barcode_ende);

                            if
                                v_barcode_ende is null
                    ...
                    v_laenge_nr := in_laenge - nvl(
                        length(v_barcode_begin),
                        0
                    ) - nvl(
                        length(v_barcode_ende),
                        0
                    );
        ...
        or v_barcode_ende = 'DDMMYYYY' then
            v_mit_datum := true;
            v_barcode_ende := to_char(sysdate, v_barcode_ende);
        end if;

        v_lid := in_nummer;
        if v_mit_charge then
      -- Bsp.:     AAAAAACCCCCC__MMMMmM
            if lvs_p_base.get_charge_bez(in_sid,                    -- in_sid        in lvs_charge.sid%type,
             ...
             then
                if in_basis_id is not null then
                    if in_basis_id = c.basis_lte then
                        v_lid := v_charge.charge_lte_lfdn + 1;
                        update lvs_charge t
                        set
                            t.charge_lte_lfdn = v_lid
                        where
                            t.charge_id = v_charge.charge_id;

                    else
                        v_lid := v_charge.charge_lhm_lfdn + 1;
                        update lvs_charge t
                        set
                            t.charge_lhm_lfdn = v_lid
                        where
                            t.charge_id = v_charge.charge_id;
            ...
            then
                if in_basis_id = c.basis_lte then
                    update lvs_charge t
                    set
                        t.charge_lte_lfdn = t.charge_lte_lfdn + 1
                    where
                        t.charge_id = v_charge.charge_id;
                    ...
                    update lvs_charge t
                    set
                        t.charge_lhm_lfdn = t.charge_lhm_lfdn + 1
                    where
                        t.charge_id = v_charge.charge_id;
        ...
        if v_barcode_lfdnkey is not null then
        ...
        v_return := nvl(v_barcode_begin, '')
                    || nvl(v_barcode_nr, '')
                    || v_barcode_ende;

        if length(v_return) > in_laenge then
            raise_isi_error(10,
 ...
 *******************************************************************************/
    ...
    begin
        if in_lte_hoehe != 0 then
            v_lte_hoehe := in_lte_hoehe;
        else
            v_lte_hoehe := null;
        end if;

        if in_lte_breite != 0 then
            v_lte_breite := in_lte_breite;
        else
            v_lte_breite := null;
        end if;

        if in_lte_tiefe != 0 then
            v_lte_tiefe := in_lte_tiefe;
        else
            v_lte_tiefe := null;
        end if;

        if in_lte_name is null then
 ...
 *******************************************************************************/
        ...
        lvs_lte_artikel_erz_v358_v2(in_sid,         -- in_sid                 in isi_sid.sid%type,
         ...
         in_menge_basis, -- in_menge_basis         in lvs_lam.menge_basis%type,
                                    ...
                                    in_lam_sel5,                       -- LAM_SEL5 N VARCHAR2(15)  Y     Parameter zusätzliche Selectionsparameter
                                     ...
                                     in_lam_sel9,                       -- LAM_SEL9 N VARCHAR2(15)  Y     Parameter zusätzliche Selectionsparameter
                                    in_lam_sel10,                      -- LAM_SEL10  N VARCHAR2(15)  Y     Parameter zusätzliche Selectionsparameter
                                     in_best_nr, in_best_pos, in_li_nr_lief);                    -- in lvs_lam.li_nr_lief%type
        commit;
    end;
    ...
    -- Init der Felder 0 nicht moeglich
        ...
        v_lte_breite := in_lte_breite;
        v_lte_tiefe := in_lte_tiefe;
        if v_lte_hoehe = 0 then
            v_lte_hoehe := null;
        end if;
        if v_lte_breite = 0 then
            v_lte_breite := null;
        end if;
        if v_lte_tiefe = 0 then
            v_lte_tiefe := null;
        end if;
        ...
        close c_lte;
        if
            ( v_lte.lte_akt_lhm = 0                                   -- LTE ist bereits bekannt aber leer, dann ausbuchen
        ...
        then
            lvs_p_lte.lvs_korr_te_ausbuchen(v_lte.sid, v_lte.firma_nr, v_lte.lte_id, v_lte.lte_status, v_lte.sid,
                                            v_lte.firma_nr, v_lte.lgr_ort, v_lte.lgr_platz, in_login_id);
            ...
            if not v_found then
                ...
                if not v_found then
                    raise_isi_error(c.fmid_artikelnummer_fehlt,
                                    lc.ec_p1(lc.o_tp1_default_u_artikel_fehlt,
                                             to_char(in_artikel)));
                end if;
      ...
      */
            ...
            if in_packschema_kopf_id is not null then
            ...
            v_lte.lte_id := lvs_lte_insert_v358(in_sid,                   -- SID
                                                in_firma_nr,              -- Firma
                                                nvl(in_lte_name, v_artikel.lte_name),  -- Palettemtype Bsp. 'EURO'
            ...
            then
                raise_isi_error(c.fmid_zuggang_buchen,
                                lc.ec(lc.o_txt_fehler_ware_anlegen_lte));
            end if;

            while in_menge > v_menge_ges loop
                if in_lhm_menge = 0 then
                    v_menge := in_menge;
                    v_menge_ges := in_menge;
                else
                    if in_menge > in_lhm_menge then
                        if v_menge_ges + in_lhm_menge > in_menge then
        ...
        -- -AG- BugFix:  20.08.2009 Fehler beim Berechnen des MHD Datums
                if not lvs_einl.lvs_lam_zugang_owner_size(--in_sid, in_firma_nr,
                v_artikel.artikel_id,
                                                          ...
                                                          in_best_pos,
                                                          null,
                                                          nvl(in_prod_datum, sysdate),
                                                          nvl(in_zug_datum, sysdate),
                                                          in_login_id,
                                                          v_menge,
                                                          in_lhm_name,
                                                          null,
        ...
        then
            raise_isi_error(c.fmid_lte_id_schon_vorhanden,
        ...
        or v_lte.lte_status = c.lte_ag_stat -- AG 23.06.2014 Auch im Status AG überschreiben wenn Werte gesetzt
         then
            if
                v_artikel.lte_menge > in_menge  -- -AG- 2020.12.04 Funktion verbesser - Wenn die Menge nicht die Vollmenge der LTE ist, dann Anteil berechnen
            ...
            update lvs_lte l
            set
                l.lte_vol_hoehe = nvl(v_lte_hoehe, l.lte_vol_hoehe),
                l.lte_vol_breite = nvl(v_lte_breite, l.lte_vol_breite),
                l.lte_vol_tiefe = nvl(v_lte_tiefe, l.lte_vol_tiefe),
                l.lte_vol = nvl(v_lte_tiefe, l.lte_vol_tiefe) * nvl(v_lte_hoehe, l.lte_vol_hoehe) * nvl(v_lte_breite, l.lte_vol_breite
                ) / 1000000000,
 ...
 *******************************************************************************/
    ...
    begin
        ...
        or v_lte.lte_akt_lhm = 0 then
            ...
            if not v_found then
                raise_isi_error(20,
                                lc.ec(lc.o_txt_default_artikel_fehlt));
            end if;
            ...
            v_lte.lte_id := lvs_lte_insert_v358(in_sid,                   -- SID
             ...
             in_login_id,              -- Login ID aktuelle User
                                                null,                     -- Lagerort
                                                 ...
                                                 c.lte_pf_stat,            -- Status Palletiert Fertig
                                                 in_sep_nve, null,
                                                null, null, null, null, null,                     -- Auto Depal ist unbekannt
                                                null,                     -- in_wickelprogramm      in lvs_lte.wickelprogramm%type,
                                                 null);                    -- in_wickelprogramm_einl in lvs_lte.wickelprogramm_einl%type)
            v_lhm_id := null;
            v_maske := isi_allg.c_get_firma_cfg_param(in_sid, in_firma_nr, 'LTE_GET_DUMMY_ART',   -- in_kategorie             in isi_firma_cfg.kategorie%type,
             ...
             'LVS_LTE_MASKE',       -- in_parameter_name        in isi_firma_cfg.parameter_name%type,
                                                      'DB',                  -- in_modul_name            in isi_firma_cfg.modul_name%type,
                                                       'CFG', 'Keine',               -- in_default_param_wert    in isi_firma_cfg.parameter_wert%type,
                                                       'STRING');             -- in_default_param_typ     in isi_firma_cfg.parameter_typ%type,
            if in_lte_id like v_maske                  -- Bsp Maske ist 115-9_________
             then
                v_art_id := isi_allg.c_get_firma_cfg_param(in_sid, in_firma_nr, 'LTE_GET_DUMMY_ART',   -- in_kategorie             in isi_firma_cfg.kategorie%type,
                 null,                  -- in_kategorie_ix          in isi_firma_cfg.kategorie_ix%type,
                 'LVS_LTE_MASKE'
                                                                                                           || '_'
                                                                                                           || v_maske,               -- in_parameter_name        in isi_firma_cfg.parameter_name%type,
                                                           'DB',                  -- in_modul_name            in isi_firma_cfg.modul_name%type,
                                                            'CFG', 0,                     -- in_default_param_wert    in isi_firma_cfg.parameter_wert%type,
      ...
      -- -AG- BugFix:  20.08.2009 Fehler beim Berechnen des MHD Datums
            if not lvs_einl.lvs_lam_zugang(in_sid, in_firma_nr, 0, in_lte_id, v_lhm_id,
                                           ...
                                           sysdate, in_login_id, v_artikel.lte_menge, null, null,
                                           null, null, null, null, null,
                                           null, null, null, null, null,
                                           null, null, null,     -- v_artikel.artikel_p1,
                                            null,     -- v_artikel.artikel_p2,
                                            null,     -- v_artikel.artikel_p3,
                                           null,     -- v_artikel.artikel_p4,
                                            null,     -- v_artikel.artikel_p5,
                                           ...
                                           v_lhm_id,      -- FAE ID nicht bekannt
                                            null, null, null, 0)             -- -AG- Hier bleibt die Nummer immer 0, da hier nicht das modul BDE greift
                                            > 0 then
                raise_isi_error(30,
                                lc.ec(lc.o_txt_fehler_ware_anlegen_lte));
            end if;
        ...
        or v_lte.lte_status = c.lte_af_stat
        or v_lte.lte_status = c.lte_ag_stat
        or v_lte.waren_typ = c.leerpal then
            update lvs_lte l
            set
                l.lte_vol_hoehe = in_lte_hoehe,
 ...
 *******************************************************************************/
    ...
    begin
        v_result := lvs_lte_erzeugen(in_sid, in_firma_nr, in_linie, in_lgr_platz, in_login_id,
                                     null, null);
 ...
 *******************************************************************************/
  ...
  -- Holen des Auftrags genau für diese Leitzahl an dieser Maschine
        ...
        where
            ...
            and fa_a.leitzahl = v_order_pos.leitzahl
            and fa_a.fa_ag = v_order_pos.fa_ag
            and nvl(fa_a.fa_upos, 0) = nvl(v_order_pos.fa_upos, 0);
        ...
        where
            ...
            and pos.vorgang_typ = decode(v_prod_linie_waren.owner_address_id, null, 'WEE',  -- BUS = WE-Konsi-Lager dann ist in der ISI-Order Typ BK und Vorg_typ 'WEK'
             'KWE')
    ...
    begin
        ...
        if not v_found then
            raise_isi_error(10,
                            lc.ec_p1(lc.o_tp1_lvs_prod_linie_fehlt,
                                     to_char(in_linie)));
        end if;
      ...
      -- Damit wird die erste LHM angelegt
        ...
        if v_prod_linie_waren.artikel_id is null then        -- Alles gelesen und ungültige Daten vorhanden!!!
            v_err_nr := 10;
        ...
        if not v_found then
            close c_linie_waren;
            raise_isi_error(20,
                            lc.ec_p1(lc.o_tp1_lvs_prod_l_ware_leer,
                                     to_char(in_linie)));
    ...
    -- 20180210 Die Charge wir aus dem sufix und der eigentlichen Charge zusammengebaut
        ...
        if
            v_prod_linie_waren.hersteller_kuerzel_liste is not null
            and v_prod_linie_waren.hersteller_kuerzel_liste != ';'
            and isi_p_base.get_artikel_ctrl_typ(in_sid,
                                                v_prod_linie_waren.artikel_id,
                                                substr(v_prod_linie_waren.hersteller_kuerzel_liste,
                                                       1,
                                                       length(v_prod_linie_waren.hersteller_kuerzel_liste) - 1),
                                                v_art_ctrl)
        then
        ...
        if v_prod_linie_waren.lte_id is null then
            v_lte_id := null;
            v_kunden_nr := null;                                    -- INIT keine LTE_ID
            if isi_p_base.get_isi_artikel(in_sid, v_prod_linie_waren.artikel_id, v_artikel) then
                v_kunden_nr := v_artikel.artikel_fuer_kunde_etikett;
        -- -MM- Erweiterung 28.07.2014 Kundenspezifische LHM und LTE IDs
                v_lte_id := lvs_p_lte.lvs_lte_lhm_next_id_v35(in_sid, in_firma_nr, c.basis_lte, v_charge, v_prod_linie_waren.artikel_id
                ,
                                                              v_kunden_nr, v_typ, v_h_tag);
        ...
        v_lte_id := lvs_lte_insert_v358(in_sid,                   -- SID
         ...
         in_login_id,              -- Login ID aktuelle User
                                        v_prod_linie.linie_lagerort, in_lgr_platz,             -- Lagerplatz, NULL ist keiner
                                         v_lte_status,             -- Status Palletiert Fertig
                                         null, v_prod_linie.lte_eti_druck_status,-- Druckstatus
                                        v_charge_id,                      -- Charge ID
                                         v_charge,        -- Charge
        ...
        if v_prod_linie.res_string is not null then
            update lvs_lte lte                                          -- Eintrag ist bereits vorhanden
            set
                lte.res_string_statisch = v_prod_linie.res_string
            where
                    lte.sid = in_sid
    ...
    -- Initialisierung durch Werte aus LinieWaren
      ...
      --------------------------------------------------------------------------------------------------------------------
        ...
        if v_prod_linie_waren.lhm_id is null -- -AG- 2017.07.24 ID wurde übergeben
         ...
         then
            if v_prod_linie_waren.menge_je_karton is not null then
        -- -MM- Erweiterung 28.07.2014 Kundenspezifische LHM und LTE IDs
                v_lhm_id := lvs_p_lte.lvs_lte_lhm_next_id_v35(in_sid, in_firma_nr, c.basis_lhm, v_charge, v_prod_linie_waren.artikel_id
                ,
                                                              v_kunden_nr, v_typ, v_h_tag);
      ...
      --v_prod_linie_waren.menge_je_karton := v_prod_linie_waren.menge;
        ...
        if v_prod_linie_waren.lhm_eti_druck_status = 'RM' then
    ...
    -- -AG- 15.09.2008 Naechsten Karton eintragen im FA
        ...
        where
                fa.sid = in_sid
            and fa.firma_nr = in_firma_nr
            and fa.leitzahl = v_prod_linie_waren.leitzahl
            and fa.fa_ag = v_prod_linie_waren.fa_pos
            and nvl(fa.fa_upos, 0) = nvl(v_prod_linie_waren.fa_upos, 0)
        returning fa.lhm_anz_ist into v_lhm_anz_ist;

        if lvs_einl.lvs_lam_zugang_owner_size(v_prod_linie_waren.artikel_id,
                                              v_lte_id,
                                              v_lhm_id,
                                              v_charge_id,
                                              null,
                                              v_order_pos.leitzahl,
                                              v_order_pos.fa_ag,
                                              v_order_pos.fa_upos,
                                              v_prod_linie_waren.auftragsnr,
                                              v_prod_linie_waren.best_nr,
                                              v_prod_linie_waren.best_pos,
                                              v_prod_linie.res_id,
                                              v_prod_linie_waren.produktionsdatum,
                                              sysdate,
                                              in_login_id,
                                              v_prod_linie_waren.menge_je_karton,
                                              v_prod_linie_waren.lhm_name,
                                              v_prod_linie_waren.kunden_nr,
                                              v_prod_linie_waren.kd_art_nr,
                                              v_prod_linie_waren.mhd,
                                              v_prod_linie_waren.zeichnung,
                                              v_prod_linie_waren.zeichnung_index,
                                              null,
                                              v_prod_linie_waren.lieferant_nr,
                                              v_prod_linie_waren.li_nr_lief,
                                              v_prod_linie_waren.lte_id_lieferant,
                                              v_prod_linie_waren.sonst_id_lieferant,
                                              null,
                                              null,
                                              v_prod_linie_waren.labor_status,
                                              v_prod_linie_waren.lam_p1,
                                              v_prod_linie_waren.lam_p2,
                                              v_prod_linie_waren.lam_p3,
                                              v_prod_linie_waren.lam_p4,
                                              v_prod_linie_waren.lam_p5,
                                              v_prod_linie_waren.lam_p6,
                                              v_prod_linie_waren.lam_p7,
                                              v_prod_linie_waren.lam_p8,
                                              v_prod_linie_waren.lam_p9,
            ...
            if v_vor_lhm_id is not null then
                if in_et_drucker_name is not null then
        ...
        -- ggf. weitere LHMs für aktuelle LTE anlegen
          ...
          -- -MM- Erweiterung 28.07.2014 Kundenspezifische LHM und LTE IDs
                    v_lhm_id := lvs_p_lte.lvs_lte_lhm_next_id_v35(in_sid, in_firma_nr, c.basis_lhm, v_charge, v_prod_linie_waren.artikel_id
                    ,
                                                                  v_prod_linie_waren.kunden_nr, v_typ, v_h_tag);

                    v_vor_lhm_id := v_lhm_id;
                    if v_prod_linie_waren.menge_je_karton + v_menge_erzeugt > v_prod_linie_waren.menge then
            -- Restmenge?
      ...
      -- weitere Positionen auf der LTE (laut prod_linie_waren)
            loop
                ...
                v_lhm_eti_druck_status := v_prod_linie_waren.lhm_eti_druck_status;
                if v_prod_linie_waren.lhm_eti_druck_status = 'RM' then
                    if v_gleiche_menge != v_prod_linie_waren.menge_je_karton then
            -- Unterschiedlich zum ersten Eintrag, also andere Menge
                ...
                if v_prod_linie_waren.artikel_id is not null then -- Wenn keine Artikel_ID eingetragen dann übergehen !!!
              ...
              -- -MM- Erweiterung 28.07.2014 Kundenspezifische LHM und LTE IDs
                    ...
                    if lvs_einl.lvs_lam_zugang_owner_size(v_prod_linie_waren.artikel_id, v_lte_id, v_lhm_id, v_charge_id, null,
                                                          v_order_pos.leitzahl, v_order_pos.fa_ag, v_order_pos.fa_upos, v_prod_linie_waren.auftragsnr
                                                          , v_prod_linie_waren.best_nr,
                                                          v_prod_linie_waren.best_pos, v_prod_linie.res_id, v_prod_linie_waren.produktionsdatum
                                                          , sysdate, in_login_id,
                                                          v_prod_linie_waren.menge_je_karton, v_prod_linie_waren.lhm_name, v_prod_linie_waren.kunden_nr
                                                          , v_prod_linie_waren.kd_art_nr, v_prod_linie_waren.mhd,
                                                          v_prod_linie_waren.zeichnung, v_prod_linie_waren.zeichnung_index, null, v_prod_linie_waren.lieferant_nr
                                                          , v_prod_linie_waren.li_nr_lief,
                                                          ...
                                                          , v_prod_linie_waren.lam_p4, v_prod_linie_waren.lam_p5,
                                                          v_prod_linie_waren.lam_p6, v_prod_linie_waren.lam_p7, v_prod_linie_waren.lam_p8
                                                          , v_prod_linie_waren.lam_p9, v_prod_linie_waren.lam_p10,
                                                          v_lhm_eti_druck_status, v_prod_linie_waren.lam_text, v_prod_linie_waren.labor_text
                                                          , v_lhm_id, null,
                                                          null, v_prod_linie_waren.qs_status, v_lhm_anz_ist, v_prod_linie_waren.owner_address_id
                                                          , null,
                                                          null, null, v_prod_linie_waren.lam_sel1, v_prod_linie_waren.lam_sel2, v_prod_linie_waren.lam_sel3
                                                          ,
                                                          v_prod_linie_waren.lam_sel4, v_prod_linie_waren.lam_sel5, v_prod_linie_waren.lam_sel6
                                                          , v_prod_linie_waren.lam_sel7, v_prod_linie_waren.lam_sel8,
                                                          v_prod_linie_waren.lam_sel9, v_prod_linie_waren.lam_sel10, v_prod_linie_waren.hersteller_kuerzel_liste
                                                          ) <= 0 -- -AG- 2010.01.13 BugFix
                                                           then
                        raise_isi_error(30,
                                        lc.ec(lc.o_txt_fehler_ware_anlegen_lte));
                    else
                ...
                -- -MM- Erweiterung 28.07.2014 Kundenspezifische LHM und LTE IDs
                  ...
                  -- Restmenge?
                                ...
                                null,
                                                                      ...
                                                                      v_prod_linie_waren.best_pos, v_prod_linie.res_id, v_prod_linie_waren.produktionsdatum
                                                                      , sysdate, in_login_id,
                                                                      v_prod_linie_waren.menge_je_karton, v_prod_linie_waren.lhm_name
                                                                      , v_prod_linie_waren.kunden_nr, v_prod_linie_waren.kd_art_nr, v_prod_linie_waren.mhd
                                                                      ,
                                                                      v_prod_linie_waren.zeichnung, v_prod_linie_waren.zeichnung_index
                                                                      , null, v_prod_linie_waren.lieferant_nr, v_prod_linie_waren.li_nr_lief
                                                                      ,
                                                                      v_prod_linie_waren.lte_id_lieferant, v_prod_linie_waren.sonst_id_lieferant
                                                                      , null, null, v_prod_linie_waren.labor_status,
                                                                      v_prod_linie_waren.lam_p1, v_prod_linie_waren.lam_p2, v_prod_linie_waren.lam_p3
                                                                      ...
                                                                      null, v_prod_linie_waren.qs_status, v_lhm_anz_ist, v_prod_linie_waren.owner_address_id
                                                                      , null,
                                                                      null, null, v_prod_linie_waren.lam_sel1, v_prod_linie_waren.lam_sel2
                                                                      , v_prod_linie_waren.lam_sel3,
                                                                      v_prod_linie_waren.lam_sel4, v_prod_linie_waren.lam_sel5, v_prod_linie_waren.lam_sel6
                                                                      , v_prod_linie_waren.lam_sel7, v_prod_linie_waren.lam_sel8,
                                                                      v_prod_linie_waren.lam_sel9, v_prod_linie_waren.lam_sel10, v_prod_linie_waren.hersteller_kuerzel_liste
        ...
        update lvs_lte
        set
            lte_vol_hoehe = v_prod_linie.lte_vol_hoehe,
            lte_vol_breite = v_prod_linie.lte_vol_breite,
            lte_vol_tiefe = v_prod_linie.lte_vol_tiefe,
            lte_vol = v_prod_linie.lte_vol_hoehe * v_prod_linie.lte_vol_breite * v_prod_linie.lte_vol_tiefe / 1000000000
        where
            lte_id = v_lte_id;
 ...
 *******************************************************************************/
    ...
    begin
        return lvs_c_lte_insert_v35(in_sid, in_firma_nr, in_lte_name, in_lte_id, in_ls_login_id,
                                    in_lgr_ort, in_lgr_platz, in_lte_status, in_sep_nve, in_lte_eti_druck_status,
                                    in_charge_id, in_charge, in_artikel_id, null);                   --in_packschema_kopf_id
    end;
 ...
 *******************************************************************************/
    ...
    begin
        return lvs_c_lte_insert_v358(in_sid, in_firma_nr, in_lte_name, in_lte_id, in_ls_login_id,
                                     in_lgr_ort, in_lgr_platz, in_lte_status, in_sep_nve, in_lte_eti_druck_status,
                                     in_charge_id, in_charge, in_artikel_id, in_packschema_kopf_id,   --in_packschema_kopf_id
 ...
 *******************************************************************************/
    ...
    begin
        v_lte_id := lvs_lte_insert_v358(in_sid, in_firma_nr, in_lte_name, in_lte_id, in_ls_login_id,
                                        in_lgr_ort, in_lgr_platz, in_lte_status, in_sep_nve, in_lte_eti_druck_status,
                                        in_charge_id, in_charge, in_artikel_id, in_packschema_kopf_id, in_auto_depal,
    ...
    function lvs_c_lte_insert_v359 (
        ...
        v_lte_id lvs_lte.lte_id%type;       -- ID des LTE
    begin
        v_lte_id := lvs_lte_insert_v359(in_sid, in_firma_nr, in_lte_name, in_lte_id, in_ls_login_id,
                                        in_lgr_ort, in_lgr_platz, in_lte_status, in_sep_nve, in_lte_eti_druck_status,
                                        in_charge_id, in_charge, in_artikel_id, in_packschema_kopf_id, in_auto_depal,
                                        in_wickelprogramm, in_wickelprogramm_einl, in_typ, in_h_tag);
 ...
 *******************************************************************************/
    ...
    begin
        return lvs_lte_insert_v35(in_sid, in_firma_nr, in_lte_name, in_lte_id, in_ls_login_id,
                                  in_lgr_ort, in_lgr_platz, in_lte_status, in_sep_nve, in_lte_eti_druck_status,
                                  in_charge_id, in_charge, in_artikel_id, null);   --in_packschema_kopf_id
    end;
 ...
 *******************************************************************************/
    ...
    begin
        return lvs_lte_insert_v358(in_sid, in_firma_nr, in_lte_name, in_lte_id, in_ls_login_id,
                                   in_lgr_ort, in_lgr_platz, in_lte_status, in_sep_nve, in_lte_eti_druck_status,
                                   in_charge_id, in_charge, in_artikel_id, in_packschema_kopf_id,     --in_packschema_kopf_id
 ...
 *******************************************************************************/
    ...
    begin
        return lvs_lte_insert_v359(in_sid,                   -- SID
                                   ...
                                   nvl(in_lte_status, c.lte_pf_stat),       -- Status Palletiert Fertig
                                   in_sep_nve,               -- Seperate NVE Nummer
                                   null,                     -- Druckstatus
    ...
    function lvs_lte_insert_v359 (
        ...
        order by
            lte.letzte_inventur_datum desc;
        ...
        where
                lte_cfg.sid = in_sid
            and lte_cfg.firma_nr = in_firma_nr
            and lte_cfg.lte_name = in_lte_name;
    ...
    begin
        ...
        close c_lte_cfg;
        if not v_found then
            raise_isi_error(c.fmid_palettetyp_fehlt,
                            lc.ec_p1(lc.o_tp1_paletten_typ_fehlt,
                                     to_char(nvl(in_lte_name, '????'))));
        end if;
        ...
        if v_lgr_platz is not null then
            ...
            close c_lgr;
            if not v_found then
                raise_isi_error(c.fmid_quelle_existiert_nicht,
                                lc.ec_p2(lc.o_tp2_platz_existiert_nicht, v_lgr_platz, in_lte_id));
            else
                v_lgr_ort := v_lgr.lgr_ort;
                if v_lgr.akt_inventur_id is not null then     -- Fehler Lagerplatz ist in Inventur
                    raise_isi_error(c.fmid_lagerplatz_gesperrt,
                                    lc.ec_p1(lc.o_tp1_lagerplatz_in_inventur, v_lgr_platz));
                end if;

                if v_lgr.gesperrt <> c.lgr_gesperrt_f then     -- Fehler Lagerplatz ist gesperrt
                    raise_isi_error(c.fmid_lagerplatz_gesperrt,
                                    lc.ec_p2(lc.o_tp2_lagerplatz_gesperrt, v_lgr_platz, v_lgr.gesp_grund));
                end if;
        ...
        if v_lte_id is null then                   -- Keine Nummer Übergeben
            if
                nvl(v_lgr.lgr_verwendung, c.we) <> c.we
                and nvl(v_lgr.lgr_verwendung, c.we) <> c.lgr_typ_puffer
            then     -- Fehler Lagerplatz ist kein WarenEingang
                raise_isi_error(c.fmid_platz_kein_we,
                                lc.ec_p1(lc.o_tp1_platz_kein_we, v_lgr_platz));
            end if;
      -- Holen der neuen Nummer
            if v_lte_cfg.virtuell != 'L' -- Virtuell gleich nach Verarbeitung löschen
             then
        -- -MM- Erweiterung 28.07.2014 Kundenspezifische LHM und LTE IDs
                v_lte_id := lvs_lte_lhm_next_id_v35(in_sid, in_firma_nr, c.basis_lte, in_charge, in_artikel_id,
                                                    null, in_typ, in_h_tag);

            else
        -- -MM- Erweiterung 28.07.2014 Kundenspezifische LHM und LTE IDs
                v_lte_id := lvs_lte_lhm_next_id_v35(in_sid, in_firma_nr, c.basis_lte_vl, in_charge, in_artikel_id,
                                                    null, in_typ, in_h_tag);
            end if;
            ...
            if v_firma.lte_barcode_type = c.lte_barcode_spez then
                isi_utils.spez_barcode_lfdn(in_sid, in_firma_nr, v_lte_id, c.basis_lte, v_barcode_id);
        -- Z.Zt. Nur Charge 'CCC'
                update lvs_charge c
                set
                    c.charge_lte_lfdn = nvl(v_barcode_id,
                                            nvl(c.charge_lte_lfdn, 0) + 1)
                where
                        c.charge_id = in_charge_id
                    and nvl(c.charge_lte_lfdn, 0) < nvl(v_barcode_id,
                                                        nvl(c.charge_lte_lfdn, 0) + 1);
        ...
        then
            lvs_p_lte.lvs_korr_te_ausbuchen(v_lte.sid, v_lte.firma_nr, v_lte.lte_id, v_lte.lte_status, v_lte.sid,
                                            v_lte.firma_nr, v_lte.lgr_ort, v_lte.lgr_platz, in_ls_login_id);
        ...
        if not v_found then
            ...
            close c_lte_hist;
            v_sep_nve := null;
            if v_firma.sep_nve_kopf is not null then
                if in_sep_nve is null then
                    v_err_nr := 22;
                    v_err_text := lc.ec_p1(lc.o_tp1_seq_vorgang, 'seq_sep_nve_id');
                    select
                        seq_sep_nve_nr.nextval
                    into v_sep_nve_nr
                    from
                        dual;  -- Hole neue Nummer aus der Seq.LTE
                    v_sep_nve := v_firma.sep_nve_kopf
                                 || lpad(
                        to_char(v_sep_nve_nr),
                        17 - length(v_firma.sep_nve_kopf),
                        '0'
                    );
      ...
      -- Volumen rechnen in m3
            ...
            insert into lvs_lte values ( in_sid,
                                         in_firma_nr,
                                         v_lte_id,
                                         in_lte_name,
                                         v_lgr_ort,
            ...
            if
                v_lte.lgr_platz is not null
                and                              -- LTE bereits registriert und ist auf einem
                 v_lte.lgr_platz <> in_lgr_platz
            then                         -- anderen Lagerplatz
                if
                    nvl(v_lgr.lgr_verwendung, ' ') <> c.we
                    and nvl(v_lgr.lgr_verwendung, c.we) <> c.lgr_typ_puffer
                then           -- Fehler Lagerplatz ist kein WarenEingang
                    raise_isi_error(c.fmid_platz_kein_we,
                                    lc.ec_p1(lc.o_tp1_platz_kein_we, v_lgr_platz));
 ...
 *******************************************************************************/
    ...
    --  LTE status prüfen nur PF und KF paletten dürfen gelöscht werden
        if
            (
                ( v_lte.lte_status <> c.lte_pf_stat )   -- palettiert frei
                and ( v_lte.lte_status <> c.lte_kf_stat )
            )  -- Korrekturstatus
            and v_lte.lte_status <> nvl(in_status, '')
        then
            raise_isi_error(10,
                            lc.ec_p2(lc.o_tp2_lte_id_st_n_loeschbar,
                                     nvl(in_lte_id, 'NULL'),
                                     v_lte.lte_status));
        end if;
    --  hat die LTE einen Ziellagerplatz transport aktiv ?
        if (
            v_lte.ziel_lgr_platz is not null
            and v_lte.lte_status <> c.lte_pf_stat    -- palettiert frei
            and v_lte.lte_status <> c.lte_kf_stat
        )   -- Korrekturstatus
         then
            raise_isi_error(11,
                            lc.ec_p2(lc.o_tp2_lte_id_trans_n_loeschbar,
                                     nvl(in_lte_id, 'NULL'),
    ...
    -- der für das löschen verantwortliche user wird in der lte.ls_login_id festgehalten -BW-
        update lvs_lte lte
        set
            lte.ls_login_id = in_ls_login_id
        where
                lte.lte_id = in_lte_id
            and lte.sid = in_sid;
    -- LTE Loeschen
 ...
 *******************************************************************************/
    function lvs_lte_lhm_next_id_kd (
        in_sid         in isi_sid.sid%type,
        in_firma_nr    in isi_firma.firma_nr%type,
        in_barcode_ref in varchar2,
        in_charge      in lvs_charge.charge_bez%type,
    ...
    begin
        if not isi_p_base.get_adress_next_lfdn_id(in_sid, in_adr.adress_id, v_next_nr) then
            raise_isi_error(10,
                            lc.ec(lc.o_txt_id_leangenueberlauf));
        end if;
        ...
        reset_isi_error();
        v_next_barcode := format_barcode(in_sid, v_kopf, v_next_nr, v_laenge_barcode, v_seq_barcode,
                                         in_charge, in_artikel_id, in_barcode_ref, null, in_typ,
                                         in_h_tag);
 ...
 *******************************************************************************/
    function lvs_lte_lhm_next_id_std (
        in_sid         in isi_sid.sid%type,
        in_firma_nr    in isi_firma.firma_nr%type,
        in_barcode_ref in varchar2,
        in_charge      in lvs_charge.charge_bez%type,
    ...
    -- Daten Firenstamm holen
        ...
        if in_barcode_ref = c.basis_lte then
            if v_isi_firma.lte_barcode_basis = 'SEQ' then
                v_seq_barcode := 'SEQ';
                select
                    seq_lam_lgr_id.nextval
                into v_next_nr
                from
                    dual;  -- Hole neue Nummer aus der Seq. (Gilt auch für LTE)
            elsif v_isi_firma.lte_barcode_basis = c.basis_lte then
                v_seq_barcode := 'LTE';
                select
                    seq_lte_lgr_id.nextval
                into v_next_nr
                from
                    dual;  -- Hole neue Nummer aus der Seq.LTE
            end if;
        ...
        if in_barcode_ref = c.basis_lte_vl then
            ...
            v_laenge_barcode := v_isi_firma.lhm_barcode_laenge + length(c.basis_lte_vl) - nvl(
                length(v_isi_firma.lhm_barcode_kopf),
                0
            );
 ...
 *******************************************************************************/
    ...
    begin
        lvs_get_druck_daten(in_lte_id, in_kunden_nr, null, out_lte_sid, out_lte_firma_nr,
                            out_rave_datei, out_rave_report_name, out_job_daten_typ, out_job_daten, out_anz_drucke);
    end;
    ...
    -------------------------------------------------------------------------------------------------------
        ...
        select
            max(nvl(lam.owner_address_id, 0))
        from
            lvs_lam lam
        where
            lam.lte_id = in_id;
        ...
        select
            stradd_distinct(a.artikel_fuer_kunde_etikett)
        from
            lvs_lam     lam,
            isi_artikel a
        where
        ...
        from
            isi_adressen adr
        where
                adr.sid = nvl(v_lte.sid, v_lhm.sid)
        ...
        from
            isi_firma firma
        where
                firma.sid = nvl(v_lte.sid, v_lhm.sid)
        ...
        from
            isi_etiketten et
        where
                et.sid = nvl(v_lte.sid, v_lhm.sid)
    ...
    begin
        ...
        if not v_found then               -- Ganz schlecht, weil es diese LTE nicht gibt
            ...
            close c_lhm;
            v_lte.lte_id := null;
            if not v_found then               -- Ganz schlecht, weil es auch diese LHM nicht gibt
                raise_isi_error(10,
                                lc.ec_p1(lc.o_tp1_lte_id_fehlt,
                                         nvl(
                          to_char(in_id),
            ...
            if v_adr_id is not null then
                open c_eigentuemer;
                fetch c_eigentuemer into v_kunde;
                close c_eigentuemer;
                if v_kunde.lhm_etiketten_name is null
                   or v_kunde.lhm_etiketten_typ is null then
                    v_adr_id := null;
                end if;
            ...
            if v_adr_id is not null then
                ...
                if v_kunde.lte_etiketten_name is null
                   or v_kunde.lte_etiketten_typ is null then
                    v_adr_id := null;
                end if;
        ...
        if v_adr_id is null
           or in_kunden_nr is not null then
            if in_kunden_nr is not null then
      ...
      -- -AG- BugFix: 15.12.2010 Artikeldaten müssen hier gelesen werden
            ...
            close c_art;
            out_anz_drucke := nvl(v_art.anz_etikett_je_lte,
                                  nvl(v_firma.anz_etikett_je_lte, 1));
            ...
            or v_kunde.lte_etiketten_typ is null then
                if not v_found then
                    raise_isi_error(10,
                                    lc.ec_p1(lc.o_tp1_firmenstamm_fehlt,
                                             nvl(
                              to_char(out_lte_firma_nr),
                              'NULL'
                          )));
                end if;
            ...
            or v_kunde.lhm_etiketten_typ is null then
                ...
                if not v_found then
                    raise_isi_error(20,
                                    lc.ec_p1(lc.o_tp1_lhm_id_ohne_bestand,
                                             nvl(
                              to_char(in_id),
                              'NULL'
                          )));
      ...
      -- out_anz_drucke := 1;
            out_anz_drucke := isi_allg.get_firma_cfg_param(v_lhm.sid, v_lhm.firma_nr, 'CFG',                    -- in_kategorie             in isi_firma_cfg.kategorie%type,
        ...
        close c_etiketten;
        if not v_found then               -- Ganz schlecht, weil es kein Etikettenlayout gibt
            raise_isi_error(20,
                            lc.ec_p1(lc.o_tp1_etiketten_layout_fehlt, v_etikett));
        end if;
    ...
    -- (-AM-) 080826  Seaquist 4878, Karton-Versand-Etikett bzw. Vormontagelabel
        ...
        or v_etiketten.rave_datei = 'etiketten_seaquist_Krt.rav' then
            out_job_daten := z_seaquist_druck.vda_etikett_vers_krt(v_firma.sid, v_firma.firma_nr, in_id);
        elsif v_etiketten.rave_datei = 'etiketten_seaquist_LteVers.rav' then
      ...
      -- out_job_daten := z_gtp_get_print_data(v_firma.sid, v_firma.firma_nr, null, null, null, in_id, v_etiketten.rave_datei);
        ...
        or v_etikett_typ is not null then
            ...
            raise_isi_error(20,
                            lc.ec_p1(lc.o_tp1_eti_datei_unterstuetzt, v_etiketten.rave_datei));
        end if;
 ...
 *******************************************************************************/
    ...
    begin
        lvs_lte_get_druck_daten(in_lte_id, in_kunden_nr, v_lte_sid, v_lte_firma_nr, v_rave_datei,
                                v_rave_report_name, v_job_daten_typ, v_job_daten, v_anzahl_drucke);

        if lvs_p_base.get_lam_by_lhm_id(v_lte_sid, v_lte_firma_nr, in_lte_id, v_lam) then
            if
                v_lam.res_id is not null
                and isi_p_base.get_resource(v_lte_sid, v_lam.res_id, v_res)
            then
                if
                    v_res.kategorie = 'PRMSD'
                    and v_res.kategorie_typ is not null
                then
                    v_rave_report_name := v_rave_report_name
                                          || '_'
                                          || v_res.kategorie_typ;
                end if;
        ...
        print_engine.insert_new_job(v_lte_sid, v_lte_firma_nr, v_rave_datei, v_rave_report_name, v_job_daten_typ,
                                    v_job_daten, in_drucker_name, v_anzahl_drucke, v_job_nr);

        update lvs_lte l
        set
            l.lte_eti_druck_status = 'D'
        where
                l.sid = v_lte_sid
 ...
 *******************************************************************************/
    ...
    begin
        ...
        loop
            ...
            v_lte_id := lvs_lte_erzeugen(in_sid, in_firma_nr, in_linie, in_lgr_platz, in_login_id,
                                         in_drucker, in_et_drucker_name);

            if in_drucker is not null then
                result := lvs_lte_drucken(v_lte_id,
                                          nvl(in_kunden_nr, v_kunden_nr),
                                          in_drucker);
 ...
 *******************************************************************************/
    ...
    begin
        lvs_lte_lief_erzeugen(in_lte_id, in_liefers, in_kunde);
        commit;
    exception
    -- Im Fehlerfall is der Fehlertext bereits gesetzt.
        when v_error then  -- Update 2011 show Exception Source Line
    ...
    procedure lvs_lte_lief_erzeugen (
        ...
        where
            ...
            and nvl(ord.charge_id,
                    nvl(v_lam.charge_id, -1)) = nvl(v_lam.charge_id, -1)
            and nvl(ord.seriennr_id,
                    nvl(v_lam.serie_id, -1)) = nvl(v_lam.serie_id, -1);
    ...
    begin
        ...
        if v_lte.waren_typ = c.leerpal then
            raise_isi_error(11,
                            lc.ec_p1(lc.o_tp1_lte_id_ist_leer,
                                     nvl(in_lte_id, 'NULL')));
        end if;
        ...
        if v_found then
            raise_isi_error(15,
                            lc.ec_p3(lc.o_tp3_lte_m_liefers_vorhaden,
                                     nvl(in_lte_id, 'NULL'),
                                     v_liefs.li_nr,
                                     v_liefs.li_pos_nr));
        end if;
        ...
        loop
            ...
            if
                v_order_pos.sid = v_lte.sid
                and v_order_pos.firma_nr = v_lte.firma_nr
                and v_order_pos.li_nr = in_liefers
                and v_order_pos.artikel_id = v_lam.artikel_id
                and nvl(v_order_pos.charge_id,
                        nvl(v_lam.charge_id, -1)) = nvl(v_lam.charge_id, -1)
                and nvl(v_order_pos.seriennr_id,
                        nvl(v_lam.serie_id, -1)) = nvl(v_lam.serie_id, -1)
            then
                v_found := true;
            else
            ...
            v_err_nr := null;
            v_err_text := null;
            lvs_ausl.lvs_liefers_erzeuge_daten(v_transport, v_lam, v_order_pos, null, v_lte,
                                               v_lhm);
 ...
 *******************************************************************************/
    ...
    procedure lvs_lte_trans_h_get_lief_kd (
        ...
        select
            *
        from
            isi_transport_hist trh
        where
                trh.sid = v_lte.sid
            and trh.firma_nr = v_lte.firma_nr
            and trh.lte_id = in_lte_id
            and trh.transp_id in (
                select
                    max(trh2.transp_id)
                from
                    isi_transport_hist trh2
                where
                        trh2.sid = v_lte.sid
    ...
    begin
        ...
        if not v_found then
            raise_isi_error(10,
                            lc.ec_p1(lc.o_tp1_lte_id_fehlt, in_lte_id));
        end if;
 ...
 *******************************************************************************/
    ...
    begin
        ...
        then
            ...
            loop
                ...
                close c_order_pos;
                if v_order_pos.li_nr = in_lief_nr then
                    if in_grund is not null then
        ...
        -- Wenn Auftrag vom HOST dann melden
                    ...
                    v_err_nr := 20;
                    v_err_text := lc.ec_p1(lc.o_tp1_lte_liefers_diff,
                                           nvl(in_lte_id, 'NULL'));
            ...
            if c_lam%rowcount = 0 then
                v_err_nr := 10;
                v_err_text := lc.ec_p1(lc.o_tp1_lte_id_ausgel,
                                       nvl(in_lte_id, 'NULL'));
  ...
  -- Im Fehlerfall is der Fehlertext bereits gesetzt.
    ...
    begin
        ...
        v_result := lvs_transport.lvs_transp_lte(in_sid, in_firma_nr, in_modul_erzeuger, null, c.c_false,
                                                 null, in_user_id, null, null, in_prio,
                                                 0, 0, 0, null, in_zu_lgr_platz,
                                                 in_lte_id, null, c.c_false, null, null,
                                                 null, null,             -- in_fahrzeuge_IDs Transport soll in jedem Fall erzeugt werden, auch wenn das Fahrzeug gestört ist
                                                  null, v_transport_gruppe, v_transp_id,
                                                 null, in_fetig_bis);

        if v_result != 0 then
            raise_isi_error(10,
                            c.decode_function_fehler(v_result));
        end if;
  ...
  -- Im Fehlerfall is der Fehlertext bereits gesetzt.
    ...
    begin
        lvs_lte_transp_gen(in_sid, in_firma_nr, in_modul_erzeuger, in_lte_id, in_zu_lgr_platz,
                           in_prio, in_user_id, in_fetig_bis);
 ...
 *******************************************************************************/
    ...
    begin
        ...
        if not v_found then
            raise_isi_error(10,
                            lc.ec_p2(lc.o_tp2_platz_existiert_nicht, in_lgr_platz, in_lte_id));
        end if;
        ...
        v_lgr_text := lvs_platz.lvs_platz_einl_pruef_err_t_r30(v_lte, v_basis_lte_name, v_lte_cfg.flaechen_stellplatz_erf, v_lgr, 'E'
        ,
                                                               null);
 ...
 *******************************************************************************/
  ...
  -- Zum Lesen der Positionen die bereits fertig sind und auf 'E' gesetzt werden müssen
    ...
    begin
        ...
        if
            nvl(v_lte.order_vorgang_id, 0) != in_tour_nr
            and nvl(in_tour_nr, 0) > 0 -- WK 2015-09-16: 0 wie NULL gleich behandeln
        then
            raise_isi_error(10,
                            lc.ec_p2(lc.o_tp2_lte_id_res_err,
                                     nvl(in_lte_id, 'NULL'),
                                     to_char(in_tour_nr)));
        end if;
        ...
        if v_lte.lte_status = c.lte_af_stat then
            ...
            if not v_found then
                raise_isi_error(30,
                                lc.ec_p1(lc.o_tp1_lagerplatz_fehlt,
      ...
      --                 nicht umgehen kann
                ...
                then
                    open c_lam_ausl;
                    fetch c_lam_ausl into v_lam_anz;
    ...
    -- 23.02.2008 -AG- In den Abgangsbuchungen werden jetzt auch Lieferscheinnummer und Pos. gespeichert
            v_lam.lam_id := lvs_ausl.lvs_lam_abgang(v_lte.sid, v_lte.firma_nr, v_lam.artikel_id, v_lte.lte_id, null,
                                                    v_order_pos.auftrag, null, sysdate, in_user_id, null,
                                                    null, null, null, null, null,
            ...
            if v_lam.lam_id < 0 then
                raise_isi_error(15,
                                lc.ec_p1(lc.o_tp1_lte_id_gesp_ware_ausl,
    ...
    -- reservierung
           ...
           -- WK 2015-09-16: Reservierung ebenfalls zurücksetzen bei AG (für Wiederverwendung)
            ...
            raise_isi_error(20,
                            lc.ec_p2(lc.o_tp2_lte_id_st_n_ausl_bar, in_lte_id, v_lte.lte_status));
        end if;
  ...
  -- da jetzt alle Mengen und Chargen gebucht sind
    ...
    -- Die Felder für das Schnittstelleschreiben verbiegen
            ...
            if v_order_pos.li_extern = 'B' then
...
-- -AG- 14.09.2016 Neue Funktion für R$, mit Uebergabe der Transport-ID
    ...
    begin
        v_err_nr := null;
        if lvs_p_base.get_transp_by_transp_id(in_sid, in_transport_id, v_transp) then
            v_result := lvs_p_lte.lvs_suche_neue_lte(v_transp, in_user_id);
        else
            raise_isi_error(40,
                            lc.ec_p1(lc.o_tp1_transp_data_nf, in_transport_id));
        end if;
    ...
    function lvs_suche_neue_lte (
        ...
        v_result varchar2(255);
    begin
        v_result := lvs_p_lte.lvs_suche_neue_lte_old_crtl(in_transport, in_user_id, 'AUSB', null);
        return ( v_result );
    end;
    ...
    function lvs_suche_neue_lte_old_crtl (
        ...
        v_result varchar2(255);
    begin
        v_result := lvs_p_lte.lvs_suche_neue_lte_old_crtl_31(in_transport, in_user_id, in_lte_crtl, in_lte_id);
        return ( v_result );
    end;
  ...
  -- Tabelle für die Erzeugung von Staffeltransporten
        ...
        where
            ...
            and not exists (
                ...
                where
                        x.lte_id = lam.lte_id
                    and x.order_pos_auf_id is not null
            )
       ...
       -- 20190606 (W24210-368 Auslagerungen RBG_5 werden nicht substituiert)
            and ( v_e_lam_fa_ag is null
                  or nvl(lam.leitzahl, -1) = nvl(v_e_lam_leitzahl,
                                                 nvl(lam.leitzahl, -1)) )
  ...
  --CMe 20220811 MHD berücksichtigen
        ...
        from
            ...
            lvs_fahrzeuge        f
        where
                fg.lgr_gruppe_id = v_lgr_neue_quelle.lgr_gruppe_id
  ...
  -- Erst mal die LTE-Daten lesen
        ...
        v_found := c_lte%found;
        close c_lte;
        if lvs_p_lgr_grp_fahrzeuge.chk_lte_lgr_zugriff_ok(v_lte.lte_id) = 'T' then
            return ( v_ret );
        end if;
  ...
  -- Schlimmer Fehler, konnte die LTE-Daten nicht lesen (Ist evtl. schon Ausgelagert und aus den Daten genommen?)
        ...
        if not lvs_p_base.get_transport(v_lte.sid, in_transport_id, v_transport) then
            return ( v_ret );
        end if;

  -- LTE ist nicht mehr am urspr. Platz
        if v_lte.lgr_platz != v_transport.lgr_platz_quelle then
    -- Wenn die Palette bereits am Ziel ist, dann ist alles OK
            if v_lte.lgr_platz = v_transport.lgr_platz_ziel then
      ...
      -- v_ret := 'LTE bereits am Ziel. Transport gelöscht.';
            ...
            fetch c_lam_g into
                ...
                v_max_lam_sel9,
                v_max_lam_sel10,
                v_min_owner_address_id,
                v_max_owner_address_id,
                v_min_labor_status,
                v_max_labor_status,
                v_lgr_ort,
                v_lgr_adresse,
    ...
    -- Charge FA-Auftrag und Arbeitsgang der Palette ist relevant für die Sortierung zur findung der neuen Palette
            ...
            if not v_found then
                raise_isi_error(30,
                                lc.ec_p1(lc.o_tp1_q_lgr_platz_fehlt,
    ...
    -- Wenn dispo auf einlagerung lagerplatz wieder um LTE Menge ,Gewicht .. Entlasten
            ...
            if v_lgr_quelle.lgr_platz is not null then
                lvs_platz.lvs_platz_ausl_disp_ruecks(v_lte, v_lgr_quelle);
            end if;

            update lvs_lte lte
            set
                lte.lte_status = v_neuer_status,
                lte.order_vorgang_id = null,
                lte.order_auf_id = null,
                lte.ziel_lgr_ort = null,
                lte.ziel_lgr_platz = null
            where
                    lte.sid = v_transport.sid
        ...
        -- Irgend etwas passendes gefunden
          ...
          -- Eine Palette aus ISIOrder (Wegen reservierungen müssen die Mengen passen)
            ...
            --CMe 20220808 auf der neuen Ziel LTE alles ohne Reservierung zusammenfassen
                            lvs_p_lte_lhm.unite_lams_without_reservation(
                                in_sid          => v_transport.sid,
            ...
            -- Lagerbestand an den Auftrag binden
                            if v_res_menge > 0 then
                                ...
                                loop
                                    ...
                                    close c_lam_e_lte;
                                    exit when not v_found;
                                    if v_e_lam.menge < v_g_lam.menge then
            ...
            --              auf Grund von Mangel handelt und sich auf der LTE bereits reserviertes Material befindet
                            lvs_p_lte_lhm.add_amount_to_reservation(
                                in_sid          => v_transport.sid,
                                in_firma_nr     => v_transport.firma_nr,
                                in_lte_id       => v_lam_lte_id,
                                in_auf_id       => v_auf_id,
            ...
            -- return v_ret;       -- Kein Hinweis allse OK
                        ...
                        if lvs_p_base.get_lte(v_lam_lte_id, v_lte) then
                            if lvs_p_base.get_lgr_platz(v_lte.lgr_platz, v_lgr_quelle) then
  ...
  -- Tabelle für die Erzeugung von Staffeltransporten
        ...
        where
            ...
            and lte.lgr_ort = ort.lgr_ort
            and lte.order_vorgang_id is null
            and nvl(lam.leitzahl, -1) = nvl(v_e_lam_leitzahl,
  ...
  -- AG sicherheitshalber hier mit original LTE_ID nochmal
    ...
    function lvs_suche_neuen_platz_v349 (
        ...
        from
            lvs_lte lte
        where
            lte.lte_id = in_transport.lte_id;
        ...
        from
            lvs_lgr lgr
        where
            lgr.lgr_platz = in_transport.lgr_platz_ziel;

    begin
        if nvl(in_prorgamm_nr, 0) = 0 then
            open c_lte;
            fetch c_lte into v_lte;
            v_found := c_lte%found;
            close c_lte;
    ...
    -- Schlimmer Fehler, konnte die LTE-Daten nicht lesen (Ist evtl. schon Ausgelagert und aus den Daten genommen?)
            if not v_found then
                raise_isi_error(10,
                                lc.ec_p1(lc.o_tp1_lte_id_fehlt, in_transport.lte_id));
            end if;
    ...
    -- Ziel ist ein WE
            ...
            or v_lgr_ziel.lgr_verwendung = c.lgr_typ_ep then
                raise_isi_error(11,
                                lc.ec(lc.o_txt_ep_we_wa_voll_n_moeglich));
            end if;

            v_fahrzeuge_ids := ';'
                               || to_char(in_transport.res_id)
                               || ';';
            open c_lte_cfg;
            fetch c_lte_cfg into v_lte_cfg;
            close c_lte_cfg;
            ...
            v_basis_lte_name := nvl(v_lte_cfg.basis_lte_name, v_lte.lte_name);
            lvs_platz.lvs_suche_um_platz(v_lte, v_basis_lte_name, v_lte_cfg.flaechen_stellplatz_erf, v_lgr_ziel, v_fahrzeuge_ids,
                                         v_lgr_ziel_neu);
            ...
            update isi_transport t
            set
                t.lgr_platz_ziel = v_lgr_ziel_neu.lgr_platz,
            ...
            update lvs_lte lte
            set
                lte.ziel_lgr_platz = v_lgr_ziel_neu.lgr_platz
            where
                lte.lte_id = in_transport.lte_id;
    -- Abhängig von Firma_CFG "ZIEL_VOLL_SPERRE_AKTIV" den ursprünglichen Lagerplatz sperren !!!
            v_lagerplatz_sperren := isi_allg.get_firma_cfg_param(in_transport.sid, in_transport.firma_nr, 'CFG',                    -- in_kategorie             in isi_firma_cfg.kategorie%type,
            ...
            if
                v_lgr_ziel.lgr_typ != c.bkl1
                and v_lgr_ziel.gesperrt != c.lgr_gesperrt_g
                and v_lagerplatz_sperren = c.c_true
            then
                lvs_platz.lvs_c_set_lgr_sperr_status(c.lgr_gesperrt_g,
                                                     v_lgr_ziel.lgr_platz,
                                                     in_transport.modul_bearbeiter
                                                     || ' '
                                                     || to_char(sysdate, 'yyyy.mm.dd'));
            else
                if v_lagerplatz_sperren = c.c_false then
        -- damit die leeren  Plätze nicht wieder verwendet werden, Reservierungsstring ändern !
                    ...
                    or v_lgr_ziel.lgr_typ = c.durchl1 then
            ...
            raise_isi_error(999,
                            lc.ec_p1(lc.o_tp1_feherabhandlung_fehlt,
                                     nvl(
                      to_char(in_prorgamm_nr),
                      'NULL'
                  )));
        end if;
  ...
  -- Im Fehlerfall is der Fehlertext bereits gesetzt.
    ...
    function lvs_c_suche_neuen_platz (
        ...
        v_ret varchar2(100);
    begin
        v_ret := lc.ec_p1(lc.o_tp1_neuen_platz_anfahern,
                          nvl(
                                   lvs_suche_neuen_platz_v349(in_transport, in_user_id, in_prorgamm_nr),
                                   'Fehlt'
                               ));
    ...
    function lvs_c_suche_neuen_platz_v349 (
        ...
        v_ret varchar2(100);
    begin
        v_ret := lvs_suche_neuen_platz_v349(in_transport, in_user_id, in_prorgamm_nr);
        commit;
        return v_ret;
    exception
    -- Im Fehlerfall is der Fehlertext bereits gesetzt.
...
-------------------------------------------------------------------------------------------------------
    ...
    begin
        ...
        loop
            ...
            if nvl(v_lte_cfg.lte_name, 'ISI_Fehlt') != v_lte.lte_name then
                v_lte_cfg := null;
                open c_lte_cfg;
                fetch c_lte_cfg into v_lte_cfg;
                close c_lte_cfg;
            ...
            if v_lte.lte_letzte_buchung < sysdate - v_lte_cfg.lte_id_gueltig_tage then
                begin
                    lvs_p_lte.lvs_korr_te_ausbuchen(v_lte.sid, v_lte.firma_nr, v_lte.lte_id, v_lte.lte_status, v_lte.sid,
                                                    v_lte.firma_nr, v_lte.lgr_ort, v_lte.lgr_platz, in_login_id);
  ...
  -- Im Fehlerfall is der Fehler bereits gesetzt, als lam_id wird 0 zurückgegeben.
        when others then
            if c_lte_leer%isopen then
                close c_lte_leer;
            end if;
            if c_lte_o_lgr%isopen then
                close c_lte_o_lgr;
            end if;
            if c_lhm_cfg%isopen then
                close c_lhm_cfg;
            end if;
            if c_lam_del%isopen then
                close c_lam_del;
 ...
 *******************************************************************************/
    ...
    begin
        ...
        or v_lte.lte_name != nvl(in_chk_lte_name, v_lte.lte_name) then
            return false;
        end if;
 ...
 *******************************************************************************/
    ...
    begin
        ...
        update lvs_lte t
        set
            t.lte_vol_hoehe = nvl(in_chk_hoehe, v_lte.lte_vol_hoehe),
  ...
  ---------------------------------------------------------------------------------------------------
    ...
    -- LTE auf neuen lagerplatz buchen !
        ...
        update lvs_lam lam
        set
            lam.lgr_platz = in_ist_lgr_platz
        where
            lam.lte_id = in_lte_id;
     ...
     --  and (lam.lgr_platz != in_ist_lgr_platz or lam.lgr_platz is null);
        update lvs_lhm lhm
        set
            lhm.lgr_platz = in_ist_lgr_platz
        where
            lhm.lte_id = in_lte_id;
     /* LHa - 20220318 - P71141-125 - Bedingung wurde auskommentiert. Es führt ansonsten dazu, 
                                      dass der lgr_platz in der LVS_LTE anders ist als in der
  ...
  -------------------------------------------------------------------------
    ...
    begin
        lvs_korr_te_einbuchen(in_te_sid, in_te_firma_nr, in_lte_id, in_lte_status, in_lgr_sid,
                              in_lgr_firma_nr, in_lgr_einl_ort, in_lgr_einl_lagerplatz, in_ls_login_id, in_lgr_platz_pruefen);
  ...
  -------------------------------------------------------------------------
    ...
    begin
        ...
        if v_lgr_platz is null then
            raise_isi_error(c.fmid_lte_id_null,
                            lc.ec_p2(lc.o_tp2_lte_buch_platz_err, in_lte_id, 'NULL'));
        end if;
    ...
    -- LTE Einlesen
        ...
        close c_lvs_lte;
        if not v_found then
            raise_isi_error(c.fmid_lte_id_fehlt,
                            lc.ec_p1(lc.o_tp1_lte_id_fehlt, in_lte_id));
        end if;
    ...
    -- lagerplatz aus lvs_lgr lesen
        ...
        close c_lgr;
        if not v_found then
            raise_isi_error(c.fmid_lager_platz_fehlt,
                            lc.ec_p2(lc.o_tp2_platz_existiert_nicht, v_lgr_platz, in_lte_id));
        end if;
        ...
        close c_lte_cfg;
        v_basis_lte_name := nvl(v_lte_cfg.basis_lte_name, v_lte.lte_name);
        if v_lgr.lte_namen = 'Keine' then
            v_lgr.lte_namen := v_lgr.lte_namen_cfg;
        end if;

        v_err_text := lvs_platz.lvs_platz_einl_pruef_err_t_r30(v_lte, v_basis_lte_name, v_lte_cfg.flaechen_stellplatz_erf, v_lgr, 'K'
        ,
                                                               null);

        if
            v_err_text is not null
    ...
    -- -AG- 29.04.2009 BugFix: LTE mit Menge 0 hat hier den Staus LF
        ...
        then
         ...
         -- 23.02.2008 -AG- In den Abgangsbuchungen werden jetzt auch Lieferscheinnummer und Pos. gespeichert
                insert into lvs_lam_bh values ( v_lam.sid,
                                                v_lam.firma_nr,
                                                v_vorg_id,
                                                c.lam_bh_inv,
                                                v_lam_bh_id,
    ...
    -- der Status richtig gesetzt werden Abhängig von Transport Ziel. Ticket: E70397-603
        if not lvs_p_base.get_transport_by_lte_id(in_lte_id, v_transport) then
            v_transport := null;
  ...
  -------------------------------------------------------------------------
    ...
    -- lagerplatz aus lvs_lgr lesen
        ...
        if not v_found then
            raise_isi_error(c.fmid_lager_platz_fehlt,
                            lc.ec_p2(lc.o_tp2_platz_existiert_nicht, v_lte.lgr_platz, in_lte_id));
        end if;
      ...
      -- muss die Disponierung zurückgesetzt werden
        ...
        lvs_te_lagerziel_umbuchen_353(in_te_sid, in_te_firma_nr, in_lte_id, null, null,
                                      null, v_lte.ziel_lgr_platz, v_lte.ziel_lgr_ort, v_neuer_status, in_lte_status,
                                      v_lte.ziel_lgr_platz_n_freif, v_lte.ziel_lgr_ort_n_freif, systimestamp, v_lte.order_auf_id, v_lte.order_vorgang_id
                                      ,
        ...
        where
            ...
            and t.lte_offset_z = (
                ...
                from
                    lvs_lte x
                where
                    x.lgr_platz = v_lgr_platz
            );

    begin
        v_x_pos := in_x_pos;
        v_y_pos := in_y_pos;
        v_lgr_platz := lvs_platz.lvs_suche_flaechenplatz(in_sid, in_firma_nr, in_lgr_ort, v_x_pos, v_y_pos,
                                                         v_lgr_vol_breite, v_lgr_vol_tiefe);
    ...
    begin
      ...
      -- Jetzt mit datum letzten Eintrag suchen und Zählen dass nur ein LAM auf Palette
            ...
            fetch c_lam_bh_last_ag into
                ...
                v_last_ag_anz;
            close c_lam_bh_last_ag;
            if ( v_last_ag_anz != 1 ) then
                return ( -2 );
            end if;
            open c_lam_bh;
            fetch c_lam_bh into v_lam_bh;
            close c_lam_bh;
            if v_lam_bh.menge < io_menge then
            ...
            update lvs_lam_bh t
            set
                t.menge = v_lam_bh.menge - io_menge
            where
  ...
  */
    ...
    begin
        ...
        if ( v_found ) then
            v_ret := lvs_suche_neue_lte_old_crtl_or(in_transport, in_user_id, in_lte_crtl, in_lte_id);
        else
            open c_check_is_fa;
            ...
            close c_check_is_fa;
            if ( v_found ) then
                v_ret := lvs_suche_neue_lte_old_crtl_fa(in_transport, in_user_id, in_lte_crtl, in_lte_id);
            else
                v_ret := lvs_suche_neue_lte_old_crtl_or(in_transport, in_user_id, in_lte_crtl, in_lte_id);
            end if;
  ...
  */
            ...
            (
                ...
                where
                    ...
                    and ( lgr.lgr_verwendung = c.r_lgr_typ_lager
                          or -- Lagertypen und Verwendungstypen erlaubt (Lager)
                           lgr.lgr_verwendung = c.r_lgr_typ_lagerp
                          or -- Nur PP's wenn Palette grade fertig befüllt Palette kann raus
                           lgr.lgr_verwendung = c.r_lgr_typ_puffer
                          or -- Nur PP's wenn Palette grade fertig befüllt Palette kann raus
                           ( lgr.lgr_verwendung = c.r_lgr_typ_wa
                               and  -- Nur WA's wenn der WA_TYP BDEPUSH oder BDEICEPUSH
  ...
  --CMe 20220811 MHD berücksichtigen
        ...
        from
            ...
            join lvs_lgr lgr on lgr.lgr_platz = lte.lgr_platz
        where
                lam.order_pos_auf_id = v_min_auf_id
    ...
    -- T = PRUEFEN - Es dürfen KEINE Reservierungen auf der LTE sein
        v_dispo_res_lte_chk := isi_allg.get_firma_cfg_param(in_transport.sid, in_transport.firma_nr, 'AUSL_BDE_PUSH', 'RES', 'BDE_AUSL_LTE_CHK_DISPO'
        ,
                                                            'BDE', 'CFG', 'F', 'BOOLEAN');
    ...
    -- Neue LTE darf nicht in der selben Lagerplatzgruppe stehen, wie das Transport Ziel
        ...
        if ( nvl(in_transport.quelle_leer_progr_nr, 0) = 0 ) then
            v_found := lvs_p_base.get_lte(v_lte_id,  -- in_lte_id in lvs_lte.lte_id%type,
             v_lte_t);  -- io_lte    in out lvs_lte%rowtype) return boolean is
      ...
      -- LTE ist nicht mehr am urspr. Platz
            if v_lte_t.lgr_platz != in_transport.lgr_platz_quelle then
        -- Wenn die Palette bereits am Ziel ist, dann ist alles OK
                if v_lte_t.lgr_platz != in_transport.lgr_platz_ziel then
                    delete isi_transport t
                    where
                            t.sid = in_transport.sid
          ...
          -- BugFix: Lagerort
                    update isi_transport t
                    set
                        t.lgr_platz_quelle = v_lte_t.lgr_platz,
                    ...
                    v_ret := lc.ec_p1(lc.o_tp1_neuen_platz_anfahern,
                                      nvl(v_lte_t.lgr_platz, 'NULL'));

                    return ( v_ret );
                end if;
            else
                savepoint keine_neue_lte;
                if ( in_lte_crtl = 'AUSB' ) then
          -- Quelle ist ein WA
                    if ( v_lgr_quelle.lgr_verwendung = c.lgr_typ_we ) then
                        ...
                        if in_transport.vorgang_id is not null then
        ...
        -- Hole alle Reservierungen von der Transport LTE
          ...
          -- Hole Ersatz LTE
            ...
            -- Ersetze soviel wie möglich (am besten vollständig) 
                        ...
                        loop
                            ...
                            if ( v_lams.menge < v_res_menge ) then
              ...
              -- Wenn gewechselt wurde immer für die LTE die kleinste order_pos_auf_id eintragen
                            ...
                            where
                                lte.lte_id = v_e_lte_id;

                            if
                                ( v_min_auf_id = v_g_auf_id )
                                and ( v_min_lte_id is null )
                            then
          ...
          -- Es wurde nicht getauscht für die LAM. Loop verlassen
                    ...
                    v_found := lvs_p_base.get_lam(in_transport.sid, in_transport.firma_nr, v_g_lam_id, v_g_lams);
          
          --CMe 20220808 Am besten mit NVL arbeiten                      
                    if ( nvl(v_g_res_menge, 0) - nvl(v_menge_sum, 0) > 0 ) then
                        update lvs_lam lam
                        set
                            lam.res_menge = nvl(lam.res_menge, 0) - nvl(v_menge_sum, 0)
                        where
                            lam.lam_id = v_g_lams.lam_id;
          ...
          -- Ok mind. die kleinste auf_id wurde ersetzt Transport umschreiben
                    update isi_transport t
                    set
                        t.lgr_ort_quelle = v_lte_n.lgr_ort,
                    ...
                    if ( in_transport.transp_typ = 'A' ) then
                        v_c_lte_status_neu := c.lte_ad_stat;
                    elsif ( in_transport.transp_typ = 'E' ) then
                    ...
                    update lvs_lte lte
                    set
                        lte.lte_status = v_c_lte_status_neu
                    where
                        lte.lte_id = v_lte_n.lte_id;

                    if ( v_lte_t.lgr_platz is null ) then
                        v_lte_status_neu := c.lte_kf_stat;
                    else
                        v_lte_status_neu := c.lte_lf_stat;
                    end if;

                    v_found := lvs_p_base.get_lgr_platz(in_transport.lgr_platz_quelle, v_lgr_quelle);
                    if ( v_found ) then
                        ...
                        or ( v_lgr_quelle.lgr_verwendung = c.lgr_typ_lager ) then
                            v_lte_status_neu := c.lte_lf_stat;
                        ...
                        or ( v_lgr_quelle.lgr_verwendung = c.lgr_typ_wep ) then
                            v_lte_status_neu := c.lte_bf_stat;
                        end if;

                    end if;

                    v_found := lvs_p_base.get_lgr_platz(v_lte_n.lgr_platz, v_lgr_quelle_e);
                    if ( v_found ) then
                        lvs_platz.lvs_platz_ausl_disp_setzen(v_lte_n, v_lgr_quelle_e);
                    end if;
                    v_found := lvs_p_base.get_lgr_platz(in_transport.lgr_platz_ziel, v_lgr_ziel);
                    if ( v_found ) then
                    ...
                    if ( in_lte_crtl = 'AUSB' ) then
                        lvs_p_lte.lvs_korr_te_ausbuchen(v_lte_t.sid, v_lte_t.firma_nr, v_lte_t.lte_id, v_lte_t.lte_status, v_lte_t.sid
                        ,
                                                        v_lte_t.firma_nr, v_lte_t.lgr_ort, v_lte_t.lgr_platz, in_user_id);

                        v_ret := lc.ec_p1(lc.o_tp1_lte_gewechselt_n_platz,
                                          nvl(in_transport.lgr_platz_quelle, 'NULL'));
          ...
          --CMe 20220819: Nur zurücksetzen wenn auch ausgebucht wird. Hat zu Fehlern geführt
            ...
            raise_isi_error(990,
                            lc.ec_p1(lc.o_tp1_feherabhandlung_fehlt,
                                     nvl(
                      to_char(in_transport.quelle_leer_progr_nr),
                      'NULL'
                  )));
        end if;
    ...
    exception
        when v_error then
            if c_lam_g%isopen then
                close c_lam_g;
            end if;
            if c_lam_e%isopen then
                close c_lam_e;
            end if;
            if c_get_lams%isopen then
                close c_get_lams;
            end if;
            v_err_text := v_err_text
                          || cr_lf()
  ...
  */
  ...
  -- STD Ptrogramm fuer Platz ist leer und Palette soll geholt werden !!
    ...
    -- LTE ist nicht mehr am urspr. Platz
            if v_lte.lgr_platz != in_transport.lgr_platz_quelle then
      -- Wenn die Palette bereits am Ziel ist, dann ist alles OK
                if v_lte.lgr_platz = in_transport.lgr_platz_ziel then
                    delete isi_transport t
                    where
                            t.sid = in_transport.sid
                        and t.firma_nr = in_transport.firma_nr
        ...
        -- BugFix: Lagerort
                    update isi_transport t
                    set
                        t.lgr_platz_quelle = v_lte.lgr_platz,
      ...
      -- Palettendaten von allen Auftragsdatten lösen und Status setzetn
                ...
                or v_lgr_quelle.lgr_verwendung = c.lgr_typ_wep then
                    v_lte_status_neu := c.lte_bf_stat;
                elsif v_lte.lgr_platz is null then
                    v_lte_status_neu := c.lte_kf_stat;
                else
                    v_lte_status_neu := c.lte_lf_stat;
                end if;
      ...
      */
      ...
      -- Erst mal alle Daten Holen
          ...
          -- Irgend etwas passendes gefunden
            ...
            -- Eine Palette aus ISIOrder (Wegen reservierungen müssen die Mengen passen)
                            if
                                in_transport.vorgang_id is not null
                                and v_e_menge != v_menge
                            then
                                delete isi_transport t
                                where
              ...
              --              auf Grund von Mangel handelt und sich auf der LTE bereits reserviertes Material befindet
                                ...
                                if lvs_p_base.get_lte(v_lam_lte_id, v_lte) then
                                    ...
                                    if lvs_p_base.get_lgr_platz(in_transport.lgr_platz_ziel, v_lgr_ziel) then
                                        lvs_platz.lvs_platz_einl_disp_setzen(v_lte, v_lgr_ziel);
                                    end if;
...
-- sqlcl_snapshot {"hash":"208aa0384b9a7e376209f5319ba3a088a01c365e","type":"PACKAGE_BODY","name":"LVS_P_LTE","schemaName":"DIRKSPZM32","sxml":""}