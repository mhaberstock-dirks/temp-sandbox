create or replace function dirkspzm32.get_pause_time (
...
begin
  -- Erst mal keine Pausenzeit fuer diesen Zeitraum
    ...
    if pzm_lohnauswertung.v_pzm_sim_on -- In der Simulation berechen, wieviel Pausenzeit für die Schicht gerechnet werden muss
     then
        time_std := get_pause_time_day(p_sa_kurzname, p_beginn, p_ende, v_anw_dauer, 0,
                                       p_pb_id, v_pause_unbezahlt);
  ...
  -- Wenn Schitdaten gefunden
    ...
    -- Wenn die Pause1 im Zeitraum ist
        ...
        or ( v_ende between p_beginn and p_ende ) then
      -- Ueberschneidungen Pause Zeitraum dann Pausenzeit reduzieren
            if v_beginn < p_beginn then
                v_beginn := p_beginn;
            end if;
            if v_ende > p_ende then
                v_ende := p_ende;
            end if;
...
-- sqlcl_snapshot {"hash":"9bbbbb20f8146f3e1ff66da8a1c3370d7b93d1f4","type":"FUNCTION","name":"GET_PAUSE_TIME","schemaName":"DIRKSPZM32","sxml":""}