...
begin
  ...
  if in_status = pzm_p_zeiterfassung.STATUS_ABWESEND then
    ...
    if in_calc_ist_start is null and in_calc_ist_ende is null then
    ...
    if in_work_location is not null and in_work_location != v_work_location then
  ...
  if in_kst_id is not null and in_kst_id != v_kst_id then -- Kostenstelle wurde geaendert
