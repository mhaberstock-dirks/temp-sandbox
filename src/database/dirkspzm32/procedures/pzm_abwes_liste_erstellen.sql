create or replace procedure dirkspzm32.pzm_abwes_liste_erstellen (
      ...
      -- Fix: Variablen zu Beginn jedes Tages zurücksetzen um "Mitnahme" von Werten des Vortags zu verhindern
        ...
        -- Fix: Erlaubt Krankheit ('K%') auch wenn Arbeitsstunden vorhanden sind (v_anw_std > 0)
                if
                    ...
                    and v_schicht_vorh
                    and ( v_anw_std = 0
                    or v_aa_kurzname like 'K%' )
                then
                    v_ze_handled := true;
                    if v_aa_id is not null then
                        if v_last_aa_id = v_aa_id then
                            update pzm_abwes_liste t
                            set
                                t.abwes_liste_bis = v_current_date,
            ...
            if not v_ze_handled -- Wenn ZE keine Abwesenheit erzeugt hat (z.B. weil gearbeitet wurde), prüfe Anträge/Meldungen
             ...
             then
                if
                    v_feiertag = 'F'
                    and v_wochentag_id < 6
                    and v_current_date <= trunc(sysdate)
                or v_current_date = trunc(sysdate) then
                    if v_aa_kurzname not like 'K%'
                       or v_aa_kurzname = 'KUG' then
                        v_last_aa_id := null;
                    end if;
                end if;
                ...
                fetch c_am into
                    ...
                    v_aa_farbe,
                    v_found_end_date,
                    v_tag_faktor; -- Enddatum & Faktor abrufen
                v_found := c_am%found;
                close c_am;
                if v_found then
...
-- sqlcl_snapshot {"hash":"bd8187d9d06007613e0b7467407bc466b938445a","type":"PROCEDURE","name":"PZM_ABWES_LISTE_ERSTELLEN","schemaName":"DIRKSPZM32","sxml":""}