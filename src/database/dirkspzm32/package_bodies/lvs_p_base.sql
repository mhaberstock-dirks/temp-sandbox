...
package body DIRKSPZM32.lvs_p_base is
  ...
  function get_lte_cfg(in_sid      in lvs_lte_cfg.sid%type,
                       in_lte_name in lvs_lte_cfg.lte_name%type,
                       io_lte_cfg  in out lvs_lte_cfg%rowtype) return boolean is
    v_result boolean;
    cursor c_lte_cfg is
      select *
        from lvs_lte_cfg t
  ...
  function get_lhm_cfg(in_sid      in lvs_lhm_cfg.sid%type,
                       in_lhm_name in lvs_lhm_cfg.lhm_name%type,
                       io_lhm_cfg  in out lvs_lhm_cfg%rowtype) return boolean is
    v_result boolean;
    cursor c_lhm_cfg is
      select *
        from lvs_lhm_cfg t
  ...
  function get_lgr_ort(in_sid      in lvs_lgr_ort.sid%type,
                       in_firma_nr in lvs_lgr_ort.firma_nr%type,
                       in_lgr_ort  in lvs_lgr_ort.lgr_ort%type,
                       io_lgr_ort  in out lvs_lgr_ort%rowtype) return boolean is
    v_result boolean;
    cursor c_lgr_ort is
      select *
        from lvs_lgr_ort t
  ...
  function get_lgr_platz(in_lgr_platz in lvs_lgr.lgr_platz%type,
                         io_lgr       in out lvs_lgr%rowtype) return boolean is
    v_result boolean;
  ...
  function get_lgr_komm_info(in_lgr_platz in lvs_lgr.lgr_platz%type,
  ...
  begin
    open c_lgr_info;
    fetch c_lgr_info into out_komm_direkt_moegl, out_komm_ausweich_lgr_platz,
                          out_komm_picken_moegl, out_komm_neu_lte_lgr_platz;
  ...
  function get_lgr_wa_info(in_lgr_platz in lvs_lgr.lgr_platz%type,
                           out_wa_menge_uebelief out lvs_lgr.wa_menge_ueberlief%type
                          ) return boolean is
    cursor c_lgr_info is
      select lgr.wa_menge_ueberlief
        from lvs_lgr lgr
       where lgr.lgr_platz = in_lgr_platz
  ...
  function get_artikel_status(in_sid            in lvs_artikel_status.sid%type,
    ...
    cursor c_artikel_status is
      select t.*
        from lvs_artikel_status t
       where t.sid = in_sid
         and t.firma_nr = in_firma_nr
         and t.artikel_id = in_artikel_id
         and (t.leitzahl = in_leitzahl or (in_leitzahl is null and t.leitzahl is null))
         and (t.fa_ag = in_fa_ag or (in_fa_ag is null and t.fa_ag is null));
  begin
    ...
    if not v_result and in_auto_insert
    then
  ...
  procedure set_artikel_letztes_einl_datum(in_sid                in lvs_artikel_status.sid%type,
                                           in_firma_nr           in lvs_artikel_status.firma_nr%type,
                                           in_artikel_id         in lvs_artikel_status.artikel_id%type,
                                           in_leitzahl           in lvs_artikel_status.leitzahl%type,
                                           in_fa_ag              in lvs_artikel_status.fa_ag%type,
  ...
  begin
    if not get_artikel_status(in_sid, in_firma_nr, in_artikel_id, in_leitzahl, in_fa_ag,
                              v_artikel_status, true)
    then
      raise_isi_error(10, LC.ec_p1(LC.O_TP1_ARTIKEL_ID_STATUS_ERST, nvl(to_char(in_artikel_id), 'null')));
    end if;

    -- Neuen Wert speichern
    update lvs_artikel_status t
       set t.letztes_einl_datum = case when in_letztes_einl_datum > nvl(t.letztes_einl_datum, in_letztes_einl_datum - 1)
                                       then in_letztes_einl_datum
                                       else t.letztes_einl_datum
                                       end
     where t.sid = v_artikel_status.sid
  ...
  procedure set_artikel_letztes_ausl_datum(in_sid                in lvs_artikel_status.sid%type,
                                           in_firma_nr           in lvs_artikel_status.firma_nr%type,
                                           in_artikel_id         in lvs_artikel_status.artikel_id%type,
                                           in_leitzahl           in lvs_artikel_status.leitzahl%type,
                                           in_fa_ag              in lvs_artikel_status.fa_ag%type,
  ...
  begin
    ...
    -- Neuen Wert speichern
    update lvs_artikel_status t
       set t.letztes_ausl_datum = case when in_letztes_ausl_datum > nvl(t.letztes_ausl_datum, in_letztes_ausl_datum - 1)
                                       then in_letztes_ausl_datum
                                       else t.letztes_ausl_datum
                                       end
     where t.sid = v_artikel_status.sid
       and t.artikel_status_id = v_artikel_status.artikel_status_id
  ...
  procedure set_artikel_letzte_buchung(in_sid            in lvs_artikel_status.sid%type,
                                       in_firma_nr       in lvs_artikel_status.firma_nr%type,
                                       in_artikel_id     in lvs_artikel_status.artikel_id%type,
                                       in_leitzahl       in lvs_artikel_status.leitzahl%type,
                                       in_fa_ag          in lvs_artikel_status.fa_ag%type,
  ...
  begin
    ...
    -- Neuen Wert speichern
    update lvs_artikel_status t
       set t.letzte_buchung = case when in_letzte_buchung > nvl(t.letzte_buchung, in_letzte_buchung - 1)
                                   then in_letzte_buchung
                                   else t.letzte_buchung
                                   end
     where t.sid = v_artikel_status.sid
       and t.artikel_status_id = v_artikel_status.artikel_status_id
  ...
  function get_lte(in_lte_id in lvs_lte.lte_id%type,
                   io_lte    in out lvs_lte%rowtype) return boolean is
    v_result boolean;
    cursor c_lte is
      select *
        from lvs_lte t
  ...
  function get_lte_hist(in_lte_id in lvs_lte.lte_id%type,
                        io_lte    in out lvs_lte%rowtype) return boolean is
    v_result boolean;
    cursor c_lte is
      select *
  ...
  function get_lhm(in_lhm_id in lvs_lhm.lhm_id%type,
                   io_lhm    in out lvs_lhm%rowtype) return boolean is
    v_result boolean;
    cursor c_lhm is
      select *
        from lvs_lhm t
  ...
  function get_lhm_by_lte_id(in_lte_id in lvs_lhm.lte_id%type,
    ...
    cursor c_lhm is
      select *
        from lvs_lhm t
       where t.lte_id = in_lte_id
         and (t.lhm_id = in_lhm_id or in_lhm_id is null);
  begin
    if not get_lte(in_lte_id, v_lte)
    then
      raise_isi_error(10, LC.ec_p1(LC.O_TP1_LTE_ID_FEHLT, nvl(in_lte_id, 'null')));
    end if;

    if in_lhm_id is null and nvl(v_lte.lte_akt_lhm, 0) > 1
    then
      raise_isi_error(20, LC.ec_p1(LC.O_TP1_LHM_AUF_LTE_N_UNINQUE, nvl(in_lte_id, 'null')));
    end if;
  ...
  function get_lam(in_sid      in lvs_lam.sid%type,
                   in_firma_nr in lvs_lam.firma_nr%type,
                   in_lam_id   in lvs_lam.lam_id%type,
                   io_lam      in out lvs_lam%rowtype) return boolean is
    v_result boolean;
    cursor c_lam is
      select *
        from lvs_lam t
  ...
  function get_lam_by_lhm_id(in_sid      in lvs_lam.sid%type,
                             in_firma_nr in lvs_lam.firma_nr%type,
  ...
  function get_lam_by_lte_id(in_sid      in lvs_lam.sid%type,
                             in_firma_nr in lvs_lam.firma_nr%type,
  ...
  function get_charge(in_charge_id in lvs_charge.charge_id%type,
                      io_charge    in out lvs_charge%rowtype) return boolean is
    v_result boolean;
    cursor c_charge is
      select *
        from lvs_charge t
       where t.charge_id = in_charge_id;
  begin
    open c_charge;
  ...
  function get_charge_bez(in_sid        in lvs_charge.sid%type,
  ...
  function get_transport(in_sid       in isi_transport.sid%type,
                         in_transp_id in isi_transport.transp_id%type,
                         io_transport in out isi_transport%rowtype) return boolean is
    v_result boolean;
  ...
  function get_transport_by_lte_id(in_lte_id in isi_transport.lte_id%type,
                                   io_transport in out isi_transport%rowtype) return boolean is
    v_result boolean;
    cursor c_transport is
      select *
  ...
  function get_fahrzeug(in_res_id   in lvs_fahrzeuge.res_id%type,
                        io_fahrzeug in out lvs_fahrzeuge%rowtype) return boolean is
    v_result boolean;
    cursor c_fahrzeuge is
      select *
        from lvs_fahrzeuge t
  ...
  function get_lvs_lgr_ort_ue_platz(in_sid                  in lvs_lam.sid%type,
                                    in_firma_nr             in lvs_lam.firma_nr%type,
                                    ...
                                    io_lvs_lgr_ort_ue_platz in out lvs_lgr_ort_ue_platz%rowtype) return boolean is
    ...
    cursor c_lvs_lgr_ort_ue_platz is
      select *
       ...
       where t.sid = in_sid
         ...
         and  (t.lte_name = in_lte_name
            or (in_lte_name is NULL and t.lte_name is NULL)
      ...
      order by nvl(t.lte_name, in_lte_name || 'X');
  begin
  ...
  function lagerbestand_vorhanden_menge(in_sid                in lvs_lam.sid%type,
                                        in_firma_nr           in lvs_lam.firma_nr%type,
                                        ...
                                        out_anzahl_lam        out number) return boolean is
    v_result boolean;
    ...
    cursor c_lam_art is
      select count(*),
        ...
        from lvs_lam l,
             lvs_lte lte
       where l.artikel_id = in_artikel_id
         and nvl(l.leitzahl, -1) = nvl(in_leitzahl, nvl(l.leitzahl, -1))
         and nvl(l.fa_ag, -1) = nvl(in_fa_ag, nvl(l.fa_ag, -1))
         and nvl(l.zeichnung_index, '@@NULL@@') = nvl(in_zeichnung_index, nvl(l.zeichnung_index, '@@NULL@@'))
         ...
         and (lte.lgr_ort = in_lgr_ort or in_lgr_ort is null); -- (optional) nur in diesem Lagerort
  begin
    open c_lam_art;
    fetch c_lam_art into out_anzahl_lam, out_menge_ges;
    v_result := c_lam_art%found;
    close c_lam_art;

    -- v_result korrigieren, falls keine Mengen vorhanden sind.
    v_result := v_result and out_anzahl_lam > 0 and out_menge_ges > 0;

    return (v_result);
  end;

  function lagerbestand_verfuegbar_menge(in_sid                in lvs_lam.sid%type,
                                         in_firma_nr           in lvs_lam.firma_nr%type,
                                         in_artikel_id         in lvs_lam.artikel_id%type,
                                         in_leitzahl           in lvs_lam.leitzahl%type,
                                         in_fa_ag              in lvs_lam.fa_ag%type,
  ...
  function lagerbestand_vorhanden(in_sid                in lvs_lam.sid%type,
                                  in_firma_nr           in lvs_lam.firma_nr%type,
                                  in_artikel_id         in lvs_lam.artikel_id%type,
                                  in_leitzahl           in lvs_lam.leitzahl%type,
                                  in_fa_ag              in lvs_lam.fa_ag%type,
                                  in_zeichnung_index    in lvs_lam.zeichnung_index%type,
                                  in_lgr_ort            in lvs_lgr.lgr_ort%type) return boolean is
    v_menge number;
    v_count number;
    v_result boolean;
  begin
    v_result := lagerbestand_vorhanden_menge(in_sid, in_firma_nr, in_artikel_id,
                                             in_leitzahl, in_fa_ag, in_zeichnung_index,
                                             in_lgr_ort, v_menge, v_count);
    return (v_result);
  end;

  function lagerbestand_verfuegbar(in_sid                in lvs_lam.sid%type,
                                   in_firma_nr           in lvs_lam.firma_nr%type,
                                   in_artikel_id         in lvs_lam.artikel_id%type,
                                   in_leitzahl           in lvs_lam.leitzahl%type,
                                   in_fa_ag              in lvs_lam.fa_ag%type,
    ...
    v_result boolean;
  begin
    v_result := lagerbestand_verfuegbar_menge(in_sid, in_firma_nr, in_artikel_id,
                                              in_leitzahl, in_fa_ag, in_zeichnung_index,
                                              in_lgr_ort, v_menge, v_count);
    return (v_result);
  end;

  function get_packschema_kopf(in_sid                in lvs_packschema_kopf.sid%type,
                               in_firma_nr           in lvs_packschema_kopf.firma_nr%type,
                               in_packschema_kopf_id in lvs_packschema_kopf.packschema_kopf_id%type,
                               io_packschema_kopf    in out lvs_packschema_kopf%rowtype) return boolean
                               is
    v_result boolean;

    CURSOR c_packschema_kopf is
      select t.*
        from lvs_packschema_kopf t
       where t.sid = in_sid
         and t.firma_nr = in_firma_nr
  ...
  --
  -------------------------------------------------------------------------------------------------------
  function get_transp_by_lte_id(in_sid              in  isi_sid.sid%type,
  ...
  --
  -------------------------------------------------------------------------------------------------------
  function get_transp_by_transp_id(in_sid              in  isi_sid.sid%type,
                                   in_transp_id        in  isi_transport.transp_id%type,
                                   out_transp          out isi_transport%rowtype
                                  ) return boolean is
  v_found boolean;
  ...
  --
  -------------------------------------------------------------------------------------------------------
  function get_transp_grp_by_lte_id(in_sid              in  isi_sid.sid%type,
                                    in_lte_id       in  lvs_lte.lte_id%type,
  ...
  --
  -------------------------------------------------------------------------------------------------------
  function get_transp_grp_by_lte_grp_id(in_sid              in  isi_sid.sid%type,
  ...
  --
  -------------------------------------------------------------------------------------------------------
  function get_transp_grp_by_transp_id(in_sid              in  isi_sid.sid%type,
  ...
  function get_transp_id_by_lte_id(in_sid              in  isi_sid.sid%type,
                                   in_lte_id           in  lvs_lte.lte_id%type,
  ...
  function get_lgr_ort_fuellg_chk_by_p (in_sid                       in  isi_sid.sid%type,
    ...
    CURSOR c_lvs_lgr_ort_fuellg_check is
      select *
        from lvs_lgr_ort_fuellg_check t
       where t.sid = in_sid
         and t.firma_nr = in_firma_nr
         and t.lgr_ort = in_lgr_ort
         and nvl(t.lgr_gruppe_id, nvl(in_lgr_gruppe_id, -1)) = nvl(in_lgr_gruppe_id, -1)
         and nvl(t.lte_name, nvl(in_lte_name, 'XXXX')) = nvl(in_lte_name, 'XXXX');
   begin
