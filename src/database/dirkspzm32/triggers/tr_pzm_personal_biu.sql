create or replace editionable trigger dirkspzm32.tr_pzm_personal_biu before
...
begin
    ...
    -- -WK- 20110805: Bugfix: Fehlende Gutschrift, wenn Urlaubskonto-Art erst im nachhinein gesetzt wurde
        if nvl(:new.pers_urlaub_anspr_aa_id,
               -1) <> :old.pers_urlaub_anspr_aa_id then
            update pzm_konten t
            set
                t.aktiv = 'F'
            where
                    t.konto_nr = v_uk_konto_old.konto_nr
                and t.firma_nr = 1
                and t.sid = '01';
            ...
            set
                t.aktiv = 'T'
            where
                    t.konto_nr = v_uk_konto.konto_nr
                and t.firma_nr = 1
                and t.sid = '01';

            if nvl(:new.pers_urlaub_anspr_wert,
                   0) > 0 then
        -- Einheitwechsel, Umrechnung!
                ...
                if v_d_arb_std_pro_tag is not null then
                    if :old.pers_urlaub_anspr_aa_id = :new.pers_urlaub_anspr_aa_id then
                        if
                            upper(v_uk_konto.buch_einheit) = 'HH24'
                            and upper(v_uk_konto_old.buch_einheit) = 'DD'
                        then
                            :new.pers_urlaub_anspr_wert := :old.pers_urlaub_anspr_wert * v_d_arb_std_pro_tag;
                        elsif
                            upper(v_uk_konto.buch_einheit) = 'DD'
                            and upper(v_uk_konto_old.buch_einheit) = 'HH24'
                        then
                            :new.pers_urlaub_anspr_wert := round(:old.pers_urlaub_anspr_wert / v_d_arb_std_pro_tag);
                        end if;

                    end if;
                elsif :old.pers_urlaub_anspr_aa_id is not null then
                    raise_application_error(-20000,
                                            'In dem zugeordneten Schichtmodell <'
                                            || nvl(
                        nvl(v_schichtmodell.sm_name, v_schichtmodell_abt.sm_name),
                        'Nicht eingetragen bei ' || :new.pers_nr
                    )
...
-- sqlcl_snapshot {"hash":"7204666d02da8d5f1934f6f7ce98f80a10063376","type":"TRIGGER","name":"TR_PZM_PERSONAL_BIU","schemaName":"DIRKSPZM32","sxml":""}