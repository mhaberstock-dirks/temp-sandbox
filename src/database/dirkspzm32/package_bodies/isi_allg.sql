create or replace package body dirkspzm32.isi_allg is
  /*
  ...
  */
  ...
  -- ohne COMMIT
  --------------------------------------------------------------------------------
    function get_arbeitsplatz_cfg (
        in_arbeitsplatz_id     in isi_arbeitsplatz_cfg.arbeitsplatz_id%type,
    ...
    -------------------------------------------------------------------------------------------------------
        ...
        select
            *
        from
            isi_arbeitsplatz_cfg a
        where
                a.arbeitsplatz_id = in_arbeitsplatz_id
  ...
  -- ohne COMMIT
  --------------------------------------------------------------------------------
    procedure set_arbeitsplatz_cfg (
        in_tcpip           in varchar2,
    ...
    begin
        ...
        if v_found then
            if get_arbeitsplatz_cfg(v_arbeitsplatz.arbeitsplatz_id, in_modul_name, in_modul_funktion, v_param) = 1 then
                update isi_arbeitsplatz_cfg a
                set
                    a.modul_parameter = in_modul_parameter
                where
                        a.arbeitsplatz_id = v_arbeitsplatz.arbeitsplatz_id
                    and upper(a.modul_name) = upper(in_modul_name)
                    and a.modul_funktion = in_modul_funktion;

            else
                insert into isi_arbeitsplatz_cfg a values ( null,                           -- APP_CFG_ID            NUMBER not null,
            ...
            v_err_nr := 10;
            v_err_text := 'Fehler Arbeitsplatz <'
                          || in_tcpip
                          || '> fehlt in der Arbeitsplatzkonfiguration.';
            raise v_error;
        end if;
  ...
  -- mit COMMIT
  --------------------------------------------------------------------------------
    procedure c_set_arbeitsplatz_cfg (
        in_tcpip           in varchar2,
        in_modul_name      in isi_arbeitsplatz_param.modul_name%type,
        in_modul_funktion  in isi_arbeitsplatz_param.modul_funktion%type,
        in_modul_parameter in isi_arbeitsplatz_param.modul_parameter%type
  ...
  --------------------------------------------------------------------------------
    ...
    begin
        v_result := get_global_param(in_modul_name, in_parameter_name, in_default_param_wert, in_default_param_typ);
        commit;
        return v_result;
    end;
  ...
  --------------------------------------------------------------------------------
    ...
    begin
        return get_firma_cfg_param('01', 1, 'CFG', null, in_parameter_name,
                                   in_modul_name, 'CFG', in_default_param_wert, in_default_param_typ);
    end;
  ...
  -- mit COMMIT
  --------------------------------------------------------------------------------
    function c_get_firma_cfg_param (
        ...
        if v_found then
            ...
            begin
                insert into isi_firma_cfg t (
                    ...
                    t.typ
                )                 --  TYP            VARCHAR2(10) default 'CFG' not null,
                 values ( in_sid,
                           in_firma_nr,
                           in_kategorie,
                           in_kategorie_ix,
  ...
  --
  --------------------------------------------------------------------------------
    function get_artikel_cfg (
        in_sid         in isi_artikel_cfg.sid%type,
        io_artikel_cfg in out isi_artikel_cfg%rowtype
    ) return boolean is
        v_result boolean;
  ...
  --
  --------------------------------------------------------------------------------
    function get_artikel_by_artikel_id (
        in_sid          in isi_artikel.sid%type,
        in_artikel_id   in isi_artikel.artikel_id%type,
        io_artikel_satz in out isi_artikel%rowtype
    ) return boolean is
  ...
  --
  --------------------------------------------------------------------------------
    function get_artikel_by_artikel_nr (
    ...
    function get_kd_artikel_by_artikel_id (
        in_sid             in isi_artikel.sid%type,
        in_artikel_id      in isi_artikel.artikel_id%type,
        in_kunden_nr       in isi_adressen.adr_nr%type,
        io_kd_artikel_satz in out isi_artikel_kunde%rowtype
    ) return boolean is
  ...
  --------------------------------------------------------------------------------
    function get_firma (
        ...
        io_firma    in out isi_firma%rowtype
    ) return boolean is
  ...
  --
  --------------------------------------------------------------------------------
    function get_sid_status (
        in_sid in isi_sid.sid%type
    ) return varchar2 is
        v_result varchar2(10);
        v_sid    isi_sid%rowtype;
    begin
        if not get_sid(in_sid, v_sid) then
            raise_isi_error(-10,
                            'Für SID ID '
                            || nvl(in_sid, '(null)')
  ...
  --------------------------------------------------------------------------------
    function get_user_by_login_id (
        ...
        io_user     in out isi_user%rowtype
    ) return boolean is
  ...
  --
  --------------------------------------------------------------------------------
    function get_user_by_transpoder (
  ...
  --
  --------------------------------------------------------------------------------
    procedure c_set_client_info (
        v_cli in varchar2
    ) is -- v$session.client_info%type
    begin
...
-- sqlcl_snapshot {"hash":"bd9134ad251a4197b070b74157811b097bbfb1ab","type":"PACKAGE_BODY","name":"ISI_ALLG","schemaName":"DIRKSPZM32","sxml":""}