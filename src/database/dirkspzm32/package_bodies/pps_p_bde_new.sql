create or replace package body dirkspzm32.pps_p_bde_new is
  ...
  --            Eingebaut fuer projekt HAG (Nur Rohstoffe des Eigentümers verwenden)
    ...
    begin
        pps_p_bde_new.create_bde_fa_dispo_db31(in_sid, in_firma_nr, in_dispo_charge_rein, in_hersteller_liste, in_leitzahl,
                     ...
                     then
                        ...
                        if v_found -- LTE hat Reservierung
                         then
                            ...
                            open c_fa_auf_res;
                            fetch c_fa_auf_res into v_bde_fa_res; -- Das ist der in der Reihenfolge letzte Auftrag aus der LTE-Reservierung
                            if c_fa_auf_res%found then
                                loop
                --CMe 20220221 (P71141-40): Bei einen Auftrag wo der Zielplatz vom Typ DURCHL1
                --                          muss das bereits reservierte entweder zum sleben Auftrag
                --                          oder zum gleichen Batch gehören
                                    fetch c_fa_auf_res into v_bde_fa_res;
                                    v_found := c_fa_auf_res%found;
                                    exit when not v_found
                                    or v_bde_fa_res.leitzahl != v_fa_leit_res;
                                end loop;

                                if ( v_bde_fa_res.leitzahl = v_bde_fa.leitzahl )
                                or ( v_bde_fa_res.abnr = v_bde_fa.abnr )
                                or ( not v_found ) then
...
-- sqlcl_snapshot {"hash":"0e0d290594340db4bf4ea00ca121a1563e08d13c","type":"PACKAGE_BODY","name":"PPS_P_BDE_NEW","schemaName":"DIRKSPZM32","sxml":""}