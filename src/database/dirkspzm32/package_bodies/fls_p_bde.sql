...
package body DIRKSPZM32.fls_p_bde is
  ...
  -- Standard Fehlerhandling für Exceptions
	...
	-------------------------------------------------------------------------------------------------------
	function get_next_new_fa(out_fa_nr out bde_fa_auftrag.leitzahl%type) return varchar2 is
		v_result varchar2(1);
		cursor c_bde_fa_auftr is
			select t.fa_nr
				from bde_fa_kopf t
			 where t.status = 'N'
			 order by t.termin_soll_start,
								t.fa_nr;
	...
	begin
		...
		open c_bde_fa_auftr;
		fetch c_bde_fa_auftr
			into out_fa_nr;
		v_found := c_bde_fa_auftr%found;
		close c_bde_fa_auftr;

		if v_found and out_fa_nr is not null
		then
			v_result := c.C_TRUE;
		end if;
	...
	-------------------------------------------------------------------------------------------------------
  function c_start_next_new_fa(in_sid        in bde_fa_kopf.sid%type,
                               in_firma_nr   in bde_fa_kopf.firma_nr%type,
                               out_fa_nr     out bde_fa_kopf.fa_nr%type) return varchar2 is
		v_result varchar2(1);
  begin
    v_result := get_next_new_fa(out_fa_nr);
    if v_result = c.C_TRUE
    then
      -- Status im Kopf nachziehen
      update bde_fa_kopf t
         set t.status = 'B'
       where t.fa_nr = out_fa_nr
  ...
  exception
	...
	-------------------------------------------------------------------------------------------------------
	procedure c_fa_ag_anmelden(in_sid      in bde_fa_auftrag.sid%type,
														 ...
														 out_vorg_id out bde_pd_prod.vorg_id%type) is
	begin
  ...
  exception
	...
	-------------------------------------------------------------------------------------------------------
	procedure c_fa_ag_abmelden(in_sid           in bde_fa_auftrag.sid%type,
														 in_firma_nr      in bde_fa_auftrag.firma_nr%type,
														 in_fa_nr         in bde_fa_auftrag.leitzahl%type,
														 in_fa_ag         in bde_fa_auftrag.fa_ag%type,
														 in_fa_upos       in bde_fa_auftrag.fa_upos%type,
														 ...
														 in_menge_schrott in bde_pd_prod.schrott%type) is
	begin
  ...
  exception
	...
	--
	-------------------------------------------------------------------------------------------------------
	procedure c_fa_ag_menge_buchen(in_sid           in bde_fa_auftrag.sid%type,
																 in_firma_nr      in bde_fa_auftrag.firma_nr%type,
																 ...
																 in_fae_id_pos    in bde_pd_prod.fae_id_position%type) is
		v_res_zust_akt isi_resource_zust_akt%rowtype;
	begin
		if not isi_p_base.get_res_zust_akt(in_sid,
																			 in_res_id,
																			 v_res_zust_akt)
		then
			raise_isi_error(10,
	...
	--
	-------------------------------------------------------------------------------------------------------
	procedure c_fa_ag_bereit(in_sid      in bde_fa_auftrag.sid%type,
													 in_firma_nr in bde_fa_auftrag.firma_nr%type,
													 in_fa_nr    in bde_fa_auftrag.leitzahl%type,
													 in_fa_ag    in bde_fa_auftrag.fa_ag%type,
													 in_fa_upos  in bde_fa_auftrag.fa_upos%type,
	...
	exception
		...
		when v_error then
			rollback;
			raise_application_error(-20000 - v_err_nr,
															v_err_text);
		when others then
			rollback;
	...
	--
	-------------------------------------------------------------------------------------------------------
	procedure c_fa_ag_fe_erzeugen(in_sid      in bde_fa_auftrag.sid%type,
																in_firma_nr in bde_fa_auftrag.firma_nr%type,
																in_fa_nr    in bde_fa_auftrag.leitzahl%type,
																in_fa_ag    in bde_fa_auftrag.fa_ag%type,
																in_fa_upos  in bde_fa_auftrag.fa_upos%type,
																in_res_id   in isi_resource.res_id%type,
																in_login_id in isi_user.login_id%type,
																out_lam_id  out lvs_lam.lam_id%type) is
    ...
    cursor c_ma_mengen is
      select fa.leitzahl,
       ...
       where fa.leitzahl = in_fa_nr
         ...
         and ag_stl.fa_ag = (select min(t.fa_ag)
                               from bde_fa_auftrag_stl t,
                                    bde_fa_auftrag t1
                              where t.leitzahl = ag_stl.leitzahl
                                ...
                                and t1.satzart = 'V')
       order by ag_stl.ma_fa_ag, ag_stl.ma_fa_upos, ag_stl.prod_menge_ix;

	begin
		if not bde_p_base.get_fa_kopf(in_sid,
																	in_firma_nr,
																	in_fa_nr,
																	v_bde_fa_kopf)
		then
		...
		if nvl(in_fa_ag, 0) > 0
			 and not bde_p_base.get_fa_ag(in_sid,
																		in_firma_nr,
																		in_fa_nr,
																		in_fa_ag,
																		in_fa_upos,
    ...
    if v_bde_fa_kopf.serie_auto_inc = c.c_true
    then
      v_serie_id := v_serie_id + 1;
      -- letzte seriennummer speichern
      update bde_fa_kopf t
         set t.serie_id = v_serie_id
       where t.sid = v_bde_fa_kopf.sid
    ...
    insert into lvs_lam
       ...
       in_fa_nr,
			 ...
			 v_bde_fa_kopf.zeichnung_index)
		returning lam_id into out_lam_id;
    ...
    open c_ma_mengen;
    loop
      fetch c_ma_mengen into v_fa_nr,
                             v_ma_fa_ag,
                             v_ma_fa_upos,
                             v_prod_menge_ix,
                             v_menge_p_einheit,
      ...
      update bde_pd_lam_stl_daten t
         set t.stl_lam_ab_menge = v_menge_p_einheit
       where t.fert_lam_id = out_lam_id
         and t.fa_nr = v_fa_nr
         and t.fa_ag = v_fa_ag
	...
	--
	-------------------------------------------------------------------------------------------------------
  procedure c_ma_remove_all_res_lam_akt(in_sid    in isi_resource_lam_akt.sid%type,
  ...
  begin
	...
	--
	-------------------------------------------------------------------------------------------------------
  procedure c_ma_add_new_res_lam_akt(in_sid        in isi_resource_lam_akt.sid%type,
  ...
  begin
    ...
    v_res_lam_akt := null;
    v_lam := null;
    if isi_p_base.get_res_lam_akt(in_sid, in_res_id, in_artikel_id, v_res_lam_akt)
    then
      -- Prüfen, ob beim vorhandenen Datensatz die Menge = 0 ist, kann er gelöscht werden
      if lvs_p_base.get_lam(in_sid, in_firma_nr, v_res_lam_akt.lam_id, v_lam)
      then
        if v_lam.menge > 0
        then
          raise_isi_error(20,
       ...
       in_charge_id,
			 ...
			 in_zeichnung,
			 in_zindex)
		returning lam_id into v_lam.lam_id;
	...
	--
	-------------------------------------------------------------------------------------------------------
  procedure c_ma_add_res_lam_akt_menge(in_sid        in isi_resource_lam_akt.sid%type,
                                       in_firma_nr   in lvs_lam.firma_nr%type,
                                       in_res_id     in isi_resource_lam_akt.res_id%type,
                                       in_artikel_id in isi_resource_lam_akt.artikel_id%type,
  ...
  begin
    ...
    if isi_p_base.get_res_lam_akt(in_sid, in_res_id, in_artikel_id, v_res_lam_akt)
    then
      if v_res_lam_akt.lam_id != in_lam_id
      then
        raise_isi_error(30,
	...
	--
	-------------------------------------------------------------------------------------------------------
  procedure c_ma_reduce_res_lam_akt_menge(in_sid        in isi_resource_lam_akt.sid%type,
                                          in_firma_nr   in lvs_lam.firma_nr%type,
                                          in_res_id     in isi_resource_lam_akt.res_id%type,
                                          in_artikel_id in isi_resource_lam_akt.artikel_id%type,
                                          in_lam_id     in isi_resource_lam_akt.lam_id%type,
    ...
    if not isi_p_base.get_res_lam_akt(in_sid, in_res_id, in_artikel_id, v_res_lam_akt)
    then
      raise_isi_error(30,
  ...
  exception
	...
	--
	-------------------------------------------------------------------------------------------------------
  procedure c_ma_set_res_lam_akt_params(in_sid        in isi_resource_lam_akt.sid%type,
                                        in_res_id     in isi_resource_lam_akt.res_id%type,
                                        in_artikel_id in isi_resource_lam_akt.artikel_id%type,
  ...
  begin
    ...
    update isi_resource_lam_akt t
       set t.res_lam_params = in_params_csv
     where t.sid = in_sid
       and t.res_id = in_res_id
       and t.artikel_id = in_artikel_id;
  ...
  exception
	...
	--
	-------------------------------------------------------------------------------------------------------
  procedure c_fe_ag_bereit(in_sid         in bde_pd_lam_stl_daten.sid%type,
  ...
  begin
    update bde_pd_lam_stl_daten t
       set t.status = 'A', -- kann angefangen werden
           t.aend_datum = sysdate,
           t.aend_login_id = -1,
           t.res_id = nvl(in_res_id, t.res_id)
     where t.fert_lam_id = in_fert_lam_id
  ...
  exception
	...
	--
	-------------------------------------------------------------------------------------------------------
  procedure c_fe_ag_reset(in_sid         in bde_pd_lam_stl_daten.sid%type,
													in_firma_nr    in bde_pd_lam_stl_daten.firma_nr%type,
													in_fert_lam_id in bde_pd_lam_stl_daten.fert_lam_id%type,
                          in_fa_nr       in bde_pd_lam_stl_daten.fa_nr%type,
													in_fa_ag       in bde_pd_lam_stl_daten.fa_ag%type,
  ...
  begin
    update bde_pd_lam_stl_daten t
       set t.status = 'N', -- Neu, noch nicht bearbeitet
           t.aend_datum = sysdate,
           t.aend_login_id = -1
     where t.fert_lam_id = in_fert_lam_id
       and t.fa_nr = in_fa_nr
       ...
       and t.firma_nr = in_firma_nr
       and t.status = 'A'
       and (t.res_id = in_res_id or in_res_id is null);
  ...
  -- fertiggemeldeten Stücklistenpositionen)
	-------------------------------------------------------------------------------------------------------
  procedure c_fe_ag_fertig(in_sid           in bde_pd_lam_stl_daten.sid%type,
													 in_firma_nr      in bde_pd_lam_stl_daten.firma_nr%type,
													 in_fert_lam_id   in bde_pd_lam_stl_daten.fert_lam_id%type,
                           in_fa_nr         in bde_pd_lam_stl_daten.fa_nr%type,
													 in_fa_ag         in bde_pd_lam_stl_daten.fa_ag%type,
  ...
  begin
    ...
    if not bde_p_base.get_fa_ag(in_sid, in_firma_nr, in_fa_nr, in_fa_ag, in_fa_upos, v_fa_ag)
    then
			raise_isi_error(20,
											lc.ec_p3(lc.o_tp3_fa_auftrg_fehlt,
                               to_char(in_fa_nr),
    ...
    update bde_pd_lam_stl_daten t
       set t.status = 'F', -- fertig verrrichtet
           t.aend_datum = sysdate,
           t.aend_login_id = -1
           --t.res_id = in_res_id
     where t.fert_lam_id = in_fert_lam_id
    ...
    update lvs_lam t
       set t.leitzahl = in_fa_nr,
           ...
           t.menge = v_lam_menge
     where t.lam_id = in_fert_lam_id
       and t.firma_nr = in_firma_nr
       and t.sid = in_sid;

    commit;

    if v_fa_ag.freig_status = 'AP'
    then
      c_fa_ag_menge_buchen(in_sid, in_firma_nr, 'PP', in_fa_nr, in_fa_ag, in_fa_upos, in_res_id,
                         null, v_menge_io, v_menge_nio, v_menge_schrott, v_lam.fae_id, v_lam.fae_id_position);
    end if;
  exception
		-- Im Fehlerfall is der v_err_nr bereits gesetzt
		when v_error then
	...
	--
	-------------------------------------------------------------------------------------------------------
  procedure c_fe_ag_stl_lam_status(in_sid                 in bde_pd_lam_stl_daten.sid%type,
													         in_firma_nr            in bde_pd_lam_stl_daten.firma_nr%type,
  ...
  begin
    update bde_pd_lam_stl_daten t
       set t.status = in_status,
           ...
           t.aend_login_id = -1,
           t.res_id = nvl(in_res_id, t.res_id)
     where t.pd_lam_stl_daten_id = in_fe_lam_stl_daten_id
       and t.firma_nr = in_firma_nr
       and t.sid = in_sid;
  ...
  exception
	...
	--
	-------------------------------------------------------------------------------------------------------
  procedure c_fe_ag_stl_lam_fertig(in_sid           in bde_pd_lam_stl_daten.sid%type,
													         in_firma_nr      in bde_pd_lam_stl_daten.firma_nr%type,
													         in_fert_lam_id   in bde_pd_lam_stl_daten.fert_lam_id%type,
                                   in_fa_nr         in bde_pd_lam_stl_daten.fa_nr%type,
													         in_fa_ag         in bde_pd_lam_stl_daten.fa_ag%type,
  ...
  exception
	...
	-------------------------------------------------------------------------------------------------------
  procedure c_fe_ag_stl_lam_ist_menge(in_sid           in bde_pd_lam_stl_daten.sid%type,
													            in_firma_nr      in bde_pd_lam_stl_daten.firma_nr%type,
													            in_fert_lam_id   in bde_pd_lam_stl_daten.fert_lam_id%type,
                                      in_fa_nr         in bde_pd_lam_stl_daten.fa_nr%type,
													            in_fa_ag         in bde_pd_lam_stl_daten.fa_ag%type,
  ...
  begin
    update bde_pd_lam_stl_daten t
       set t.stl_lam_ist_menge = in_ist_menge
     where t.fert_lam_id = in_fert_lam_id
       and t.fa_nr = in_fa_nr
       and t.fa_ag = in_fa_ag
       and t.fa_upos = in_fa_upos
  ...
  exception
	...
	--
	-------------------------------------------------------------------------------------------------------
  procedure c_fe_ag_upd_result_params(in_sid           in bde_pd_lam_stl_daten.sid%type,
                                      in_firma_nr      in bde_pd_lam_stl_daten.firma_nr%type,
                                      in_fert_lam_id   in bde_pd_lam_stl_daten.fert_lam_id%type,
                                      in_fa_nr         in bde_pd_lam_stl_daten.fa_nr%type,
                                      in_fa_ag         in bde_pd_lam_stl_daten.fa_ag%type,
  ...
  begin
    update bde_pd_lam_stl_daten t
       set t.result_params = in_result_params
     where t.fert_lam_id = in_fert_lam_id
       and t.fa_nr = in_fa_nr
       and t.fa_ag = in_fa_ag
       and t.fa_upos = in_fa_upos
  ...
  exception
	...
	--
	-------------------------------------------------------------------------------------------------------
  procedure c_fe_ag_stl_lam_res_status(in_sid           in bde_pd_lam_stl_daten.sid%type,
													             in_firma_nr      in bde_pd_lam_stl_daten.firma_nr%type,
                                       in_fert_lam_id   in bde_pd_lam_stl_daten.fert_lam_id%type,
                                       in_fa_nr         in bde_pd_lam_stl_daten.fa_nr%type,
                                       in_fa_ag         in bde_pd_lam_stl_daten.fa_ag%type,
  ...
  begin
    update bde_pd_lam_stl_daten t
       set t.res_id = nvl(in_res_id, t.res_id),
  ...
  -- Wenn die IST-Menge am Ende des AG nicht korrekt ist, muss ein Bediener die Werte korrigieren.
	-------------------------------------------------------------------------------------------------------
  procedure c_fe_ag_res_anmelden(in_sid           in bde_pd_lam_stl_daten.sid%type,
													       in_firma_nr      in bde_pd_lam_stl_daten.firma_nr%type,
													       in_fert_lam_id   in bde_pd_lam_stl_daten.fert_lam_id%type,
                                 in_fa_nr         in bde_pd_lam_stl_daten.fa_nr%type,
													       in_fa_ag         in bde_pd_lam_stl_daten.fa_ag%type,
    ...
    v_res_zust_akt isi_resource_zust_akt%rowtype;
  begin
    if not isi_p_base.get_res_zust_akt(in_sid, in_res_id, v_res_zust_akt)
    then
			raise_isi_error(10,
											lc.ec_p1(lc.o_tp1_resourcenzustand_fehlt,
															 in_res_id));
    ...
    if v_res_zust_akt.fert_lam_id is not null and v_res_zust_akt.fert_lam_id != in_fert_lam_id
    then
			raise_isi_error(20,
											lc.ec_p1('lc.o_tp1_res_zust_fe_lam_belegt',
															 in_res_id));
    end if;

    if v_res_zust_akt.leitzahl is not null
    then
      if v_res_zust_akt.leitzahl != in_fa_nr
      then
        raise_isi_error(30,
                        lc.ec_p1('lc.o_tp1_res_zust_fa_nr_falsch',
                                 in_res_id));
      end if;

      if v_res_zust_akt.fa_ag != in_fa_ag
      then
        raise_isi_error(40,
                        lc.ec_p1('lc.o_tp1_res_zust_fa_ag_falsch',
                                 in_res_id));
      end if;
    else
    ...
    update isi_resource_zust_akt t
       set t.fert_lam_id = in_fert_lam_id
     where t.res_id = in_res_id
       and t.firma_nr = in_firma_nr
       and t.sid = in_sid;

    update bde_pd_lam_stl_daten t
       set t.status = 'AP', -- Produktion läuft
           t.stl_lam_ist_menge = decode(nvl(t.stl_lam_ist_menge, 0),
                                        0, nvl(t.stl_lam_ab_menge, 0), -- wenn noch keine IST-Menge, dann Sollmenge übenehmen
                                        t.stl_lam_ist_menge) -- ansonsten IST-Menge beibehalten
     where t.fert_lam_id = in_fert_lam_id
       and t.fa_nr = in_fa_nr
       and t.fa_ag = in_fa_ag
       and t.fa_upos = in_fa_upos
  ...
  exception
	...
	--
	-------------------------------------------------------------------------------------------------------
  procedure c_fe_ag_res_abmelden(in_sid           in bde_pd_lam_stl_daten.sid%type,
													       in_firma_nr      in bde_pd_lam_stl_daten.firma_nr%type,
													       in_fert_lam_id   in bde_pd_lam_stl_daten.fert_lam_id%type,
													       in_res_id        in bde_pd_lam_stl_daten.res_id%type
                                ) is
  ...
  begin
    ...
    if v_res_zust_akt.fert_lam_id is not null
    then
      if v_res_zust_akt.fert_lam_id != in_fert_lam_id
      then
        raise_isi_error(20,
                        lc.ec_p1('lc.o_tp1_res_zust_fe_lam_falsch',
                                 in_res_id));
      end if;

      update isi_resource_zust_akt t
         set t.fert_lam_id = null
       where t.res_id = in_res_id
         and t.firma_nr = in_firma_nr
         and t.sid = in_sid;
    end if;
  ...
  exception
	...
	--
	-------------------------------------------------------------------------------------------------------
  procedure c_fe_update_qs_status(in_sid           in lvs_lam.sid%type,
													        in_firma_nr      in lvs_lam.firma_nr%type,
  ...
  exception
	...
	--
	-------------------------------------------------------------------------------------------------------
  procedure c_fe_update_last_fa_ag(in_sid           in lvs_lam.sid%type,
													         in_firma_nr      in lvs_lam.firma_nr%type,
													         in_fert_lam_id   in lvs_lam.lam_id%type,
  ...
  exception
	...
	--
	-------------------------------------------------------------------------------------------------------
  procedure c_fe_entnahme_buchen(in_sid           in lvs_lam.sid%type,
													       in_firma_nr      in lvs_lam.firma_nr%type,
													       in_fert_lam_id   in lvs_lam.lam_id%type,
    ...
    cursor c_nio_daten is
      select count(*)
        from bde_pd_nio_daten t
       where t.fert_lam_id = in_fert_lam_id;
  begin
    if isi_p_base.get_resource(in_sid, in_res_id, v_resource)
    then
      if isi_p_base.get_res_zust_akt(in_sid, in_res_id, v_res_zust)
      then
        if v_res_zust.fert_lam_id = in_fert_lam_id
        then
          update isi_resource_zust_akt t
             set t.fert_lam_id = null
           where t.res_id = in_res_id
    ...
    */
    ...
    update lvs_lam t
       set t.lgr_platz = null,
           t.labor_status = decode(t.labor_status,
                                   'P', decode(v_nio_count, 0, 'F', 'G'),
                                   t.labor_status),
           t.prod_datum = sysdate,
  ...
  exception
	...
	--
	-------------------------------------------------------------------------------------------------------
  procedure c_fe_nio_insert(in_sid                 in  bde_pd_nio_daten.sid%type,
    ...
    v_res_zust  isi_resource_zust_akt%rowtype;
  begin
    if not isi_p_base.get_res_zust_akt(in_sid, in_res_id, v_res_zust)
    then
			raise_isi_error(10,
											lc.ec_p1(lc.o_tp1_resourcenzustand_fehlt,
                               to_char(in_res_id)));
  ...
  exception
	...
	--
	-------------------------------------------------------------------------------------------------------
  procedure c_fe_nio_update_status(in_sid          in bde_pd_nio_daten.sid%type,
													         in_firma_nr     in bde_pd_nio_daten.firma_nr%type,
  ...
  begin
    ...
    update bde_pd_nio_daten t
       set t.nio_status = in_nio_status,
           t.aend_datum = sysdate,
           t.res_id = in_res_id,
           t.res_status_id = v_res_zust.status_id
     where t.nio_daten_id = in_nio_daten_id
       and t.firma_nr = in_firma_nr
       and t.sid = in_sid;
  ...
  exception
	...
	--
	-------------------------------------------------------------------------------------------------------
  procedure c_fe_nio_update_params(in_sid          in bde_pd_nio_daten.sid%type,
													         in_firma_nr     in bde_pd_nio_daten.firma_nr%type,
                                   in_nio_daten_id in bde_pd_nio_daten.nio_daten_id%type,
                                   in_nio_params   in bde_pd_nio_daten.nio_params%type
                                  ) is
  begin
    update bde_pd_nio_daten t
       set t.nio_params = in_nio_params
     where t.nio_daten_id = in_nio_daten_id
       and t.firma_nr = in_firma_nr
       and t.sid = in_sid;
  ...
  exception
	...
	--
	-------------------------------------------------------------------------------------------------------
  procedure c_res_status_beg(in_sid          in bde_pd_nio_daten.sid%type,
                             in_firma_nr     in bde_pd_nio_daten.firma_nr%type,
                             in_res_id       in isi_resource.res_id%type,
