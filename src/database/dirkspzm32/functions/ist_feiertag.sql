create or replace function dirkspzm32.ist_feiertag (
    ...
    where
        ...
        and kst.kst_nr (+) = nvl(in_kst_id,
                                 nvl(p.pers_kst_id,
                                     nvl(abt.abt_kst_id, pb.pb_kst_id)))
        and akst.adress_id (+) = kst.kst_adress_id
        and p.pers_nr = in_pers_nr
        and nvl(v_region_code,
                nvl(p.pers_region_code,
                    nvl(akst.region_code,
                        nvl(aa.region_code,
                            nvl(ap.region_code, rc.column_value))))) like rc.column_value || '%'
...
-- sqlcl_snapshot {"hash":"aae0a44e85e1b22923833f3eceaa9d4284501d76","type":"FUNCTION","name":"IST_FEIERTAG","schemaName":"DIRKSPZM32","sxml":""}