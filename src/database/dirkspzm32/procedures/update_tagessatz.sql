...
procedure DIRKSPZM32.UPDATE_TAGESSATZ(p_pers_nr in number,
                                             ...
                                             p_day_pause_bez_std in number) is
...
begin
  v_auto_ueb_std_genehmigung := nvl(pzm_p_base.get_allg_parameter_mandant(p_pers_nr, 'AUTO_UEBERSTD_GENEHMIGUNG'), 'F') != 'F';

  open c_tagessatz;
  fetch c_tagessatz into v_isi_pzm_ze_tagessatz;

  if c_tagessatz%found
  then
    v_kst_id := nvl(v_isi_pzm_ze_tagessatz.ts_day_kst_id, get_pers_kst_id(p_pers_nr));
    v_abt_id := nvl(v_isi_pzm_ze_tagessatz.ts_day_abt_id, get_pers_abt_id(p_pers_nr));
    v_pb_id := nvl(v_isi_pzm_ze_tagessatz.ts_day_pb_id, get_pers_pb_id(p_pers_nr));

    if v_isi_pzm_ze_tagessatz.ts_abschluss is null
    then
      if v_auto_ueb_std_genehmigung
      ...
      then
        ...
        update pzm_ze_tagessatz t
           set t.ts_ueb_ok_datum = null
         where current of c_tagessatz;
      end if;

      update pzm_ze_tagessatz t
         set t.ts_day_ist_start = p_day_begin,
      ...
      if v_auto_ueb_std_genehmigung
         and (p_day_ueb_std > 0 or p_day_flex_std > 0)
      then
        -- ?berstunden automatisch genehmigen
        update pzm_ze_tagessatz t
           set t.ts_ueb_ok_datum = sysdate
         where current of c_tagessatz;
      end if;
    else
      update pzm_ze_tagessatz t
