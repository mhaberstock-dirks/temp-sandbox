  ...
  CREATE OR REPLACE EDITIONABLE TRIGGER "TR_LVS_LAM_BH_BI" 
  before insert on lvs_lam_bh
  for each row
declare
    -------------------------------------------------------------------------------------------------------
  ...
  cursor c_lam_akt is                    -- Cursor für den Lagerbestand der aktuellen Buchung
    select *
      from lvs_lam lam_akt
     where lam_akt.sid = :new.sid
  ...
  cursor c_lam_lte is                    -- Anzahl und MIN, MAX Temperatur der LHM's auf dieser Transporteinheit
    select count(lam_lte.lam_id),
     ...
     where lam_lte.sid = :new.sid and
           lam_lte.lte_id = :new.lte_id and
           lam_lte.menge > 0 and
           art.sid = :new.sid and
           art.artikel_id = lam_lte.artikel_id;

  cursor c_lgr is                        -- Lesen des Lagerplatz
    select *
    from lvs_lgr lgr
    where lgr.lgr_platz = :new.lgr_platz;

  cursor c_lte is                        -- Lesen der Transporteinheit
    select *
    from lvs_lte lte
    where lte.lte_id = :new.lte_id;

  cursor c_lhm is                        -- Lesen des Lagerhilfsmittel
    select *
    from lvs_lhm lhm
    where lhm.lhm_id = :new.lhm_id;
  ...
  cursor c_send_bew is                    -- Suche eintrag in der Schnittstelle
    select *
      from s_send_bew bew
      where bew.lam_id = v_lam_akt.lam_id
    ...
    select *
     ...
     where pos.sid = :new.sid
       and pos.vorgang_typ = decode(:new.bus,
                                    c.LAM_BH_BUS_ZUG_KONSI, 'KWE',  -- BUS = WE-Konsi-Lager dann ist in der ISI-Order Typ BK und Vorg_typ 'WEK'
                                    'WEE')
...
begin
  ...
  if  :new.bus != 1               -- Keine Inventurbuchung
  ...
  and :new.bus != 23              -- KONSI hat keinen Einfluss auf INVENTUR
                                  ...
                                  --!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  then
    if v_art.akt_inventur_id is not NULL
    then
      v_err_text :=
      'Fehler: Buchung nicht möglich. ' ||
      'Artikel ' ||v_art.artikel || ' ' || v_art.bezeichnung1 ||
      ' ist in Inventur.';
    ...
    CLOSE c_lgr_ort;
    if v_ziel_ort.akt_inventur_id is not NULL
    then
      v_err_text :=
      'Fehler: Buchung nicht möglich. ' ||
      'Lagerort ' || v_ziel_ort.lgr_ort || ' ' || v_ziel_ort.lgr_ort_text ||
      ' ist in Inventur.';
      v_err_nr := c.FMID_Inventur_Ort;
      raise v_error;
    end if;
    if v_lam_akt.akt_inventur_id is not NULL
    then
      v_err_text :=
      'Fehler: Buchung nicht möglich. ' ||
      'Lagerbestand für LHM ' || v_lam_akt.lhm_id ||
      ' ist in Inventur.';
  ...
  then
    begin
      lvs_p_base.set_artikel_letzte_buchung(v_lam_akt.sid,
                                            ...
                                            :new.buch_datum);
    exception
      when others then NULL;  -- Wenn kein Upodate dann egal
    end;
  end if;

  -- Lesen der User-Daten zum übermitteln der Personalnummer zu Hostsystemen
  if not isi_allg.get_user_by_login_id(:new.sid, :new.ls_login_id, v_user)
  then
    v_user.pers_nr := NULL;
  end if;
  ...
  --
    ...
    then
      ...
      v_lam_akt.lam_kg := v_lam_akt.lam_kg + nvl(:new.lam_bh_kg, 0);      -- Gewicht addieren im Material

      update lvs_lte
         set lte_akt_kg = lte_akt_kg + nvl(:new.lam_bh_kg, 0), -- WK 2015-10-01: wie bei LAM nur Delta (Änderung) buchen
             lte_letzte_buchung = :new.buch_datum
       where lte_id = :new.lte_id;

      update lvs_lgr
         set lgr_akt_kg = v_lgr.lgr_akt_kg + nvl(:new.lam_bh_kg, 0) -- WK 2015-10-01: wie bei LAM nur Delta (Änderung) buchen
       where lgr_platz = :new.lgr_platz;
    end if;
    ...
    then
      ...
      then -- -WK- Bugfix lte_akt_lhm < 0
        v_lte.lte_akt_lhm := v_lte.lte_akt_lhm - 1;
        update lvs_lte lte
           set lte.lte_akt_lhm = v_lte.lte_akt_lhm
         where lte_id = :new.lte_id;
      end if;
    else -- -AG- BugFix damit die Kartonzähler nach Inverur OK
      if v_lam_akt.menge = :new.menge
      then
        v_lte.lte_akt_lhm := v_lte.lte_akt_lhm + 1;
        update lvs_lte lte
           set lte.lte_akt_lhm = v_lte.lte_akt_lhm
    ...
    update lvs_lam lam
       set lam.lte_id = decode(:new.bus, 14, v_lam_akt.lte_id, lam.lte_id),  -- -AG- 05.08.2016 Bug Fix, bei Menge 0 muss die LAM von der LTE getrennt werden.
           lam.lgr_platz = v_lgr_platz,
           lam.menge = decode(:new.bus, 14, v_lam_akt.menge, lam.menge),
           lam.lam_kg = decode(:new.bus, 14, v_lam_akt.lam_kg, lam.lam_kg),
           ...
           lam.letzte_inventur_datum = decode(:new.bus, 14, sysdate, lam.letzte_inventur_datum),
           lam.letzte_inventur_login_id = decode(:new.bus, 14, :new.ls_login_id, lam.letzte_inventur_login_id)
     where lam.sid = :new.sid
       and lam.lam_id = :new.lam_id;

    update lvs_lhm lhm
       set lhm.lgr_platz = v_lgr_platz
     where lhm.lhm_id = :new.lhm_id;
    ...
    if :new.menge > 0 and
       v_lte.res_string = v_lte.lte_name
    then
      ...
      if v_lte.res_string_statisch is null then
        v_lte.res_string := lvs_util.get_res_string_v359(:new.sid, :new.firma_nr, v_lte.waren_typ, v_lte.res_artikel_id,
                                                       v_lam_akt.hersteller_kuerzel_liste,
                                                       v_lam_akt.fa_ag, v_lam_akt.charge_id, v_lam_akt.serie_id,
                                                       v_lam_akt.leitzahl, v_lam_akt.kunden_nr, v_lam_akt.lieferant_nr,
                                                       v_lam_akt.best_nr, v_lam_akt.lam_mhd, 1, v_lam_akt.labor_status,
                                                       v_lte.lte_voll, v_lam_akt.owner_address_id, v_res_mhd);
      else
        v_lte.res_string := v_lte.res_string_statisch;
      end if;
      update lvs_lte lte
         set lte.res_string = v_lte.res_string,
             lte.res_artikel_id = v_lte.res_artikel_id
       where lte.lte_id = v_lte.lte_id;
    end if;

    if v_lam_akt.lte_id is NULL
    then
      v_lam_akt.lte_id := v_lte.lte_id; -- AG 20170309 Damit die LTE-ID in der Schnittstelle geschrieben wird
    end if;
    s_schnittstelle.write_host_bew(NULL, v_lam_akt, :new.lam_bh_id, :new.bus, :new.vorg_typ, NULL, 'UE', v_lgr, NULL, :new.ls_login_id, :new.menge);
---------------------------------------------------------------------------------------------------------------------------------
  ...
  then
    v_buch_kg := nvl(:new.lam_bh_kg, 0);                                -- Gewicht Netto übergeben

    if nvl(v_lam_akt.menge, 0) = 0 and
       :new.menge > 0 then                                      -- Erste Befüllung dann
    ...
    then
      ...
      update s_send_bew bew
         set bew.status = v_send_bew_status,
             bew.lagerort = v_ziel_ort.host_lgr_ort,
             bew.lte_nr = v_lam_akt.lte_id,
             bew.menge = :new.menge,
             bew.lam_bh_id = :new.lam_bh_id,
             bew.pers_nr = v_user.pers_nr,
             bew.aktion = decode(:new.bus, 22, 'KWE', bew.aktion),  -- Bei Konsi muss WEK in der Schnittstelle gebucht werden
      ...
      then
        ...
        if v_sid.sid_schnittstelle is NULL then
          update isi_order_pos pos
             set pos.ist_menge = pos.ist_menge + decode(pos.menge_basis,
                                                        c.BASIS_LTE, 1,
                                                        :new.menge)
           where pos.sid = :new.sid
             and pos.vorgang_typ = decode(:new.bus,
      ...
      s_schnittstelle.write_host_bew(v_order_pos, v_lam_akt, :new.lam_bh_id, :new.bus, :new.vorg_typ, v_send_bew_tabelle, v_send_bew_status, v_lgr, NULL, :new.ls_login_id);
    end if;
    if v_lam_akt.labor_status != c.LAB_STAT_F -- Gesperrte Ware muss auf dem HOST in die Spreeware gebucht werden
    and :new.bus = 22                         -- Zugang Konsi
    then
      s_schnittstelle.write_host_bew(NULL, v_lam_akt, NULL, 5, c.LAM_BH_BUS_SP, NULL, 'UE', v_lgr, NULL, :new.ls_login_id, :new.menge);
    end if;

    UPDATE lvs_lam
        SET menge = v_lam_akt.menge,
    ...
    UPDATE lvs_lhm
        SET lhm_akt_kg = v_lam_akt.lam_kg,
            lhm_letzte_buchung = :new.buch_datum
        WHERE lhm_id = :new.lhm_id;
    ...
    if v_lte_cfg.lte_vol_hoehe_fest != c.C_TRUE
    ...
    if v_lte.waren_typ != 'MP' then
      ...
      begin
        ...
        if v_lam_akt.menge_basis = c.BASIS_LTE
        then
           v_art_lte_menge := 1;
           v_anz_lagen := v_art_lte_lhm_lagen;
        elsif v_lam_akt.menge_basis = c.BASIS_LHM
        then
          v_art_lte_menge := v_art_lte_lhm_menge;
          v_lte_akt_lhm := lvs_p_lte_lhm.lvs_lte_lhm_best(:new.sid, :new.firma_nr, :new.lte_id, 'LTE');
      ...
      if lvs_p_lte_lhm.lvs_lte_lhm_best(:new.sid, :new.firma_nr, :new.lte_id, 'LTE') >= v_art_lte_menge * v_firma.proz_anbruch / 100
    ...
    UPDATE lvs_lte
        SET lte_akt_kg = nvl(v_lte.lte_akt_kg, 0),
...
---------------------------------------------------------------------------------------------------------------------------------
  ...
  then
    ...
    then
      ...
      if v_lam_akt.labor_status = c.LAB_STAT_F
      then
        update bde_fa_auftrag fa
           set fa.ag_ist_mg = fa.ag_ist_mg - :new.menge,
               fa.ag_ist_mg_schrott = fa.ag_ist_mg_schrott + :new.menge
         where fa.sid      = v_fa_auftrag.sid
        ...
        update bde_pd_prod pa
           set pa.menge_a = nvl(pa.menge_a, 0) - :new.menge,
               pa.schrott = nvl(pa.schrott, 0) + :new.menge
         where pa.sid      = v_fa_auftrag.sid
           and pa.vorg_typ = 'PA'
        ...
        update bde_pd_prod pp
           set pp.menge_a = nvl(pp.menge_a, 0) - :new.menge,
               pp.schrott = nvl(pp.schrott, 0) + :new.menge
         where pp.sid      = v_fa_auftrag.sid
           and pp.vorg_typ = 'PP'
        ...
        update bde_pd_prod pa
           set pa.menge_b = nvl(pa.menge_b, 0) - :new.menge,
               pa.schrott = nvl(pa.schrott, 0) + :new.menge
         where pa.sid      = v_fa_auftrag.sid
           and pa.vorg_typ = 'PA'
           and pa.firma_nr = v_fa_auftrag.firma_nr
        ...
        update bde_pd_prod pp
           set pp.menge_b = nvl(pp.menge_b, 0) - :new.menge,
               pp.schrott = nvl(pp.schrott, 0) + :new.menge
         where pp.sid      = v_fa_auftrag.sid
           and pp.vorg_typ = 'PP'
           and pp.firma_nr = v_fa_auftrag.firma_nr
        ...
        update bde_fa_auftrag fa
           set fa.ag_ist_mg_b = fa.ag_ist_mg_b - :new.menge,
               fa.ag_ist_mg_schrott = fa.ag_ist_mg_schrott + :new.menge
         where fa.sid      = v_fa_auftrag.sid
           and fa.firma_nr = v_fa_auftrag.firma_nr
           and fa.leitzahl = v_fa_auftrag.leitzahl
    ...
    if v_lam_akt.labor_status != c.LAB_STAT_F -- Gesperrte Ware muss auf dem HOST erst aus der Spreeware um dann abgebucht zu werden
    and :new.bus != 23                        -- Abgang KONSI
    then
      s_schnittstelle.write_host_bew(NULL, v_lam_akt, NULL, 5, c.LAM_BH_BUS_SP, NULL, 'UE', v_lgr, NULL, :new.ls_login_id, :new.menge * -1);
    end if;
    ...
    UPDATE lvs_lte
        SET lte_akt_kg = v_lte.lte_akt_kg,
            lte_akt_lhm = v_lte.lte_akt_lhm,
            lte_letzte_buchung = :new.buch_datum,
            -- Bei Teilentnahme (normaerweise immer aus einem Behaelter) kein Groessenkorrekturen (VOL in LTE)
            lte_vol_hoehe      = decode(:new.bus, 13, lte_vol_hoehe, v_lte.lte_vol_hoehe),
            lte_vol_breite     = decode(:new.bus, 13, lte_vol_breite, v_lte.lte_vol_breite),
            lte_vol_tiefe      = decode(:new.bus, 13, lte_vol_tiefe, v_lte.lte_vol_tiefe),
            lte_vol            = decode(:new.bus, 13, lte_vol, v_lte.lte_vol),
            wert_klasse        = nvl(v_lte.wert_klasse, 0),
            gefahren_klasse    = nvl(v_lte.gefahren_klasse, 0),
            res_artikel_id     = v_lte.res_artikel_id,
            waren_typ          = v_lte.waren_typ,
...
---------------------------------------------------------------------------------------------------------------------------------
  -- Umlagerung -- Umlagerung -- Umlagerung -- Umlagerung -- Umlagerung -- Umlagerung -- Umlagerung -- Umlagerung -- Umlagerung
    ...
    if  :new.vorg_typ = c.LAM_BH_ZUGAGNG
    and :new.lgr_platz is not NULL
    ...
    then
      ...
      then
        v_err_text :=
        'Fehler: Der Lagerplatz ist für KONSI reserviert. ' ||
        'LTE ' || v_lam_akt.lte_id;
        if nvl(v_lam_akt.owner_address_id, 0) = 0
        then
    ...
    then
      ...
      then
        if (isi_p_order_base.get_order_pos(:new.sid, v_lam_akt.order_pos_auf_id, v_order_pos)
        ...
        then
          v_err_text :=
          'Fehler: Die LTE ' || v_lam_akt.lte_id ||
...
/
ALTER TRIGGER "TR_LVS_LAM_BH_BI" ENABLE;


-- sqlcl_snapshot {"hash":"c02939d1ea4093189753796253a09dff6d1dc561","type":"TRIGGER","name":"TR_LVS_LAM_BH_BI","schemaName":"DIRKSPZM32","sxml":""}