create or replace package body dirkspzm32.pps_p_bde is
  ...
  -- für das Beziehungswissen welche Teile sind in diesem AG relevant
    type t_stl_tab is
        table of bde_fa_auftrag_stl%rowtype index by binary_integer;
    v_stl_tab_ix        number;
    v_stl_tab_i         number;
    v_stl_tab           t_stl_tab;
  ...
  -- AG und STL für das Beziehungswissen welche Teile sind in diesem AG relevant
    type t_pps_stl_tab is
        table of pps_plan_auftrag_ag_stl%rowtype index by binary_integer;
  ...
  ---------------------------------------------------------------------------------------
    ...
    procedure create_bde_fa_auf_a_plan_dis_h (
        in_sid               in isi_sid.sid%type,
        in_firma_nr          in isi_firma.firma_nr%type,
        in_artikel_id        in isi_artikel.artikel_id%type,
        in_charge_bez        in lvs_charge.charge_bez%type,
  ...
  --
    procedure crt_bde_fa_auf_a_plan_h_35_11 (
        in_sid               in isi_sid.sid%type,
        in_firma_nr          in isi_firma.firma_nr%type,
        in_artikel_id        in isi_artikel.artikel_id%type,
        in_charge_bez        in lvs_charge.charge_bez%type,
        ...
        crt_bde_fa_auft_a_plan_35_11(in_sid, -- in isi_sid.sid%type,
         ...
         in_res_id, -- in isi_resource.res_id%type,
                                     ...
                                     in_anz_lhm, -- in bde_fa_auftrag.lhm_anz%type,
                                      ...
                                      in_serie_auto_inc, -- in bde_fa_kopf.serie_auto_inc%type,
                                     in_fa_gruppe, -- in bde_fa_kopf.fa_gruppe%type,
                                      in_dispo_charge_rein, in_hersteller_liste, --in_hersteller_liste  in pps_simple_fa.hersteller_kuerzel_liste%type,
                                      in_lam_sel1, -- in lvs_lam.LAM_SEL1%type,
                                      in_lam_sel2, -- in lvs_lam.LAM_SEL2%type,
                                     in_lam_sel3, -- in lvs_lam.LAM_SEL3%type,
                                      in_lam_sel4, -- in lvs_lam.LAM_SEL4%type,
    ...
    procedure crt_bde_fa_auft_a_plan_35_11 (
        in_sid               in isi_sid.sid%type,
        in_firma_nr          in isi_firma.firma_nr%type,
        in_artikel_id        in isi_artikel.artikel_id%type,
        in_charge_bez        in lvs_charge.charge_bez%type,
    ...
    procedure crt_bde_fa_auft_a_plan_db31 (
        in_sid               in isi_sid.sid%type,
        in_firma_nr          in isi_firma.firma_nr%type,
        in_artikel_id        in isi_artikel.artikel_id%type,
        in_charge_bez        in lvs_charge.charge_bez%type,
    ...
    begin
        pps_p_bde.crt_bde_fa_auft_a_plan_db31_f(
            in_sid               => in_sid,
            in_firma_nr          => in_firma_nr,
            in_artikel_id        => in_artikel_id,
            in_charge_bez        => in_charge_bez,
            in_res_id            => in_res_id,
            in_login_id          => in_login_id,
            in_menge             => in_menge,
            in_ab_text1          => in_ab_text1,
            in_ab_text2          => in_ab_text2,
            in_ab_text3          => in_ab_text3,
            in_soll_betriebsart  => in_soll_betriebsart,
            in_kunden_nr         => in_kunden_nr,
            in_kd_art_nr         => in_kd_art_nr,
            in_ag_name1          => in_ag_name1,
            in_ag_name2          => in_ag_name2,
            in_ag_text1          => in_ag_text1,
            in_ag_text2          => in_ag_text2,
            in_ag_text3          => in_ag_text3,
            in_kenz_lhm_druck    => in_kenz_lhm_druck,
            in_anz_lte           => in_anz_lte,
            in_anz_lhm           => in_anz_lhm,
            in_bestnr_kd         => in_bestnr_kd,
            in_abnr              => in_abnr,
            in_serie_id          => in_serie_id,
            in_serie_auto_inc    => in_serie_auto_inc,
            in_fa_gruppe         => in_fa_gruppe,
            in_dispo_charge_rein => in_dispo_charge_rein,
            in_hersteller_liste  => in_hersteller_liste,
            in_lam_sel1          => in_lam_sel1,
            in_lam_sel2          => in_lam_sel2,
            in_lam_sel3          => in_lam_sel3,
            in_lam_sel4          => in_lam_sel4,
            in_lam_sel5          => in_lam_sel5,
            in_lam_sel6          => in_lam_sel6,
            in_lam_sel7          => in_lam_sel7,
            in_lam_sel8          => in_lam_sel8,
            in_lam_sel9          => in_lam_sel9,
            in_lam_sel10         => in_lam_sel10,
            in_adress_id         => in_adress_id,
            in_lohn_arbeit       => in_lohn_arbeit,
            in_seq_nr            => in_seq_nr,
            in_lead_leitzahl     => in_lead_leitzahl,
            in_primaer_leitzahl  => in_primaer_leitzahl,
            in_fixed_id          => 0,
            in_fixed_lte_id      => null,
            in_out_leitzahl      => in_out_leitzahl
        );
    end;
  ...
  ---------------------------------------------------------------------------------------
    ...
    procedure create_bde_fa_auftrag_db31 (
        in_ag_satzart              in bde_fa_auftrag.satzart%type,
        in_kenz_letzter            in bde_fa_auftrag.kenz_letzt_ag%type,
        in_artikel_id              in isi_artikel.artikel_id%type,
        in_zeichnung               in isi_artikel.zeichnung%type,
        ...
        select
            pps_fhm.*
        from
            pps_arb_plan_ag_fhm pps_fhm,
            isi_resource        r,
            isi_resource        rmpg
        where
                pps_fhm.sid = in_bde_fa_plan.sid
            ...
            and rmpg.typ = 'MPG'
            and ( pps_fhm.fhm_grp = r.res_name
                                    || '-'
                                    || in_bde_fa_plan.arb_plan_pos_id
                  or pps_fhm.fhm_grp = rmpg.res_name
                                       || '-'
                                       || in_bde_fa_plan.arb_plan_pos_id );
        ...
        select
            x.*
        from
            isi_artikel_hersteller x
        where
            ( x.herstellerkuerzel = in_hersteller_liste
              or x.herstellerkuerzel || ';' = in_hersteller_liste )
    ...
    -- Pruefen, ob der MAV-Wert ereicht ist, wenn ja dann Fehler
        if in_fa_ag >= v_ag_max then
            raise_isi_error(10,
                            lc.ec_p2(lc.o_tp2_bde_fa_ag_zu_gross, in_out_leitzahl, in_fa_ag));
        end if;

    -- Keine Leitzahl uebergeben, dann aus SEQ holen
        if in_out_leitzahl is null then
            select
                seq_bde_fa_auftrag_leitzahl.nextval
        ...
        if in_ag_satzart = 'MA' then
            v_charge_id := get_charge_id(in_bde_fa_plan.sid, in_bde_fa_plan.firma_nr, null, in_out_leitzahl
                                                                                            || '/'
                                                                                            || in_fa_ag, in_artikel_id);
        ...
        if in_ag_satzart != 'MA' then
            if in_kenz_letzter = 1 then
                v_charge_id := get_charge_id(in_bde_fa_plan.sid,
                                             ...
                                             null,
                                             nvl(in_charge_bez, in_out_leitzahl
                                                                || '/'
                                                                || in_fa_ag),
                                             in_artikel_id);
            else
            ...
            loop
                ...
                insert into bde_fa_auftrag_fhm values ( in_bde_fa_plan.sid, -- SID                     VARCHAR2(2) not null,
    ...
    -- Bei MA die Mengen er;mitteln
      ...
      -- Hotfix CMe 20210806 Ende
            ...
            if isi_p_base.get_isi_artikel(in_bde_fa_plan.sid, in_artikel_id, v_artikel) then
                if not isi_p_base.get_isi_artikel_kd(in_bde_fa_plan.sid, in_artikel_id, in_kunden_nr, v_artikel_kunde) then
                    v_artikel_kunde := null;
                end if;

                if isi_p_base.get_artikel_gruppe(in_bde_fa_plan.sid, v_artikel.art_gruppe_id, v_artikel_gruppe) then
                    if v_artikel_gruppe.art_params_cfg is not null then
                        if isi_p_base.get_artikel_ctrl(in_bde_fa_plan.sid, in_artikel_id, in_zeichnung, in_zeichnung_index, in_out_leitzahl
                        ,
                                                       in_fa_ag, v_artikel_ctrl) then
                            if isi_utils.is_param_list_comlete(v_artikel_ctrl.prod_params, v_artikel_gruppe.art_params_cfg, c.c_true) = c.c_false
                            then
                                raise_isi_error(30,
                                                lc.ec_p3(lc.o_tp3_art_prod_param_nio, in_out_leitzahl, in_fa_ag, v_artikel.artikel));
            ...
            v_found := isi_p_base.get_isi_artikel(in_bde_fa_plan.sid, in_artikel_id, v_artikel);
            v_ag_menge := in_menge;
        end if;
    ...
    -- -AG- 20190917 - Einbau mitarbeiteranmeldung an Maschine zu FA mit Arbeitszeiterfassung
        insert into bde_fa_auftrag t values ( in_bde_fa_plan.sid, -- SID                     VARCHAR2(2) not null,
                                              in_bde_fa_plan.firma_nr, -- FIRMA_NR                NUMBER(2) not null,
                                              in_abnr, -- ABNR                    VARCHAR2(20),
                                              in_out_leitzahl, -- LEITZAHL                NUMBER not null,
                                              in_fa_ag, -- FA_AG                   NUMBER not null,
       ...
       -- RUEST_ZEIT_GEPL         NUMBER,
                                              ...
                                              decode(in_ag_satzart, 'MA', 0, v_ag_menge * in_bde_fa_plan.soll_zeit_p_einh),
       -- PROD_ZEIT_GEPL          NUMBER,
                                              0, -- PROD_ZEIT_IST           NUMBER,
                                              decode(in_ag_satzart, 'MA', 0, v_ag_menge * in_bde_fa_plan.soll_zeit_p_einh * in_bde_fa_plan.soll_zeit_stoer_p_std / 60
                                              ),
       ...
       -- ZEIT_EINHEIT            NUMBER,
                                              ...
                                              case
                                                  when in_ag_satzart = 'MA'
                                                       and in_dispo_charge_rein = 'T' then
                                                      0
                                                  else
                                                      null
  ...
  Erzeugt aus PPS-Daten --> BDE-Daten
  -------------------------------------------------------------------------------*/
    procedure c_create_bde_fa_auf_a_pla_disp (
        in_sid               in isi_sid.sid%type,
        in_firma_nr          in isi_firma.firma_nr%type,
        in_artikel_id        in isi_artikel.artikel_id%type,
        in_charge_bez        in lvs_charge.charge_bez%type,
    ...
    procedure c_create_bde_fa_auftrag_a_pps (
        ...
        select
            t2.*
        from
            pps_plan_auftrag_ag_stl t,
            pps_plan_auftrag_stl    t2
        where
                v_bde_fa_plan.sid = t.sid
    ...
    -- Lesen der Schrrittweite der Arbeitsgaenge
        v_ag_schrittweite := isi_allg.c_get_firma_cfg_param(in_sid, in_firma_nr, 'BDE_FA', -- in_kategorie             in isi_firma_cfg.kategorie%type,
    ...
    -- Lesen des Maxwert der Arbeitsgaenge
        v_ag_max := isi_allg.c_get_firma_cfg_param(in_sid, in_firma_nr, 'BDE_FA', -- in_kategorie             in isi_firma_cfg.kategorie%type,
         null, -- in_kategorie_ix          in isi_firma_cfg.kategorie_ix%type,
    ...
    if v_anz_bde_beg > 0 then
      raise_isi_error(5,
        ...
        if not check_bde_fa_for_delete(in_sid, in_firma_nr, in_plan_auf_id, v_anz_bde_beg) then
            raise_isi_error(5,
                            lc.ec_p2(lc.o_tp2_arb_pps_ag_bde_beg, in_plan_auf_id, v_anz_bde_beg));
        end if;
    ...
    -- Einbau der Tabelle wenn eine FA erstellt wurde
        ...
        where
            t.leitzahl = in_plan_auf_id;

        open c_pps_plan_auftrag;
        fetch c_pps_plan_auftrag into v_pps_plan_auftrag;
    ...
    -- Lesen der Artikeldaten
        ...
        if not v_found then
            raise_isi_error(20,
                            lc.ec_p1(lc.o_tp1_artikel_id_fehlt, v_pps_plan_auftrag.artikel_id));
        end if;
    ...
    -- Erst mal den Kopfsatz schreiben
        insert into bde_fa_kopf values ( in_sid, -- SID                     VARCHAR2(2) not null,
    ...
    -- Recordstruktur v_bde_fa_plan_crt kopiert, die dann für die Erzeugung genutzt wird.
        open c_bde_fa_plan;
        fetch c_bde_fa_plan into v_bde_fa_plan;
    ...
    -- Ab jetzt alle Eintraege der c_bde_fa_plan abarbeiten
      ...
      -- dann den letzten schreiben
            if v_bde_fa_plan_crt.arb_plan_pos_id != v_bde_fa_plan.arb_plan_pos_id then
                ...
                fetch c_pps_plan_auftrag_ag into v_pps_plan_auftrag_ag;
        ...
        -- Übertragen der Artikeldaten für LTE-Name etc. aus Artikel oder KD-Artikel
                if v_artikel_kd.lte_name is not null then
                    if nvl(v_artikel_kd.lte_menge, 0) = 0 then
                        ...
                        v_lte_anz := round(v_pps_plan_auftrag.menge / v_artikel_kd.lte_menge, 0);
                    end if;
                else
                    if nvl(v_artikel.lte_menge, 0) = 0 then
                        v_lte_anz := 1;
                    else
                        v_lte_anz := round(v_pps_plan_auftrag.menge / v_artikel.lte_menge, 0);
                    end if;
                end if;
        -- Übertragen der Artikeldaten für LHM-Name etc. aus Artikel oder KD-Artikel
                if v_artikel_kd.lhm_name is not null then
                    if nvl(v_artikel_kd.lhm_menge, 0) = 0 then
                        ...
                        v_lhm_anz := round(v_pps_plan_auftrag.menge / v_artikel_kd.lhm_menge, 0);
                    end if;
                else
                    if nvl(v_artikel.lhm_menge, 0) = 0 then
                        v_lhm_anz := 1;
                    else
                        v_lhm_anz := round(v_pps_plan_auftrag.menge / v_artikel.lhm_menge, 0);
                    end if;
                end if;

                v_mit_quitt_gruppe := v_mit_quitt_gruppe
                or ( v_bde_fa_plan_crt.quitt_gruppe_ag is not null );
        ...
        -- Suchen, ob diese STL-Referenz auch eine 'MA'terialanforderung ist
                ...
                fetch c_stl_pos_fuer_ma_in_ag into v_pps_plan_auftrag_stl;
                close c_stl_pos_fuer_ma_in_ag;
        -- Diese STL-Referenz ist auch eine 'MA'terialanforderung (Kleinser AG für die Ref)
                if v_pps_plan_auftrag_stl.plan_auf_stl_id = v_bde_fa_plan.stueckliste_pos_id then
                    ...
                    loop
                        v_stl_tab_i := v_stl_tab_i + 1;
                        exit when v_stl_tab_i > v_stl_tab_ix
                        or v_stl_tab(v_stl_tab_i).stueckliste_pos_id = v_bde_fa_plan.stueckliste_pos_id;
                    end loop;

                    if v_stl_tab(v_stl_tab_i).stueckliste_pos_id = v_bde_fa_plan.stueckliste_pos_id then
    ...
    -- und der Letzte 'V'errichtensatz ist noch nicht geschrieben
      ...
      -- Relation AG - STL werden erstellt
            ...
            insert into bde_fa_auftrag_stl values ( v_stl_tab(v_stl_tab_i).sid,
    ...
    -- Der Letzte 'V'errichtensatz wird geschrieben
        ...
        if v_mit_quitt_gruppe then
            ...
            loop
                fetch c_bde_fa_quitt_grp into
                    v_fa_ag_quitt_gruppe,
                    v_quitt_gruppe;
                exit when c_bde_fa_quitt_grp%notfound;
                update bde_fa_auftrag t
                set
                    t.quitt_gruppe_ag = v_fa_ag_quitt_gruppe
                where
                        t.sid = in_sid
                    and t.firma_nr = in_firma_nr
                    and t.leitzahl = in_plan_auf_id
  ...
  ---------------------------------------------------------------------------------------
    ...
    procedure create_pps_auftrag_a_plan (
        in_sid              in isi_sid.sid%type,
        in_firma_nr         in isi_firma.firma_nr%type,
        in_artikel_id       in isi_artikel.artikel_id%type,
        in_charge_bez       in lvs_charge.charge_bez%type,
        ...
        select
            ...
            pag.nio_res_id,
            nvl(pag.prod_params,
                nvl(v_artikel_arb_plan.prod_params, v_artikel_grp_arb_plan.prod_params)),
            pag.quitt_gruppe_ag,
            pag.soll_zeit_ruest,
            pag.soll_zeit_p_einh,
        ...
        cursor c_stl_pos_fuer_ma_in_ag is
        select
            t.stueckliste_pos_id
        from
            pps_arb_plan_ag_stl t,
            pps_arb_plan_ag     t2
        where
                v_bde_fa_plan.sid = t2.sid
            ...
            and t.stueckliste_pos_id not in (
                select
                    t3.stueckliste_pos_id
                from
                    pps_arb_plan_ag_stl t3, pps_arb_plan_ag     t4
                where
                        t.sid = t4.sid
        ...
        where
            ...
            and v_status_loeschbar not like '%'
                                            || t.status
                                            || ';%';
        ...
        v_ag_max := isi_allg.c_get_firma_cfg_param(in_sid, in_firma_nr, 'BDE_FA', -- in_kategorie             in isi_firma_cfg.kategorie%type,
         ...
         'BDE_FA_AG_MAX', -- in_parameter_name        in isi_firma_cfg.parameter_name%type,
                                                   'BDE', -- in_modul_name            in isi_firma_cfg.modul_name%type,
                                                    ...
                                                    '1000', -- in_default_param_wert    in isi_firma_cfg.parameter_wert%type,
                                                    'INTEGER'); -- in_default_param_typ
        v_status_loeschbar := isi_allg.c_get_firma_cfg_param(in_sid, in_firma_nr, 'PPS_PLAN_AUFTRAG', -- in_kategorie             in isi_firma_cfg.kategorie%type,
         null, -- in_kategorie_ix          in isi_firma_cfg.kategorie_ix%type,
    ...
    -- Prufen ob der Auftrag loeschbar ist
        ...
        fetch c_auftrag_in_bde_begonnen into v_status_loeschbar;
        v_found := c_auftrag_in_bde_begonnen%found;
        close c_auftrag_in_bde_begonnen;
        if v_found then
            raise_isi_error(5,
                            lc.ec_p2(lc.o_tp2_arb_pps_ag_bde_beg, in_out_leitzahl, 1));
        end if;
    ...
    -- Lesen Arbeitsplan für den Artikel
        ...
        fetch c_artikel_arb_plan into v_artikel_arb_plan;
    ...
    -- Kein Arbeitsplan für den Artikel gefunden, dann über die Artikelgruppe suchen
        if not v_found then
            ...
            fetch c_artikel_grp_arb_plan into v_artikel_grp_arb_plan;
    ...
    -- Kein Arbeitsplan für den Artikel oder Gruppe gefunden, dann Fehler
        if not v_found then
            raise_isi_error(20,
                            lc.ec_p1(lc.o_tp1_artikel_arb_plan_fehlt, v_artikel.artikel));
        end if;
    ...
    -- Erst mal den Kopfsatz schreiben
        insert into pps_plan_auftrag values ( in_sid, -- SID                     VARCHAR2(2) not null,
                                              in_firma_nr, -- FIRMA_NR                NUMBER(2) not null,  PLAN_AUF_AG_ID   NUMBER not null,
    ...
    -- Ab jetzt alle Eintraege der c_bde_fa_plan abarbeiten
      ...
      -- dann den letzten schreiben
        ...
        -- Suchen, ob diese STL-Referenz auch eine 'MA'terialanforderung ist
                ...
                open c_stl_pos_fuer_ma_in_ag;
                fetch c_stl_pos_fuer_ma_in_ag into v_stueckliste_pos_id;
                close c_stl_pos_fuer_ma_in_ag;
        -- Diese STL-Referenz ist auch eine 'MA'terialanforderung (Kleinser AG für die Ref)
        ...
        -- Naechsten Eintrag in der STL-Ref Tabelle
                ...
                v_pps_stl_tab(v_pps_stl_tab_ix).plan_auf_stl_id := in_out_leitzahl * v_ag_max + v_bde_fa_plan.stueckliste_pos_id;
            end if;
    ...
    -- und der Letzte 'V'errichtensatz ist noch nicht geschrieben
      ...
      -- Relation AG - STL werden erstellt
            ...
            insert into pps_plan_auftrag_ag_stl values ( v_pps_stl_tab(v_pps_stl_tab_i).sid,
  ...
  ---------------------------------------------------------------------------------------
    ...
    -- Keine Leitzahl uebergeben, dann aus SEQ holen
        ...
        select
            seq_pps_plan_auftrag_ag.nextval
        into v_plan_auf_ag_id
        from
            dual;

    -- PPS-Eintrag schreiben
        insert into pps_plan_auftrag_ag values ( in_bde_fa_plan.sid, -- SID                     VARCHAR2(2) not null,
                                                 ...
                                                 in_menge * in_bde_fa_plan.soll_zeit_p_einh * in_bde_fa_plan.soll_zeit_stoer_p_std / 60
                                                 , -- SOLL_ZEIT_STOER  NUMBER default 0 not null,
  ...
  ---------------------------------------------------------------------------------------
    ...
    -- PPS-Eintrag schreiben
        insert into pps_plan_auftrag_stl values ( in_bde_fa_plan.sid, -- SID                     VARCHAR2(2) not null,
                                                  in_bde_fa_plan.firma_nr, -- FIRMA_NR                NUMBER(2) not null,
  ...
  --            Eingebaut fuer projekt HAG (Nur Rohstoffe des Eigentümers verwenden)
    ...
    begin
        pps_p_bde.create_bde_fa_dispo_db31(in_sid, in_firma_nr, in_dispo_charge_rein, in_hersteller_liste, in_leitzahl,
                                           in_fa_ag, in_fa_upos, in_komplett, in_user_id, in_ice,
                                           null);
    end;
  ...
  06.07.2022   DB31_7      (-CMe-)  Wenn sich die Palette bereits im Transport befindet und das Ziel ist ein ICE Platz,
                                    dann muss geprüft werden, ob das Transport Ziel ein Durchlauflager ist.
  15.07.2022   DB31_8      (-CMe-)  Sortierung in c_ltes über die feste LTE ID in den Produktionsparameter mit neuer Funktion
                                    comp_w_fixed_id_in_params. Ticket: P71208-7
  11.04.2023   DB31_9      (-CMe-)  In c_ltes nur alle LAM's berücksichtigen, die noch im Laborstatus (F)rei ist
  */

    procedure create_bde_fa_dispo_db31 (
        in_sid               in isi_sid.sid%type,
        in_firma_nr          in isi_firma.firma_nr%type,
        in_dispo_charge_rein in varchar2,
        in_hersteller_liste  in pps_simple_fa.hersteller_kuerzel_liste%type,
    ...
    -- 20180203 AG Erweiterung, LTE,s im BDE nur mit einem FA reservieren über pruefen reserviernung
        ...
        v_check_t_ziel_seq        boolean;
        v_t_ziel_lgr              lvs_lgr%rowtype;
        v_anz_res_on_lte          number;
        ...
        where
                fag.leitzahl = v_bde_fa.leitzahl
        ...
        from
            isi_artikel        a,
            isi_artikel_gruppe ag
        where
                a.artikel_id = v_bde_fa.ag_artikel_id
        ...
        from
            (
                ...
                where
                    ...
                    and ( ( l.owner_address_id = nvl(v_bde_fa.adress_id, v_bde_fak.adress_id)
                            and -- genau von diesem Kunden
                             v_bde_fa.lohn_arbeit = c.c_true )
                          or -- Und Konsi benutzen
                           l.owner_address_id is null
                          or  -- oder kein Keine KONSI-Ware
                    ...
                    and ( lgr.wa_typ = 'BDEPUSH'
                          or v_bde_push_q_chk = c.c_false ) -- BDE-Push eingestellt auf der Quelle oder ist nich Pflicht
                    and nvl(
                        decode(v_bde_fa.ma_res_charge_id, -- ID 0 nicht moeglich - 0 Bedeutet Chargenrein
                         0, null, v_bde_fa.ma_res_charge_id),
                        nvl(l.charge_id, 0)
                    ) = nvl(l.charge_id, 0)
                  ...
                  --and nvl(l.leitzahl, v_bde_fa.leitzahl) = v_bde_fa.leitzahl -- Leitzahl beruecksichtigen
                    and ( nvl(l.leitzahl, v_bde_fa.leitzahl) = v_bde_fa.leitzahl
                          or l.fa_ag is null )
                    and l.fa_ag is null
                    and l.order_pos_auf_id is null
                    and ( v_dispo_res_lte_chk = c.c_false
                          or nvl(lte.order_vorgang_id, v_bde_fa.leitzahl) = v_bde_fa.leitzahl )
                    and lgr.lgr_typ != 'DURCHL1' -- Durchlauflager mit begrenzten Zugriff
                    and ( ( lte.lte_status like '%F' )
                          or
                       -- CMe 20220221 (P71141-40): Wenn in Transport Status ist die LTe zulässig, 
                       --                           solange v_bde_ausl_ltr_chk_transp 0 'T'
                           ( ( lte.lte_status like '%T'
                                 or lte.lte_status like '%D' )
                               and v_bde_ausl_ltr_chk_transp = 'T' )
                          or ( ( lte.lte_status like '%T'
                                 or lte.lte_status like '%D' )
                               and ( v_bde_fak.soll_betriebsart = 'JITSEQ'
                                     or v_bde_fak.soll_betriebsart = 'EXPRESS' )
                               and nvl(zlgr.lgr_typ, 'XX') != 'DURCHL1'
                               and nvl(zlgr.wa_typ, 'XX') in ( 'BDEPUSH', 'BDEICEPUSH' ) ) )
        ...
        where
            lam_s.g_menge >= ( v_bde_fa.ag_soll_mg - v_bde_fa.ma_res_menge )
                ...
                where
                  ...
                  --and nvl(l.leitzahl, v_bde_fa.leitzahl) = v_bde_fa.leitzahl -- Leitzahl beruecksichtigen
                    ...
                    and ( v_hersteller_liste like ( '%;'
                      ...
                      -- CMe Hotfix 20210806
                           v_hersteller_liste = ';'
                          or v_hersteller_liste is null
                          or l.hersteller_kuerzel_liste is null )
                    and ( ( lte.lte_status like '%F' )
                          or
                       -- CMe 20220221 (P71141-40): Wenn in Transport Status ist die LTe zulässig, 
                       --                           solange v_bde_ausl_ltr_chk_transp 0 'T'
        ...
        where
                nvl(lam_s.hersteller_kuerzel_liste, ';') = nvl(
                    nvl(v_bde_fa.ma_hersteller_kuerzel_liste, lam_s.hersteller_kuerzel_liste),
                    ';'
                )
            and ( lam_s.g_menge >= ( v_bde_fa.ag_soll_mg - v_bde_fa.ma_res_menge )
                  or nvl(v_bde_fa.ma_reserviert, c.c_false) = c.c_true
                  or in_komplett = c.c_false )
        order by
            lam_s.lam_mhd;
    ...
    -- fuer Auslagerungen identisch gehalten werden
        select
            ...
            pps_p_bde.chk_lte_on_res_lgr_grp(in_sid, in_firma_nr, lte.lte_id, v_bde_fa.auf_id) sort3
        from
            lvs_lte     lte,
            lvs_lam     lam,
            lvs_lgr     lgr,
        ...
        where
         ...
         --and nvl(lam.leitzahl, v_bde_fa.leitzahl) = v_bde_fa.leitzahl -- Leitzahl beruecksichtigen
            and ( nvl(lam.leitzahl, v_bde_fa.leitzahl) = v_bde_fa.leitzahl
                  or lam.fa_ag is null ) -- Leitzahl beruecksichtigen
            ...
            and nvl(lam.charge_id, -1) = nvl(v_lams_c.charge_id,
                                             nvl(lam.charge_id, -1)) -- Charge Passt?
            and nvl(lam.hersteller_kuerzel_liste, 'Keine') = nvl(v_lams_h.hersteller_kuerzel_liste,
            ...
            -- 2017.09.13 AG Jetzt auch verlängerte Werkbank und Verwendung von Kunden-KONSI-Beständen
            --           Eingebaut für HAG und Verwendung von Rohstoffen, die genau einem Kunden oder Lieferadresse geoeren
            and ( ( lam.owner_address_id = nvl(v_bde_fa.adress_id, v_bde_fak.adress_id)
                    and -- genau von diesem Kunden
                     v_bde_fa.lohn_arbeit = c.c_true )
                  or -- Und Konsi benutzen
                   lam.owner_address_id is null
                  or -- oder kein Keine KONSI-Ware
                   v_bde_check_konsi = c.c_false )
            and lgr.gesperrt = c.lgr_gesperrt_f
            and lvs_p_lgr_grp_fahrzeuge.chk_lgr_platz_zugriff_ok(lgr.lgr_platz) = c.c_true
         ...
         -- CMe 20210325 Hotfix fuer E70397-596 (V_R_LTE_% PAletten dürfen nicht verwendet werden)
            and lte.lte_id not like 'V_R_LTE_%'
         -- CMe 20230411 (W23910-343): Nur berücksichtigen wenn die LAM's im Laborstatus 'F' ist
            and lam.labor_status = c.lab_stat_f
        group by
            lte.lte_id,
            lte.lte_voll,
       ...
       -- CMe 20220715 Sortierung überdie Fixed LTE ID in den Produktionsparametern
        order by
            comp_w_fixed_id_in_params(v_bde_fak.prod_params, lte.lte_id) desc,
            decode(
                nvl((
                    select
                        count(x.lam_id)
                    from
                             lvs_lam x
                        join bde_fa_auftrag y on y.auf_id = x.order_pos_auf_id
                    where
                        (y.abnr = v_bde_fa.abnr
                         or y.leitzahl = v_bde_fa.leitzahl
                         and x.lte_id = lte.lte_id)
                ),
                    0),
                0,
                0,
                1
            ) desc, -- Bevorzugung LTE's die für die gleich abnr oder Leitzahl reserviert sind
            decode(
        ...
        order by
            fa.termin_start_gepl desc,
            fa.abnr desc,
            fa.leitzahl desc;

        cursor c_check_trans_z_durchl is
        select
            lgr.*
        from
            lvs_lgr lgr
        where
                1 = 1
            and lgr.sid = v_lte_transport.sid
            and lgr.firma_nr = v_lte_transport.firma_nr
            and lgr.lgr_platz = v_lte_transport.lgr_platz_ziel
   ...
   --                          oder zum gleichen Batch gehören
        ...
        from
            bde_fa_auftrag fa
        where
            ( fa.termin_start_gepl >= v_fa_start_gepl
              or ( fa.termin_start_gepl is null
                   and v_fa_start_gepl is null
                   and fa.leitzahl >= v_fa_leit_res ) )
        ...
        cursor c_check_anz_lte_res is
        select
            count(lam.lam_id) anz_res
        from
            lvs_lam        lam
            left outer join bde_fa_auftrag fag on fag.auf_id = lam.order_pos_auf_id
        where
                1 = 1
            and lam.sid = in_sid
            and lam.firma_nr = in_firma_nr
            and lam.lte_id = v_lte_id
            and fag.leitzahl <> v_bde_fa.leitzahl
            and fag.abnr <> v_bde_fa.abnr;

        cursor c_check_anz_res_lams is
        select
            count(lam.lam_id)
        from
            lvs_lam lam
        ...
        where
            ...
            and lam.lam_id <> (
                ...
                from
                    lvs_lam x
                where
                        x.lte_id = v_lte.lte_id
                    and x.order_pos_auf_id = v_bde_fa.auf_id
            );
    ...
    --end if;
        ...
        if
            in_min_res_menge is not null
            and in_komplett = c.c_true -- Fehlerhafter Aufruf der Funktion. Wenn MIN-Menge gesetzt, dann muss Komplett Reserviern False sein
        then
            raise_isi_error(c.fmid_param_fehlen,
                            lc.ec_p1(lc.o_tp1_res_error, v_bde_fa.leitzahl));
        end if;

    -- - AG - 20211020 - Neuer Scahleter CHECK-KONSI - Default T = Pürfen
        v_bde_check_konsi := isi_allg.c_get_firma_cfg_param(in_sid, in_firma_nr, 'BDE_CHECK', 'KONSI', 'BDE_CHECK_KONSI',
                                                            'BDE', 'CFG', 'T', 'BOOLEAN');

    -- CMe 20210810 Ende
        v_streng_fifo := isi_allg.get_firma_cfg_param(in_sid, in_firma_nr, 'AUSL_STRAT', null, 'STRENG_FIFO',
                                                      'LVS', 'CFG', 'F', 'BOOLEAN');

        v_bde_push_q_chk := isi_allg.get_firma_cfg_param(in_sid, in_firma_nr, 'AUSL_BDE_PUSH', 'RES', 'BDE_AUSL_QPLATZ_PUSH_SET',
                                                         'BDE', 'CFG', 'F', 'BOOLEAN');
    ...
    -- T = PRUEFEN - Es dürfen KEINE Reservierungen auf der LTE sein
        v_dispo_res_lte_chk := isi_allg.get_firma_cfg_param(in_sid, in_firma_nr, 'AUSL_BDE_PUSH', 'RES', 'BDE_AUSL_LTE_CHK_DISPO',
                                                            'BDE', 'CFG', 'F', 'BOOLEAN');
    ...
    -- T = LTE darf im Transport sein
        v_bde_ausl_ltr_chk_transp := isi_allg.get_firma_cfg_param(in_sid, in_firma_nr, 'AUSL_BDE_PUSH', 'RES', 'BDE_AUSL_LTE_CHK_TRANSP'
        ,
                                                                  'BDE', 'CFG', 'F', 'BOOLEAN');
        ...
        loop
            ...
            v_prod_params := null;
            open c_art_grp;
            fetch c_art_grp into v_prod_params;
            close c_art_grp;
      ...
      --               0 ist (P70397-849).
            if
                ( nvl(v_bde_fa.ma_reserviert, c.c_false) = c.c_false
                or nvl(v_bde_fa.ma_res_menge, 0) < v_bde_fa.ag_soll_mg )
                and ( v_dispo
                or (
                    in_min_res_menge is not null
                    and in_min_res_menge > 0
                ) )
            then
                open c_bde_fa_v;
                fetch c_bde_fa_v into v_res_roh_platz;
                close c_bde_fa_v;
                v_lams_a := null;
                v_lams_h := null;
                v_lams_c := null;
                if in_dispo_charge_rein = c.c_true then
                    open c_lams_c;
                    fetch c_lams_c into v_lams_c;
                    v_found := c_lams_c%found;
                    close c_lams_c;
                elsif in_dispo_charge_rein = 'H'
                or in_dispo_charge_rein = 'HERSTELLERVORGABE' then
                    ...
                    open c_lams_h;
                    fetch c_lams_h into v_lams_h;
                    v_found := c_lams_h%found;
                    close c_lams_h;
                else
                    open c_lams_a;
                    fetch c_lams_a into v_lams_a;
                ...
                if isi_p_base.get_resource(in_sid, v_bde_fa.res_id, v_res) then
                    if lvs_p_base.get_lgr_platz(v_res.lager_roh, v_lgr) then
                        if in_ice = c.c_true
                        or v_bde_fak.soll_betriebsart = 'JITSEQ' then
            ...
            -- Dann muss die LTE auf Reservierungn gepueft werden.
                ...
                if not v_found then
                    if v_bde_fa.ma_reserviert = c.c_true then
                        ...
                        raise_isi_error(c.fmid_bestand_reicht_nicht,
                                        lc.ec_p3(lc.o_tp3_bestands_mg_fehlt_bde, v_bde_fa.leitzahl, v_bde_fa.fa_ag, v_bde_fa.fa_upos)
                                        );
                    end if;
                end if;
                ...
                loop
                    ...
                    if v_check_seq_res -- Wir fahren zu einem Ziel, dass Durchlauf-Lager ist STRENG FIFO mit reduzierten Zugriff auf Ware
                     then
                        open c_lte_lam_res;
                        fetch c_lte_lam_res into
                            v_fa_leit_res,
                            v_fa_start_gepl; -- Höchste Leitzahl oder Start geplant aus den Reservierungen der LTE
              ...
              -- CMe 20220505: P71141-175 Prüfen ob bereits eine Reservierung schon vorhanden ist, und ob 
              -- diese Reservierung für die gleiche Leitzahl oder ABNR ist 
              ...
              if c_fa_auf_res%FOUND then
                ...
                if (v_bde_fa_res.leitzahl = v_bde_fa.leitzahl) or
                   ...
                   (not v_found) then*/
                            ...
                            open c_check_anz_lte_res;
                            fetch c_check_anz_lte_res into v_anz_res_on_lte;
                            v_found := c_check_anz_lte_res%found;
                            close c_check_anz_lte_res;
                            if ( not v_found )
                            or ( v_anz_res_on_lte = 0 ) then
                                v_check_t_ziel_seq := false;
                -- CMe 20220221 (P71141-40) Auch hier die Transporte berücksichtigen
                                ...
                                open c_check_trans_z_durchl;
                                fetch c_check_trans_z_durchl into v_t_ziel_lgr;
                                v_check_t_ziel_seq := c_check_trans_z_durchl%found;
                                close c_check_trans_z_durchl;
                                v_lte_dispo_ziele := get_lte_dipo_ziele(
                                    in_sid      => v_bde_fa.sid,
                                ...
                                if
                                    ( v_found )
                                    and ( v_bde_ausl_ltr_chk_transp = 'T' )
                                    and ( v_lte_dispo_ziele = v_lgr_gruppe_fa )
                                    and ( not v_check_t_ziel_seq )
                                then
          ...
          -- ob der FA die selbe Lagergruppe als ziel hat wie der anliegende Transport
            ...
            --CMe 20220808 (P71141-237): Alle nicht reservierten lams zusammenfasssen
                        lvs_p_lte_lhm.unite_lams_without_reservation(
                            in_sid          => v_lte.sid,
                            in_firma_nr     => v_lte.firma_nr,
                            in_lte_id       => v_lte.lte_id,
                            in_user_id      => in_user_id,
                            in_consider_mhd => 'T',
                            in_min_mhd      => sysdate
                        );

                        v_result := lvs_ausl.lvs_lte_reserv_user_ziel(in_sid, in_firma_nr, v_bde_fa.leitzahl, v_bde_fa.auf_id, v_lte_id
                        ,
                                                                      v_bde_fa.ag_artikel_id, in_user_id, v_res_roh_platz);
            
            -- Es soll nicht die ganze lte reserviert werden, also Teilreservierungen auf LHMs
            -- zuviel reservierte LAMs zurücksetzen
                        v_delta_menge := v_menge_sum - ( v_bde_fa.ag_soll_mg - nvl(v_bde_fa.ma_res_menge, 0) );
                      ...
                      -- nur Teilmenge reserviert lassen (für spätere kommissionierung)
                                            ...
                                            v_lte_lams.res_menge := v_lte_lams.menge - v_delta_menge;
                                        ...
                                        if v_menge = 0 -- Von dieser Palette ist nichts mehr Reserviert
                                         ...
                                         then
                                            v_result := lvs_ausl.lvs_lte_res_rueck(in_sid, in_firma_nr, v_bde_fa.leitzahl, v_bde_fa.auf_id
                                            , v_lte.lte_id,
                                                                                   v_bde_fa.leitzahl, v_lte.lgr_platz, c.c_true);
            ...
            --              auf Grund von Mangel handelt und sich auf der LTE bereits reserviertes Materila befindet
                        open c_check_anz_res_lams;
                        fetch c_check_anz_res_lams into v_anz_res_lam;
                        close c_check_anz_res_lams;
                        if ( v_anz_res_lam > 1 ) then
                            lvs_p_lte_lhm.add_amount_to_reservation(
                                in_sid          => v_lte.sid,
                                in_firma_nr     => v_lte.firma_nr,
                                in_lte_id       => v_lte.lte_id,
                                in_auf_id       => v_bde_fa.auf_id,
                                in_user_id      => in_user_id,
                                in_consider_mhd => 'T',
                                in_min_mhd      => sysdate
                            );
                    ...
                    exit when v_menge_sum >= ( v_bde_fa.ag_soll_mg - nvl(v_bde_fa.ma_res_menge, 0) )
                    or v_menge_sum >= in_min_res_menge; -- Fertig wenn Menge erreicht
                end loop;

                close c_ltes;
                if
                    in_komplett = c.c_true
                    and v_menge_sum < ( v_bde_fa.ag_soll_mg - nvl(v_bde_fa.ma_res_menge, 0) )
                then
                    raise_isi_error(c.fmid_bestand_reicht_nicht,
                                    lc.ec_p3(lc.o_tp3_bestands_mg_fehlt_bde, v_bde_fa.leitzahl, v_bde_fa.fa_ag, v_bde_fa.fa_upos));
                end if;
        ...
        -- Hersteller oder Rohcharge im MA-Satz hinterlegen
                ...
                set
                    fa.ma_reserviert = 'T',
                    fa.ma_res_menge = nvl(fa.ma_res_menge, 0) + v_menge_sum,
  ...
  __________________________________________________
  Description
  Erzeugt für die Menge der übergeben LAM die nicht reserviert ist eine neue LAM und
  pakt die entsprechende Menge auf die neue LAM um
  __________________________________________________
  TODO
  none.
  __________________________________________________
  Date         Ver.        AUTOR    Comment
  -----------  ---------   ------   ---------------
  09.09.2022   DB31_1      (-CMe-)  Es wird beim erzeugen der neuen LAM geprüft, ob die Daten der orginal
                                    LHM vorhanden sind. Wenn nicht dann wird für die neue LHM ein Datensatz
                                    basierend auf der CFG erzeugt. Für die LHM der original LAM werden ggf.
                                    ebenfall neue Daten / Einträge erzeugt.
                                    
                                    Sonst werden für die neue LHM die LHM daten der original LAM verwendet.
                                    
                                    Ticket: (W23910-279)
  */
    procedure create_new_lam_f_rest (
        in_sid      in isi_sid.sid%type,
        in_firma_nr in isi_firma.firma_nr%type,
        ...
        v_org_lhm_id        lvs_lhm.lhm_id%type;
        cursor c_charge is
        select
            t.*
        from
        ...
        where
                1 = 1
            and lhm.sid = in_lam.sid
            and lhm.firma_nr = in_lam.firma_nr
            and lhm.lhm_id = v_org_lhm_id;

    begin
        v_org_lhm_id := in_lam.lhm_id;
        open c_charge;
        ...
        if
            in_lam.hersteller_kuerzel_liste is not null
            and in_lam.hersteller_kuerzel_liste != ';'
            and isi_p_base.get_artikel_ctrl_typ(in_lam.sid,
                                                in_lam.artikel_id,
        ...
        v_lhm_id := lvs_p_lte.lvs_lte_lhm_next_id_v35(in_lam.sid, in_lam.firma_nr, c.basis_lhm, v_charge.charge_bez, in_lam.artikel_id
        ,
    ...
    --                           verwendet. Sonst werden die Daten aus der LHM CFG geladen und eingetragen
      ...
      -- CMe 09.09.2022 (W23910-279): Wenn es die LHM aus der in_lam nicht gibt, dann muss diese
      --                              auf jedenfall erzeugt werden, dass ansonsten im Prozess zu
      --                              Fehlern kommen kann. 
            if ( v_org_lhm_id is not null ) then
                insert into lvs_lhm (
                    sid,
                    firma_nr,
                    lhm_id,
    ...
    -- -AG- 06.09.2010 Erweiterung LFDN in der Charge
        insert into lvs_lam values ( in_lam.sid, -- SID                 VARCHAR2(2) not null,
        ...
        begin
            if ( nvl(in_lam.menge, 0) <> 0 ) then
                v_lam_bh_kg := ( nvl(in_lam.lam_kg, 0) * nvl(in_lam.menge - in_lam.res_menge, 0) ) / nvl(in_lam.menge, 0);
    ...
    -- Abgangsbuchungen
        insert into lvs_lam_bh values ( in_sid, -- SID               VARCHAR2(2) not null,
                                        ...
                                        in_lam.lgr_platz, -- LGR_PLATZ         VARCHAR2(30),
                                        in_lam.lte_id, -- LTE_ID            VARCHAR2(19),
                                        v_org_lhm_id, -- LHM_ID            VARCHAR2(19),
                                        in_lam.charge_id, -- CHARGE_ID         NUMBER,
                                        in_lam.serie_id, -- SERIE_ID          NUMBER,
    ...
    procedure storno_bde_fa_dispo (
        ...
        from
            lvs_lam l
        where
            l.order_pos_auf_id = v_bde_fa.auf_id
        group by
            l.lte_id;
    ...
    begin
        ...
        loop
            ...
            loop
                ...
                v_result := lvs_ausl.lvs_lte_res_rueck(in_sid, in_firma_nr, v_bde_fa.leitzahl, v_bde_fa.auf_id, v_lte_id,
                                                       v_bde_fa.leitzahl, v_bde_fa.auf_id, c.c_true);
            ...
            set
                ...
                fa.ma_res_charge_id =
                    case
                        when fa.ma_res_charge_id is not null then
                            0
                        else
                            null
                    end,
  ...
  --------------------------------------------------------------------------------
    ...
    function check_bde_fa_for_delete (
  ...
  --------------------------------------------------------------------------------
    ...
    function get_lte_dipo_ziele (
        ...
        select
            stradd_distinct(lgr_platz_gruppe)
        from
            (
                select
                    lgr.lgr_platz_gruppe
    ...
    function chk_lte_on_res_lgr_grp (
        in_sid      in lvs_lte.sid%type,
        in_firma_nr in lvs_lte.firma_nr%type,
        in_lte_id   in lvs_lte.lte_id%type,
        ...
        v_lte_platz_grp  lvs_lgr.lgr_platz_gruppe%type;
        v_fa_lgr_platz   lvs_lgr%rowtype;
        v_transport_id   isi_transport.transp_id%type;
        v_lgr_platz_ziel lvs_lgr.lgr_platz_gruppe%type;
        cursor c_get_lgr_platz_grp_fa is
        select
            lgr.*
        from
                 lvs_lgr lgr
            join isi_resource   res on res.lager_roh = lgr.lgr_platz
            join bde_fa_auftrag fag on fag.res_id = res.res_id
        where
            fag.auf_id = in_auf_id;

        cursor c_get_lgr_platz_grp_lte is
        select
            lgr.lgr_platz_gruppe
        from
                 lvs_lgr lgr
            join lvs_lte lte on lte.lgr_platz = lgr.lgr_platz
        where
            lte.lte_id = v_lvs_lte.lte_id;

        cursor c_chk_in_trans is
        select
            t.transp_id,
            nvl(z.lgr_platz_gruppe, 'KEIN_ZIEL_GRP')
        from
            isi_transport t
            left join lvs_lgr       z on z.lgr_platz = t.lgr_platz_ziel
        where
            t.lte_id = v_lvs_lte.lte_id;

    begin
        v_result := 1;
        v_faktor := 100;
        v_found := lvs_p_base.get_lte(in_lte_id, v_lvs_lte);
        if ( v_found ) then
            open c_get_lgr_platz_grp_fa;
            fetch c_get_lgr_platz_grp_fa into v_fa_lgr_platz;
            close c_get_lgr_platz_grp_fa;
            open c_chk_in_trans;
            fetch c_chk_in_trans into
                v_transport_id,
                v_lgr_platz_ziel;
            v_found := c_chk_in_trans%found;
            close c_chk_in_trans;
            if ( v_found ) then
                if ( v_fa_lgr_platz.lgr_platz_gruppe <> v_lgr_platz_ziel ) then
                    v_result := v_result + ( 20 / v_faktor );
                else
                    v_result := v_result - ( 15 / v_faktor );
                end if;
            end if;

            if ( v_fa_lgr_platz.lgr_typ = 'DURCHL1' ) then
                v_result := v_result + ( 100 / v_faktor );
            else
                v_lte_platz_grp := pps_p_bde.get_lte_dipo_ziele(
                    in_sid      => v_lvs_lte.sid,
                    in_firma_nr => v_lvs_lte.firma_nr,
                    in_lte_id   => v_lvs_lte.lte_id
                );

                if ( v_lte_platz_grp is not null ) then
                    if ( v_lte_platz_grp != v_fa_lgr_platz.lgr_platz_gruppe ) then
                        v_result := v_result + ( 10 / v_faktor );
                    end if;

                else
                    open c_get_lgr_platz_grp_lte;
                    fetch c_get_lgr_platz_grp_lte into v_lte_platz_grp;
                    v_found := c_get_lgr_platz_grp_lte%found;
                    close c_get_lgr_platz_grp_lte;
                    if ( not v_found ) then
                        v_result := v_result + ( 100 / v_faktor );
                    else
                        if ( v_lte_platz_grp != v_fa_lgr_platz.lgr_platz_gruppe ) then
  ...
  __________________________________________________
  Description
  Die Funktion prüft ob in den übergeben Produktionsparamtern eine feste LTE ID vergeben ist.
  Wenn eine feste ID vorhanden ist, wird geprüft, ob die übergeben LTE ID mit der in den Produktionsparametern
  idetisch ist. Wenn dies der Fall ist, wird eine 1 zurück gegeben, ansonsten eine 0
  ...
  Date         Ver.        AUTOR    Comment
  -----------  ---------   ------   ---------------
  15.07.2022   DB31_1      (-CMe-)  Neue Logik erstellt  
  */
    function comp_w_fixed_id_in_params (
        in_params in bde_fa_kopf.prod_params%type,
        in_lte_id in lvs_lte.lte_id%type
    ) return number is

        v_result              number;
        v_fixed_lte_param     varchar2(100);
        v_fixed_lte_id        lvs_lte.lte_id%type;
        v_chk_compare_allowed varchar2(1);
        cursor c_get_seperated_params is
        select
            regexp_substr(in_params, '[^;]+', 1, level) as transport_id
        from
            dual
        connect by
            regexp_substr(in_params, '[^;]+', 1, level) is not null;

    begin
        v_result := 0;
        v_fixed_lte_id := null;
        v_chk_compare_allowed := isi_allg.get_firma_cfg_param(
            in_sid                => '01',
            in_firma_nr           => 1,
            in_kategorie          => 'CFG',
            in_kategorie_ix       => null,
            in_parameter_name     => 'FIXED_ID_COMPARE_ALLOWED',
            in_modul_name         => 'PPS',
            in_typ                => 'CFG',
            in_default_param_wert => 'T',
            in_default_param_typ  => 'String'
        );

        if ( v_chk_compare_allowed = 'T' ) then
            open c_get_seperated_params;
            loop
                fetch c_get_seperated_params into v_fixed_lte_param;
                exit when c_get_seperated_params%notfound;
                if ( instr(v_fixed_lte_param, 'FIX-LTE-ID') > 0 ) then
          -- Wert entweder 'FIX-LTE-ID={LTE_ID}' oder 'FIX-LTE-ID=LTE_ID'
                    if ( instr(v_fixed_lte_param, '{') > 0 ) then
                        v_fixed_lte_param := substr(v_fixed_lte_param,
                                                    instr(v_fixed_lte_param, '{') + 1);
                        v_fixed_lte_id := substr(v_fixed_lte_param,
                                                 0,
                                                 length(v_fixed_lte_param) - 1);
                    else
                        v_fixed_lte_id := substr(v_fixed_lte_param,
                                                 instr(v_fixed_lte_param, '=') + 1);
                    end if;

                    exit when v_fixed_lte_id is not null;
                end if;

            end loop;

            close c_get_seperated_params;
            if
                ( v_fixed_lte_id is not null )
                and ( v_fixed_lte_id = in_lte_id )
            then
                v_result := 1;
            end if;
  ...
  __________________________________________________
  Description
  Erstellt Fertigungsaufträge gemäß den Fertigungsplänen. Erweiterung
  für die Berücksichtigung von festen LTE's durch die Planung aus dem APS.
  ...
  Date         Ver.        AUTOR    Comment
  -----------  ---------   ------   ---------------
  22.07.2022   DB31_1      (-CMe-)  Neue Logik erstellt  
  */
    procedure crt_bde_fa_auft_a_plan_db31_f (
        in_sid               in isi_sid.sid%type,
        in_firma_nr          in isi_firma.firma_nr%type,
        in_artikel_id        in isi_artikel.artikel_id%type,
        in_charge_bez        in lvs_charge.charge_bez%type,
    ...
    -- mehr als ene RESOURCE gilt die unterschiedliche Taktzeiten haben
        ...
        select
            ...
            nvl((
                select
                    pagrl.maschinenzeitfaktor
                from
                    pps_arb_plan_ag_res_id_liste pagrl
                where
                        pag.arb_plan_id = pagrl.arb_plan_id
        ...
        where
            ...
            and t.stueckliste_pos_id not in (
                select
                    t.stueckliste_pos_id
                from
                    pps_arb_plan_ag_stl t, pps_arb_plan_ag     t2
                where
                        v_bde_fa_plan.sid = t2.sid
                    and v_bde_fa_plan.firma_nr = t2.firma_nr
                    and v_bde_fa_plan.arb_plan_id = t2.arb_plan_id
    ...
    if v_anz_bde_beg > 0 then
      raise_isi_error(5,
        ...
        if not check_bde_fa_for_delete(in_sid, in_firma_nr, in_out_leitzahl, v_anz_bde_beg) then
            raise_isi_error(5,
                            lc.ec_p2(lc.o_tp2_arb_pps_ag_bde_beg, in_out_leitzahl, v_anz_bde_beg));
        end if;
    -- Loeschen derBDE-Daten fuer diesen FA_Auftrag
        delete bde_fa_kopf t
        where
    ...
    -- Übertragen der Artikeldaten für LHM-Name etc. aus Artikel oder KD-Artikel
        if v_artikel_kd.lhm_name is not null then
            if
                nvl(v_artikel_kd.lhm_menge, 0) = 0
        ...
        if v_artikel_kd.lhm_name is null
           or nvl(v_artikel_kd.lhm_menge, 0) = 0 then
            if
                nvl(v_artikel.lhm_menge, 0) = 0
    ...
    -- CMe 20220722 (P71208-6): Die Prod Params die feste LTE hinzufügen                     
        if
            ( in_fixed_id > 0 )
            and ( in_fixed_lte_id is not null )
        then
            v_bde_fa_plan.prod_params := v_bde_fa_plan.prod_params
                                         || 'FIX-LTE-ID='
                                         || in_fixed_lte_id;
        end if;
    ...
    -- Dann am Ende den Kopfsatz schreiben
        ...
        update bde_fa_auftrag t
        set
            t.lohn_arbeit = in_lohn_arbeit,
            t.adress_id = in_adress_id
        where
            t.leitzahl = in_out_leitzahl;
    ...
    -- Einbau der Tabelle wenn eine FA erstellt wurde
        ...
        if isi_allg.c_get_firma_cfg_param(in_sid, in_firma_nr, 'BDE_DISPO',           -- in_kategorie             in isi_firma_cfg.kategorie%type,
    ...
    end crt_bde_fa_auft_a_plan_db31_f;
