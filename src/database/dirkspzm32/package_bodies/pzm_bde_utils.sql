...
package body DIRKSPZM32.PZM_BDE_UTILS is
  ...
  --
  -------------------------------------------------------------------------------------------------------
  function get_bde_zeiten_by_pers_nr(in_pers_nr         in pzm_ze_bde_zeiten.ze_bde_pers_nr%type,
  ...
  function close_bde_zeiten_by_pers_nr(in_pers_nr         in pzm_ze_bde_zeiten.ze_bde_pers_nr%type,
  ...
  begin
    update pzm_ze_bde_zeiten t
       set t.ze_bde_day_ist_ende = in_datum
     where t.ze_bde_pers_nr = in_pers_nr
       and t.ze_bde_basis = 'BDE'
       and t.ze_bde_day_ist_ende is NULL
  ...
  function start_zeit_fa_pers_nr(in_pers_nr in isi_user.pers_nr%type,
                                 in_ts_datum    in pzm_ze_bde_zeiten.ze_bde_datum%type,
                                 in_leitzahl    in bde_fa_auftrag.leitzahl%type,
                                 in_fa_ag       in bde_fa_auftrag.fa_ag%type,
                                 in_fa_upos     in bde_fa_auftrag.fa_upos%type,
  ...
  begin
    ...
    if isi_allg.get_user_by_pers_nr(in_pers_nr, v_isi_user)
        ...
        then
          begin
            bde_pd_prod_p_ag_b(v_isi_user.sid,      -- in_sid,
                               ...
                               in_fa_ag,            -- in bde_fa_auftrag.fa_ag%type,
                               in_fa_upos,          -- in bde_fa_auftrag.fa_upos%type,
                               v_bde_fa_auftrag.res_id,
                               NULL,                -- Akt Terminal
                               v_isi_user.login_id);
          exception
            when others then v_result := 3;     -- FA im BDE nicht zu buchen
          end;
        else
          ...
          exception
            when others then v_result := 0;     -- FA im BDE nicht zu buchen, hier egal
          end;
  ...
  function c_start_zeit_fa_pers_nr(in_pers_nr     in isi_user.pers_nr%type,
    ...
    v_result number;
  begin
    v_result := start_zeit_fa_pers_nr (in_pers_nr,
                                       in_ts_datum,
                                       in_leitzahl,
                                       in_fa_ag,
                                       in_fa_upos,
  ...
  function c_pzm_pers_nr_zeit_fa_buchen(in_pers_nr      in pzm_personal.pers_nr%type,
    ...
    v_result number;
  begin
    v_result := pzm_bde_utils.pzm_pers_nr_zeit_fa_buchen(in_pers_nr,
                                           in_sa_kurz_name,
                                           in_datum_beg,
                                           in_datum_ende,
                                           in_zeit_std,
  ...
  begin
    ...
    then
      ...
      then
        ...
        v_found_bde := get_bde_zeiten_by_pers_nr(in_pers_nr,
                                                 in_datum,
                                                 NULL,
                                                 NULL,
                                                 NULL,
        ...
        then
          ...
          */
          update pzm_ze_bde_zeiten t -- Gebuchte Zeiten aus dem Satz nehmen
             set t.ze_bde_zeit_min = 0, -- round(t.ze_bde_zeit_min - v_bde_zeiten.ze_bde_zeit_min, 0)
                 t.ze_bde_status_text =
                 case when t.ze_bde_verbucht_status = 'UE' 
                      then substr('OldMin.=' || to_char(t.ze_bde_zeit_min) || ' ' || t.ze_bde_status_text, 1, 255)
                      else t.ze_bde_status_text
                      end
           where t.ze_bde_pers_nr = v_bde_zeiten.ze_bde_pers_nr
        ...
        update pzm_ze_bde_zeiten t
           set t.ze_bde_day_ist_ende = in_datum_ende
         where t.ze_bde_pers_nr = in_pers_nr
           and t.ze_bde_basis = 'BDE'
           and t.ze_bde_datum = in_datum
           and t.ze_bde_day_ist_start < in_datum_ende
        ...
        LOOP   
          ...
          or v_bde_zeiten.ze_bde_day_ist_ende = NULL
          then
            v_bde_zeiten.ze_bde_day_ist_ende := in_datum_ende;
          end If;
            
          v_bde_minuten_ges := ((  case when nvl(v_bde_zeiten.ze_bde_day_ist_ende, in_datum_ende) >= in_datum_ende 
                                        then in_datum_ende
                                        else nvl(v_bde_zeiten.ze_bde_day_ist_ende, in_datum_ende)
                                        end
                                 - case when v_bde_zeiten.ze_bde_day_ist_start >= in_datum_beg
                                        then v_bde_zeiten.ze_bde_day_ist_start
                                        else in_datum_beg
          ...
          update bde_fa_auftrag t
           ...
           where t.leitzahl = v_bde_zeiten.ze_bde_leitzahl
             ...
             and t.fa_upos = v_bde_zeiten.ze_bde_fa_upos;
          update pzm_ze_bde_zeiten t -- Gebuchte Zeiten aus dem Satz nehmen
             set t.ze_bde_zeit_min = nvl(v_bde_minuten_ges, 0),
...
--                   and t.ze_bde_day_ist_ende > in_datum_ende)
          ...
          if v_bde_zeiten.ze_bde_day_ist_start < v_bde_zeiten.ze_bde_day_ist_ende
          then
            insert into pzm_ze_bde_zeiten
              (ze_bde_pers_nr, 
               ze_bde_datum, 
        ...
        then
          ...
          then
            ...
            v_result := start_zeit_fa_pers_nr (v_bde_zeiten_open.ze_bde_pers_nr,
                                               sysdate,
                                               v_bde_zeiten_open.ze_bde_leitzahl,
                                               v_bde_zeiten_open.ze_bde_fa_ag,
                                               v_bde_zeiten_open.ze_bde_fa_upos,
                                               sysdate);
          end if;
        end if;
      end if;
  ...
  function c_pzm_ze_tagessatz_buchen(in_datum        in pzm_ze_tagessatz.ts_datum%type) 
    ...
    CURSOR c_ze_tagessatz is
      select *
       ...
       where t.ts_datum = trunc(in_datum);
  ...
  begin
    ...
    then -- Das ist in diesem Modus nicht möglich
      ...
      LOOP
        ...
        if v_ze_tagessatz.ts_pers_nr = 9100000700
        then
          v_ze_tagessatz.ts_pers_nr := v_ze_tagessatz.ts_pers_nr;
        end if;
        if v_ze_tagessatz.ts_day_arb_std + v_ze_tagessatz.ts_day_ueb_std  + v_ze_tagessatz.ts_day_flex_std > 0
        then
          v_ts_result := pzm_bde_utils.c_PZM_pers_nr_zeit_fa_buchen(v_ze_tagessatz.ts_pers_nr,         -- => :in_pers_nr,
                                                                    ...
                                                                    v_ze_tagessatz.ts_day_wert_ende
                                                                        - v_ze_tagessatz.ts_day_abw_std / 24,   -- => :in_datum_ende,
                                                                    v_ze_tagessatz.ts_day_arb_std +
                                                                    v_ze_tagessatz.ts_day_ueb_std +
                                                                    v_ze_tagessatz.ts_day_flex_std,     -- => in_zeit_min => :in_zeit_min,
          ...
          update pzm_ze_bde_zeiten t                                -- Wenn keine Anmeldung, dann wird der Satz geschlossen (Urlaub, Krank, ...)
             set t.ze_bde_day_ist_ende = t.ze_bde_day_ist_start,
                 t.ze_bde_verbucht_status = 'N' -- 'U' Erst mal auf U damit im Nachgang Freigegeben werden kann
           where t.ze_bde_pers_nr = v_ze_tagessatz.ts_pers_nr
             and t.ze_bde_basis = 'BDE'
             and t.ze_bde_datum = in_datum
             and t.ze_bde_day_ist_ende is NULL;
        end if;
