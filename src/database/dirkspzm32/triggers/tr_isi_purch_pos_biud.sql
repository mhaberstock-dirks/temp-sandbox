...
declare
  ...
  CURSOR c_isi_purch_kopf is
   select *
     from isi_purch_kopf s
    where s.id = v_kopf_id;

  CURSOR c_kunden is
    select *
      from isi_adressen t
     where t.adress_id = v_header.kunde_id;
...
begin
  ...
  then
    if :new.id is null
    then
      select seq_isi_purch_pos.nextval into :new.id from dual;
    end if;
    ...
    if not v_found then
      v_err_nr := 20;
      v_err_text := lc.ec_p1(lc.o_tp1_artikel_id_fehlt  , to_char(:new.artikel_id));
    end if;

    v_liefer_datum := v_header.liefer_datum;
    if v_liefer_datum is null
    then
      v_liefer_datum := trunc(sysdate);
    end if;

    if not isi_p_base.get_isi_artikel_nr_by_id(:new.sid, :new.artikel_id, v_artikel)
    then
      v_err_nr := 20;
      v_err_text := lc.ec_p1(lc.o_tp1_artikel_id_fehlt  , to_char(:new.artikel_id));
    end if;

    if v_header.vorg_typ ='BE' or v_header.vorg_typ ='AN'
    then
      insert into s_rcv_auftr
           values (:new.firma_nr,       -- FIRMA_NR
                   :new.id,             -- AUF_ID
    ...
    then
      ...
      then
        v_err_nr := 20;
        v_err_text := lc.ec_p1(lc.O_TP1_TXT_ADRESSE_NF  , to_char(v_header.kunde_id));
      end if;

      insert into s_rcv_kunden_auftr_pos
           values ( :new.sid,                    -- SID
  ...
  then
    update s_rcv_auftr a
       set a.status = 'L'
     where a.auf_id = :old.id;

    update s_rcv_kunden_auftr_pos kap
       set kap.status = 'D'
     where kap.kunden_auftr_pos_id = :old.id;
  end if;

  if updating
  then
    if :new.wek_ok = 'T' and ((:old.wek_ok is null) or  (:old.wek_ok = 'F'))
    then
    ...
    then
      update s_rcv_auftr a
         set a.POS_NR = :new.pos,
             a.soll_mg = :new.menge,
             a.liefer_datum  = v_liefer_datum
       where a.auf_id = :new.id;
    end if;
    ...
    then
      update s_rcv_kunden_auftr_pos a
         set a.POS_NR = :new.pos,
             ...
             a.liefer_datum  = v_liefer_datum
       where a.kunden_auftr_pos_id = :new.id;
    end if;
  end if;
exception
