  ...
  */
  ...
  begin
   ...
   v_print_daten :=
    ...
    'VdaPackstueckNr='                                     || v_VdaPackstueckNr || cr_lf() ||
    ...
    'VdaPackstueckNrBarcode='                              || v_VdaPackstueckNrBarcode || cr_lf() ||
  ...
  function std_etikett(in_sid                 in isi_sid.sid%type,
    ...
    CURSOR c_lte is
      select lte.sid,
             ...
             decode(min(lam.mengeneinheit_basis), max(lam.mengeneinheit_basis), min(lam.mengeneinheit_basis) , NULL) menge_basis,
             decode(min(lam.leitzahl), max(lam.leitzahl), min(lam.leitzahl) , NULL) leitzahl,
             decode(trunc(min(lam.lam_mhd)), trunc(max(lam.lam_mhd)), trunc(min(lam.lam_mhd)) , NULL) lam_mhd,
             decode(trunc(min(lam.zug_datum)), trunc(max(lam.zug_datum)), trunc(min(lam.zug_datum)) , NULL) zug_datum,
             decode(trunc(min(lam.prod_datum)), trunc(max(lam.prod_datum)), trunc(min(lam.prod_datum)) , NULL) prod_datum,
             decode(min(lam.zeichnung), max(lam.zeichnung), min(lam.zeichnung) , NULL) zeichnung,
             decode(min(lam.zeichnung_index), max(lam.zeichnung_index), min(lam.zeichnung_index) , NULL) zeichnung_index,
             decode(min(lam.artikel_id), max(lam.artikel_id), min(lam.artikel_id) , NULL) artikel_id,
       ...
       where  lte.sid = lam.sid (+)
         ...
         and lte.lte_id = lam.lte_id (+)
       group by lte.sid, lte.firma_nr, lte.order_vorgang_id, lte.lte_voll, lte.lte_id, lte.lte_name, lte.ziel_lgr_platz,
                lte.lte_akt_lhm, lte.lgr_platz, lte.lgr_ort, lte.ziel_lgr_ort;
    ...
    CURSOR c_lhm is
      select lam.sid,
       ...
       where lhm.lhm_id = in_id
         and lhm.lte_id = lte.lte_id
         and lhm.lhm_id = lam.lhm_id
       group by lam.sid, lam.firma_nr, lte.order_vorgang_id, lam.lte_id, lhm.lhm_id, lhm.lhm_name, lte.ziel_lgr_platz, lte.lgr_platz, lte.lgr_ort, lte.ziel_lgr_ort;
  ...
  begin
    ...
    then
      ...
      FETCH c_lte into     v_lte_sid, v_lte_firma_nr, v_lte_lte_id, v_lhm_lte_id, v_lte_lte_name, v_lte_ziel_lgr_platz,
                           v_lte_lgrort, v_LaborStatus, v_LaborStatusText,
                           v_lte_lte_akt_lhm,v_lte_order_vorgang_id,  v_lte_lte_voll, v_lhm_lhm_name, v_lam_charge_id,
                           v_lam_kunden_nr, v_lam_best_nr, v_brutto, v_netto,
                           v_order_pos_auf_id, v_lam_memge, v_lam_memge_basis, v_lam_leitzahl,
                           v_lam_mhd_datum,v_lam_zug_datum, 
                           v_lam_prod_datum, v_lam_zeichnung, v_lam_zeichnung_index,
   ...
   then
     lvs_p_lte_lhm.spez_barcode_result(in_sid,                      -- in_sid        in lvs_charge.sid%type,
                                       in_firma_nr,
                                       v_stdpackstuecknr,            -- in  lvs_lam.lhm_id%type,
     ...
     for i in 1..length(v_stdPackstueckNr)
     loop
       if upper(substr(v_bc_format, i, 1)) = 'M'
       then
         if v_stdPackstueckNrSollMenge is NULL
         then
           v_stdPackstueckNrSollMenge := substr(v_stdPackstueckNr, i, 1);
         else
           v_stdPackstueckNrSollMenge := v_stdPackstueckNrSollMenge || substr(v_stdPackstueckNr, i, 1);
         end if;
       end if;
     end loop ;
     ...
     loop
       if upper(substr(v_bc_format, i, 1)) = 'M'
       then
         if v_stdPackstueckNrMenge is NULL
         then
           v_stdPackstueckNrMenge := substr(v_stdPackstueckNr, i, 1);
         else
           v_stdPackstueckNrMenge := v_stdPackstueckNrMenge || substr(v_stdPackstueckNr, i, 1);
         end if;
       end if;
     end loop ;
   ...
   if v_hersteller.kuerzel is not NULL
   then
     v_stdSachNrLieferant := v_stdSachNrLieferant || ' (' || v_hersteller.kuerzel || ')';
   end if;
   ...
   v_print_daten :=
    ...
    'stdLagerOrt='                                        || v_stdLagerOrt || CHR(13) || CHR(10) ||
    'stdLaborStatus='                                     || v_stdLaborStatus || CHR(13) || CHR(10) ||
    'stdLaborStatusText='                                 || v_stdLaborStatusText || CHR(13) || CHR(10) ||
    ...
    'stdTyp='                                             || v_stdTyp || CHR(13) || CHR(10) ||
    'stdDatum='                                           || v_stdDatum || CHR(13) || CHR(10) ||
    'stdProdDatum='                                       || v_stdProdDatum || CHR(13) || CHR(10) ||
    'stdZugDatum='                                        || v_stdZugDatum || CHR(13) || CHR(10) ||
    'stdMhdDatum='                                        || v_stdMhdDatum || CHR(13) || CHR(10) ||
    ...
    'stdPackstueckNr='                                     || v_stdPackstueckNr || CHR(13) || CHR(10) ||
    'stdChargenNr='                                       || v_stdChargenNr || CHR(13) || CHR(10) ||
    ...
    'stdPackstueckNrBarcode='                              || v_stdPackstueckNrBarcode || CHR(13) || CHR(10) ||
    'stdSachNrLieferantBarcode='                          || v_stdSachNrLieferantBarcode || CHR(13) || CHR(10) ||
    'stdChangenNrBarcode='                                || v_stdChargenNrBarcode || CHR(13) || CHR(10) ||
    ...
    'stdPackstueckNrMenge='                                || v_stdPackstueckNrMenge || CHR(13) || CHR(10) ||
    'stdPackstueckNrSollMenge='                            || v_stdPackstueckNrSollMenge || CHR(13) || CHR(10) ||
    'stdReifenTyp='                                       || v_stdReifenTyp   || CHR(13) || CHR(10) ||
    'stdFelgenTyp='                                       || v_stdFelgenTyp   || CHR(13) || CHR(10) ||
    'stdVentil='                                          || v_stdVentil      || CHR(13) || CHR(10) ||
    'stdstdTyp='                                          || v_stdTyp   || CHR(13) || CHR(10) ||
    'stdReifenHersteller='                                || v_stdReifenHersteller      || CHR(13) || CHR(10) ||
    'stdEtiDatum='                                        || v_stdEtiDatum   || CHR(13) || CHR(10) ||
    'stdEtiZeit='                                         || v_stdEtiZeit      || CHR(13) || CHR(10) ||
    'stdArtikelP1='                                       || v_stdArtikelP1   || CHR(13) || CHR(10) ||
    'stdArtikelP2='                                       || v_stdArtikelP2   || CHR(13) || CHR(10) ||
    'stdArtikelP3='                                       || v_stdArtikelP3   || CHR(13) || CHR(10) ||
    'stdArtikelP4='                                       || v_stdArtikelP4   || CHR(13) || CHR(10) ||
    'stdArtikelP5='                                       || v_stdArtikelP5   || CHR(13) || CHR(10) ||
    'stdArtikelP6='                                       || v_stdArtikelP6   || CHR(13) || CHR(10) ||
    'stdArtikelP7='                                       || v_stdArtikelP7   || CHR(13) || CHR(10) ||
    'stdArtikelP8='                                       || v_stdArtikelP8   || CHR(13) || CHR(10) ||
    'stdArtikelP9='                                       || v_stdArtikelP9   || CHR(13) || CHR(10) ||
    'stdArtikelP10='                                      || v_stdArtikelP10;
  ...
  begin
    ...
    'stdPackstueckNr='                                     || '01' || CHR(13) || CHR(10) ||
    ...
    'stdPackstueckNrBarcode='                              || '12345612345601010000' || CHR(13) || CHR(10) ||
    ...
    'stdEinsVonX='                                        || '1-1' || CHR(13) || CHR(10) ||
    'stdSpezBarcode='                                     || '1234567890' || CHR(13) || CHR(10) ||
    'stdArtikelP1='                                       || '4'    || CHR(13) || CHR(10) || -- Layout 4 bei Sasol
    'stdArtikelP2='                                       || 'P2'   || CHR(13) || CHR(10) ||
    'stdArtikelP3='                                       || 'P3'   || CHR(13) || CHR(10) ||
    'stdArtikelP4='                                       || 'P4'   || CHR(13) || CHR(10) ||
    'stdArtikelP5='                                       || 'P5'   || CHR(13) || CHR(10) ||
    'stdArtikelP6='                                       || 'P6'   || CHR(13) || CHR(10) ||
    'stdArtikelP7='                                       || 'P7'   || CHR(13) || CHR(10) ||
    'stdArtikelP8='                                       || 'P8'   || CHR(13) || CHR(10) ||
    'stdArtikelP9='                                       || 'P9'   || CHR(13) || CHR(10) ||
    'stdArtikelP10='                                      || 'P10';
