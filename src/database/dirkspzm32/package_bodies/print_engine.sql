...
package body DIRKSPZM32.print_engine is
  ...
  *******************************************************************************/
	procedure c_insert_new_job(in_sid              in varchar2,
													   ...
													   out_job_nr          out pe_jobs.job_nr%type) is
		-------------------------------------------------------------------------------------------------------
  ...
  * procedure C_INSERT_NEW_JOB(...)
  *******************************************************************************/
	procedure insert_new_job(in_sid              in varchar2,
													 in_firma_nr         in number,
													 in_rave_datei       in pe_jobs.rave_datei%type,
													 in_rave_report_name in pe_jobs.rave_report_name%type,
													 in_job_daten_typ    in pe_jobs.job_daten_typ%type,
    ...
    if print_allg.get_drucker_layout_cfg(in_sid, in_firma_nr,
                                         in_drucker_name,
                                         v_layout_name,
                                         v_layout_datei,
                                         v_dr_layout_cfg)
    then
      ...
      if v_dr_layout_cfg.dest_anzahl is not null
      then
    ...
    -- Printengine kann die Anzahl noch nicht verarbeiten darum in LOOP
		select pe_seq_pe_jobs.nextval into out_job_nr from dual;
    ...
    then
      ...
      delete from pe_jobs t
       where t.sid = in_sid
		...
		v_err_nr   := 10;
		v_err_text := 'Einfügen des Druckjobs: ' || out_job_nr ||
									' nicht möglich';
		insert into pe_jobs
		values
			(in_sid,
  ...
  *******************************************************************************/
	function get_job_status(in_sid              in varchar2,
													in_firma_nr         in number,
		...
		cursor c_print_job is
			select pe_jobs.status,
						 pe_jobs.status_text
				from pe_jobs
			 where pe_jobs.sid = in_sid
	...
	begin
		...
		fetch c_print_job
			into out_job_status, out_job_status_text;
  ...
  * function GET_JOB_DATEN(...) return boolean
  *******************************************************************************/
	function get_job_daten(in_sid          in varchar2,
												 in_firma_nr     in number,
												 in_job_nr       in pe_jobs.job_nr%type,
		...
		cursor c_drucker_typ is
			select pe_drucker_cfg.drucker_typ
				from pe_drucker_cfg
			 where pe_drucker_cfg.sid = in_sid
  ...
  *******************************************************************************/
	function get_next_job(in_sid          in varchar2,
												in_firma_nr     in number,
												out_job_data    out pe_jobs%rowtype,
												out_drucker_typ out pe_drucker_cfg.drucker_typ%type)
		return boolean is
    ...
    -- -AG- Anpassung der Sortierung für die Abarbeitung (Nach Problemen mit Sasol)
		cursor c_print_job is
			select p.*
				from pe_jobs p
			 where p.status = job_status_neu
				 and p.sid = in_sid
				 and p.firma_nr = in_firma_nr
         and not exists (select p2.drucker_name -- Nur Jobs von druckern berücksichtigen
                           from pe_jobs p2      -- die noch nicht drucken
                          where p2.status = job_status_drucken
                            and p2.drucker_name = p.drucker_name)
       order by decode(p.status_text, NULL, 0, 1), -- Erst mal die noch kein Problem hatten
  ...
  *******************************************************************************/
	procedure c_set_job_status(in_sid             in varchar2,
														 in_firma_nr        in number,
														 in_job_nr          in pe_jobs.job_nr%type,
														 in_job_status      in varchar2,
														 in_job_status_text in varchar2) is
	begin
		update pe_jobs
			 set status      = in_job_status,
					 status_text = in_job_status_text
		 where sid = in_sid
			 and firma_nr = in_firma_nr
			 and job_nr = in_job_nr;

		if in_job_status = job_status_neu
		then
			update pe_jobs t
				 set t.job_timestamp = sysdate
			 where sid = in_sid
				 and firma_nr = in_firma_nr
				 and job_nr = in_job_nr;
		end if;
  ...
  * procedure C_SET_JOB_STATUS((...)
  *******************************************************************************/
	procedure c_reset_jobs(in_sid in varchar2, in_firma_nr in number) is
	begin
		update pe_jobs
			 set status      = job_status_fehler,
					 status_text = 'Reset',
					 status_time = sysdate
		 where sid = in_sid
			 and firma_nr = in_firma_nr
  ...
  *******************************************************************************/
	procedure c_reprint_last_job(in_sid             in varchar2,
  														 in_firma_nr        in number,
														   in_printer_name    in pe_jobs.drucker_name%type) is
	begin
		update pe_jobs j
			 set status      = 'N'
		 where j.sid = in_sid
			 and j.firma_nr = in_firma_nr
       and j.job_nr = (select max(j.job_nr)
                         from pe_jobs j
                        where j.sid = in_sid
                          and j.firma_nr = in_firma_nr
