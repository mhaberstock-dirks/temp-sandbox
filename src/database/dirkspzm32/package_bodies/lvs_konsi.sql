create or replace package body dirkspzm32.lvs_konsi is
  /*
...
*/
  --------------------------------------------------------------------------------
    ...
    begin
        change_goods_owner(in_lte_id, in_owner_address_id, in_login_id, in_res_id);
        commit;
    exception
        when others then
            rollback;
...
*/
  --------------------------------------------------------------------------------
    ...
    begin
        ...
        if v_lte.lgr_platz is null then
            raise_isi_error(15,
                            lc.ec_p1(lc.o_tp1_lte_ohne_lgr_platz, in_lte_id));
        end if;

        if v_lte.lte_status != 'LF' then
            raise_isi_error(40,
                            lc.ec_p1(lc.o_tp1_lte_id_w_transportiert, in_lte_id));
        end if;

        if
            in_owner_address_id is not null
            and not isi_p_base.get_adress_nr_by_id('01', in_owner_address_id, v_owner_adr_nr)
        then
            raise_isi_error(20,
                            lc.ec_p1(lc.o_tp1_txt_adresse_nf, in_owner_address_id));
        end if;
      ...
      -- Merken letzte LAM für RES_STRING abholen
            ...
            if v_buch_vorg_id is null then
      ...
      -- Eigentümerwechsel buchen
            ...
            ) values ( v_buch_vorg_id,         -- VORG_ID           NUMBER not null,
      ...
      -- Eigentümer ändern
            update lvs_lam t
            set
                t.owner_address_id = in_owner_address_id
            where
                t.lam_id = v_lam_id;

      -- Erforderliche Daten für die Schnittstelle holen
            v_found := isi_p_order_base.get_order_pos_by_id_pos_type(v_lam.best_nr, 'KWE', v_lam.best_pos, v_order_pos);

            v_found := lvs_p_base.get_lgr_platz(v_lte.lgr_platz, v_lgr);

      -- Schnittstelle schreiben
            s_schnittstelle.write_host_bew(v_order_pos, v_lam, v_lam_bh_id, c.lam_bh_bus_wke_konsi, c.lam_bh_wke,
                                           'S_AUF', 'UE', null, v_lgr, in_login_id,
                                           v_lam_bh_menge);

        end loop;

        close c_lam;
        if lvs_p_base.get_lam('01', 1, v_lam_id, v_lam) then
      -- Der Reservierungsstring muss für die neue
      -- Eigentümersituation ermittelt und gesetzt werden
            v_res_string := lvs_util.get_res_string_v359(v_lam.sid, v_lam.firma_nr, v_lte.waren_typ, v_lte.res_artikel_id, v_lam.hersteller_kuerzel_liste
            ,
   ...
   */
    ...
    ----------------------------------------------------------------------------
        ...
        select
            count(bh.lam_bh_id) count_lam_bh,
            nvl(
                count(lam.lam_id),
                0
            )                   count_lam
        ...
        where
                bh.vorg_id = v_lam_bh_ref.vorg_id
        ...
        from
            lvs_lam_bh t
        where
                t.vorg_id = v_lam_bh_ref.vorg_id
    ...
    -- Plausibilitäten prüfen
        if not lvs_p_base.get_lam_bh_by_id(in_lam_bh_id, v_lam_bh_ref) then
        ...
        fetch c_lam_bh_vorg_count into
            v_count_lam_bh,
            v_count_lam;
        ...
        if not v_found then
            raise_isi_error(50,
                            lc.ec_p1(lc.o_tp1_lte_id_update_err, in_lte_id));
        end if;

        if v_count_lam_bh != v_count_lam then
        ...
        loop
            ...
            if ( lvs_p_base.get_lam('01', 1, v_lam_bh.lam_id, v_lam) ) then
        -- Stornierung durchführen
                update lvs_lam_bh bh
                set
                    bh.ls_login_id = in_login_id,
                    bh.change_menge = bh.menge,
                    bh.menge = 0,
                    bh.last_change_date = sysdate,
                    bh.last_change_login_id = in_login_id
                where
                    bh.lam_bh_id = v_lam_bh.lam_bh_id;

        -- Eigentümer zurücksetzen
                update lvs_lam lam
                set
                    lam.owner_address_id = v_lam_bh_ref.owner_address_id -- vorherigen Eigentümer eintragen
                where
        ...
        -- Schnittstelle schreiben
                s_schnittstelle.write_host_bew(v_order_pos, v_lam, v_lam_bh.lam_bh_id, c.lam_bh_bus_wke_konsi, c.lam_bh_wke,
                                               'S_AUF', 'UE', null, v_lgr, in_login_id,
                                               (v_lam_bh.menge * -1));

            else
                raise_isi_error(60,
                                lc.ec_p1(lc.o_tp1_lam_id_fehlt, v_lam_bh.lam_id));
            end if;

        end loop;

        close c_lam_bh;
        if lvs_p_base.get_lam('01', 1, v_lam_bh_ref.lam_id, v_lam_ref) then
      -- Der Reservierungsstring muss für die neue
      -- Eigentümersituation ermittelt und gesetzt werden
            v_res_string := lvs_util.get_res_string_v359(v_lam_ref.sid, v_lam_ref.firma_nr, v_lte.waren_typ, v_lte.res_artikel_id, v_lam.hersteller_kuerzel_liste
            ,
                                                         v_lam_ref.fa_ag, v_lam_ref.charge_id, v_lam_ref.serie_id, v_lam_ref.leitzahl
                                                         , v_lam_ref.kunden_nr,
                                                         v_lam_ref.lieferant_nr, v_lam_ref.best_nr, v_lam_ref.lam_mhd, 1, v_lam_ref.labor_status
                                                         ,
                                                         v_lte.lte_voll, v_lam_ref.owner_address_id, v_res_mhd);
...
-- sqlcl_snapshot {"hash":"917f7725ed8badaa659943c5597848a278c3ef82","type":"PACKAGE_BODY","name":"LVS_KONSI","schemaName":"DIRKSPZM32","sxml":""}