...
package body DIRKSPZM32.LVS_P_INVENTUR is
  ...
  begin
    if not isi_allg.get_artikel_by_artikel_id(in_sid, in_artikel_id, v_artikel)
    then
      v_err_nr := 5;
      v_err_text := LC.ec_p1(LC.O_TP1_ARTIKEL_ID_FEHLT, nvl(to_char(in_artikel_id), 'null'));
      raise v_error;
    end if;
    ...
    then
      v_err_nr := 10;
      v_err_text := LC.ec_p1(LC.O_TP1_INV_ARTIKEL_STATUS_ERST, nvl(to_char(v_artikel.artikel), 'null'));
      raise v_error;
    end if;

    if v_artikel_status.akt_inventur_id is not null
       and v_artikel_status.akt_inventur_id != in_akt_inventur_id
    then
      v_err_nr := 20;
      v_err_text := LC.ec_p2(LC.O_TP2_INV_ARTIKEL_IST_INVERTUR, nvl(to_char(v_artikel.artikel), 'null'), to_char(v_artikel_status.akt_inventur_id));
      raise v_error;
    end if;

    -- Neuen Wert speichern
    update lvs_artikel_status t
       set t.akt_inventur_id = in_akt_inventur_id
  ...
  */
  ...
  begin
    open c_lam_art;
    fetch c_lam_art into v_count, v_menge;
    v_result := c_lam_art%found;
    close c_lam_art;

    -- v_result korrigieren, falls keine Mengen vorhanden sind.
    v_result := v_result and v_count > 0 and v_menge > 0;
