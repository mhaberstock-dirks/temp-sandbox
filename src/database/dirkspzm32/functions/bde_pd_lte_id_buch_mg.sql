create or replace 
function DIRKSPZM32.bde_pd_lte_id_buch_mg
(
...
out_msg          out varchar2
  ...
  CURSOR c_lam_bh is
    select *
      from lvs_lam_bh t 
   where t.lhm_id = v_res_zus.lte_id  -- Ist die LHM_ID die gebucht wurde
     and t.bus = 2;
...
begin
  ...
  update isi_resource_zust_akt
     set lte_id = in_barcode                            -- Ergebnis im aktuellen Maschinenzustand SPEICHERN
   where sid = in_sid and
         res_id = in_res_id;
  update lvs_lte t
     set t.lte_id = in_barcode
   where t.lte_id = v_res_zus.lte_id;
         
  v_result := bde_scanner.bde_c_scanner_buch_spez(in_scanner_name,
                                                  v_res_zus.lte_id,
                                                  v_res_zus.lte_id,
                                                  out_msg);
  if   v_result != in_menge
  ...
  then
    ...
    then
      ...
      out_msg := 'PRODUKTION ' || to_char(in_menge) || ' OK';
      v_result := in_menge;
    end if;
  end if;
...
-- sqlcl_snapshot {"hash":"d93c5af89448c2978f63faecd56a45088930892b","type":"FUNCTION","name":"BDE_PD_LTE_ID_BUCH_MG","schemaName":"DIRKSPZM32","sxml":""}