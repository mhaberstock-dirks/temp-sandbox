...
package body DIRKSPZM32.isi_p_order is
  ...
  procedure rest_mg_wai_ma_pruefen_rt(in_sid             in isi_sid.sid%type,
  ...
  begin
    abbruch_transporte_359(in_sid, in_firma_nr, in_vorgang_id, NULL, NULL, in_auf_id, in_vorgang_typ, in_satzart, in_user_id, 'T', in_res_rueck);
    ...
    cursor c_pos is
      select pos.*
       ...
       where pos.sid = in_sid
         ...
         and (pos.li_nr = in_li_nr or in_li_nr is NULL)                   -- optional nur einen Lieferschein bearbeiten
         and (pos.li_pos_nr = in_li_pos_nr or in_li_pos_nr is NULL)       -- optional nur einen Lieferscheinposition bearbeiten
         and ((pos.status in ('D', 'R', 'T') and in_del_transp = c.C_TRUE)
           or  pos.status in ('D', 'R', 'V'))
         and (pos.auf_id = in_auf_id or in_auf_id is null)                -- optional nur eine auf_id bearbeiten
       order by pos.transport_gruppe desc,
                pos.vorgang_id,
                pos.li_nr,
                pos.li_pos_nr;
  ...
  begin
    ...
    update isi_order_kopf kopf
       ...
       where kopf.sid = in_sid
         ...
         and (kopf.li_nr = in_li_nr or in_li_nr is NULL)
    ...
    loop
      ...
      then
        ...
        loop
          ...
          v_result := lvs_ausl.lvs_lte_res_rueck (v_pos.sid,
                                                  v_pos.firma_nr,
                                                  v_pos.vorgang_id,
                                                  v_pos.auf_id,
                                                  v_lte.lte_id,
        ...
        delete isi_komm_order t
          where t.auf_id = v_pos.auf_id;

        update isi_order_pos pos
           set pos.freigegeben_datum = null,                    -- FreigabeDatum setzen
               pos.ware_disponiert = 'F',
               pos.status = 'N'
           where pos.sid = v_pos.sid
  ...
  begin
    abbruch_transporte(in_sid, in_firma_nr, in_vorgang_id, in_auf_id, in_vorgang_typ, in_satzart, in_user_id, in_res_rueck);
  ...
  begin
    abbruch_transporte(in_sid, in_firma_nr, in_tour_nr, null, 'WAE', 'LI', in_user_id, in_res_rueck);
  ...
  begin
    abbruch_transporte_359(in_sid, in_firma_nr, in_tour_nr, in_li_nr, in_li_pos_nr, null, 'WAE', 'LI', in_user_id, in_del_transp, in_res_rueck);
  ...
  --------------------------------------------------------------------------------
  --******************************************************************************
  procedure vorbereiten_trans_lief(in_sid         in isi_sid.sid%type,
                                    in_firma_nr    in isi_firma.firma_nr%type,
                                    in_tour_nr     in isi_order_kopf.vorgang_id%type,
                                    in_user_id     in isi_user.login_id%type
                                   ) is
      ...
      select pos.*
       ...
       where pos.sid = in_sid
         ...
         and (pos.vorgang_id =  in_tour_nr
           or pos.vorgang_id in (select t.vorgang_id
                                   from isi_order_kopf t
                                  where t.sid = in_sid
                                    and t.firma_nr = in_firma_nr
                                    and t.lkw_nr = (select decode(min(kopf.lkw_nr), max(kopf.lkw_nr), min(kopf.lkw_nr), NULL)
                                                      from isi_order_kopf kopf
                                                     where kopf.sid = in_sid
                                                       and kopf.firma_nr = in_firma_nr
                                                       and kopf.vorgang_id = in_tour_nr
                                                       and kopf.vorgang_typ in ('WAE', 'KWA', 'WAI', 'WUI')
                                                       and kopf.satzart  in ('LI', 'RL', 'LK', 'MAK', 'LAK', 'LNK'))
                                    and t.lkw_nr > 0
                                    and t.freigegeben_datum = v_freigabe_datum))
         and (pos.li_nr = in_li_nr or in_li_nr is NULL)
         and (pos.li_pos_nr = in_li_pos_nr or in_li_pos_nr is NULL)
    ...
    CURSOR c_transport is
      select *
       ...
       where tra.sid = in_sid
         ...
         and (tra.li_nr = in_li_nr or in_li_nr is NULL)
         and (tra.li_pos_nr = in_li_pos_nr or in_li_pos_nr is NULL)
  ...
  begin
    ...
    -- KONSI-Lager berücksichtigen
      update isi_order_kopf kopf
         set kopf.freigegeben_datum = v_freigabe_datum,
    ...
    LOOP
      ...
      if v_pos.arbeitsplatz_id is NULL
      ...
      then
        ...
        update isi_order_pos pos
           set pos.arbeitsplatz_id = v_pos.arbeitsplatz_id,
               pos.ziel = v_pos.ziel
           where pos.sid = v_pos.sid
             and pos.firma_nr = v_pos.firma_nr
             and pos.auf_id = v_pos.auf_id;
      ...
      if v_pos.satzart in ('MAK', 'LAK', 'LNK')
      ...
      if in_aktivieren = C.C_TRUE then
        update isi_order_pos pos
           set pos.freigegeben_datum = v_freigabe_datum,                    -- FreigabeDatum setzen
      ...
      if v_pos.menge_basis = c.BASIS_LTE
      then
        OPEN c_lte_auf_id;
      ...
      or nvl(v_menge, 0) != v_pos.soll_menge - v_pos.ist_menge
      then
        if (nvl(v_menge, 0) < v_pos.soll_menge - v_pos.ist_menge
        and nvl(v_pos.wa_menge_ueberlief, 'EX') != 'ULHM'
        and nvl(v_pos.wa_menge_ueberlief, 'EX') != 'ULTE')
        or (nvl(v_menge, 0) = 0)
        then
          if v_ret_text is NULL
          then
            v_ret_text :=  LC.ec_p5(LC.O_TP5_BESTANDS_MG_FEHLT_L1, to_char(v_pos.li_nr), to_char(v_pos.li_pos_nr), v_pos.soll_menge, v_pos.mengeneinheit, v_menge) || CHR(13) || CHR(10)|| to_char(v_pos.li_nr)
                                       || '/' || to_char(v_pos.li_pos_nr)
                                       || ' ' || v_pos.soll_menge
                                       || ' ' || v_pos.mengeneinheit
                                       || ' ' || v_menge
                                       || ' -> ' || to_char(v_menge - v_pos.soll_menge) || CHR(13) || CHR(10);
          else
            v_ret_text := v_ret_text || to_char(v_pos.li_nr)
                                       || '/' || to_char(v_pos.li_pos_nr)
                                       || ' ' || v_pos.soll_menge
                                       || ' ' || v_pos.mengeneinheit
                                       || ' ' || v_menge
        ...
        elsif nvl(v_menge, 0) > v_pos.soll_menge - v_pos.ist_menge
        and nvl(v_pos.wa_menge_ueberlief, 'EX') != 'OLHM'
        and nvl(v_pos.wa_menge_ueberlief, 'EX') != 'OLTE'
        then
          if v_ret_text is NULL
          then
            v_ret_text :=  LC.ec_p5(LC.O_TP5_BESTANDS_MG_Z_VIEL_L1, to_char(v_pos.li_nr), to_char(v_pos.li_pos_nr), v_pos.soll_menge, v_pos.mengeneinheit, v_menge) || CHR(13) || CHR(10)|| to_char(v_pos.li_nr)
                                       || '/' || to_char(v_pos.li_pos_nr)
                                       || ' ' || v_pos.soll_menge
                                       || ' ' || v_pos.mengeneinheit
                                       || ' ' || v_menge
  ...
  begin
    ...
    LOOP
      ...
      if v_order_pos.freigegeben_datum is not NULL then
        ...
        LOOP
          ...
          v_result := lvs_ausl.lvs_lte_res_rueck (v_order_pos.sid,
                                                  v_order_pos.firma_nr,
    ...
    delete from isi_order_pos p
      where p.sid = in_sid
        ...
        and p.satzart  in ('LI', 'RL', 'LK', 'MAK', 'LAK', 'LNK');


    -- Jetzt prüfen ob noch POSITIONEN
    v_found := False;
    if in_lief_pos is not NULL
    then
      v_pos_nr := NULL;
      OPEN c_order_pos;
      FETCH c_order_pos into v_order_pos;
  ...
  begin
    ...
    RES_LIEF_POS(in_sid, in_firma_nr, in_lief_nr, in_lief_pos, in_user_id, in_typ, v_satzart, in_tour_nr);
    ...
    if nvl(v_menge, 0) < v_pos.soll_menge - v_pos.ist_menge then
      v_err_nr := 20;
      v_err_text := LC.ec_p5(LC.O_TP5_BESTANDS_MG_FEHLT_L1, to_char(in_lief_nr), to_char(in_lief_pos), v_pos.soll_menge, v_pos.mengeneinheit, v_menge) || CHR(13) || CHR(10);
      ...
      v_err_nr := 30;
      v_err_text := LC.ec_p5(LC.O_TP5_BESTANDS_MG_Z_VIEL_L1, to_char(in_lief_nr), to_char(in_lief_pos), v_pos.soll_menge, v_pos.mengeneinheit, v_menge) || CHR(13) || CHR(10);
  ...
  begin
    ...
    v_result := lvs_ausl.lvs_pruefe_lief_pos(in_sid, in_firma_nr, in_lief_nr, in_lief_pos, v_kopf, in_typ, in_satzart, in_tour_nr);
    ...
    select sum(nvl(l.res_menge, 0)) into v_lte_menge
      from lvs_lam l  
     where l.order_pos_auf_id = v_pos.auf_id;

    -- -AG- 20250516 - Bei Unterlieferung und Reservierung < SOLL, dann SOLL = Reservierte Menge
    if v_pos.wa_menge_ueberlief in ('ULTE', 'ULHM')
    and v_lte_menge < v_pos.soll_menge
    and isi_allg.get_firma_cfg_param(v_pos.sid,
                                     v_pos.firma_nr,
                                     'AUSL_RESERVIEREN',       -- in_kategorie             in isi_firma_cfg.kategorie%type,
  ...
  procedure res_rueck_order_pos(in_sid         in isi_sid.sid%type,
    ...
    cursor c_pos is
      select pos.*
       ...
       where pos.sid = in_sid
         and pos.firma_nr = in_firma_nr
         and (pos.vorgang_typ = in_vorgang_typ or in_vorgang_typ is null)
         and (pos.satzart = in_satzart or in_satzart is null)
         and pos.vorgang_id = in_vorgang_id
    ...
    cursor c_lam is
      select lam.lte_id,
       ...
       group by lam.lte_id, lam.lgr_platz;
  ...
  begin
    ...
    OPEN c_lam;
    LOOP
      FETCH c_lam into v_lam_lte_id, v_lam_lgr_platz;
    ...
    if v_pos.vorgang_typ in ('WAE', 'KWA', 'WAI', 'WUI') and v_pos.satzart  in ('LI', 'RL', 'LK', 'MAK', 'LAK', 'LNK')
    then
      update isi_order_kopf ok
         set ok.status = 'N'                  -- Neu ist jetzt 'N' nicht mehr NULL
       where ok.sid = v_pos.sid
         ...
         and ok.status != 'E'
         and 0 = (Select count(*) from isi_order_pos op
                   where op.vorgang_id = v_pos.Vorgang_Id
                     and ok.vorgang_typ = v_pos.vorgang_typ
                     and ok.satzart = v_pos.satzart
                     and (op.status <> 'N' and op.status <> 'E'));
    end if;

    if v_pos.vorgang_typ = 'WAI' and v_pos.satzart = 'MA'
    then
      update isi_order_kopf ok
         set ok.status = 'N'                  -- Neu ist jetzt 'N' nicht mehr NULL
       where ok.sid = v_pos.sid
  ...
  begin
    res_rueck_order_pos(in_sid, in_firma_nr, in_user_id, in_vorgang_id, in_auf_id, in_vorgang_typ, in_satzart);
    commit;
  end;
  ...
  begin
    ...
    fetch c_order_lief_pos into v_vorgang_id, v_auf_id, v_satzart;
    ...
    res_rueck_order_pos(in_sid, in_firma_nr, in_user_id, v_vorgang_id, v_auf_id, in_typ, v_satzart);
  exception
  ...
  begin
    ...
    RES_RUECK_LIEF_POS(in_sid, in_firma_nr, in_lief_nr, in_lief_pos, in_user_id, in_typ, in_tour_nr);
    commit;
  exception
    -- Im Fehlerfall is der Fehler bereits gesetzt, als lam_id wird 0 zurückgegeben.
    when v_error then
  ...
  begin
    ...
    LOOP
      ...
      v_result := lvs_ausl.lvs_lte_suche_buch_ausl(v_pos,
                                                   v_kopf,
                                                   v_lgr_orte,
                                                   in_user_id,
                                                   in_aktivieren);
      ...
      if nvl(v_menge, 0) < v_pos.soll_menge - v_pos.ist_menge then
        if v_ret_text is NULL
        then
          v_ret_text :=  LC.ec_p5(LC.O_TP5_BESTANDS_UL_FEHLT_L1, to_char(v_pos.li_nr), to_char(v_pos.li_pos_nr), v_pos.soll_menge, v_pos.mengeneinheit, v_menge) || CHR(13) || CHR(10)|| to_char(v_pos.li_nr)
                                     || '/' || to_char(v_pos.li_pos_nr)
                                     || ' ' || v_pos.soll_menge
                                     || ' ' || v_pos.mengeneinheit
                                     || ' ' || v_menge
        ...
        if v_ret_text is NULL
        then
          v_ret_text :=  LC.ec_p5(LC.O_TP5_BESTANDS_UL_Z_VIEL_L1, to_char(v_pos.li_nr), to_char(v_pos.li_pos_nr), v_pos.soll_menge, v_pos.mengeneinheit, v_menge) || CHR(13) || CHR(10)|| to_char(v_pos.li_nr)
                                     || '/' || to_char(v_pos.li_pos_nr)
                                     || ' ' || v_pos.soll_menge
                                     || ' ' || v_pos.mengeneinheit
                                     || ' ' || v_menge
  ...
  begin
    ...
    delete from isi_order_pos p
      where p.sid = in_sid
        and p.firma_nr = in_firma_nr
        and (p.vorgang_typ = in_vorgang_typ or in_vorgang_typ is null)
    ...
    delete from isi_order_kopf k
      where k.sid = in_sid
        and k.firma_nr = in_firma_nr
        and (k.vorgang_typ = in_vorgang_typ or in_vorgang_typ is null)
  ...
  function is_vorbestellung(in_sid in isi_order_pos.sid%type,
                            in_auf_id in isi_order_pos.auf_id%type) return varchar2 is
    ...
    cursor c_lam_menge is
      select nvl(sum(lam.menge), 0)
       ...
       where lam.sid = v_order_pos.sid
         ...
         and nvl(lam.zeichnung_index, 'Keine_') = nvl(v_order_pos.zeichnung_index, nvl(lam.zeichnung_index, 'Keine_'))
         and (lam.leitzahl = v_order_pos.leitzahl or v_order_pos.leitzahl is null)
         and (lam.fa_ag = v_order_pos.fa_ag or v_order_pos.fa_ag is null)
         and lam.lgr_platz is not null
         and lam.order_pos_auf_id is null
         and lam.labor_status = 'F'
         and lam.lte_id = (select t2.lte_id
                             from lvs_lte t2
                            where t2.lte_id = lam.lte_id
  ...
  begin
    ...
    open c_order_pos;
    fetch c_order_pos into v_order_pos;
    if c_order_pos%found
    then
      open c_lam_menge;
      fetch c_lam_menge into v_lam_menge;
      if c_lam_menge%found
      then
        if v_lam_menge = 0
        then
          v_ret_val := c.c_true;
  ...
  function get_liefer_lgr_orte_fifo(in_sid in isi_order_pos.sid%type,
                                    in_auf_id in isi_order_pos.auf_id%type,
                                    in_adress_id in lvs_lgr_ort.adress_id%type) return varchar2 is
    ...
    cursor c_lam_menge is
      select stradd_distinct(to_char(x.lgr_ort)) lgr_orte
        from (select lte.lgr_ort
                from lvs_lam lam,
                     lvs_lte lte,
                     lvs_lgr lgr,
                     lvs_lgr_ort lo
               where lam.sid = v_order_pos.sid
                 ...
                 and (lo.adress_id = in_adress_id or in_adress_id is null)
               order by lam.prod_datum, lo.lgr_ort
             ) x;
   begin
    v_ret_val := null;
    ...
    then
      ...
      if c_lam_menge%found and v_ret_val is not null
      then
        v_ret_val := v_ret_val || ';';
        null;
      end if;
  ...
  function c_order_v_delete(in_von_datum in varchar2,
    ...
    cursor c_pos is
      select op.*
        from isi_order_pos op
       where op.order_datum between to_date(nvl(in_von_datum, '01.01.1900') || ' 00:00:00','dd.mm.yyyy hh24:mi:ss')
                                 and to_date(in_bis_datum || ' 23:59:59','dd.mm.yyyy hh24:mi:ss')
         and (op.login_id = in_login_id or in_login_id is null)
         and op.vorgang_typ = nvl(in_vorgang_typ, 'WAI')
         and op.status = 'N'
         -- pruefen, wieviel auf Lager ist
         and (select nvl(sum(lam.menge), 0)
                from lvs_lam lam
               where lam.sid = op.sid
                 and lam.artikel_id = op.artikel_id
                 and (lam.zeichnung_index = op.zeichnung_index or op.zeichnung_index is null)
                 and (lam.leitzahl = op.leitzahl or op.leitzahl is null)
                 and (lam.fa_ag = op.fa_ag or op.fa_ag is null)
                 and lam.lgr_platz is not NULL
                 and lam.order_pos_auf_id is null
                 and lam.labor_status = 'F'
                 and lam.lte_id = (select t2.lte_id
  ...
  begin
    for v_pos in c_pos loop
      ...
      select count(*) into v_rec_count                        -- im Kopf vorhanden Positionen zählen
        from isi_order_pos op                                 -- 99 als default, wenn Vorgang_ID nicht da, dann nicht löschen
       where op.sid = in_sid
         and op.firma_nr = in_firma_nr
         and (op.vorgang_typ = in_vorgang_typ or in_vorgang_typ is null)
         and (op.satzart = in_satzart or in_satzart is null)
...
--------------------------------------------------------------------------------
  function TOUR_SUCHE_UM_PLATZ (in_lte              in  lvs_lte%ROWTYPE,
    ...
    CURSOR c_um_lgr is
      select lgr.*
       ...
       where lgr.sid                 = in_lvs_akt_lgr.sid
         ...
         -- Ticket: E70397-623
         and instr(nvl(lgr.lte_namen, v_basis_lte_name), v_basis_lte_name) > 0
         and (nvl(lgr.res_string, to_char(in_tranport_gruppe)) = to_char(in_tranport_gruppe)
             or  lgr.lgr_ort          = in_lvs_akt_lgr.lgr_ort)
         and in_lvs_akt_lgr.sid = qlo.sid
         and in_lvs_akt_lgr.firma_nr = qlo.firma_nr
         and in_lvs_akt_lgr.lgr_ort = qlo.lgr_ort
         and (qlo.verl_vorb_lgr_orte like ('%;' || to_char(lgr.lgr_ort) || ';%')
    ...
    CURSOR c_fahrzeuge is
      select f.res_id
        from lvs_fahrzeuge f,
             lvs_lgr_grp_fahrzeug gf
       where gf.lgr_ort = out_lvs_lgr.lgr_ort
  ...
  begin
      ...
      if isi_allg.get_firma_cfg_param(in_lte.sid,
                                in_lte.firma_nr,
                                'AUSL_VORBER',            -- in_kategorie             in isi_firma_cfg.kategorie%type,
    ...
    LOOP
     ...
     then
        ...
        if v_lgr_platz_gruppe = '1-38250.4750'
        then
          NULL;
        end if;
        ...
        if v_found then
          ...
          CLOSE c_fahrzeuge;
          if out_fahrzeuge_IDs is NOT NULL
          then
            out_fahrzeuge_IDs := out_fahrzeuge_IDs || ';';
          end if;

          v_err_text := lvs_platz.lvs_platz_einl_pruef_err_t_R30(in_lte,
                                                                 v_basis_lte_name,
...
--------------------------------------------------------------------------------
  procedure TOUR_VORB_UML (in_lte              in lvs_lte%ROWTYPE,
  ...
  BEGIN
    ...
    or lvs_ausl.v_transport_gruppe is NULL
    then
      if isi_allg.get_firma_cfg_param(in_lte.sid,
                                in_lte.firma_nr,
                                'AUSL_STRAT',             -- in_kategorie             in isi_firma_cfg.kategorie%type,
      ...
      then
       if lvs_ausl.v_order_transport_gruppe is NOT NULL
        then
      ...
      lvs_ausl.v_transport_gruppe := NULL;
      select max(lte.transport_gruppe) into lvs_ausl.v_transport_gruppe
        from lvs_lte lte
       where lte.order_auf_id in (select p.auf_id
                                    from isi_order_pos p
                                   where p.vorgang_id = (select p2.vorgang_id from isi_order_pos p2 where p2.auf_id = in_order_pos.auf_id)
                                     and p.transport_gruppe = (select p2.transport_gruppe from isi_order_pos p2 where p2.auf_id = in_order_pos.auf_id))
         and lte.transport_gruppe is not NULL;
      if nvl(lvs_ausl.v_transport_gruppe, 0) = 0 -- Noch keine LTE Vorbereitet
      then
        select seq_transport_gruppe.nextval into lvs_ausl.v_transport_gruppe from dual; -- Dann neue Nummer holen
      end if;
      dbms_output.put_line('auf_id=' || to_char(in_order_pos.auf_id) || ' Tgpr=' || to_char(lvs_ausl.v_transport_gruppe));
      lvs_ausl.v_order_transport_gruppe := in_order_pos.transport_gruppe;
      if lvs_ausl.v_sperr_transport_gruppe
      then
        update lvs_lgr t
           set t.res_string = to_char(lvs_ausl.v_transport_gruppe)
         where t.res_string = to_char(v_last_transp_grp)
           and t.lgr_akt_te < t.lgr_max_te;
      end if;
    end if;

    if to_char(lvs_ausl.v_transport_gruppe) = v_quelle_lvs_lgr_rec.res_string
    then
      return;
    end if;

    v_err_text := tour_suche_um_platz (in_lte,  v_quelle_lvs_lgr_rec, in_lgr_platz_grp, lvs_ausl.v_transport_gruppe, v_ziel_lvs_lgr_rec, v_fahrzeuge_IDs);

    if v_err_text is NULL
    and v_ziel_lvs_lgr_rec.lgr_platz is NULL
    then
      if lvs_p_base.get_lgr_ort(v_quelle_lvs_lgr_rec.sid,
                                v_quelle_lvs_lgr_rec.firma_nr,
                                v_quelle_lvs_lgr_rec.lgr_ort,
                                v_lgr_ort)
    ...
    or v_ziel_lvs_lgr_rec.lgr_platz is NULL
    then
      v_err_nr := 10;
      v_err_text := LC.ec(LC.O_TXT_VORB_FREI_PLAETZE_N_AUSR);
    ...
    v_transp_lte := lvs_transport.lvs_transp_lte (
     in_lte.sid,                     -- in_sid                  IN isi_sid.sid%TYPE,
  ...
  procedure TOUR_VORB_TRANS(in_sid         in isi_sid.sid%type,
    ...
    CURSOR c_lgr_ort is                             -- Lesen des Lagerplatz
     select *
       from lvs_lgr_ort ort
      where ort.lgr_ort = v_lte.lgr_ort
        and ort.sid = in_sid;
    ...
    CURSOR c_lam_auf_id is
      select lam.lte_id
       ...
       where lam.sid = v_pos.sid
         ...
         and nvl(to_char(lte.transport_gruppe), lte.res_string || 'X') != nvl(lgr.res_string, 'xx') -- Wenn Transportgruppe = Res-Struíng dann bereits vortereitet
       group by lam.lte_id,
       ...
       order by decode (lgr.lgr_typ,
                        c.STAP_FLAE1, nvl(min(lte.lte_offset_z * -1),0),
                        c.STAP_FLAE2, nvl(min(lte.lte_offset_z * -1),0),
                        0),
    ...
    LOOP
      ...
      LOOP
        ...
        then
          ...
          then
            tour_vorb_uml(v_lte, 'ORD', v_Quell_lgr_ort.Lgr_Ort_Modul, 0, in_platz_grp, in_tour_nr, v_pos, in_user_id, in_lkw_nr);
          end if;
        else
          exit;
        end if;
  ...
  --------------------------------------------------------------------------------
  --******************************************************************************
  function tour_tour_vl(in_sid         in isi_sid.sid%type,
                        in_firma_nr    in isi_firma.firma_nr%type,
                        in_tour_nr     in isi_order_kopf.vorgang_id%type,
    ...
    CURSOR c_order_lte_ges_anz is
      select count(lte.order_vorgang_id)
        from lvs_lte lte,
             lvs_lte_cfg cfg,
             lvs_lgr lgr
       where lte.sid = in_sid
         ...
         and lte.lgr_platz = lgr.lgr_platz
         and nvl(to_char(lte.transport_gruppe), lte.res_string || 'X') != nvl(lgr.res_string, 'xx') -- Wenn Transportgruppe = Res-Struíng dann bereits vortereitet
       group by lte.order_vorgang_id;

    CURSOR c_order_lte_anz is
      select count(lte.order_vorgang_id),
             nvl(cfg.basis_lte_name, lte.lte_name) lte_name,
             decode (max(lte.lgr_ort),min(lte.lgr_ort),max(lte.lgr_ort),NULL) lgr_ort
        from lvs_lte lte,
             lvs_lte_cfg cfg,
             lvs_lgr lgr
       where lte.sid = in_sid
       ...
       group by lte.order_vorgang_id, nvl(cfg.basis_lte_name, lte.lte_name);

    CURSOR c_vorb_lgr_plaetze is
      select gb.*
        from (select t.lgr_ort,
                     t.lgr_platz_gruppe,
                     t.lgr_typ,
                     (abs(t.lgr_ort - in_lgr_wa.lgr_ort)  * 100000000 +
                      abs(trunc((t.lgr_dim_r + 1) / 2) - in_lgr_wa.lgr_dim_r) +

                      abs(t.lgr_dim_e - in_lgr_wa.lgr_dim_e) +
                      decode (t.lgr_typ,
                              c.STAP_FLAE1, case when abs(t.lgr_dim_p - in_lgr_wa.lgr_dim_p) >
                                                      abs(min(t.lgr_dim_t) - in_lgr_wa.lgr_dim_t)
                                                 then abs(t.lgr_dim_p - in_lgr_wa.lgr_dim_p)
                                                 else abs(min(t.lgr_dim_t) - in_lgr_wa.lgr_dim_t)
                                            end,
                              c.STAP_FLAE2, case when abs(t.lgr_dim_p - in_lgr_wa.lgr_dim_p) >
                                                      abs(min(t.lgr_dim_t) - in_lgr_wa.lgr_dim_t)
                                                 then abs(t.lgr_dim_p - in_lgr_wa.lgr_dim_p)
                                                 else abs(min(t.lgr_dim_t) - in_lgr_wa.lgr_dim_t)
                                            end,
                              abs(t.lgr_dim_p - in_lgr_wa.lgr_dim_p))) porder2,
                     min(t.lgr_verwendung) verwendung,
                     sum(t.lgr_akt_te) LTE,
                     sum(t.lgr_max_te) max_lte,
                     sum(t.lgr_einl_te_verfueg) lgr_einl_te_verfueg
                from lvs_lgr t
               where (t.lgr_verwendung = c.R_Lgr_Typ_Lager
                  or  t.lgr_verwendung = c.R_Lgr_Typ_Puffer)
                 and t.sid                 = in_sid
                 and t.firma_nr            = in_firma_nr
                 and nvl(v_lgr_platz_grp, '?')
                                           not like '%;' || t.lgr_platz_gruppe || ';%'
                 and nvl(v_lgr_platz_grp, '?')
                                           not like t.lgr_platz_gruppe || ';%'
                 and (v_lgr_orte like '%;' || to_char(t.lgr_ort) || ';%'
                      or v_lgr_orte like to_char(t.lgr_ort) || ';%'
                     )
                 and t.gesperrt            = c.R_LGR_GESPERRT_F
                 and t.akt_inventur_id    is NULL
                 and nvl(t.lte_namen, v_lte_name || ';') like '%' || v_lte_name || ';%'

              group by t.lgr_platz_gruppe, t.lgr_verwendung, t.lgr_typ, t.lgr_ort, t.lgr_dim_g, t.lgr_dim_r, t.lgr_dim_p, t.lgr_dim_e) gb,
         lvs_lgr lgr,
         lvs_lgr_ort ort
       where lgr.lgr_platz = gb.lgr_platz_gruppe
          and (
                 (
                    (
                       (
                          (gb.lte = 0
                            or lgr.res_string in (select to_char(lte.transport_gruppe)
                                                 from lvs_lte lte
                                                where lte.order_vorgang_id = in_tour_nr
                                                 and lte.transport_gruppe is not NULL)
                          )
        ...
        order by decode(ort.verl_vorb_lgr_orte, NULL, porder2, lte * - 1), gb.lgr_platz_gruppe, decode(ort.verl_vorb_lgr_orte, NULL, abs(lgr.lgr_dim_platz - in_lgr_wa.lgr_dim_platz), 0);
  ...
  begin
    ...
    if nvl(v_lte_anz, 0) = 0
    then
      return (v_lgr_platz_grp);
    end if;


    v_lgr_orte := get_lgr_orte_vorbereitung(in_sid,
                                            in_firma_nr,
                                            in_tour_nr,
                                            in_ziel);
    v_lte_anz := 0;
    ...
    LOOP
      FETCH c_order_lte_anz into v_lte_name_anz, v_lte_name, v_lgr_ort;
      ...
      LOOP
        ...
        FETCH c_vorb_lgr_plaetze into v_lgr.lgr_ort, v_lgr.lgr_platz_gruppe,
                                      v_lgr.lgr_typ, v_abstand_abs,
                                      v_lgr.lgr_verwendung, v_lgr.lgr_akt_te, v_lgr.lgr_max_te,
                                      v_lgr.lgr_einl_te_verfueg;
        EXIT when c_vorb_lgr_plaetze%NOTFOUND;

        v_freie_plaetze := nvl(v_freie_plaetze, 0) + v_lgr.lgr_max_te;
        v_lgr_platz_grp := nvl(v_lgr_platz_grp, '') || v_lgr.lgr_platz_gruppe || ';';
    ...
    if v_freie_plaetze < v_lte_name_anz
    then
      v_err_nr := 30;
      v_err_text := LC.ec(LC.O_TXT_FREI_PLAETZE_N_AUSR);
      -- v_err_text := 'Fehler: Nicht genügend freie Lagerplätze. ';
    ...
    LOOP
      ...
      LOOP
        ...
        if v_freie_plaetze_typ = 0
        then
          if v_lte_typen is NULL
          then
            v_lte_typen := v_lte_name;
          else
            v_lte_typen := v_lte_typen || '; ' || v_lte_name;
          end if;
        end if;
      ...
      then
        v_err_nr := 10;
        v_err_text := LC.ec_p2(LC.O_TP2_LGR_LTE_TPY_ART_ERR, nvl(v_lte_typen, '????'),  v_lte_name);
        -- v_err_text := 'Fehler: Es sind noch Transporte für diese Tour eingetragen, die nicht gelöscht werden konnten. Bitte Transporte prüfen und nochmals versuchen.';
        raise v_error;
        CLOSE c_order_lte_anz;
      end if;
  ...
  begin
    ...
    v_lgr_platz_grp :=  tour_tour_vl(in_sid,
                                     in_firma_nr,
                                     in_tour_nr,
                                     v_order_kopf.ziel,
                                     v_lgr_wa
  ...
  procedure C_CHECK_LIEF_FUER_LIEF_ENDE(in_sid         in isi_sid.sid%type,
    ...
    cursor c_order_kopf is
     select min(k.vorgang_typ), k.vorgang_id, k.satzart
       from isi_order_kopf k
      where k.sid = in_sid
        and k.firma_nr = in_firma_nr
        and (k.vorgang_id = in_tour_nr
         or (in_tour_nr is NULL
            and k.li_nr = in_lief_nr))
        and k.satzart  in ('LI', 'RL', 'LK', 'MAK', 'LAK', 'LNK')
     group by k.vorgang_id, k.satzart;
  ...
  begin
    begin
      ...
      FETCH c_order_kopf
        into v_order_vorgang_typ, v_order_vorgang_id, v_order_satzart;
      CLOSE c_order_kopf;
      ...
      commit;
    exception
      when others then NULL; -- Order konnte nicht geschlossen werden, kein Fehler melden
    end;
 ...
 --******************************************************************************
  ...
  begin
    ...
    LOOP
      ...
      update isi_order_pos p
         set p.status = v_order_pos.status
        where p.sid = in_sid
          and p.firma_nr = in_firma_nr
  ...
  procedure LTE_LIEF_ENDE(in_out_order_pos   in out isi_order_pos%rowtype,
    ...
    CURSOR c_lte is
      select *
        from lvs_lte lte
       where lte.sid = in_out_order_pos.sid
         and lte.firma_nr = in_out_order_pos.firma_nr
         and lte.order_auf_id = in_out_order_pos.auf_id;

    CURSOR c_liefs is
      select *
        from isi_liefs liefs
       where liefs.sid = in_out_order_pos.sid
  ...
  begin
      ...
      then
        ...
        then
          ...
          then
            ...
            v_lam.lam_id := lvs_ausl.lvs_lam_abgang (in_out_order_pos.sid, in_out_order_pos.firma_nr, v_lam.artikel_id,
                                                     v_lte.lte_id, NULL, in_out_order_pos.auftrag,
                                                     NULL, sysdate, in_user_id, NULL, NULL, NULL, NULL, NULL, NULL, c.LAM_BH_BUS_ABG,
                                                     in_out_order_pos.vorgang_id,
                                                     in_out_order_pos.li_nr,
                                                     in_out_order_pos.li_pos_nr);
            if v_lam.lam_id <= 0
            then
              v_err_nr := 10;
              v_err_text := LC.ec_P1(LC.O_TP1_LTE_ID_GESP_WARE_AUSL, nvl(v_lte.lte_id, 'NULL'));
        ...
        elsif v_lte.lte_status = c.LTE_AD_STAT
        then
          CLOSE c_lte;
          v_err_nr := 20;
          ...
          v_result := lvs_ausl.lvs_lte_res_rueck(in_out_order_pos.sid,
                                                 in_out_order_pos.firma_nr,
                                                 in_out_order_pos.vorgang_id,
                                                 in_out_order_pos.auf_id,
                                                 v_lte.lte_id,
                                                 in_out_order_pos.vorgang_id,
                                                 v_lte.lgr_platz,
                                                 c.c_true);
    ...
    if in_out_order_pos.li_extern = 'B'
    then
      ...
      select *
       ...
       where pos.sid = in_sid
         ...
         and ( (    pos.satzart = 'BE'
              ...
              or
               (    pos.satzart = 'BK'                  -- KONSI-Bestellung
  ...
  begin
    ...
    LOOP
      ...
      if v_lam.order_pos_auf_id is NULL
      then
        v_err_nr := 10;
  ...
  begin
    ...
    then
      ...
      if out_vorgang_id is not null
      then

        begin
          insert into isi_order_tour
          select out_vorgang_id,
                 t.tour,
  ...
  begin
    ...
    then
      v_err_nr := 10;
      v_err_text := LC.ec_p1(LC.O_TP1_LTE_ID_FEHLT, nvl(to_char(in_transp_id), '(NULL)'));
    ...
    loop
      fetch c_transport_mengen into v_order_auf_id, v_transp_menge, v_transp_z_index;
      ...
      then
        v_err_nr := 10;
        v_err_text := LC.ec_p1(LC.O_TP1_ORDER_AUF_ID_FEHLT, nvl(to_char(v_transport.auf_id), '(NULL)'));
      ...
      then
        ...
        v_order.lvs_info := nvl(v_transp_z_index, '0000') || ', ' || nvl(to_char(v_transport.lgr_ort_quelle), '--');

        update isi_order_pos t
           set t.lvs_info = v_order.lvs_info
         where t.sid = v_order.sid
           and t.firma_nr = v_order.firma_nr
           and t.auf_id = v_order.auf_id;
        ...
        update isi_order_kopf ok
           set ok.status = v_order.status,
               ok.freigegeben_datum = v_order.freigegeben_datum
         where ok.sid = v_order.sid
  ...
  procedure rest_mg_wai_ma_pruefen(in_sid             in isi_sid.sid%type,
                                   in_firma_nr        in isi_firma.firma_nr%type,
                                   in_auf_id          in isi_order_pos.auf_id%type) is
    -------------------------------------------------------------------------------------------------------
    -- Standard Fehler Felder für Exception
    -------------------------------------------------------------------------------------------------------
    v_error     exception;
  ...
  begin
    ...
    then
      v_err_nr := 10;
      v_err_text := LC.ec_p1(LC.O_TP1_ORDER_AUF_ID_FEHLT, nvl(to_char(in_auf_id), '(NULL)'));
  ...
  procedure rest_mg_wai_ma_pruefen_rt(in_sid             in isi_sid.sid%type,
                                      in_firma_nr        in isi_firma.firma_nr%type,
                                      in_order_pos       in isi_order_pos%rowtype) is
    -------------------------------------------------------------------------------------------------------
    -- Standard Fehler Felder für Exception
    -------------------------------------------------------------------------------------------------------
    v_error     exception;
    ...
    cursor c_order_restmenge is
      select t.auf_id
        from isi_order_pos t
       where t.vorgang_id = in_order_pos.vorgang_id
         and t.status = 'N'
  ...
  begin
    ...
    if c_anz_auslager_dispos%notfound
    then
      v_anz_ausl_dispos := 0;
    end if;
    ...
    if c_transport_mengen%notfound
    then
      v_sum_transp_menge := 0;
    end if;
    close c_transport_mengen;

    if (nvl(v_anz_ausl_dispos, 0) = 0)
    ...
    then
      ...
      if in_order_pos.status in ('T', 'A', 'F', 'D', 'R')
      then
        update isi_order_pos t
           set t.status = 'N',
    ...
    elsif (in_order_pos.ist_menge + v_sum_transp_menge) < in_order_pos.soll_menge
    ...
    then
      ...
      then
        ...
        then
          ...
          if not lvs_p_base.lagerbestand_vorhanden(in_order_pos.sid,
                                                   in_order_pos.firma_nr,
                                                   in_order_pos.artikel_id,
                                                   in_order_pos.leitzahl,
                                                   in_order_pos.fa_ag,
                                                   in_order_pos.zeichnung_index,
                                                   null)
          then
          ...
          update isi_order_pos op
             set op.soll_menge = (op.ist_menge + v_sum_transp_menge)
           where op.sid = in_order_pos.sid
  ...
  procedure man_lvs_wai_ma_order(io_order_pos in out isi_order_pos%rowtype) is
    v_artikel isi_artikel%rowtype;

    cursor c_artikel is
      select a.*
        from isi_artikel a
       where a.sid = io_order_pos.sid
  ...
  begin
    ...
    values
      ...
      ) returning vorgang_id, li_nr into io_order_pos.vorgang_id, io_order_pos.li_nr;

      insert into isi_order_pos
      values io_order_pos
      returning auf_id into io_order_pos.auf_id;
  end;
  function get_lgr_orte_vorbereitung (in_sid            in isi_sid.sid%type,
    ...
    CURSOR c_lvs_lgr_ort_ue_platz is
      select *
        from lvs_lgr_ort_ue_platz t
       where t.sid = in_sid
         and t.firma_nr = in_firma_nr
         and t.lgr_ort_quelle = v_akt_ort
         and t.lgr_ort_ziel = v_z_platz.lgr_ort
         and  (t.lte_name = v_lte_name
            or (v_lte_name is NULL and t.lte_name is NULL)
            or t.lte_name is NULL)
      order by nvl(t.lte_name, v_lte_name || 'X');

    CURSOR c_order_lte_orte is
      select lo.verl_vorb_lgr_orte,
             lo.lgr_ort,
             decode (min(lte.lte_name), max(lte.lte_name), min(lte.lte_name), NULL)
        from lvs_lte lte,
             lvs_lgr_ort lo
       where lte.sid = in_sid
       ...
       group by lo.lgr_ort, lo.verl_vorb_lgr_orte;

  begin
    if not lvs_p_base.get_lgr_platz(in_transport_ziel, v_z_platz)
    then
      v_err_nr := 5;
    ...
    LOOP
      FETCH c_order_lte_orte into v_akt_orte, v_q_ort, v_lte_name;
      EXIT when c_order_lte_orte%NOTFOUND;


      v_q_orte := nvl(v_q_orte, '') || to_char(v_q_ort) || ';';
      if v_akt_orte is NULL
      then
        v_akt_orte := v_q_orte;
      end if;
      ...
      LOOP
        ...
        then
          ...
          if (v_ende_pos>0)
          then
            if v_last_orte not like '%;' || substr(v_akt_orte, v_start_pos, v_ende_pos-v_start_pos) || ';%'
            or v_last_orte not like substr(v_akt_orte, v_start_pos, v_ende_pos-v_start_pos) || ';%'
            or v_last_orte is NULL
            then
              if v_neue_orte is NULL
              then
                v_neue_orte := substr(v_akt_orte, v_start_pos, v_ende_pos-v_start_pos) || ';';
              else
                v_neue_orte := v_neue_orte || substr(v_akt_orte, v_start_pos, v_ende_pos-v_start_pos) || ';';
              end if;
            end if;
          end if;
        end if;
    ...
    if v_last_orte is NULL
    then
      v_err_nr := 10;
      v_err_text := LC.ec_p2(LC.O_TP2_WEG_VON_NACH_FALSCH, nvl(to_char(v_ort_o_weg), nvl(v_q_orte, '??')), in_transport_ziel);
      raise_application_error(-20000 - v_err_nr, v_err_text);
    end if;
    return (v_last_orte);
  ...
  */
  ...
  function gen_trans_check_order(in_sid             in isi_sid.sid%type,
                                 ...
                                 in_auf_id          in isi_order_pos.auf_id%type) return boolean is
  ...
  cursor c_get_transp_gruppe is
    select max(lte.transport_gruppe) into lvs_ausl.v_transport_gruppe
     ...
     where lte.order_auf_id in (select p.auf_id
                                  from isi_order_pos p
                                 where p.vorgang_id = (select p2.vorgang_id from isi_order_pos p2 where p2.auf_id = v_order_pos.auf_id)
                                   and p.transport_gruppe = (select p2.transport_gruppe from isi_order_pos p2 where p2.auf_id = v_order_pos.auf_id))
       and lte.transport_gruppe is not NULL;

  cursor c_get_lvs_lam is
    select lam.*
      from lvs_lam lam
     where lam.sid = v_lvs_lte.sid
       and lam.firma_nr = v_lvs_lte.firma_nr
       and lam.lam_id = (select max(x.lam_id)
                           from lvs_lam x
                          where x.sid = v_lvs_lte.sid
  ...
  cursor c_get_order_pos_by_fag is
    select opos.*
      ...
      join isi_order_pos opos on opos.auftrag = fag.kunden_ab and
                                 opos.pos_nr = fag.kunden_ab_pos
     where fag.sid = in_sid
       and fag.firma_nr = in_firma_nr
       and fag.kenz_letzt_ag = 1
       and fag.leitzahl = (select x.leitzahl
                             from bde_fa_auftrag x
                             join lvs_lam lam on lam.leitzahl = x.leitzahl
                            where lam.lam_id = v_lvs_lam.lam_id
                              and x.kenz_letzt_ag = 1);

  cursor c_get_order_kopf is
     select okop.*
       from isi_order_kopf okop
      where okop.sid = v_order_pos.sid
  ...
  begin
    ...
    then
      v_err_nr := 11;
      v_err_text := 'Quell Lagerplatz ' || in_quell_lgr_platz || ' umbekannt !';
      raise v_error;
    end if;
    ...
    then
      v_err_nr := 13;
      v_err_text := 'LAM fuer Lte Id ' || v_lvs_lte.lte_id || 'umbekannt';
      raise v_error;
    end if;
    -- Lte auf die Quelle umbuchen
   if (v_lvs_lte.lgr_platz is null) or
      (v_lvs_lte.lgr_platz <> v_lgr_quelle.lgr_platz)
   then
     lvs_transport.lvs_lte_transport(in_lte_id => in_lte_id,
   ...
   then
     ...
     if (v_found)
     then
       if (v_order_pos.quell_lagerorte is not null)
       then
         ...
         update isi_order_kopf okop
            set okop.quell_lagerorte = v_lgr_einl_ziele
          where okop.sid = v_order_kopf.sid
            and okop.firma_nr = v_order_kopf.firma_nr
            and okop.vorgang_id = v_order_kopf.vorgang_id;

         update isi_order_pos opos
            set opos.quell_lagerorte = v_lgr_einl_ziele
          where opos.sid = v_order_kopf.sid
       ...
       if (v_order_kopf.status = 'N') or
          (v_order_kopf.status = 'D')
       then
       ...
       if (v_order_kopf.status = 'V') or
          (v_order_kopf.status = 'R')
       then
         ...
         if (v_found) or
            (v_lte_transp_gruppe is not null)
         then
           update lvs_lte lte
              set lte.transport_gruppe = v_lte_transp_gruppe
             where lte.lte_id = v_lvs_lte.lte_id;
             ...
             then
               v_err_nr := 21;
               v_err_text := 'Ziel Lagerplatz ' || in_quell_lgr_platz || ' umbekannt !';
               raise v_error;
             end if;
             ...
             if v_err_text is not NULL
             or v_lgr_ziel.lgr_platz is NULL
             then
                return (v_trans_created);
             end if;

             v_trans_typ := NULL;
             if v_order_pos.vorgang_typ like ('WE%')
             then
               v_trans_typ := 'E';
             elsif v_order_pos.vorgang_typ like ('WA%')
             then
               v_trans_typ := 'A';
             elsif v_order_pos.vorgang_typ like ('WU%')
             then
               v_trans_typ := 'U';
             end if;
