...
package body DIRKSPZM32.DW is
  ...
  function set_zins_proz(in_zins_proz in number) return number is
  begin
  ...
  procedure C_Take_LVS_Snapshot_LTE_Typ(in_sid        in dw_lvs_bestand.sid%type,
                                        ...
                                        in_wert_datum in dw_lvs_bestand.wert_datum%type default null) is
  begin
  ...
  procedure Take_LVS_Snapshot_LTE_Typ (in_sid        in dw_lvs_bestand.sid%type,
    ...
    cursor c_lte_typen is
      select l.lte_name,
       ...
       where l.sid = in_sid
         ...
         and (l.lgr_ort = in_lgr_ort or in_lgr_ort is null)
    ...
    cursor c_doppel_eintraege is
      select count(*)
        from dw_lvs_bestand t
       where t.wert_datum = in_wert_datum;
  begin
    isi_p_log.c_isi_system_meldung('01',
                                   1,
                                   'DB_HUF_JOBS',
                                   'ORA-DB',
                                   'dw.take_lvs_snapshot_lte_typ',
                                   NULL,
                                   NULL,
                                   NULL,
                                   NULL,
    ...
    then
      ...
      then
        isi_p_log.c_isi_system_meldung('01',
                                       ...
                                       NULL,
                                       NULL,
                                       'Es wurden Bereits Daten für den ' || in_wert_datum || ' erfasst.',
                                       'WL');
        return;
      end if;
    end if;
    ...
    begin
      open c_lte_typen;
        loop
          fetch c_lte_typen into v_lte_name,
                                 v_lgr_ort,
                                 v_lte_status,
                                 v_lte_basis_name,
                                 v_lte_anzahl;
          ...
          insert into dw_lvs_bestand
          values (
            in_sid,                           -- SID	VARCHAR2(2)	N			Datenbank für Konsolidierung
    ...
    cursor c_lam_mengen is
      select l.artikel_id,
       ...
       where l.sid = in_sid
         ...
         and (lg.lgr_ort = in_lgr_ort or in_lgr_ort is null)
       group by
             l.artikel_id,
             l.fa_ag,
             l.menge_basis,
  ...
  begin
    ...
    isi_p_log.c_isi_system_meldung('01',
                                   ...
                                   'LVS_DW_JOB',
                                   'ORA-DB',
                                   'dw.take_lvs_snapshot_lam_menge',
                                   NULL,
                                   NULL,
                                   NULL,
                                   NULL,
    ...
    begin
      open c_lam_mengen;
        loop
          fetch c_lam_mengen into     v_artikel_id,
                                      v_ag,
                                      v_menge_basis,
          ...
          values (
            ...
            in_firma_nr,                      -- FIRMA_NR	NUMBER(2)	N			Firmennummer in der Datenbank
            null,                             -- DW_STAT_ID	NUMBER	N			Laufende Nummer
            'DW_LAM_ART_MENGE_IN_LAGERORT_TAG_' ||
            v_mengeneinheit_basis,            -- STAT_NAME	VARCHAR2(200)	N			Name der Statistik z.B. HUF_DW_STATISITIK
  ...
  procedure C_Set_LVS_LAM_BH_AbgangTag(in_sid            in dw_lvs_bestand.sid%type,
                                       in_firma_nr       in dw_lvs_bestand.firma_nr%type,
                                       in_lgr_ort        in dw_lvs_bestand.lgr_ort%type,
                                       in_wert_datum     in dw_lvs_bestand.wert_datum%type) is
  ...
  begin
    isi_p_log.c_isi_system_meldung('01',
                                   ...
                                   NULL,
                                   NULL,
                                   'gestartet für ' || to_char(trunc(in_wert_datum), 'yyyy-mm-dd'),
                                   'IL');

    if in_wert_datum is NULL
    then
      isi_p_log.c_isi_system_meldung('01',
                                     1,
                                     'LVS_DW_JOB',
    ...
    begin
      open c_lam_bh;
        loop
          fetch c_lam_bh into     v_artikel_id,
                                      v_ag,
                                      v_menge_basis,
                                      v_mengeneinheit_basis,
                                      v_menge,
      ...
      open c_lam_bh_hist;
        loop
          fetch c_lam_bh_hist into    v_artikel_id,
                                      v_ag,
                                      v_menge_basis,
                                      v_mengeneinheit_basis,
                                      v_menge,
          ...
          then
            update dw_lvs_bestand  t
               set t.sum_menge = t.sum_menge + v_menge,
  ...
  procedure C_Set_LVS_LAM_BH_AbgTag_v_b(in_sid            in dw_lvs_bestand.sid%type,
                                        ...
                                        in_wert_datum_bis in dw_lvs_bestand.wert_datum%type) is
    v_datum date;
  begin
    -- Call the procedure
    v_datum := trunc(in_wert_datum_von);
    LOOP
      C_Set_LVS_LAM_BH_AbgangTag(in_sid,
                                 in_firma_nr,
                                 in_lgr_ort,
                                 v_datum);
