create or replace 
package body DIRKSPZM32.lvs_komm is

  function get_komm_anz_lam_fuer_lte(in_sid              in lvs_lam.sid%type,
  ...
  --------------------------------------------------------------------------------
  procedure lvs_c_komm_direct (in_sid           in isi_sid.sid%type,
                               ...
                               in_ziel_lte_id   in lvs_lte.lte_id%type) is

    v_komm_neu_lam_id lvs_lam.lam_id%type;
    v_komm_neu_lhm_id lvs_lhm.lhm_id%type;
  begin
    lvs_komm_direct(in_sid, in_firma_nr, in_res_id, in_user_id, in_quelle_lte_id, in_lam_id,
                    in_menge, in_ziel_lte_id, v_komm_neu_lam_id, v_komm_neu_lhm_id);
    commit;
  end;

  procedure lvs_komm_direct (in_sid           in isi_sid.sid%type,
                             in_firma_nr      in isi_firma.firma_nr%type,
                             in_res_id        in isi_resource.res_id%type,
                             in_user_id       in isi_user.login_id%type,
                             in_quelle_lte_id in lvs_lte.lte_id%type,
                             ...
                             out_neu_lhm_id   out lvs_lhm.lhm_id%type) is
  begin
  ...
  procedure lvs_komm_direct_R359 (in_sid           in isi_sid.sid%type,
                                   in_firma_nr      in isi_firma.firma_nr%type,
                                   in_res_id        in isi_resource.res_id%type,
                                   in_user_id       in isi_user.login_id%type,
                                   in_quelle_lte_id in lvs_lte.lte_id%type,
    ...
    CURSOR c_lam_res is
      select count(lam.lte_id)
        from lvs_lam lam
       where lam.lte_id = in_quelle_lte_id
    ...
    CURSOR c_lam_res_ziel is
      select count(lam.lte_id)
        from lvs_lam lam
       where lam.lte_id = v_ziel_lte_id
         and lam.order_pos_auf_id is not NULL
       group by lam.lte_id;
  ...
  begin
    ...
    if in_ziel_lte_id = c.LTE_KOMM_GLEICH_LTE
    then
    ...
    if v_quelle_lte.lgr_platz is NULL
    then
      v_err_nr := 10;
    ...
    if v_ziel_lte.lgr_platz is NULL
    then
      v_err_nr := 25;
    ...
    if v_ziel_lte.res_artikel_id <> c.MISCHPAL
    and v_ziel_lte.res_artikel_id <> to_char(v_lam.artikel_id) then
      OPEN c_ziel_lam;
      ...
      if v_found then
        update lvs_lte lte
           set lte.res_artikel_id = c.MISCHPAL
         where lte.sid      = in_sid
           and lte.firma_nr = in_firma_nr
           and lte.lte_id   = v_ziel_lte_id;
      end if;
    ...
    then
      ...
      or v_order_pos.satzart != 'MA'
      then
        v_vorgang_id := v_order_pos.vorgang_id;
        if v_ziel_lte.order_auf_id is NULL
        or nvl(v_ziel_lte.order_auf_id, 0) = v_order_pos.auf_id
        then
          v_auf_id := v_order_pos.auf_id;
        end if;
        if v_ziel_lte.order_vorgang_id is not NULL
        and v_auf_id is not NULL
        then
          update lvs_lte t
             set t.order_vorgang_id = v_vorgang_id,
                 t.order_auf_id = v_auf_id
           where t.lte_id = v_ziel_lte.lte_id;
        elsif v_ziel_lte.order_vorgang_id is NULL
        then
          v_anz_res := lvs_ausl.lvs_lte_reservieren(in_sid,
                                                    in_firma_nr,
                                                    v_vorgang_id,
                                                    v_auf_id,
                                                    v_ziel_lte.lte_id,
      ...
      if bde_p_base.get_fa_by_auf_id(in_sid, in_firma_nr, v_lam.order_pos_auf_id, v_bde_auf)
      then
    ...
    then -- Bei Nachschub nicht LNK, MAK mit KOMM dann von AF abziehen
      update isi_order_pos p
         set p.ist_menge = p.ist_menge + (nvl(in_menge, 0) * -1),
             p.brutto_kg = p.brutto_kg + (nvl(v_lam_bh_kg, 0) * -1)
       where p.sid = v_quelle_lte.sid
    ...
    then -- Bei Nachschub LNK, MAK mit KOMM menge addieren
      update isi_order_pos p
         set p.ist_menge = p.ist_menge + (nvl(in_menge, 0)),
             p.brutto_kg = p.brutto_kg + (nvl(v_lam_bh_kg, 0)),
             p.status = case when (p.ist_menge + (nvl(in_menge, 0)) >= p.soll_menge)
                            then 'X'
                            else p.status
                            end
       where p.sid = v_quelle_lte.sid
    ...
    then
      ...
      update lvs_lam t
         set t.res_menge = v_rest_res_menge
       where t.lam_id = in_lam_id;
    end if;

    -- -AG- 2015.08.10 Buchen in der KOMM_ORDER wenn Auftrag vorhanden
    update isi_komm_order t
       set t.komm_ist_menge = nvl(t.komm_ist_menge, 0) + in_menge
  ...
  --------------------------------------------------------------------------------
  procedure LVS_C_LTE_PLATZ_LOESCHEN (in_sid      in isi_sid.sid%type,
                                      in_firma_nr in isi_firma.firma_nr%type,
  ...
  function get_packschema_kopf_id (in_sid      in isi_sid.sid%type,
                                   in_firma_nr in isi_firma.firma_nr%type,
                                   in_lte_id   in lvs_lte.lte_id%type) return lvs_packschema_kopf.packschema_kopf_id%type is
  ...
  function get_packschema_lfdn (in_sid      in isi_sid.sid%type,
                                  in_firma_nr in isi_firma.firma_nr%type,
                                  in_lte_id   in lvs_lte.lte_id%type) return number is
    v_error    exception;
    v_err_nr   number;
    v_err_text varchar2(255);
  ...
  function get_packschema_max_lfdn (in_sid                  in isi_sid.sid%type,
                                    in_firma_nr             in isi_firma.firma_nr%type,
                                    in_packschema_kopf_id   in lvs_lte.lte_id%type) return number
                                    is
   v_lfdn                           number;

   CURSOR c_pack_max_lfdn is
     select count(pp.packschema_pos_nr) * pk.anz_lagen
  ...
  function get_packschema_max_lage (in_sid                  in isi_sid.sid%type,
                                    in_firma_nr             in isi_firma.firma_nr%type,
                                    in_packschema_kopf_id   in lvs_lte.lte_id%type) return number
                                    is
   v_lfdn                           number;
   ...
   CURSOR c_pack_max_lfdn is
     select count(pp.packschema_pos_nr)
       from lvs_packschema_pos pp
      where pp.sid = in_sid
        and pp.firma_nr = in_firma_nr
        and pp.packschema_kopf_id = in_packschema_kopf_id
  ...
  --------------------------------------------------------------------------------
  --******************************************************************************
  procedure lvs_lte_lhm_umpacken(in_sid        in isi_sid.sid%type,
                                   in_firma_nr   in isi_firma.firma_nr%type,
                                   in_user_id    in isi_user.login_id%type,
                                   in_res_id     in isi_resource.res_id%type,
    ...
    CURSOR c_lam is
      ...
      order by t.menge desc, nvl(t.packschema_lfdn, t.lhm_c_lfd_nr) desc;
  ...
  begin
    ...
    v_auf_id := NULL;
    v_vorgang_id := NULL;
    if isi_p_order_base.get_order_pos(in_sid, in_auf_id, v_order_pos)
    then
      if (v_order_pos.satzart = 'MA'
      and v_ziel_lte.order_vorgang_id = v_order_pos.vorgang_id
         )
      ...
      if bde_p_base.get_fa_by_auf_id(in_sid, in_firma_nr, in_auf_id, v_bde_auf)
      then
        v_vorgang_id := v_bde_auf.leitzahl;
        v_auf_id := v_bde_auf.auf_id;
        v_anz_res := lvs_ausl.lvs_lte_reservieren(in_sid,
    ...
    v_found := c_lam%FOUND;
    LOOP
      EXIT when i >= in_lhm_uanz or not v_found;
      -- Anpassung P80032 Belimo Uebernahme Inventurkennzeichen aus der Quelle ins Ziel
      if v_ziel_lte.lte_akt_lhm = 0                            -- -AG- 20260330 Zielpalette ist leer
      and c.C_TRUE =
          isi_allg.get_firma_cfg_param (v_ziel_lte.sid,
                                        v_ziel_lte.firma_nr,
                                        'LTE_KOMM',
                                        null,
                                        'LVS_LTE_INV_UEBERNAME',
                                        'LVS',
                                        'CFG',
                                        'F',
                                        'BOOLEAN') then
        update lvs_lte t
           set t.letzte_inventur_id = v_lam.letzte_inventur_id,
      ...
      lvs_p_lte_lhm.lvs_lhm_umpacken(in_sid, in_firma_nr, in_user_id, in_res_id, v_lam.lhm_id, in_z_lte_id); -- Dann auf neue Palette umpacken
  ...
  procedure lvs_c_lte_lhm_umpacken(in_sid        in isi_sid.sid%type,
                                   ...
                                   in_lhm_uanz   in number) is
  begin
    lvs_komm.lvs_lte_lhm_umpacken(in_sid,
                                  in_firma_nr,
                                  in_user_id,
                                  in_res_id,
                                  in_q_lte_id,
  ...
  procedure lvs_c_lte_umpack_q_platz_leer(in_q_platz_leeren   in varchar2, -- T/F T = LTE vom Quellplatz ausbuchen
  ...
  --------------------------------------------------------------------------------
  --******************************************************************************
  procedure lvs_c_lte_menge_umpacken(in_sid        in isi_sid.sid%type,
                                     in_firma_nr   in isi_firma.firma_nr%type,
                                     in_user_id    in isi_user.login_id%type,
                                     in_res_id     in isi_resource.res_id%type,
                                     in_q_lte_id   in lvs_lhm.Lhm_Id%type,
  ...
  procedure lvs_lte_menge_umpacken(in_sid        in isi_sid.sid%type,
                                   in_firma_nr   in isi_firma.firma_nr%type,
                                   in_user_id    in isi_user.login_id%type,
                                   in_res_id     in isi_resource.res_id%type,
                                   in_q_lte_id   in lvs_lhm.Lhm_Id%type,
    ...
    CURSOR c_lam_res is
      select count(lam.lte_id), max(lam.res_login_id)
        from lvs_lam lam
       where lam.lte_id = in_q_lte_id
         and lam.order_pos_auf_id is not NULL
       group by lam.lte_id;
    ...
    CURSOR c_lam_mg is
      select t.lhm_id, t.order_pos_auf_id, sum(t.menge), sum(nvl(t.res_menge, 0))
        from lvs_lam t
       where t.lte_id = in_q_lte_id
         and (( nvl(t.order_pos_auf_id, 0) = nvl(in_auf_id, 0)
      ...
      group by t.lhm_id, t.order_pos_auf_id, t.packschema_lfdn, t.lhm_c_lfd_nr
      order by sum(t.menge) desc, nvl(t.packschema_lfdn, t.lhm_c_lfd_nr) desc;
    ...
    CURSOR c_lam is
      ...
      order by nvl(t.packschema_lfdn, t.lhm_c_lfd_nr) desc;
  ...
  begin
    ...
    if  in_neg_komm = c.C_TRUE     -- Bei negativ Kommissionierung
    ...
    then
      ...
      then
        ...
        if v_ziel_lte.order_vorgang_id is not NULL                  -- LTE hat Reservierung
        and v_auf_id is not NULL
        then
          update lvs_lte t
             set t.order_vorgang_id = v_vorgang_id,
    ...
    if (not isi_p_base.get_resource(in_sid, in_res_id, v_res)) -- Jetzt RES lesen und ggf. kategorie_typ merken für Update der LTE ob die LTE zukünftig über eine KOMMPlatz in der Menge kontrolliert werden muss
      ...
      or v_res.kategorie_typ not in ('TK', 'TD')
    then
      v_res.kategorie_typ := NULL;                             -- Keine Änderung Behältertransport (TD Direkt TK mit Kontrolle)
    end if;
    OPEN c_lam_mg;
    FETCH c_lam_mg into v_lam_lhm_id, v_auf_id, v_menge, v_res_menge;
    v_found := c_lam_mg%FOUND;
    LOOP
      EXIT when not v_found or (in_neg_komm = c.C_FALSE and v_ges_menge >= in_menge);
      if  in_neg_komm = c.C_FALSE     -- Bei normaler Kommissionierung
      and v_ges_menge <= in_menge
      then
        -- habe passende Gebinde gefunden
        if v_menge <= in_menge - v_ges_menge -- Ganzes Gebinde umpacken
        and (v_res_menge = v_menge            -- -AG- 2015.09.04 Komplett Reserviert
          or in_auf_id is NULL)
        then
          v_auf_id := NULL;
          v_vorgang_id := NULL;
          lvs_p_lte_lhm.lvs_lhm_umpacken(in_sid, in_firma_nr, in_user_id, in_res_id, v_lam_lhm_id, in_z_lte_id); -- Dann auf neue Palette umpacken
          ...
          LOOP
            EXIT when v_ges_menge >= in_menge or not v_found_lam;
            ...
            lvs_komm_direct_R359 (in_sid, in_firma_nr, in_res_id, in_user_id, in_q_lte_id, v_lam.lam_id, v_menge, in_z_lte_id, v_lhm_id, v_neu_lam_id, v_neu_lhm_id);
        ...
        -- Jetzt Quell_LTE evtl. die Reservierung rausnehmen
        OPEN c_lam_res;
        FETCH c_lam_res into v_anz_res, v_login_id;
        CLOSE c_lam_res;
      ...
      then
        ...
        update lvs_lam t
           set t.order_pos_auf_id = v_auf_id,
               t.res_menge = t.menge,
               t.res_ziel_lte_id = null,
        ...
        update isi_komm_order t
           set t.komm_ist_menge = t.komm_soll_menge
         where t.auf_id = in_auf_id
           and t.lte_id = in_q_lte_id
           and nvl(t.komm_ziel_lte_id, in_z_lte_id) = in_z_lte_id;

      end if;
      FETCH c_lam_mg into v_lam_lhm_id, v_auf_id, v_menge, v_res_menge;                -- Versuch nächstes Gebinde zu lesen
      v_found := c_lam_mg%FOUND;                -- Merken ob ein weiteres gefunden wurde
    end LOOP;
    CLOSE c_lam_mg;

    if v_ges_menge != in_menge      -- Zu wenig Reserviert auf der Palette
...
-- sqlcl_snapshot {"hash":"33f5ba2cda91d94a7d33b25bd2f11d16d9a747ac","type":"PACKAGE_BODY","name":"LVS_KOMM","schemaName":"DIRKSPZM32","sxml":""}