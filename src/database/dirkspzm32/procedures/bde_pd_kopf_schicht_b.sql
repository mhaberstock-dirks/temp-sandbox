...
( in_sid         in isi_sid.sid%type,
  ...
  CURSOR c_pd_k_c is
    SELECT count(pers_nr)
      FROM bde_pd_kopf pd_k_c
      WHERE pd_k_c.sid = in_sid and
            pd_k_c.firma_nr = in_firma_nr and
            pd_k_c.res_id = in_res_id and
--            pd_k_c.sa_beginn <= v_schicht.sa_beginn and
            pd_k_c.sa_ende > v_schicht.sa_beginn
      GROUP BY pd_k_c.res_id;
...
begin
  ...
  if nvl(v_pd_k_c, 0) != 1 or  -- Nicht nur einer der von der Zeit passen kann
    ...
    bde_pd_kopf_insert(in_sid, in_firma_nr, in_res_id, in_pers_nr, v_sysdate, in_ls_login_id);
  elsif nvl(v_pd_k_c, 0) = 1 and
        v_found = true then
  ...
  update bde_pd_kopf
     set sa_kurzname = v_schicht.sa_kurzname,
