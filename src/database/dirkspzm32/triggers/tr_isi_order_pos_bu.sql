...
declare
  ...
  cursor c_s_send_bew is
    select t.bew_id
      from s_send_bew t
     where t.auf_id = :new.auf_id_extern;
...
begin
  ...
  if not v_found then
     v_err_nr := 10;
     v_err_text := 'Fehler: Artikeldaten fehlen für Artikel ID ' || :new.artikel_id || ' Auf ID ' || :new.auf_id;
     raise v_error;
  end if;

  if :new.mengeneinheit != v_art.mengeneinheit_basis
  then
    :new.mengeneinheit := v_art.mengeneinheit_basis;
  end if;
  if :new.menge_basis != v_art.menge_basis
  then
