create or replace function dirkspzm32.get_schicht_daten (
...
begin
  ...
  -- 1. Personaldaten holen
    ...
    -- ACHTUNG !!!! Dann gilt die gesetzte Schicht, auch wenn es von der zeit nicht passt !!!!
        ...
        result := is_schichtart_in_time(p_sa_kurzname, p_stempel_zeit, p_schicht_datum, v_schicht_modell.d_arb_std_pro_tag, p_sa_beginn
        ,
                                        p_sa_ende, p_sa_std);
  ...
  --     Schichtzeiten mit den registrierten Zeiten übereinstimmen
    if v_dayofweek = 1  -- Mo
     then
        open c_schichtperioden for select
                                                                  sp_sa_wot_mo
                                                              from
                                                                  pzm_schicht_perioden
                                   where
                                       sp_sm_name = v_schicht_modell.sm_name;
    ...
    loop -- jede angelegte Woche durchlaufen
        ...
        if v_kurzname is not null then
            result := is_schichtart_in_time(v_kurzname, p_stempel_zeit, p_schicht_datum, v_schicht_modell.d_arb_std_pro_tag, p_sa_beginn
            ,
                                            p_sa_ende, p_sa_std);

            if result = 1 then
                if v_merk_sa_kurzname is null then
           ...
           -- MWe 2018.05.16 Ticket:P70460-14
                    if pzm_p_base.get_schichtart_by_uix(v_kurzname, v_schichtart) then
                        if ( abs(v_diff_akt) <= abs(v_diff_merk) )
                        or (
                            v_schichtart.sa_bewertung_beginn = 1
                            and v_diff_akt <= v_diff_merk
    ...
    if c_schichtperioden%isopen then
        close c_schichtperioden;
    end if;
    v_kurzname := null;
    if v_merk_sa_kurzname is not null then -- die beste (gemerkte) schicht nochmal holen, da wir möglicherweise mehr als eine gültige haben
        if pzm_p_base.get_schichtart_by_uix(v_merk_sa_kurzname, v_schichtart) then
            if
                v_schichtart.calc_basis = 'GLEITZ'
                and p_zaehler > 0
            then
    ...
    -- Was auch eigendlich falsch ist
        if p_stempel_zeit < trunc(p_stempel_zeit) + 4 / 24 then
            result := get_schicht_daten(p_pers_nr,
...
-- sqlcl_snapshot {"hash":"19a0c4b972d8a3bfb674bf9d19662b0f06b3a67e","type":"FUNCTION","name":"GET_SCHICHT_DATEN","schemaName":"DIRKSPZM32","sxml":""}