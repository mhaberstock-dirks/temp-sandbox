...
begin
    ...
    if updating then
        ...
        if current_isi_user_login_id() is not null then
            :new.last_change_login_id := current_isi_user_login_id();
        end if;

        begin
      -- Die im ERP verbuchten Zeiten in den Fertigungsauftrag übernehmen
            if :new.ze_bde_verbucht_status = 'UE' then
                if :new.ze_bde_dwh_status = :old.ze_bde_dwh_status then
                    :new.ze_bde_dwh_status := 'N';
                end if;

                if :new.ze_bde_verbucht_status != :old.ze_bde_verbucht_status then
...
-- sqlcl_snapshot {"hash":"b1cc624a9c21d47b27df6918835275f9d1bad3ce","type":"TRIGGER","name":"TR_PZM_ZE_BDE_ZEITEN_BIUD","schemaName":"DIRKSPZM32","sxml":""}