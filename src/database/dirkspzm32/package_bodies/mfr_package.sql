create or replace package body dirkspzm32.mfr_package is
 ...
 *******************************************************************************/
    ...
    begin
        ...
        update lvs_lam lam
        set
            lam.lam_text = in_aktuelle_position
        where
            lam.lte_id = lvs_p_lte.v_lvs_lte_id;
    ...
    begin
        if not isi_p_base.get_isi_artikel_by_nr(in_artikel, v_art) then
            raise_application_error(-20000 - 1,
                                    lc.ec_p1(lc.o_tp1_artikel_fehlt, in_artikel));
        end if;
  ...
  ------------------------------------------------------------------------------------------------------------------------------------
    ...
    begin
        if isi_p_base.get_isi_artikel_by_nr(in_artikel, v_artikel) then
            ...
            loop
                ...
                v_result := lvs_transport.lvs_transp_lte(in_sid, in_firma_nr, 'MFR', null, c.c_false,
                                                         'A', in_login_id, v_order_auf_id, null, 3,
                                                         0, 0, 0, v_lte.lgr_platz, in_ziel,
                                                         v_lte.lte_id, null, c.c_false, null, null,
    ...
    begin
        select
            count(*)
        into v_anz_transporte
        from
            isi_transport t
        where
            t.lgr_platz_ziel = in_ziel;
    ...
    begin
        lvs_platz.lvs_c_transp_suche_einl_platz(in_lte_id, in_lgr_orte, in_fahrzeuge_ids, in_modul_erzeuger, in_modul_bearbeiter,
                                                in_user_id, in_prio, in_progr_nr, in_quelle_leer_progr_nr, in_ziel_voll_progr_nr,
                                                in_lgr_platz_quelle, out_lgr_platz, out_transport_id);
    end;
  ...
  -------------------------------------------------------------------------------------------------------------
    ...
    begin
        lvs_ausl.c_lam_suche_res_ausl(in_sid, in_firma_nr, in_artikel_id, null,              -- in_leitzahl => :in_leitzahl,
         null,              -- in_fa_ag => :in_fa_ag,
                                      ...
                                      0,                 -- in_menge,
                                       in_ziel_lgr_platz, null,              -- in_komm_ziel_lte_name => :in_komm_ziel_lte_name,
    ...
    begin
        if not isi_p_base.get_isi_artikel_by_nr(in_artikel, v_art) then
            ...
            c_art_id_suche_ausl(in_sid, in_firma_nr, v_art.artikel_id, in_lager_orte, in_ziel_lgr_platz,
                                in_login_id);
        end if;
    end;
    ...
    begin
        insert into mfr_transp_log (
            ...
            duration_sec
                   ...
                   in_final_dest,
                   in_is_nio,
                   in_duration_sec ) returning guid into out_guid;
  ...
  */
    procedure c_transp_log_with_nio (
        ...
        niobitmask  number;
    begin
        c_transp_log(in_type, in_element, in_source, in_target, in_origin_source,
                     in_final_dest, in_lte_id, in_mfr_unique_id, in_is_nio, out_guid,
                     in_duration_sec);

        if in_nio_bits <> 0 then
            for niobitindex in 0..15 loop
  ...
  --------------------------------------------------------------------------------
    ...
    function c_erzeuge_art_id_ausl_auf_tms (
        ...
        where
            ...
            and ( ( l.lgr_verwendung = 'Lager'
                    and lte.lte_status = 'LF' )
                  or ( l.lgr_verwendung = 'WE'
                       and lte.lte_status = 'BF' )
                  or ( l.lgr_verwendung = 'LagerP'
                       and l.lgr_dispo_einl_te = 0 ) )
            and l.gesperrt = 'F'
            and o.sid = l.sid
            ...
            and ( instr(
                lvs_lager_opt.lvs_lort_format(in_lager_orte),
                lpad(
                              to_char(l.lgr_ort),
                              3 - 1,
                              '0'
  ...
  --------------------------------------------------------------------------------
    ...
    begin
        ...
        if not isi_p_base.get_isi_artikel_by_nr(in_artikel, v_art) then
            ...
            v_result := c_erzeuge_art_id_ausl_auf_tms(in_sid, in_firma_nr, v_art.artikel_id, in_lager_orte, in_ziel_lgr_platz,
                                                      in_login_id, in_prio);
        end if;
    ...
    -- local variables here
        ...
        cursor c_max_lhm is
        select
            max(pp.packschema_pos_nr)
        from
            lvs_packschema_pos pp
        where
            pp.packschema_kopf_id = v_lte.packschema_kopf_id;
    ...
    begin
        ...
        if lvs_p_base.get_lte(in_lte_id, v_lte) then
            ...
            close c_max_lhm;
            if out_max_lhm is null then
                out_max_lhm := v_lte.lte_akt_lhm + 1;
                v_result := c.c_false;
            else
                v_result := c.c_false;
                if out_max_lhm <= v_lte.lte_akt_lhm then
                    v_result := c.c_true;
                end if;
            end if;
  ...
  -------------------------------------------------------------------------
    ...
    begin
        ...
        if in_fahrzeuge_ids is null then
            ...
            set
                t.res_id = null                  -- Damit der Auftrag nich in die AU Liste kommt
            where
                t.transp_id = out_transp_id;
  ...
  -------------------------------------------------------------------------
    ...
    -- Lesen der Artikeldaten
        v_err_nr := null;
        v_err_text := null;
        v_result := lvs_transport.lvs_transp_lte_353(in_sid, in_firma_nr, in_modul_erzeuger, in_modul_bearbeiter, in_frei_fahren,
                                                     ...
                                                     in_vorgang_id, in_fahrzeuge_ids, in_lkw_nr, in_komm_id, in_komm_lte_lhm_lagen,
                                                     in_komm_lte_lhm_pro_lage, in_komm_lhm_hoehe_lage, in_komm_packscheme_kopf_id, in_out_transport_gruppe
                                                     , v_transp_id);
    ...
    procedure c_import_mfr_element is
        ...
        select
            elem.fahrzeug
        from
            mfr_element_cfg elem
        where
            elem.fahrzeug = v_fahrzeug;
  ...
  -------------------------------------------------------------------------
    ...
    begin
        ...
        if
            lvs_p_base.get_lte(in_lte_id, v_lte) -- Lesen der LTE
            and v_lte.lte_status in ( c.lte_bf_stat, c.lte_bs_stat, c.lte_pf_stat, c.lte_kf_stat, c.lte_af_stat,
                                      c.lte_ag_stat )
        then
            change_lte_properties(v_lte,                  -- in lvs_lte%rowtype,
             in_lte_name,            -- in lvs_lte.lte_name%type,
             in_lte_hoehe,           -- in lvs_lte.lte_vol_hoehe%type,
             in_lte_breite,          -- in lvs_lte.lte_vol_breite%type,
             in_lte_tiefe,           -- in lvs_lte.lte_vol_tiefe%type,
            ...
            begin
                if
                    ( v_lte.lte_status = c.lte_bf_stat -- Palette stheht auf enem WE oder WA mit bestand und wurde vom MFR mit ID Gelesen
    ...
    begin
        v_text := '';
        select
            stradd_cr(ma.fahrzeug
                      || '; '
                      || 'AT;'
                      || ma.telegr_fc
                      || '; '
                      || ma.telegr_fe_id_anfang
                      || '; '
                      || ma.telegr_fe_id_anfang
                      || '; '
                      || '; '
                      ...
                      || '; '
                      || mq.telegr_fe_id_ende
                      || ';' || decode(w.ziel_element_id - ma.element_id, 0, ' Aufnahme', ' Abgabe'))
        into v_text
...
--     , ma.pos_nr, mq.pos_nr quell_pos_nr, mz.pos_nr ziel_pos_nr,
 --     w.Quell_element_id Quell_Weg_Quelle, w.Ziel_element_id Quell_Weg_Ziel , w.Quell_element_Id Ziel_Weg_Quelle, w.Ziel_element_Id Ziel_Weg_Ziel
        from
            ...
            mfr_wege        w
        where
                ma.element_id = in_mfr_element_id
            and
        --m.Element_id = wq.Ziel_Element_Id (+)  and
             ( ma.element_id = w.ziel_element_id
                  or ma.element_id = w.quell_element_id )
            and w.quell_element_id = mq.element_id (+)
            and w.ziel_element_id = mz.element_id (+)
        order by
            ma.pos_nr,
            mq.pos_nr,
            mz.pos_nr;
...
-- diese dürfen auch während einer Einlagerung vorgenommen werden.
    ...
    -- Update der LTE-Daten
        update lvs_lte lte
        set
            lte.lte_name = nvl(in_lte_name, lte.lte_name),
            ...
            lte.lte_vol = nvl(in_lte_tiefe, lte.lte_vol_tiefe) * nvl(in_lte_hoehe, lte.lte_vol_hoehe) * nvl(in_lte_breite, lte.lte_vol_breite
            ) / 1000000000,
    ...
    -- LTE steht auf einem Lagerpltz, dann Gewicht und Höhen prüfen
        if in_lte.lgr_platz is not null then
            if in_lte_gewicht != in_lte.lte_akt_kg then
                update lvs_lgr lgr
                set
                    lgr.lgr_akt_kg = lgr.lgr_akt_kg + in_lte.lte_akt_kg - in_lte_gewicht
                where
                    lgr.lgr_platz = in_lte.lgr_platz;

            end if;

            if in_lte_hoehe != in_lte.lte_vol_hoehe then
                update lvs_lgr lgr
                set
                    lgr.lgr_frei_hoehe = lgr.lgr_frei_hoehe + in_lte.lte_vol_hoehe - in_lte_hoehe
                where
                        lgr.lgr_platz = in_lte.lgr_platz
    ...
    -- LTE hat ein Ziel, dann Gewicht und Höhen prüfen in Quelle und Ziel anpasse (DISPO)
        if in_lte.ziel_lgr_platz is not null then
            if in_lte_gewicht != in_lte.lte_akt_kg then
                update lvs_lgr lgr
                set
                    lgr.lgr_dispo_ausl_kg = lgr.lgr_dispo_ausl_kg + in_lte.lte_akt_kg - in_lte_gewicht
                where
                    lgr.lgr_platz = in_lte.lgr_platz;

                update lvs_lgr lgr
                set
                    lgr.lgr_dispo_einl_kg = lgr_dispo_einl_kg + in_lte.lte_akt_kg - in_lte_gewicht
                where
                    lgr.lgr_platz = in_lte.ziel_lgr_platz;
            ...
            if in_lte_hoehe != in_lte.lte_vol_hoehe then
                update lvs_lgr lgr
                set
                    lgr.lgr_dispo_einl_frei_hoehe = lgr.lgr_dispo_einl_frei_hoehe + in_lte.lte_vol_hoehe - in_lte_hoehe
                where
                        lgr.lgr_platz = in_lte.lgr_platz
                    and lgr.lgr_typ in ( c.reg_fach1, c.stap_flae1, c.stap_flae2 );
...
-- Der Transport wird gelöscht gelöscht, der Grund wird zurückgegeben.
  ...
  -------------------------------------------------------------------------
    ...
    --
        ...
        from
            isi_order_pos pos
        where
            pos.auf_id_extern = v_auf_id_extern;
    ...
    begin
        ...
        if c_lam%rowcount > 1 -- Es gibt mehr als einen Eintrag
         ...
         then
            close c_lam;
            raise_application_error(-20000 - c.fmid_falscher_lte_status,
                                    lc.ec_p1(lc.o_tp1_lte_hat_mehr_eine_lhm, in_lte_id));
        ...
        if lvs_p_base.get_lte(in_lte_id, v_lte) -- Lesen der LTE
         then
            if not isi_p_base.get_isi_artikel(v_lte.sid, v_artikel_id, v_artikel) then
                raise_application_error(-20000 - c.fmid_artikelnummer_fehlt,
                                        lc.ec_p1(lc.o_tp1_artikel_fehlt, v_artikel.artikel));
            end if;
            ...
            raise_application_error(-20000 - c.fmid_lte_id_fehlt,
    ...
    -- Jetzt die ISI_ORDER bauen
        insert into s_rcv_auftr values ( v_lte.firma_nr,           -- FIRMA_NR
    ...
    -- Lesen Quell-Packschema
        ...
        begin
            ...
            if mod(v_lte.lte_akt_lhm, v_lhm_je_lage) > 0 then
    ...
    -- Lesen Ziel-Packschema
        ...
        begin
            ...
            if mod(v_lte.lte_akt_lhm,(v_lhm_je_lage * v_lte_lagen)) > 0 then
                v_lte_anz := v_lte_anz + 1;
            end if;

        exception
            when others then
                v_lte_lagen := v_lte.lte_akt_lhm;
        end;

    -- Ziel-Packschema hat mehr als eine LAM je lage
        if v_lhm_je_lage > 1 then
    ...
    -- LTE reservieren
        ...
        if lvs_transport.lvs_transp_lte(v_lte.sid,                 -- in_sid                  IN isi_sid.sid%TYPE,
                                        ...
                                        0,
                                        v_transport_gruppe,
                                        out_transport_id) != 0 then
            raise_application_error(-20000 - c.fmid_falsche_buchungsart,
                                    lc.ec_p2(lc.o_tp2_weg_von_nach_falsch, in_aktuelle_position, in_komm_lgr_platz));
        end if;
        ...
        if lvs_transport.lvs_transp_beginnen(v_lte.sid,                  -- in_sid                  IN isi_sid.sid%TYPE,
         ...
         in_lte_id,                 -- in_lte_id               IN lvs_lte.lte_id%TYPE,
                                             null)                      -- in_res_id              in isi_resource.res_id%type)
                                              != 0 then
            raise_application_error(-20000 - c.fmid_falsche_buchungsart,
                                    lc.ec_p1(lc.o_tp1_tram_mit_id_fehlt,
                                             to_char(out_transport_id)));
        end if;
        ...
        or v_komm_lgr_platz is null then
            v_komm_lgr_platz := in_komm_lgr_platz;
        end if;
        close c_komm_lgr_platz;
        insert into isi_komm_order values ( v_lte.sid,
                                            v_lte.firma_nr,
                                            null,    -- v_komm_id,
                                            v_li_nr, -- v_p_komm_id,
        ...
        update isi_transport t
        set
            t.p_komm_id = v_komm_id
        where
            t.transp_id = out_transport_id;
  ...
  --******************************************************************************
    ...
    begin
      ...
      -- MFR hat ggf. diesen Order mit Komm Order für die eingene Vewrwaltung ROBOTER angelegt und kann muss nach Beendigung geloescht werden
            if
                v_found -- KOMM_Order gefunden
                and ( v_komm_order.modul_erzeuger != 'MFR' -- Nur wenn Erzeuger der MFR ist, dann alle wieder löschen
                or (
                    v_komm_order.menge < v_komm_order.komm_ist_menge  -- Noch nicht fertig
            ...
            if v_found -- Transport gefunden
             then
                v_result_n := lvs_transport.lvs_transp_loeschen(in_sid,   -- Dann löschen
                 in_firma_nr, in_user_id, v_transport.transp_id, 'T');
            end if;

            delete isi_komm_order t
            where
                    t.modul_erzeuger = 'MFR'
                and t.lte_id = in_lte_id;                              -- Schonmal die KOMM_ORDER löschen
            isi_p_order.c_abbr_trans_lief(in_sid,                     -- Reservierungenzurücksetzen
             in_firma_nr, v_komm_order.p_komm_id, in_user_id, 'T');                       -- Mit diesem Commit ist der Eintrag in der ISI_KOMM_Order gelöscht und alle Reservierungen  entfernt.

            isi_p_order.c_del_lief(in_sid,                            -- Order komplett löschen

             in_firma_nr, v_komm_order.p_komm_id,            -- in_lief_nr

             in_user_id, v_komm_order.p_komm_id);           -- in_tour_nr
            delete isi_order_tour t
            where
                t.vorgang_id = v_komm_order.p_komm_id;           -- tour_nr
  ...
  --******************************************************************************
    ...
    begin
        if lvs_p_base.get_lgr_platz(in_neues_ziel, v_lgr) then
            update isi_order_pos t
            set
                t.ziel = in_neues_ziel
            where
                    t.sid = in_sid
                and t.firma_nr = in_firma_nr
                and t.vorgang_id = in_vorgang_id
            ...
            set
                t.pack_lgr_platz = in_neues_ziel
            where
                t.vorgang_id = in_vorgang_id;

            update isi_komm_order t
            set
                t.komm_ziel_lgr_platz = in_neues_ziel
            where
                    t.sid = in_sid
                and t.firma_nr = in_firma_nr
            ...
            loop
                ...
                v_result := lvs_platz.lvs_c_transp_neues_ziel(in_sid, in_firma_nr, -1, v_transport.transp_id, in_neues_ziel,
                                                              v_res_id);
            ...
            raise_application_error(-20000 - 1,
                                    lc.ec_p2(lc.o_tp2_platz_existiert_nicht, in_neues_ziel, 'ALL'));
        end if;
    end;
  ...
  --******************************************************************************
    ...
    -- Deutsch Sprache = 1
        ...
        if v_count = 0 then   -- SID, Firma_nr, Engine_id, MT_Sprache, MT_Gruppe, mt_index , FehlerNr, Fehlertext, Typ, Quittieren, create Date,create User,change Date,change User
            insert into meldung_texte values ( '01',
                                               1,
                                               1,
                                               1,
                                               - 1,
  ...
  --******************************************************************************
    ...
    function c_res_stat (
        ...
        where
                r.res_id = in_res_id
            and r.sid = irs.sid
            and r.firma_nr = irs.firma_nr
            and irs.res_st_id = in_res_st_id
            and r.typ = irs.res_typ
            and r.fehler_schluessel = irs.fehler_schluessel;
    ...
    begin
        ...
        or -- Maschine hatte keine Störung, nicht im Rüsten Störung kommt.
         (
            ( v_res_action = '1' )
            and ( v_res_st_id = 0 )
        ) then  -- Maschine hatte Störung nicht im Rüsten Fehler geht!
            res_status.res_status_beg(in_sid, in_firma_nr, in_res_id, in_ls_login_id, v_res_st_id,
                                      in_res_typ, null,  -- in_res_st_ug_id,
  ...
  -- in_old_PLC_connector =  Old PLC Connector   alte Schnittstellen  Nr (Telegr_Koppl_Nr)
   ...
   --******************************************************************************
    ...
    begin
        update mfr_element_cfg
        set
            mfr_element_cfg.telegr_koppl_nr = in_new_plc_connector
        where
            mfr_element_cfg.telegr_koppl_nr = in_old_plc_connector;
  ...
  --******************************************************************************
    ...
    begin
        ...
        where
            mfr_element_cfg.telegr_koppl_nr >= start_move_up_at_plc_connector;
...
-- sqlcl_snapshot {"hash":"f84582e277c6219872d46f05f554741eba81cb7b","type":"PACKAGE_BODY","name":"MFR_PACKAGE","schemaName":"DIRKSPZM32","sxml":""}