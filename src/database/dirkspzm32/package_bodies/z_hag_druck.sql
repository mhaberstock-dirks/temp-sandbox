create or replace package body dirkspzm32.z_hag_druck is
  ...
  */
  ...
  -------------------------------------------------------------------------------------------------------
    function vda_etikett (
        ...
        v_seqvon                       varchar2(255);
        v_seqbis                       varchar2(255);
        v_hagturmbehaelternr           varchar2(255);
        v_hagturmbarcode               varchar2(255);
        ...
        select
            ...
            min(pos.best_nr_kunde) min_seq,
            max(pos.best_nr_kunde) max_seq,
            substr(lam.lte_id, 5)  hagturmbehaelternr,
            max(pos.auftrag)       hagturmbarcode,
            max(pos.auf_id)        lamorderposaufid
        from
            isi_order_pos pos,
            lvs_lam       lam
        where
        ...
        where
                fa.sid = v_lte_sid
            and fa.firma_nr = v_lte_firma_nr
            and fa.auf_id = v_order_pos_auf_id;
    ...
    begin
        ...
        v_seqvon := null;
        v_seqbis := null;
        v_hagturmbehaelternr := null;
        v_hagturmbarcode := null;
        v_lam_order_pos_auf_id := null;
        v_lam_art_fanr := null;
        v_lam_art_faag := null;
        if lvs_p_lte_lhm.lvs_lte_lhm_ref(in_sid, in_firma_nr, in_id) = c.basis_lte then
            ...
            close c_fa_auf_lte;
            open c_pos_lte_all;
            fetch c_pos_lte_all into
                v_best_nr_kunde,
                v_seqvon,
                v_seqbis,
                v_hagturmbehaelternr,
                v_hagturmbarcode,
                v_lam_order_pos_auf_id;
      ...
      -- Falls der Parameter P1 nicht gefüllt, dann nur die Artikelnummer eintragen
            ...
            elsif v_order_pos_auf_id is not null then
                open c_fa;
                fetch c_fa into v_fa_auftrag;
                if c_fa%found then
                    v_vdasachnrlieferant := v_vdasachnrlieferant
                                            || '-'
                                            || nvl(
                        to_char(v_fa_auftrag.leitzahl),
                        'X'
                    )
                                            || '/'
                                            || to_char(v_fa_auftrag.fa_ag);
                end if;
            ...
            if not v_found then
                ...
                v_hagturmbarcode := v_adr.kundennrlief
                                    || '-'
                                    || v_hagturmbehaelternr
                                    || '-'
                                    || v_hagturmbarcode
        ...
        i := 1;
        loop
            if v_hag_ausg_zeile.exists(i) then
                begin
                    v_print_daten := v_print_daten
                                     || chr(13)
                                     || chr(10)
                                     || 'TurmZeile'
                                     || to_char(i)
  ...
  -------------------------------------------------------------------------------------------------------
    ...
    -- Wenn Artikelrein, dann wird der Artikelstamm jetzt gelesen
      ...
      --or v_art.abfuell_toleranz_minus > v_lam_memge / v_art.lte_lhm_menge
        ...
        if v_found then
            ...
            if v_art.artikel is not null then
                ...
                if v_lam_art_faag is not null then
                    v_stdsachnrlieferantbarcode := v_stdsachnrlieferantbarcode
                                                   || '-'
                                                   || nvl(
                        to_char(v_lam_art_fanr),
                        'X'
        ...
        if v_bc_typ = 'SPEZ' then
            lvs_p_lte_lhm.spez_barcode_result(in_sid,                      -- in_sid        in lvs_charge.sid%type,
             in_firma_nr, v_stdpackstucknr,            -- in  lvs_lam.lhm_id%type,
     ...
     -- Sollmenge wie als Barcodemenge ermitteln
            ...
            for i in 1..length(v_stdpackstucknr) loop
                if upper(substr(v_bc_format, i, 1)) = 'M' then
                    if v_stdpackstucknrsollmenge is null then
                        v_stdpackstucknrsollmenge := substr(v_stdpackstucknr, i, 1);
                    else
                        v_stdpackstucknrsollmenge := v_stdpackstucknrsollmenge
                                                     || substr(v_stdpackstucknr, i, 1);
                    end if;
            ...
            for i in 1..length(v_stdpackstucknr) loop
                if upper(substr(v_bc_format, i, 1)) = 'M' then
                    if v_stdpackstucknrmenge is null then
                        v_stdpackstucknrmenge := substr(v_stdpackstucknr, i, 1);
                    else
                        v_stdpackstucknrmenge := v_stdpackstucknrmenge
                                                 || substr(v_stdpackstucknr, i, 1);
                    end if;
        ...
        v_print_daten := 'stdSachNrLieferantDatenfeldFuerLieferanten='
                         ...
                         || chr(13)
                         || chr(10)
                         || 'stdPackstuckNrBarcode='
                         || v_stdpackstucknrbarcode
                         || chr(13)
                         || chr(10)
                         ...
                         || chr(10)
                         || 'stdPackstuckNrMenge='
                         || v_stdpackstucknrmenge
                         || chr(13)
                         || chr(10)
                         || 'stdPackstuckNrSollMenge='
                         || v_stdpackstucknrsollmenge
                         || chr(13)
                         || chr(10)
                         || 'stdAtrikelP1='
                         || v_stdatrikelp1
...
-- sqlcl_snapshot {"hash":"71b0cb250982eb6b658fbbc48512a14332fd7b17","type":"PACKAGE_BODY","name":"Z_HAG_DRUCK","schemaName":"DIRKSPZM32","sxml":""}