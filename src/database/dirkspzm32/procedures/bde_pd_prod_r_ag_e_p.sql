create or replace procedure dirkspzm32.bde_pd_prod_r_ag_e_p
/*
...
begin
  ...
  -- Wenn nicht gefunden dann setze Fehlertext !!
    ...
    if nvl(v_res_zus.akt_aufgabe, 0) != 'R' then
        v_err_nr := 20;
    ...
    -- -AG- 23.09.2008 Wenn LHM während des Ruesten gebucht wurde, dann muss der bei der Abmeldung geloescht werden
        bde_funktionen.c_set_zug_menge_lhm_fa(in_sid, in_firma_nr, v_res_zus.leitzahl, v_res_zus.fa_ag, v_res_zus.fa_upos,
                                              0);
  ...
  -- Erstmal nur die Daten ermitteln
    v_ruest_std := bde_funktionen.get_fa_zeiten_upos(in_sid, in_firma_nr, in_leitzahl, in_fa_ag, in_fa_upos,
                                                     null);
  ...
  -- -DTs- 20190919 Ermitteln der MA-Zeiten in dem FA für die aktuelle anmeldung
    v_ruest_std_erf := bde_funktionen.get_ma_erf_zeiten(v_res_zus.sid, v_res_zus.firma_nr, v_res_zus.res_id, v_res_zus.akt_aufgabe_seit
    , sysdate) * 60;
  -- Update des Aktuelle Zustands des Arbeitsgangs
    ...
    set
        ...
        fa.mde_ist_mg = nvl(fa.mde_ist_mg, 0) - nvl(fa.mde_ist_mg_t, 0), -- Mengen geht ins Rüsten
...
-- sqlcl_snapshot {"hash":"44085435076bec9e66635c3b14b5a0c44934cb7a","type":"PROCEDURE","name":"BDE_PD_PROD_R_AG_E_P","schemaName":"DIRKSPZM32","sxml":""}