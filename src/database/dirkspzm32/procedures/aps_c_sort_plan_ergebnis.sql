create or replace procedure dirkspzm32.aps_c_sort_plan_ergebnis (
    ...
    from
        (
            select --p.aps_plan_status st,
                ...
                p.jis_linie,
                p.prioritaet               prio,
                p.planreihenfolge          reihe,
                p.hersteller_kuerzel_liste herst
             -- , a.artikel_p6 reifen, a.artikel_p7 Rad, a.artikel kpr
                ,
                p.auftrag_nr,
                p.pos_nr,
                pm.materialrelation_type   r_typ,
                pm.materialrelation_valide r_valide,
                pm.materialrelation_info   r_info,
                pm.child_id,
                pe.abnr                    batch
            from
                aps_order_auftr_pos        p,
    ...
    where
        ...
        and ( x.r_typ > v_ergebnis.r_typ
              or ( x.r_typ = 2
                   and v_ergebnis.r_typ = 2
                   and x.batch like 'SISI%'
                   and v_ergebnis.batch not like 'SISI%' ) )
        and ( x.prio > v_ergebnis.prio
              or ( x.prio = v_ergebnis.prio
                   and x.reihe > v_ergebnis.reihe ) )
    order by
        x.r_typ desc,  -- 8 = Lager, 2 = Produktion
        case
            when nvl(x.batch, 'SISIx') like 'SISI%' -- Prüfen FA oder Plan
             then
                    0
            else
                1
...
begin
    if isi_allg.c_get_firma_cfg_param('01', 1, 'APS',                      -- in_kategorie             in isi_firma_cfg.kategorie%type,
    ...
    loop
        ...
        -- Ringtausch 3 
                ...
                set
                    ...
                    mr.pos_nr = 1
                where
                        mr.auftrag_nr = v_reihenfolge_fehler.auftrag_nr
        ...
        -- Besseren Eintrag nach forne
                ...
                where
                        mr.auftrag_nr = v_ergebnis.auftrag_nr
...
-- sqlcl_snapshot {"hash":"021a22472b507406894ccee9404308fc1b576bb3","type":"PROCEDURE","name":"APS_C_SORT_PLAN_ERGEBNIS","schemaName":"DIRKSPZM32","sxml":""}