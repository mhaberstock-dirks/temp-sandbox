create or replace editionable trigger dirkspzm32.tr_s_rcv_adr_biud before
    insert or update or delete on dirkspzm32.s_rcv_adr
    for each row
declare
        ...
        if not v_found then
            insert into isi_adressen (
             ...
             values ( v_sid,                        -- SID        VARCHAR2(2) not null,
                       nvl(:new.firma_nr,
                           0),        -- FIRMA_NR   NUMBER(2) not null,
                       nvl(:new.adr_art,
                           c.leer),    -- ADR_ART    VARCHAR2(1) not null,
                       nvl(:new.adr_nr,
                           c.leer),     -- ADR_NR     VARCHAR2(20) not null,
                       nvl(:new.adr_liefer,
                           c.leer) ) -- ADR_LIEFER VARCHAR2(20) not null,
                            returning adress_id into v_adress_id;
...
--                           :new.ecc_hersteller_id, -- ECC_HERSTELLER_ID          VARCHAR2(10),
  ...
  -- we need to set our old record to unactive state.
    ...
    or :old.adr_liefer <> :new.adr_liefer then
        update isi_adressen adr
        set
            adr.aktiv = c.c_false -- AKTIV VARCHAR2(1),
        where
                adr.sid = v_sid
...
-- sqlcl_snapshot {"hash":"8db90bece450e197d5a1aa1cd687e77f55b47f25","type":"TRIGGER","name":"TR_S_RCV_ADR_BIUD","schemaName":"DIRKSPZM32","sxml":""}