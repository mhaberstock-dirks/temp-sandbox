...
declare
  ...
  CURSOR c_artikel (p_artikel isi_artikel.artikel%type) is
  ...
  CURSOR c_artikel_kd is
   select *
     from isi_artikel_kunde art_kd
    where art_kd.sid     = v_sid.sid
      and art_kd.artikel_id = v_ag_artikel_id
      and art_kd.kunden_nr = :new.kunden_nr;

  CURSOR c_fa_auftrag is
   select *
     from bde_fa_auftrag auft
    where auft.sid      = v_sid.sid
      and auft.firma_nr = :new.firma_nr
      and auft.leitzahl = :new.leitzahl
      and auft.fa_ag = nvl(:new.fa_ag,0)
      and auft.fa_upos = nvl(:new.fa_upos,0);

  CURSOR c_res is
    select *
      from isi_resource res
     where res.sid = v_sid.sid
...
begin
  ...
  or :new.ag_status is NULL then
    ...
    then
      ...
      then
         v_err_nr := 1;
         v_err_text := 'Fehler:' || '  Fertigungsauftrag ' || :new.leitzahl || '/' || :new.fa_ag || ' Artikel <' || :new.ag_artikel || '> fehlt.';
         raise v_error;
      end if;
    ...
    if not v_found then
      ...
      then
        ...
        insert into bde_fa_auftrag auft (
                    ...
                    auft.ag_id)            -- AG_ID              VARCHAR2(20),
              values (
                    v_sid.sid,             -- SID      VARCHAR2(2) not null,
                    :new.firma_nr,         -- FIRMA_NR NUMBER(2) not null,
                    :new.leitzahl,         -- LEITZAHL NUMBER not null,
                    nvl(:new.fa_ag,0),     -- FA_AG              NUMBER,
                    nvl(:new.fa_upos,0),   -- FA_UPOS            NUMBER,
                    :new.satzart,          -- SATZART            VARCHAR2(2),        returning auft.leitzahl
                    nvl(:new.ab_ist,0),    -- AB_IST_MG          NUMBER,
                    nvl(:new.ag_ist_mg,0), -- AG_IST_MG          NUMBER,
                    nvl(:new.ag_ist_mg_b,0),   -- AG_IST_MG_B        NUMBER,
                    nvl(:new.ag_ist_schrott,0),-- AG_IST_MG_SCHROTT  NUMBER,
                    nvl(:new.ag_ist_ruesten,0),-- AG_IST_MG_RUESTEN  NUMBER,
                    nvl(:new.ruest_zeit_ist,0),-- RUEST_ZEIT_IST     NUMBER,
                    nvl(:new.prod_zeit_ist,0), -- PROD_ZEIT_IST      NUMBER,
                    nvl(:new.stoer_zeit_ist,0),-- STOER_ZEIT_IST     NUMBER,
                    'N',                       -- FREIG_STATUS       VARCHAR2(2),
                    nvl(:new.auf_id,0));       -- AG_ID              VARCHAR2(20),
      :new.ag_status := 'UE';
    end if;
    ...
    :new.charge := nvl(:new.charge, :new.leitzahl || '/' || :new.fa_ag);

    if :new.charge is not NULL then
      v_charge_id := get_charge_id(v_sid.sid, :new.firma_nr, NULL, :new.charge, v_ag_artikel_id);
    else
      v_charge_id := NULL;
    end if;
    ...
    update bde_fa_auftrag auft
       set                                                           -- SID                VARCHAR2(2) not null,
           ...
           auft.lhm_menge            = decode(nvl(:new.lhm_menge,0),
                                              0,
                                              auft.lhm_menge,
                                              :new.lhm_menge),       -- LAM_MENG           NUMBER,
    ...
    update bde_fa_auftrag
        set kenz_letzt_ag = NULL
      where sid = v_sid.sid and
            firma_nr = :new.firma_nr and
            leitzahl = :new.leitzahl;
    ...
    update bde_fa_auftrag f1
        set kenz_letzt_ag = 1
      where f1.sid = v_sid.sid
        and f1.firma_nr = :new.firma_nr
        and f1.leitzahl = :new.leitzahl
        and f1.fa_ag = (select max(f2.fa_ag)
                          from bde_fa_auftrag f2
                         where f1.sid = f2.sid and
                               f1.firma_nr = f2.firma_nr and
                               f1.leitzahl = f2.leitzahl and
                               f2.satzart like('V%')
                         group by f2.sid, f2.firma_nr, f2.leitzahl)
        and f1.fa_upos = (select max(f2.fa_upos)
                          from bde_fa_auftrag f2
                         where f1.sid = f2.sid and
                               f1.firma_nr = f2.firma_nr and
                               f1.leitzahl = f2.leitzahl and
                               f1.fa_ag = f2.fa_ag and
                               f2.satzart like('V%')
                         group by f2.sid, f2.firma_nr, f2.leitzahl, f2.fa_ag);
    -- -AG- 202190327 Einbau der Tabelle wenn eine FA erstellt wurde
    ...
    insert into bde_fa_auftrag_stl
      select *
        from bde_v_gen_bde_fa_auftrag_stl t
       where t.leitzahl = :new.leitzahl;
    ...
    if :old.ag_status like 'A%' then
      v_err_nr := 10;
      v_err_text := 'Fehler:' || '  Fertigungsauftrag ' || :old.leitzahl || '/' || :old.fa_ag || ' mit Status <' || :old.ag_status || '> darf nicht gelöscht werden.';
      raise v_error;
      ...
      v_err_nr := 10;
      v_err_text := 'Fehler:' || '  Fertigungsauftrag ' || :new.leitzahl ||  ' ist bereits angemeldet und darf nicht mehr gelöscht werden!';
      raise v_error;
    else
    -- c_fa_auftrag
      OPEN c_fa_auftrag;
     ...
     --if v_fa_auftrag.kenz_letzt_ag = 1 then
       delete bde_fa_auftrag auft
        where auft.sid      = v_sid.sid
          ...
          and nvl(auft.fa_upos, 0)  = nvl(:new.fa_upos, 0);
       update bde_fa_auftrag
          set kenz_letzt_ag = NULL
        where sid = v_sid.sid and
              firma_nr = :new.firma_nr and
