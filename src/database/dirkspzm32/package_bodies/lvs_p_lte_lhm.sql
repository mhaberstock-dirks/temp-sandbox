create or replace package body dirkspzm32.lvs_p_lte_lhm is

  -- Private type declarations
    procedure lvs_lhm_abpacken (
        in_lhm        in lvs_lhm%rowtype,
    ...
    function lvs_lte_lhm_pruefcheck_mod10 (
    ...
    begin
        ...
        while v_laenge > 0 loop
            v_summe := v_summe + to_number ( substr(in_barcode, v_laenge, 1) ) * v_multiplikator;

            v_laenge := v_laenge - 1; -- naechstes Zeichen
            if v_multiplikator = 3 then
                v_multiplikator := 1;
            else
                v_multiplikator := 3;
            end if;
      ...
      -- 0 ist immer 0
        ...
        if to_char(v_mod) != v_pruefziffer then
      -- Pruefziffer ist falsch
            v_err_nr := 10;
            v_err_text := lc.ec_p2(lc.o_tp2_pruefziffer_err,
                                   v_pruefziffer,
                                   to_char(v_mod));
    ...
    function lvs_lte_lhm_pruefziffer_mod10 (
        in_barcode in varchar2
    ) return varchar2 is
    -------------------------------------------------------------------------------------------------------
    ...
    -- Lesen der Artikeldaten
      ...
      -- Ist der Barcode gefüllt??
            ...
            fetch c_lhm into v_barcode; -- Lesen
            if c_lhm%notfound then
        -- Barcode ist kein LHM
                open c_lte; -- Jetzt prüfen ob der Barcode ein LTE ist (Transporteinheit)
                fetch c_lte into v_barcode; -- Lesen
                if c_lte%notfound then
          -- Wenn nicht dann
    ...
    begin
        return ( lvs_p_lte_lhm.lvs_lte_lhm_best_r30(in_sid,              -- in isi_sid.sid%type,
         in_firma_nr,         -- in isi_firma.firma_nr%type,
         in_barcode,          -- in lvs_lte.lte_id%type,
         in_barcode_ref, c.c_true) );
    end;

    function lvs_lte_lhm_best_r30 (
        in_sid              in isi_sid.sid%type,
        in_firma_nr         in isi_firma.firma_nr%type,
        in_barcode          in lvs_lte.lte_id%type,
        in_barcode_ref      in varchar2,
        in_ignor_lte_status in varchar2
    ) return number is
    -------------------------------------------------------------------------------------------------------
    ...
    -- Lesen der Artikeldaten
        ...
        if in_barcode_ref = 'LTE' then
            if lvs_p_base.get_lte(in_barcode, v_lte) then
        -- -AG- 20190401 Bug, der Status muss fast immer ignoriert wereden, ausser bei den Scann-Prozessen
                if
                    v_lte.lte_status in ( c.lte_kf_stat, c.lte_pf_stat ) -- Kein Lagerbestand mehr, dann 0
            ...
            fetch c_lam_lte into v_lam; -- Lesen
            if c_lam_lte%notfound then
        -- Keinen Lagerbestand
            ...
            close c_lam_lte;
        elsif in_barcode_ref = 'LHM' then
            open c_lam_lhm; -- Prüfen ob es Lagerbestand für diesen Barcode gibt
            fetch c_lam_lhm into v_lam; -- Lesen
            if c_lam_lhm%notfound then
        -- Keinen Lagerbestand
                v_result := 0; -- Kein Lagerbestand
            else
                v_result := nvl(v_lam.menge, 0); -- Wenn Lagerbestand vorhanden, dann aktuelle Menge zurück
    ...
    function lvs_lam_name_get (
        in_artikel_id in number,
        ...
        from
            isi_artikel art
        where
            art.artikel_id = in_artikel_id;

        cursor c_kd_art is -- Lesen des KundenArtikels !!!!
        select
            *
        from
            isi_artikel_kunde kd_art
        where
                kd_art.artikel_id = in_artikel_id
    ...
    -- Erst mal kein Fehler
        ...
        open c_kd_art;
        fetch c_kd_art into v_kd_art; -- KundenArtikel lesen
        if c_kd_art%found then
      -- KundenArtikel gefunden
        ...
        if result is null then
            open c_art;
            fetch c_art into v_art; -- Artikel lesen
            if c_art%found then
        -- Artikel gefunden
  ...
  --------------------------------------------------------------------------------
  --******************************************************************************
    procedure lvs_c_lam_sper_neue_lte_mg (
    ...
    begin
        ...
        if v_lam_bh.menge != in_menge then
            update lvs_lam_bh t
            set
                t.menge = in_menge
            where
            ...
            v_sysdate := sysdate;
            v_menge := bde_c_barcode_buch(v_lam_bh.sid, v_lam_bh.firma_nr, v_lam_bh.lhm_id, v_lam_bh.res_id, in_user_id,
                                          null, null, null, 'BESCHICKEN', null,
                                          null, v_lam_bh.leitzahl, v_lam_bh.fa_ag, v_lam_bh.fa_upos);

            if v_menge != v_lam_bh.menge - in_menge then
                update lvs_lam_bh t
                set
                    t.menge = v_lam_bh.menge - in_menge
                where
  ...
  --------------------------------------------------------------------------------
  --******************************************************************************
    procedure lvs_c_lam_sperren_neue_lte (
        in_lte_id       in lvs_lhm.lte_id%type,
        in_lhm_id       in lvs_lhm.lhm_id%type,
        in_lam_id       in lvs_lam.lam_id%type,
        in_lam_bh_id    in lvs_lam_bh.lam_bh_id%type,
    ...
    -- Erstmal die
        ...
        set
            bh.menge = 0
        where
            bh.lam_bh_id = in_lam_bh_id;

        open c_charge;
        fetch c_charge into v_charge;
        close c_charge;
        v_lte_id := null; -- INIT keine LTE_ID
        v_lte_id := lvs_p_lte.lvs_lte_insert_v358(v_lam_bh.sid, v_lam_bh.firma_nr, v_lte.lte_name, v_lte_id, in_user_id,
                                                  in_lgr_ort, in_lgr_platz, c.lte_bf_stat, null, 'SD',
                                                  v_charge.charge_id, v_charge.charge_bez, v_lam.artikel_id, v_lte.packschema_kopf_id
                                                  , null,                    -- Auto Depal ist unbekannt
                                                  null,                    -- wickelprogramm ist unbekannt,
                                                   null);                   -- wickelprogramm_einl ist unbekannt
        lvs_p_lte.lvs_korr_te_einbuchen(v_lam_bh.sid, v_lam_bh.firma_nr, v_lte_id, null, v_lam_bh.sid,
                                        v_lam_bh.firma_nr, null,  -- in_lgr_ort, -- Wird nicht verwendet
                                         v_lam_bh.lgr_platz, -1, false);

        v_lhm_id := null;
        lvs_komm.lvs_komm_direct(v_lam_bh.sid, v_lam_bh.firma_nr, v_lam_bh.res_id, in_user_id, in_lte_id,
                                 in_lam_id, v_lam_bh.menge, v_lte_id, v_lam_id, v_lhm_id);
        ...
        exception
            when others then
                isi_p_log.c_isi_system_meldung(v_lam_bh.sid, v_lam_bh.firma_nr, 'lvs_lte_drucken', 'LVS', 'Log',
                                               null, null, null, null, null,
                                               'Automatisches Buchen auf Drucker: ('
                                               || in_drucker_name
                                               || ') nicht möglich', '5');
        end;

        if c.c_true = isi_allg.get_firma_cfg_param(v_lam_bh.sid, v_lam_bh.firma_nr, 'LTE_SPERRE', null, 'LVS_LTE_AUSBUCHEN',
                                                   'LVS', 'CFG', 'T', 'BOOLEAN') then
        ...
        commit;
        lvs_c_lam_status(v_lam_id, in_user_id, v_vorg_id, v_status, c.lab_stat_txt_g);
    exception
    -- Im Fehlerfall is der Fehlertext bereits gesetzt.
        when v_error then  -- Update 2011 show Exception Source Line
            rollback;
  ...
  --------------------------------------------------------------------------------
  --******************************************************************************
    procedure lvs_c_lam_status (
        in_lam_id       in lvs_lhm.lte_id%type,
        in_user_id      in isi_user.login_id%type,
    ...
    -- Erstmal den Text aktuallisieren
        update lvs_lam lam
        set
            lam.labor_text = in_labor_text,
            lam.labor_status = in_labor_status
        where
            lam.lam_id = in_lam_id;

        if
            v_lam.labor_status != in_labor_status
            and v_lam.menge != 0
        then
            if
                v_lam.labor_status != 'F' -- Status F ==> Freigegeben
            ...
            if v_lam.menge != 0 then
                ...
                v_err_nr := null;
                v_err_text := null;
                if in_out_vorg_id is null then
                    v_err_nr := 40;
                    v_err_text := lc.ec(lc.o_txt_seq_err);
                    select
                        seq_vorg_id.nextval
  ...
  --------------------------------------------------------------------------------
  --******************************************************************************
    procedure lvs_c_lhm_sperre_abpack (
        in_sid          in isi_sid.sid%type,
        in_firma_nr     in isi_firma.firma_nr%type,
    ...
    begin
        if in_labor_status is null then
            v_labor_status := 'G';
        else
            v_labor_status := in_labor_status;
        end if;

        if in_labor_text is null then
    ...
    -- Vorgang-ID muss dann neu geholt werden (IsNULL)
        ...
        loop
            ...
            close c_art;
            lvs_c_lam_status(v_lam.lam_id, v_scanner_cfg.ls_login_id, in_out_vorg_id, v_labor_status, v_labor_text);
        end loop;
  ...
  --******************************************************************************
    ...
    begin
        lvs_lhm_umpacken(in_sid, in_firma_nr, in_user_id, in_res_id, in_lhm_id,
                         in_lte_id);
        commit;
    end;

    function lvs_c_lhm_drucken_bde (
    ...
    -------------------------------------------------------------------------------------------------------
        ...
        from
            isi_order_pos pos
        where
                pos.vorgang_id = v_lte.order_vorgang_id
    ...
    begin
        ...
        if not v_found then
            v_err_nr := 01;
            v_err_text := lc.ec_p1(lc.o_tp1_lhm_id_fehlt,
                                   nvl(in_lhm_id, 'NULL'));
    ...
    * Abpacken der Packstuecke (LHM)
    -----------------------------------------------------------------------------------------------------------------*/
        ...
        if v_lgr_platz is null then
            v_err_nr := 15;
            v_err_text := lc.ec_p1(lc.o_tp1_lhm_id_n_im_lager,
                                   nvl(in_lhm_id, 'NULL'));
      ...
      -- -AG- 2015.08.10 Buchen in der KOMM_ORDER wenn Auftrag vorhanden
            ...
            set
                t.komm_ist_menge = nvl(t.komm_ist_menge, 0) + v_lam.menge
            where
            ...
            then
                ...
                set
                    pos.ist_menge = pos.ist_menge + ( nvl(v_lam.menge, 0) * - 1 ),
            ...
            if v_lam.order_pos_auf_id is not null then
                if
                    isi_p_order_base.get_order_pos(in_sid, v_lam.order_pos_auf_id, v_order_pos)
        ...
        close c_lam;
        if
            lvs_p_base.get_lte_cfg(v_lte.sid, v_lte.lte_name, v_lte_cfg)
    ...
    * Aufpacken der Packstuecke (LHM) auf eine LTE
    ...
    -- Merken in init
        ...
        loop
            ...
            if nvl(v_lte.min_temp, -1000) < v_art.min_temp then
                v_lte.min_temp := v_art.min_temp; -- Artikel hat höhere MIN Temperatur
            end if;

            if nvl(v_lte.max_temp, 1000) > v_art.max_temp then
            ...
            if v_art.lte_name = v_lte.lte_name -- Wenn der LTE-Typ gleich dem Default im Artikel entspricht
             ...
             then                               -- Dann gibt es evtl. Einen Ueberstand des Palettenmass (In Artikel xx_max definiert)
                if nvl(v_lte.lte_vol_hoehe, 0) < v_art.lte_hoehe_max then
                    v_lte.lte_vol_hoehe := v_art.lte_hoehe_max; -- Artikel kann hoeher werden
                end if;

                if nvl(v_lte.lte_vol_breite, 0) < v_art.lte_breite_max then
                    v_lte.lte_vol_breite := v_art.lte_breite_max; -- Artikel kann breiter werden
                end if;

                if nvl(v_lte.lte_vol_tiefe, 0) < v_art.lte_tiefe_max then
            ...
            if nvl(v_lte.waren_typ, c.leerpal) = c.leerpal then
            ...
            v_lte.lte_voll := 'A';
            if
                v_lte.packschema_kopf_id is not null
                and lvs_p_base.get_packschema_kopf(in_sid, in_firma_nr, v_lte.packschema_kopf_id, v_packschema_kopf)
            then
            ...
            if v_lte.waren_typ != 'MP' then
                if v_lte.packschema_kopf_id is not null then
                    begin
                        v_anz_lagen := round(((v_lte.lte_akt_lhm + 1) / lvs_komm.get_packschema_max_lage(in_sid, in_firma_nr, v_lte.packschema_kopf_id
                        ) + 0.49),
                                             0);
                    exception
                        when others then
                    ...
                    v_lte.lte_vol_hoehe := v_lte_cfg.lte_vol_hoehe + v_anz_lagen * v_lhm.lhm_vol_hoehe;
                else
                    if v_lte_cfg.basis_lte_name = 'LHM'  -- Die Ziel-LTE ist ein Behaelter (LHM)
                     then
                        ...
                        v_lte.lte_vol_hoehe := v_lte_cfg.lte_vol_hoehe + v_lhm.lhm_vol_hoehe;   -- Neue Hoehe rechnen entweder aus der Quelle oder dem Artikel
                    end if;
                end if;

                if nvl(v_lte.lte_vol_hoehe, 0) = 0 then
                    v_lte.lte_vol_hoehe := v_lte_last_hoehe;
                end if;

                begin
                    if v_anz_lagen = 0 then
                        if
                            nvl(v_art.lhm_name, 'keine') != nvl(v_lhm.lhm_name, 'keine')
                            and v_lhm.lhm_name is not null
                        then
                            if not v_lte_cfg_found then
                            ...
                            v_art_lte_lhm_pro_lage := round(((v_lte.lte_vol_breite * v_lte.lte_vol_tiefe) /(v_lhm.lhm_vol_breite * v_lhm.lhm_vol_tiefe
                            )), 0);
                            ...
                            exception
                                when others then
                                    ...
                                    if v_art_lhm_hoehe_lage > nvl(v_lhm.lhm_vol_hoehe, 2000) then
                        ...
                        if v_art.menge_basis = c.basis_lte then
                            ...
                            v_lte_akt_lhm := lvs_p_lte_lhm.lvs_lte_lhm_best(v_lam.sid, v_lam.firma_nr, v_lam.lte_id, 'LTE');
              ...
              -- 08.10.2008 -AG- BugFix: Falsche berechnung der Hoehe, da Klammer fehlte
                            v_anz_lagen := round(((v_lte.lte_akt_lhm + 1) / v_art_lte_lhm_pro_lage) + 0.49, 0);
                ...
                if lvs_p_lte_lhm.lvs_lte_lhm_best(v_lam.sid, v_lam.firma_nr, v_lam.lte_id, 'LTE') >= ( v_art_lte_menge * v_firma.proz_anbruch / 100
                )
                or v_art.waren_typ = c.rohware
                or v_anz_lagen = 0 then
                    if
                        nvl(v_art.lhm_name, 'keine') != nvl(v_lhm.lhm_name, 'keine')
                        and v_lhm.lhm_name is not null
                        ...
                        and v_anz_lagen > 0
                    then
                        v_lte.lte_vol_hoehe := v_lte_cfg.lte_vol_hoehe + v_anz_lagen * v_art_lhm_hoehe_lage;
                    end if;
                    ...
                    if
                        nvl(v_lte_cfg.lte_vol_hoehe_fest, c.c_false) = c.c_false
                        and v_lte.packschema_kopf_id is null
                        and v_anz_lagen > 0
                    then
                        v_lte.lte_vol_hoehe := v_lte_cfg.lte_vol_hoehe + v_anz_lagen * v_art_lhm_hoehe_lage;
            ...
            if v_lte.res_string_statisch is null then
                v_lte.res_string := lvs_util.get_res_string_v359(v_lam.sid, v_lam.firma_nr, v_lte.waren_typ, v_lte.res_artikel_id, v_lam.hersteller_kuerzel_liste
                ,
            ...
            if v_art.gefahren_klasse > nvl(v_lte.gefahren_klasse, -1) then
                v_lte.gefahren_klasse := v_art.gefahren_klasse;
            end if;

            if v_art.wert_klasse > nvl(v_lte.wert_klasse, -1) then
      ...
      -- -AG- Neue LTE-Nr. übergeben
            ...
            s_schnittstelle.write_host_bew(null, v_lam, v_lam_bh_id, c.lam_bh_bus_up, c.lam_bh_umlag,
                                           null, 'UE', v_lgr_q, v_lgr_z, in_user_id);
      ...
      -- ?? Warum vor dem Aufpacken die LTE ID zuweisen?
            update lvs_lam lam
            set
                lam.lte_id = v_lte.lte_id
            where
                lam.lhm_id = v_lhm.lhm_id;
    ...
    -- oder das Rechnen gewünscht ist
        if (
            v_lhm.lhm_name is null
            and c.c_false = isi_allg.get_firma_cfg_param(in_sid, in_firma_nr, 'LTE_CALC_HOEHE_UMPACKEN', null, 'LHM_NULL',
                                                         'LVS', 'CFG', 'T', 'BOOLEAN')
        )
        or c.c_true = isi_allg.get_firma_cfg_param(in_sid, in_firma_nr, 'LTE_CALC_HOEHE_UMPACKEN', null, 'NIE',
        ...
        or v_lgr_z.lgr_typ = c.stap_flae2 then
    ...
    -- Eingebaut für HAG 3.5.11 - Rad miz LTE ID wird in ein gestell gepackt
        update lvs_lam t
        set
            t.res_menge = decode(t.order_pos_auf_id,
                                 null,
                                 null,
                                 nvl(t.res_menge, t.menge))
        where
        ...
        if
            ...
            and c.c_true = isi_allg.get_firma_cfg_param(in_sid, in_firma_nr, 'FAE_ID', null, 'LTE_LHM_UMPACKEN_CHG_LHM_ID',
                                                        'LVS', 'CFG', c.c_false, 'BOOLEAN')
        then
            begin
                update lvs_lam t
                set
            ...
            exception
                when others then         -- Stille Exception mit LOG
                    isi_p_log.isi_system_meldung(v_lam.sid, v_lam.firma_nr, 'lvs_p_lte_lhm.lvs_lhm_umpacken', 'ORA-DB', 'LTE_LHM_UMPACKEN_CHG_LHM_ID'
                    ,
                                                 null, null, null, null, null,
                                                 'Fehler bei FAE_ID' || v_q_lte_id, 'ERR');
            end;
        end if;
  ...
  --******************************************************************************
    ...
    begin
        ...
        -- Leere Paletten haben auch keinen Reservierungsstring
          ...
          -- da die letzte LHM abgepackt werden soll, bleibt nur eine Leerpalette, also Hoehe aus CFG
            ...
            if v_lte.waren_typ != 'MP' then
                if v_lte.packschema_kopf_id is not null then
                    begin
                        v_anz_lagen := round(((v_lte.lte_akt_lhm - 1) / lvs_komm.get_packschema_max_lage(in_lhm.sid, in_lhm.firma_nr,
                        v_lte.packschema_kopf_id) + 0.49),
                                             0);
                    exception
                        when others then
                            v_anz_lagen := 1;
                    ...
                    v_lte.lte_vol_hoehe := v_lte_cfg.lte_vol_hoehe + v_anz_lagen * in_lhm.lhm_vol_hoehe;
                end if;
                ...
                if
                    nvl(in_art.lhm_name, 'keine') != nvl(in_lhm.lhm_name, 'keine')
          ...
          -- LHM Typ geändert
                    ...
                    v_art_lte_lhm_lagen := v_anz_lagen;
                    v_art_lte_lhm_pro_lage := round(((v_lte.lte_vol_breite * v_lte.lte_vol_tiefe) /(in_lhm.lhm_vol_breite * in_lhm.lhm_vol_tiefe
                    )), 0);
          ...
          -- LHM Typ bleibt gleich oder kein LHM Typ gesetzt
            ...
            -- Volumenwerte vom Kunden überschreiben
                    ...
                    if nvl(v_art_lte_lhm_lagen, 0) = 0 then
                        begin
                            if nvl(v_art_lte_lhm_menge / v_art_lte_lhm_lagen, 0) != 0 then
                ...
                if in_art.menge_basis = c.basis_lte then
                    v_art_lte_menge := 1;
                    v_anz_lagen := v_art_lte_lhm_lagen;
                elsif in_art.menge_basis = c.basis_lhm then
                    v_art_lte_menge := v_art_lte_lhm_menge;
                    v_lte_akt_lhm := lvs_p_lte_lhm.lvs_lte_lhm_best(in_lhm.sid, in_lhm.firma_nr, in_lhm.lte_id, 'LTE');
                ...
                if lvs_p_lte_lhm.lvs_lte_lhm_best(in_lhm.sid, in_lhm.firma_nr, in_lhm.lte_id, 'LTE') >= v_art_lte_menge
                or in_art.waren_typ = c.rohware then
                    if
                        nvl(in_art.lhm_name, 'keine') != nvl(in_lhm.lhm_name, 'keine')
                        and in_lhm.lhm_name is not null
                        and nvl(v_lte_cfg.lte_vol_hoehe_fest, c.c_false) = c.c_false
      ...
      -- oder das Rechnen gewünscht ist
            if (
                in_lhm.lhm_name is null
                and c.c_false = isi_allg.get_firma_cfg_param(in_lhm.sid, in_lhm.firma_nr, 'LTE_CALC_HOEHE_UMPACKEN', null, 'LHM_NULL'
                ,
                                                             'LVS', 'CFG', 'T', 'BOOLEAN')
            )
            or c.c_true = isi_allg.get_firma_cfg_param(in_lhm.sid, in_lhm.firma_nr, 'LTE_CALC_HOEHE_UMPACKEN', null, 'NIE',
                                                       'LVS', 'CFG', 'F', 'BOOLEAN')
            ...
            update lvs_lte lte
            set
                lte.lte_akt_kg = nvl(v_lte.lte_akt_kg, 1) - in_lhm.lhm_akt_kg,
    ...
    -- Lagerplatz informieren
        update lvs_lgr lgr
        set
            lgr.lgr_akt_kg = lgr.lgr_akt_kg - in_lhm.lhm_akt_kg,
            lgr.lgr_frei_hoehe = lgr.lgr_frei_hoehe - v_dif_frei_hoehe
        where
            lgr.lgr_platz = in_lhm.lgr_platz;
  ...
  --******************************************************************************
    ...
    begin
        if not lvs_p_base.get_lte_cfg(in_lte.sid, in_lte.lte_name, v_lte_cfg) then
            v_lte_cfg.lte_vol_hoehe_fest := c.c_false;
        end if;

        update lvs_lte lte
        set
            lte.lte_akt_kg = nvl(in_lte.lte_akt_kg, 0) + in_lhm.lhm_akt_kg,
    ...
    -- oder das Rechnen gewünscht ist
        if
            ( in_lhm.lhm_name is not null
              or c.c_true = isi_allg.get_firma_cfg_param(in_lhm.sid, in_lhm.firma_nr, 'LTE_CALC_HOEHE_UMPACKEN', null, 'LHM_NULL',
                                                         'LVS', 'CFG', 'T', 'BOOLEAN') )
            and c.c_false = isi_allg.get_firma_cfg_param(in_lhm.sid, in_lhm.firma_nr, 'LTE_CALC_HOEHE_UMPACKEN', null, 'NIE',
                                                         'LVS', 'CFG', 'F', 'BOOLEAN')
        then
            if v_lte_cfg.lte_vol_hoehe_fest = c.c_false then
        -- -WK- hoehe nur veändern, wenn vol_hoehe nicht fest
                update lvs_lte lte
                set
                    lte.lte_vol_hoehe = in_lte.lte_vol_hoehe
                where
                    lte.lte_id = in_lte.lte_id;

                update lvs_lgr lgr
                set
                    lgr.lgr_frei_hoehe = lgr.lgr_frei_hoehe - nvl(in_dif_frei_h, 0)
                where
        ...
        update lvs_lhm lhm
        set
            lhm.lgr_platz = in_lte.lgr_platz,
            lhm.lte_id = in_lte.lte_id
        where
            lhm.lhm_id = in_lhm.lhm_id;

        if in_lte.packschema_kopf_id is null then
            v_lfdn := 0;
        else
  ...
  --------------------------------------------------------------------------------
  --******************************************************************************
    procedure lvs_c_lte_wieder_einl (
        in_lte_id  in lvs_lte.lte_id%type,
        in_lgr_ort in lvs_lgr_ort.lgr_ort%type,
        in_user_id in isi_user.login_id%type
    ) is
    begin
        lvs_lte_wieder_einl(in_lte_id, in_lgr_ort, in_user_id);
        commit;
...
--******************************************************************************
  ...
  --******************************************************************************
    ...
    begin
        ...
        -- Materialdaten aus der history holen
          ...
          -- 23.02.2008 -AG- In den Abgangsbuchungen werden jetzt auch Lieferscheinnummer und Pos. gespeichert
                    insert into lvs_lam_bh values ( v_lam_bh.sid,
                                                    v_lam_bh.firma_nr,
                                                    v_vorg_id,
                                                    c.lam_bh_zugagng,
                                                    v_lam_bh_id,
  ...
  --------------------------------------------------------------------------------
  --******************************************************************************
    procedure lvs_c_lte_set_artikel (
        in_sid        in isi_sid.sid%type,
        in_firma_nr   in isi_firma.firma_nr%type,
        in_lhm_id     in lvs_lhm.lhm_id%type,
        in_lte_id     in lvs_lte.lte_id%type,
  ...
  --------------------------------------------------------------------------------
    ...
    begin
        ...
        loop
            ...
            update lvs_lam l
            set
                l.artikel_id = v_artikel.artikel_id,
        ...
        update lvs_lte l
        set
            l.res_artikel_id = v_artikel.artikel_id
        where
                l.sid = v_lam.sid
            and l.firma_nr = v_lam.firma_nr
            and l.lte_id = in_lte_id;
  ...
  --******************************************************************************
    ...
    -- 23.02.2008 -AG- In den Abgangsbuchungen werden jetzt auch Lieferscheinnummer und Pos. gespeichert
        insert into lvs_lam_bh values ( v_lam.sid,                  -- SID               VARCHAR2(2) not null,
  ...
  -------------------------------------------------------------------------
    ...
    begin
        ...
        if in_berechnung = c.mhd_berechnen_tag
        or in_berechnung = c.mhd_berechnen_tag_ then
            result := trunc(v_start_date + v_mhd_tage);
        elsif in_berechnung = c.mhd_berechnen_woche_a
        or in_berechnung = c.mhd_berechnen_woche_a_ then
            result := trunc(v_start_date + v_mhd_tage, 'day');
        elsif in_berechnung = c.mhd_berechnen_woche_e
        or in_berechnung = c.mhd_berechnen_woche_e_ then
            result := trunc(v_start_date + v_mhd_tage, 'day') + 6;
        elsif in_berechnung = c.mhd_berechnen_monat_a
        or in_berechnung = c.mhd_berechnen_monat_a_ then
            result := trunc(v_start_date + v_mhd_tage, 'month');
        elsif in_berechnung = c.mhd_berechnen_monat_e
        or in_berechnung = c.mhd_berechnen_monat_e then
            result := trunc(last_day(v_start_date + v_mhd_tage));  -- BW Tag Genau
        elsif in_berechnung = c.mhd_berechnen_jahr_a
        or in_berechnung = c.mhd_berechnen_jahr_a_ then
            result := trunc(v_start_date + v_mhd_tage, 'syyy');
        elsif in_berechnung = c.mhd_berechnen_jahr_e
        or in_berechnung = c.mhd_berechnen_jahr_e_ then
            result := add_months(trunc(v_start_date + v_mhd_tage, 'syyy') - 1,
                                 12);
        else
            result := trunc(v_start_date + v_mhd_tage);
        end if;
  ...
  --------------------------------------------------------------------------------
    ...
    begin
        ...
        close c_lam_bh_anz;
        if v_lam_bh_anz = 1 then
            update lvs_lam l
            set
                l.sonst_id_lieferant = in_sonst_id_lieferant
            where
                    l.sid = v_lam.sid
                and l.firma_nr = v_lam.firma_nr
    ...
    -- 20180203 AG bubfix res_menge
        if v_lam.order_pos_auf_id is not null then
            if v_menge >= v_lam.res_menge then
                v_res_menge_q := null;
    ...
    -- Update des neg. Zugangs (Zuordnung zur alten LAM)
        update lvs_lam_bh l
        set
            l.lam_id = v_lam_bh.lam_id,
            l.lhm_id = v_lam_bh.lhm_id
        where
            l.lam_bh_id = v_lam_bh_id;
  ...
  -----------------------------------------------------------------------------
    procedure lvs_c_teile_lhm_auf_lte (
        in_lte_id           in lvs_lte.lte_id%type,
    ...
    -- LAM Lesen
      ...
      -- Neues LHM jetzt auf den LHM_NAMEN setzen
            update lvs_lhm t
            set
                t.lhm_name = v_lhm_cfg.lhm_name,
                t.lhm_vol_hoehe = v_lhm_cfg.lhm_vol_hoehe,
                t.lhm_vol_breite = v_lhm_cfg.lhm_vol_breite,
                t.lhm_vol_tiefe = v_lhm_cfg.lhm_vol_tiefe
            where
                t.lhm_id = v_neue_lhm_id;

            if in_eti_druck_status = 'SD' then
                update lvs_lhm t
                set
                    t.lhm_eti_druck_status = c.eti_status_soll_drucken
                where
                    t.lhm_id = v_neue_lhm_id;
        ...
        if (
            v_lam.menge < in_lhm_menge
            and in_eti_druck_status = 'RM'
        )
        or in_eti_druck_status = 'SD' then
      -- Etikett nur bei Restmenge drucken
            update lvs_lhm t
            set
                t.lhm_eti_druck_status = decode(t.lhm_eti_druck_status, c.eti_status_gedruckt, c.eti_status_neu_drucken, c.eti_status_soll_drucken
                )
            where
                t.lhm_id = v_lam.lhm_id;
        ...
        update lvs_lte t
        set
            t.lte_eti_druck_status = decode(t.lte_eti_druck_status, c.eti_status_gedruckt, c.eti_status_neu_drucken, c.eti_status_soll_drucken
            ),
    ...
    -- !!!                              in Modul ISI_UTILS.spez_barcode_gen
        ...
        while ( v_start_pos <= length(in_parameter_wert) + v_add_length ) loop
            if substr(in_parameter_wert, v_start_pos - v_add_length, 1) = '?' then
                null; -- Hier ist nicht fie ISIPlus zu tun
            elsif substr(in_parameter_wert, v_start_pos - v_add_length, 1) = 'A' then -- -AG- Artikel ist ein String und hat keine Leerzeichen
                if substr(in_barcode, v_start_pos, 1) != ' ' then
                    out_artikel := out_artikel
                                   || substr(in_barcode, v_start_pos, 1);
                end if;
            elsif substr(in_parameter_wert, v_start_pos - v_add_length, 1) = 'C' then
                out_charge := out_charge
                              || substr(in_barcode, v_start_pos, 1);
            elsif substr(in_parameter_wert, v_start_pos - v_add_length, 6) = 'PDMMYY' then
                out_prod_datum_str := substr(in_barcode, v_start_pos, 6);
                out_prod_datum_struktur := 'DDMMYY';
                v_start_pos := v_start_pos + 5;
            elsif substr(in_parameter_wert, v_start_pos - v_add_length, 5) = 'EAN06' then
                out_ean := substr(in_barcode, v_start_pos, 6);
                v_add_length := v_add_length + 1;
                v_start_pos := v_start_pos + 5;
            elsif substr(in_parameter_wert, v_start_pos - v_add_length, 5) = 'EAN13' then
                out_ean := substr(in_barcode, v_start_pos, 13);
                v_add_length := v_add_length + 8;
                v_start_pos := v_start_pos + 12;
            elsif substr(in_parameter_wert, v_start_pos - v_add_length, 5) = 'EAN14' then
                out_ean := substr(in_barcode, v_start_pos, 14);
                v_add_length := v_add_length + 9;
                v_start_pos := v_start_pos + 13;
            elsif substr(in_parameter_wert, v_start_pos - v_add_length, 1) = 'P' then
                out_lfd_nr_str := out_lfd_nr_str
                                  || substr(in_barcode, v_start_pos, 1);
            elsif substr(in_parameter_wert, v_start_pos - v_add_length, 1) = '_' then
                out_lfd_nr_str := out_lfd_nr_str
                                  || substr(in_barcode, v_start_pos, 1);
            elsif substr(in_parameter_wert, v_start_pos - v_add_length, 1) = 'M' then
                out_menge_str := out_menge_str
                                 || substr(in_barcode, v_start_pos, 1);
            elsif substr(in_parameter_wert, v_start_pos - v_add_length, 1) = 'm' then
                out_menge_str := out_menge_str
                                 || ','
                                 || substr(in_barcode, v_start_pos, 1);
            elsif substr(in_parameter_wert, v_start_pos - v_add_length, 1) = 'L' then
                out_linie_str := out_linie_str
                                 || substr(in_barcode, v_start_pos, 1);
            end if;
    ...
    -------------------------------------------------------------------------------------------------------
        ...
        from
            isi_artikel a
        where
            a.artikel_kurz = out_artikel.artikel;
    ...
    -- Sollen führende 0 aus dem Artikel gelöscht werden
        ...
        if length(out_artikel.artikel) = 0 then
            v_err_nr := 10;
    ...
    -- -AG- funktion nutzen und Artikeldaten lesen
        if not isi_p_base.get_isi_artikel_by_nr(out_artikel.artikel, v_artikel) then
      -- Gab es einen alten barcode mit anderer Struktur für die alte Artikelnummer
            v_parameter := isi_allg.c_get_firma_cfg_param(in_sid, in_firma_nr, 'CFG',                                     -- in_kategorie             in isi_firma_cfg.kategorie%type,
    ...
    --      Aber nur bis zur minimalen Laenge
        while
            length(out_charge) > isi_allg.c_get_firma_cfg_param(in_sid, in_firma_nr, 'CFG',                 -- in_kategorie             in isi_firma_cfg.kategorie%type,
             null,                  -- in_kategorie_ix          in isi_firma_cfg.kategorie_ix%type,
      ...
      -- -AG- Ist das erste zeichen das Fuellzeichen?
            and substr(out_charge, 1, 1) = isi_allg.c_get_firma_cfg_param(in_sid, in_firma_nr, 'CFG',                 -- in_kategorie             in isi_firma_cfg.kategorie%type,
             null,                  -- in_kategorie_ix          in isi_firma_cfg.kategorie_ix%type,
      ...
      -- v_err_text := 'Menge Fehlt';
        ...
        if length(v_prod_datum_str) = 0 then
            v_err_nr := 40;
    ...
    -- v_err_text := 'Fehler im Datum';
        out_prod_datum := to_date ( v_prod_datum_str,
                                    v_prod_datum_struktur );
    exception
    -- Im Fehlerfall is der Fehlertext bereits gesetzt.
        when v_error then  -- Update 2011 show Exception Source Line
            rollback;
    ...
    function insert_lhm_aus_barcode (
        in_sid            in isi_sid.sid%type,
        in_firma_nr       in isi_firma.firma_nr%type,
        in_barcode        in lvs_lam.lhm_id%type,
        in_parameter_wert in isi_firma_cfg.parameter_wert%type,
    ...
    -------------------------------------------------------------------------------------------------------
        ...
        from
            lvs_lam l
        where
            l.lte_id = v_lte_barcode;

    begin
        spez_barcode_result(in_sid, in_firma_nr, in_barcode,              -- in  lvs_lam.lhm_id%type,
         in_parameter_wert,      -- in  isi_firma_cfg.parameter_wert%type,
    ...
    -- -AG- Wenn noch Keine LTE_ID dann Gen wie früher
        if v_lte_barcode is null then
            v_lte_barcode := v_artikel.artikel
                             || v_charge
                             || nvl(v_lfd_nr_str, '');
        end if;
  ...
  */
    procedure unite_lams_without_reservation (
  ...
  --CMe 20230405 Laborstatus berücksichtigen
        ...
        where
            ...
            and ( in_consider_mhd = 'T'
                  and lam.lam_mhd >= in_min_mhd )
    ...
    begin
        ...
        loop
            ...
            loop
                ...
                if ( nvl(v_source_lam.menge, 0) <> 0 ) then
                    v_source_lam_bh_kg := ( nvl(v_source_lam.lam_kg, 0) * nvl(v_source_lam.res_menge, 0) ) / nvl(v_source_lam.menge, 0
                    );
...
/*
  ...
  06.04.2023   DB31_4      (-CMe-)  Laborstatus berücksichtigen (W23910-343)
  */
    procedure add_amount_to_reservation (
        in_sid          in lvs_lam_bh.sid%type,
        in_firma_nr     in lvs_lam_bh.firma_nr%type,
        in_lte_id       in lvs_lte.lte_id%type,
    ...
    begin
        ...
        if ( v_found ) then
            ...
            loop
                ...
                if ( nvl(v_s_lam.menge, 0) <> 0 ) then
                    v_s_lam_bh_kg := ( nvl(v_s_lam.lam_kg, 0) * nvl(v_s_lam.menge, 0) ) / nvl(v_s_lam.menge, 0);
...
-- sqlcl_snapshot {"hash":"85b6b492ad7f140228781b3091e46acae488da91","type":"PACKAGE_BODY","name":"LVS_P_LTE_LHM","schemaName":"DIRKSPZM32","sxml":""}