create or replace editionable trigger dirkspzm32.tr_s_rcv_kunden_auftr_pos_biu before
    insert or update on dirkspzm32.s_rcv_kunden_auftr_pos
    for each row
declare
    v_error exception;
    v_err_nr           number;
...
begin
  -- sid
    ...
    -- 20200727 - LHa - Nur löschen wenn der Auftrag noch Neu ist
        if
            v_found
            and v_kunden_auftr_pos.status = 'N'
        then
            delete tms_kunden_auftr_pos t
            where
                    t.kunden_auftr_pos_id = :old.kunden_auftr_pos_id
                and t.sid = :new.sid;
    ...
    if not v_found then
        ...
        v_err_text := lc.ec_p1(lc.o_tp1_txt_adresse_nf,
                               nvl(:new.adr_nr,
                                   'NULL')); -- 'Fehler: Adressdaten fehlen für Nummer <' || nvl(:new.adr_nr, 'NULL') || '>';
        raise v_error;
    end if;
    ...
    if not v_found then
        v_err_nr := 20;
        v_err_text := lc.ec_p1(lc.o_tp1_artikel_fehlt,
                               nvl(:new.artikel,
                                   'NULL')); -- 'Fehler: Artikeldaten fehlen für Artikel <' || nvl(:new.artikel, 'NULL') || '>';
        raise v_error;
    end if;
    ...
    v_found := c_kunden_auftr_pos%found;
    close c_kunden_auftr_pos;
    if :new.kunden_auftr_pos_id is null then
    ...
    if not v_found then
        if :new.kunden_auftr_pos_id is null then
            select
                seq_s_rcv_kunden_auftr_pos.nextval
            into v_kunden_auftr_pos.kunden_auftr_pos_id
            from
                dual;
        ...
        if
            ...
            and v_kunden_auftr_pos.status != 'N'
        then
            if ( :new.soll_menge != :old.soll_menge ) then
    ...
    set
        t.adress_id_liefer = v_adress_id_li,
        ...
        t.strategie = nvl(:new.strategie,
                          v_neu_std_strateg), -- STRATEGIE         VARCHAR2(10),
        ...
        t.besteller = nvl(:new.besteller,
                          t.besteller),                 -- BESTELLER         VARCHAR2(10),
        t.freigabe = nvl(:new.freigabe,
                         'M'),-- FREIGABE          CHAR(1),
...
-- sqlcl_snapshot {"hash":"dea9adb93eb44ee5f892a1f961bec87195e908e3","type":"TRIGGER","name":"TR_S_RCV_KUNDEN_AUFTR_POS_BIU","schemaName":"DIRKSPZM32","sxml":""}