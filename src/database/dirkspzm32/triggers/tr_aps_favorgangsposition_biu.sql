...
declare
  ...
  CURSOR c_res_fa is
    select r.*
      from bde_fa_auftrag_res_liste t,
           isi_resource r
      where t.sid = v_fa.sid
  ...
  CURSOR c_planparam is
    select max (t.dauerstabilerzeitraum)
      from aps_planparameter t;
...
begin
  ...
  if bde_p_base.get_fa_ag(NULL, NULL, to_number(substr(:new.fakopfnr, 3)), to_number(:new.favorgangsnr), :new.favorgangssplittnr, v_fa)
  then
    if (  (:new.typ = 2 and v_fa.satzart = 'VR')                      -- Rüsten nur beim Rüsten eintragen
       or (:new.typ = 1 and v_fa.satzart = 'V')                       -- Fertigen nur beim Fertigen eintragen
       )
    then
      v_res := NULL;
      v_res_fa := NULL;
      ...
      or    (:new.plan_res_name is not NULL
        and  isi_p_base.get_resource_by_name(:new.plan_res_name, v_res))
      then
        OPEN c_planparam;
        FETCH c_planparam into v_dauerstabilerzeitraum;
        CLOSE c_planparam;
        if sysdate + v_dauerstabilerzeitraum > to_date(:new.planstart, 'yyyymmdd hh24:mi:ss')
        or isi_allg.c_get_firma_cfg_param (v_fa.sid,
                                           v_fa.firma_nr,
                                           'APS_ERP',             -- in_kategorie             in isi_firma_cfg.kategorie%type,
        ...
        then
          update bde_fa_auftrag fa
             set fa.res_id = nvl(v_res_fa.res_id, v_res.res_id),
          ...
          update bde_fa_auftrag fa
             set fa.termin_start_gepl = to_date(:new.planstart, 'yyyymmdd hh24:mi:ss'),
                 fa.termin_ende_gepl = to_date(:new.planende, 'yyyymmdd hh24:mi:ss')
           where fa.sid = v_fa.sid
             and fa.firma_nr = v_fa.firma_nr
             and fa.leitzahl = v_fa.leitzahl
