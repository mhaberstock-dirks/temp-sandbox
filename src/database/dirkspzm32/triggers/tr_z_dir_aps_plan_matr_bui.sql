...
declare
  ...
  CURSOR c_aps_auftrag_pos is
    select * 
     ...
     where op.auftrag_nr = v_aps_auftrag_mr.auftrag_nr
  ...
  CURSOR c_aps_auftrag_mr is
    select * 
     ...
     where mr.materialrelation_type = 2
       and mr.child_id = :new.aps_plan_auftrag_nr
       and mr.aps_plan_status in ('PS', 'PE');
       
  CURSOR c_aps_pe is
    select * 
      from aps_plan_ergebnis pe
     where pe.aps_plan_auftrag_nr = :new.aps_plan_auftrag_nr
...
begin
  ...
  then
    ...
    if isi_p_base.get_isi_artikel(:new.sid, nvl(:new.child_artikel_id, v_aps_pe.artikel_id), v_artikel)
    ...
    then
      ...
      if v_aps_pe.leitzahl is NULL
      then
        if nvl(v_aps_pe.abnr, '00000000') = 00000000
        then
          ...
          or (v_aps_pe.abnr is NULL and v_aps_auftrag_pos.order_info not in ('M100'))
          or (v_aps_auftrag_pos.order_info in ('L100') and v_aps_auftrag_pos.prioritaet = 4)
          then
            update aps_order_materialrelation mr
               set mr.materialrelation_info = 'Keine Planung (Batch) für Planauftrag ' || 
                                              :new.aps_plan_auftrag_nr ||
                                              ' nicht gedeckt. Kundenauftrag ' || 
                                              v_aps_auftrag_mr.auftrag_nr || '/' ||
                                              v_aps_auftrag_mr.pos_nr || '-' ||
                                              v_aps_auftrag_mr.upos_nr || ' Status ' ||
                                              nvl(v_aps_auftrag_pos.order_info, 'Fehlt'),
                   mr.materialrelation_valide = 'I'
             where mr.auftrag_nr = v_aps_auftrag_mr.auftrag_nr
            ...
            update aps_order_materialrelation mr
               set mr.materialrelation_info = 'Keine Planung (Batch) für Planauftrag PRIO 4' || 
                   mr.materialrelation_info,
                   mr.materialrelation_valide = 'I'
             where mr.auftrag_nr = v_aps_auftrag_mr.auftrag_nr
               and mr.pos_nr = v_aps_auftrag_mr.pos_nr
               and mr.upos_nr = v_aps_auftrag_mr.upos_nr
            ...
            if :new.child_id = '-1'
            or :new.materialrelation_valide = 'I'
            then
              :new.materialrelation_info := 'Invalide Planung Rohstoff Lagerbestand Artikel ' || 
                                             v_artikel.waren_gruppe || ' ' || v_artikel.artikel || 
              ...
              update aps_order_materialrelation mr
                 set mr.materialrelation_info = 'Invalide Planung Rohstoff Lagerbestand Artikel ' || 
                                             v_artikel.waren_gruppe || ' ' || v_artikel.artikel || 
                                            ' ist nicht gedeckt. Kundenauftrag ' || 
                                            v_aps_auftrag_mr.auftrag_nr || '/' ||
                                            v_aps_auftrag_mr.pos_nr || '-' ||
              ...
              if nvl(:new.child_lte_id, '-1') = '-1'
              and :new.materialrelation_type = 8
              then
                :new.child_lte_id := :new.child_id;
              end if;
              update aps_order_materialrelation mr
                 set mr.materialrelation_info = 'Valide Planung Position ist mit dem Planauftrag ' || 
                                                :new.aps_plan_auftrag_nr ||
                                                ' gedeckt',
          ...
          then
            ...
            update aps_plan_ergebnis pe
               set pe.status = 1,
                   pe.fehler_text =  'Valide Planung Planauftrag: ' || :new.aps_plan_auftrag_nr ||
            ...
            :new.materialrelation_valide := 'F';
            update aps_order_materialrelation mr
               set mr.materialrelation_info = 'Invalide Planung Planauftrag: ' || 
                                              :new.aps_plan_auftrag_nr ||
        ...
        :new.materialrelation_info := 'Valide Planung Rohstoff Lagerbestand Artikel ' || 
                                       v_artikel.waren_gruppe || ' ' || v_artikel.artikel || 
                                       ' durch FA ' || v_aps_pe.leitzahl || ' reserviert und damit gedeckt';
        :new.materialrelation_valide := 'T';
        :new.materialrelation_type := 8;
        :new.child_id := 'FA_RES';
        :new.child_lte_id := 'FA_RES';
        update aps_order_materialrelation mr
           set mr.materialrelation_info = 'Valide Planung Position ist mit dem Fertigungsauftrag ' || 
                                          v_aps_pe.leitzahl ||
                                          ' gedeckt',
               mr.materialrelation_valide = 'T'
         where mr.auftrag_nr = v_aps_auftrag_mr.auftrag_nr
