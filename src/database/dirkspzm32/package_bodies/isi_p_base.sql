 ...
 function get_isi_artikel_by_nr(in_artikel_nr in  isi_artikel.artikel%type,
  ...
  function get_resource_by_ext_name(in_res_ext_name  in  isi_resource.res_ext_name%type,
    ...
    cursor c_resource is
      select t.*
        from isi_resource t
       where t.res_ext_name = in_res_ext_name;
  begin
    out_resource := null;
  ...
  function get_artikel_ctrl(in_sid           in  isi_sid.sid%type,
    ...
    cursor c_artikel_ctrl is
      select t.*
        from isi_artikel_ctrl t
       where t.sid = in_sid
         and t.artikel_id = in_artikel_id
         and (t.zeichnung = in_zeichnung or in_zeichnung is null)
         and (t.zeichnung_index = in_zindex or (in_zindex is null and t.zeichnung_index is null))
         and (t.leitzahl = in_leitzahl or t.leitzahl is null)
         and (t.fa_ag = in_fa_ag or t.fa_ag is null);
  begin
    open c_artikel_ctrl;
  ...
  function get_hersteller(in_herstelle_k   in  isi_hersteller.kuerzel%type,
    ...
    cursor c_hersteller is
      select t.*
        from isi_hersteller t
       where t.kuerzel = in_herstelle_k;
  begin
    open c_hersteller;
  ...
  begin
    ...
    then
      ...
      or v_adr.lte_lhm_nummernkreis_bis <= v_adr.lte_lhm_nummernkreis_aktuell
      then
        raise_isi_error(10, LC.ec(LC.O_TXT_ID_LEANGENUEBERLAUF));
      end if;

      update isi_adressen t
         set t.lte_lhm_nummernkreis_aktuell = t.lte_lhm_nummernkreis_aktuell + 1
       where t.sid = in_sid
         and t.adress_id = in_adress_id
