...
package body DIRKSPZM32.bde_rueckverfolg is
  ...
  **************************************************************************************************************************/
  function rueckverfolg_create(in_sid         in isi_sid.sid%type,
    ...
    CURSOR c_pd_prod_start is
      select pd.*
        from bde_pd_prod pd
       where pd.vorg_id = v_pd_prod_vorg_id
  ...
  begin
    ...
    v_max_rueck := isi_allg.c_get_firma_cfg_param(in_sid,
                                                  in_firma_nr,
    ...
    if in_artikel_id is NULL and in_leitzahl is NULL and in_lhm_id is NULL and
       in_lte_id is NULL and in_order_li_nr is null then
      v_err_nr   := 1;
      v_err_text := 'Keine Selektion für die Abfrage eingegeben!';
      raise v_error;
    end if;

    if in_charge_bez is NULL and in_leitzahl is NULL and in_lhm_id is NULL and
       in_lte_id is NULL and in_order_li_nr is null then
      v_err_nr   := 2;
    ...
    LOOP
      ...
      v_lam_letzte_id   := 0;
      if v_pd_prod_vorg_id is NULL and v_pd_prod_vorg_id_l = 0 then
        ...
        loop
          ...
          v_lam_bh_up       := find_lam_bh_buch(in_sid, --            in isi_sid.sid%type,
                                                ...
                                                'UP',
                                                null,
                                                in_order_li_nr); --    in bde_pd_prod.vorg_typ%type
          v_l_lam_letzte_id := v_lam_letzte_id;
          v_lam_letzte_id   := 0;
          if not v_lam_bh_up.lam_id is NULL then
            v_zeile     := nvl(v_zeile, 0) + 1;
            v_l_zeile   := v_zeile;
            v_lam       := ret_lam_v(in_sid,
                                     in_firma_nr,
                                     v_lam_bh_up.lam_id);
            v_lam_bh    := ret_lam_bh_v(in_sid,
                                        in_firma_nr,
                                        v_lam_bh_up.lam_id);
            v_anz_rueck := v_anz_rueck + 1;
            if v_anz_rueck > v_max_rueck then
              v_err_nr   := 5;
              v_err_text := 'Ergebnissmenge zu groß, bitte genauer einschrenken. Maximale Anzahl ist (MAX_RUECKVERFOLGUNG): ' ||
                            to_char(v_max_rueck);
              raise v_error;
            end if;
            ...
            v_lam_next          := ret_lam_v(in_sid,
                                             in_firma_nr,
                                             v_lam_bh_next.lam_id);
            if v_abfr_id is NULL then
              select SEQ_BDE_PD_RUECKVERFOLGUNG.nextval
                into v_abfr_id
                from dual;
              insert into bde_pd_rueckverf_sel
              values
                (in_sid,
                 in_firma_nr,
                 sysdate,
            ...
            insert into bde_pd_rueckverfolgung
            values
    ...
    update bde_pd_rueckverf_sel t set t.abfr_enddatum = sysdate where t.abfr_id = v_abfr_id;
  ...
  **************************************************************************************************************************/
  procedure rueckverfolg_weiter(in_sid        in isi_sid.sid%type,
                                ...
                                in_abfr_id    in bde_pd_rueckverfolgung.abfr_id%type) is
    -------------------------------------------------------------------------------------------------------
    -- Standard Fehler Felder für Exception
    -------------------------------------------------------------------------------------------------------
    v_error EXCEPTION;
  ...
  **************************************************************************************************************************/
  function vorwaerts_create(in_sid         in isi_sid.sid%type,
    ...
    CURSOR c_pd_prod_start is
      select pd.*
        from bde_pd_prod pd
       where pd.vorg_id = v_pd_prod_vorg_id
       order by pd.vorg_typ, pd.vorg_id;
  ...
  begin
    ...
    if in_artikel_id is NULL and in_leitzahl is NULL and in_lhm_id is NULL and
       in_lte_id is NULL and in_li_nr_lief is null then
      v_err_nr   := 1;
      v_err_text := 'Keine Selektion für die Abfrage eingegeben!';
      raise v_error;
    end if;
    ...
    if in_charge_bez is not NULL and in_artikel_id is NULL then
      v_err_nr   := 3;
      ...
      if v_pd_prod_vorg_id is NULL and v_pd_prod_vorg_id_l = 0 then
        ...
        loop
          ...
          v_lam_bh_up       := find_lam_bh_buch(in_sid, --            in isi_sid.sid%type,
                                                ...
                                                'UP',
                                                in_li_nr_lief,
                                                null); --    in bde_pd_prod.vorg_typ%type
          v_l_lam_letzte_id := v_lam_letzte_id;
          v_lam_letzte_id   := 0;
          if not v_lam_bh_up.lam_id is NULL then
            v_zeile     := nvl(v_zeile, 0) + 1;
            ...
            if v_b_lam_letzte_id != v_pd_prod.lam_id then
              ...
              values
                (in_sid, -- SID                 VARCHAR2(2) not null,
                 ...
                 null); -- QUELL_LHM_ID        VARCHAR2(20)
                 if v_lam_bh.bus = c.R_LAM_BH_BUS_ZUG_KOMM then
                   get_komm_lte_by_vorgang_id(in_sid,
                                              in_firma_nr,
                                              v_lam_bh.vorg_id,
                                              v_zeile,
                                              v_abfr_id);
  ...
  **************************************************************************************************************************/
  function find_lam_bh_buch(in_sid          in isi_sid.sid%type,
    ...
    CURSOR c_charge is
      select *
        from lvs_charge ch
       where ch.charge_bez = in_charge_bez
  ...
  begin
    ...
    if in_li_nr_lief is not null then
      v_sql_querry := 'select bh.* from lvs_v_lam lam left join lvs_v_lam_bh bh on bh.LAM_ID = lam.LAM_ID where bh.bus = decode(''' ||
                      in_vorg_type || ''', ''PP'', ' || c.R_LAM_BH_BUS_ZUG ||
                      ', ''UP'', ' || c.R_LAM_BH_BUS_UP || ', ' ||
                      c.R_LAM_BH_BUS_ABG || ') ';
    else
    ...
    if in_lhm_id is not NULL then
      v_sql_querry := v_sql_querry || ' and bh.lhm_id = ''' || in_lhm_id || '''';
      v_add_where := true;
    end if;

    if in_lte_id is not NULL then
      v_sql_querry := v_sql_querry || ' and bh.lte_id = ''' || in_lte_id || '''';
      v_add_where := true;
    end if;

    if in_leitzahl is not NULL then
      v_sql_querry := v_sql_querry || ' and bh.leitzahl = ' || in_leitzahl;
    ...
    if in_charge_bez is not NULL then
      if in_artikel_id is not NULL then
        v_sql_querry := v_sql_querry || ' and bh.artikel_id = ' ||
                        in_artikel_id;
      end if;
      OPEN c_charge;
      FETCH c_charge
        into v_charge;
      v_sql_querry := v_sql_querry || ' and bh.charge_id in (' ||
                      v_charge.charge_id;
      LOOP
        FETCH c_charge
          into v_charge;
        EXIT when c_charge%NOTFOUND;
        v_sql_querry := v_sql_querry || ' ,' || v_charge.charge_id;
      end LOOP;
      CLOSE c_charge;
    ...
    if in_li_nr_lief is not null then
      if v_add_where = true then
        v_sql_querry := v_sql_querry || ' and (lam.li_nr_lief = ''' || in_li_nr_lief || ''' or lam.li_nr_lief is null)';
      else
        v_sql_querry := v_sql_querry || ' and lam.li_nr_lief = ''' || in_li_nr_lief || '''';
      end if;
      v_add_where := true;
    end if;
    ...
    if in_order_li_nr is not null then
      if v_add_where = true then
        v_sql_querry := v_sql_querry || ' and (li.li_nr = ' || in_order_li_nr ||' or li.li_nr is null)';
      else
        v_sql_querry := v_sql_querry || ' and li.li_nr = ' || in_order_li_nr;
      end if;

      v_add_where := true;
    end if;

    if v_lam_letzte_id is not NULL and v_lam_letzte_id <> 0 then
       v_sql_querry := v_sql_querry || ' and nvl(bh.lam_id, ' ||
                       v_lam_letzte_id || ') > ' || v_lam_letzte_id;
        v_add_where := true;
    end if;

    if in_lte_id is not NULL or
       in_lhm_id is not null then
  ...
  **************************************************************************************************************************/
  ...
  begin
    ...
    LOOP
      ...
      if in_out_lam_bh.lam_id is NULL or in_vorg_type != 'PB' then
      ...
      if in_out_lam_bh.lam_id is not NULL then
        ...
        if in_test_for_abgang then
          in_out_lam_bh := ret_lam_bh_v_abgang(in_sid,
                                               in_firma_nr,
                                               in_lte_id,
                                               in_lhm_id,
                                               v_lam_id,
      ...
      if v_pd_prod.vorg_id is NULL and in_vorg_type = 'PB' and
         in_out_lam_bh.lam_id is not NULL then
  ...
  **************************************************************************************************************************/
  function ret_lam_bh_v(in_sid      in isi_sid.sid%type,
    ...
    CURSOR c_lam_bh_v is
      select /*+ INDEX(bh.IX_LAM_BH_LAM_ID lvs_lam_bh) index(bh.IX_LAM_BH_LAM_ID_hist lvs_lam_bh_hist) */
       ...
       where bh.lam_id = in_lam_id
         and (bh.bus = c.R_LAM_BH_BUS_ZUG or
             bh.bus = c.R_LAM_BH_BUS_ZUG_KOMM or
             bh.bus = c.R_LAM_BH_BUS_UP or
             bh.bus = c.R_LAM_BH_BUS_ZUG_KONSI);
  begin
    v_lam_bh := NULL;
    OPEN c_lam_bh_v;
    FETCH c_lam_bh_v
  ...
  function ret_lam_id_root(in_sid    in isi_sid.sid%type,
    ...
    Cursor c_bde_prod_pb is
      select *
        from bde_pd_prod t
       where t.vorg_id = v_bde_prod.vorg_id
         and t.vorg_typ = 'PB';

    Cursor c_lam is
      select * from lvs_lam t where t.lam_id = v_bde_prod_pb.lam_id;
  begin
  ...
  procedure get_komm_lte_by_vorgang_id(in_sid        in isi_sid.sid%type,
    ...
    Cursor c_komm_lte_lam_bh is
      select bh.*
        from lvs_lam_bh bh
       where bh.vorg_id = in_vorgang_id
  ...
  procedure get_pick_lte_by_vorgang_id(in_sid        in isi_sid.sid%type,
                                       ...
                                       in_put_lte_id in lvs_lam_bh.lte_id%type) is
    -------------------------------------------------------------------------------------------------------
    -- Standard Fehler Felder für Exception
    -------------------------------------------------------------------------------------------------------
    v_error EXCEPTION;
  ...
  begin
    if in_vorgang_id is not null and in_put_lte_id is not null then
      open c_pick_lte_lam_bh;
      fetch c_pick_lte_lam_bh
        into v_lam_bh;
 ...
 **************************************************************************************************************************/
      ...
      select bh.LTE_ID,
        ...
        from lvs_v_lam_bh bh,
             (select x.VORG_ID,
                     x.LTE_ID
                from lvs_v_lam_bh x
               where (x.LTE_ID = nvl(in_lte_id, 0) or
                      x.LAM_ID = nvl(in_lhm_id, 0))
                 and (x.bus = c.R_LAM_BH_BUS_ABG or
                      x.bus = c.R_LAM_BH_BUS_ABG_KOMM or
                      x.bus = c.R_LAM_BH_BUS_UP))y
       where bh.VORG_ID = y.VORG_ID
         and bh.LTE_ID <> y.LTE_ID
         and bh.LAM_ID > nvl(v_lam_id, 0)
         and (bh.bus = c.R_LAM_BH_BUS_ZUG or
              bh.bus = c.R_LAM_BH_BUS_ZUG_KOMM or
              bh.bus = c.R_LAM_BH_BUS_UP)
       order by bh.LAM_ID asc;
  ...
  begin
    ...
    LOOP
      OPEN c_lam_bh_v;
      FETCH c_lam_bh_v into v_lte_id,
                            v_lam_id;
