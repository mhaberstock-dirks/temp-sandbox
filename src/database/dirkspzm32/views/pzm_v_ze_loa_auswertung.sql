create or replace force editionable view dirkspzm32.pzm_v_ze_loa_auswertung (
    union_order,
    data_src,
    pers_nr,
    ts_datum,
    ...
    select
        ...
        decode(ts_ueb_ok_datum,
               ...
               decode(ts_ueb_storno_datum,
                      null,
                      nvl(ts_day_ueb_std, 0), -- genehmigte Überstunden
                      0))                                              ts_day_ueb_std,
        decode(ts_ueb_ok_datum,
               null,
               0,
               decode(ts_ueb_storno_datum,
                      null,
                      nvl(ts_day_flex_std, 0), -- genehmigte Flexi-Stunden
                      0))                                              ts_day_flex_std,
        ( decode(ts_ueb_ok_datum,
                 null,
                 0,
                 decode(ts_ueb_storno_datum,
                        null,
                        nvl(ts_day_ueb_std, 0) + nvl(ts_day_flex_std, 0), -- genehmigte Überstunden und Flexi-Stunden
                        0)) + nvl(ts_day_arb_std, 0) + nvl(ts_day_korr_std, 0) ) ts_ges_arb_std,
        (
            select
...
-- ERP/HOST (Lohnbuchhaltungssoftware) Datensätze, die erst am Monatsende verfügbar sind
    select
        ...
        nvl(loa_exp.kst_id,
            get_pers_kst_id(loa_exp.pers_nr)) kst_id,
        null                                  sa_kurzname,
        null                                  aa_id,
        null                                  ze_ist_start,
        null                                  ze_ist_ende,
...
-- sqlcl_snapshot {"hash":"a716fad92b5b68aab6a2e6665447f2df67e4d7cf","type":"VIEW","name":"PZM_V_ZE_LOA_AUSWERTUNG","schemaName":"DIRKSPZM32","sxml":""}