...
package body DIRKSPZM32.term_lvs_utils is
  ...
  */
   ...
   * public functions
   ******************************************************************************************************/
  function check_storage_cell_valid(in_scan_data             in varchar2,
    ...
    cursor c_storage_cells is
      select min(t.lgr_platz) lgr_platz,
             stradd_distinct(decode((t.lgr_max_te - t.lgr_akt_te),
                                     0, null,
                                     decode(t.lte_namen,
                                            'Keine', null,
                                            t.lgr_platz))) verf_platz_csv,
             t.lgr_platz_gruppe
        from lvs_lgr t,
             (select x.*
                from lvs_lgr x
               where x.lgr_platz like in_scan_data || '%') t1
       where t.lgr_platz_gruppe = t1.lgr_platz_gruppe
         and t.lgr_dim_g = t1.lgr_dim_g
         and t.lgr_dim_r = t1.lgr_dim_r
         and t.lgr_dim_p = t1.lgr_dim_p
         and t.lgr_dim_e = t1.lgr_dim_e
       group by t.lgr_platz_gruppe;

    cursor c_storage_cells_valid is
      select t.column_value
        from table(strsplit(v_verf_lgr_platz_csv, ';')) t
    ...
    cursor c_storage_cells_alt is
      select /*+ INDEX(ix_lvs_lgr_platz_o_punkte) lvs_lgr */
             t.lgr_platz
        from lvs_lgr t
       where replace(t.lgr_platz, '.') like in_scan_data;
  begin
    v_result := 0;
    out_found_storage_cell := null;
    open c_storage_cells;
    fetch c_storage_cells into v_lgr_platz, v_verf_lgr_platz_csv, v_lgr_platz_gruppe;
    if c_storage_cells%found
    then
      ...
      if v_result = 1
      then
        if in_expected_storage_cell is not null
           and ';' || v_verf_lgr_platz_csv || ';' like '%;' || in_expected_storage_cell || ';%'
        then
          ...
          if not c_storage_cells_valid%found
          then
            v_result := 0;
            out_found_storage_cell := null;
          end if;
    ...
    if out_found_storage_cell is null
    then
