...
TYPE BODY DIRKSPZM32."PIVOTIMPL" as
  ...
  static function ODCITableDescribe( rtype out anytype,
                                     p_stmt in varchar2,
                                     p_fmt in varchar2 := 'upper(@p@)', dummy in number )
  return number
  is
    atyp anytype;
    cur integer;
...
dbms_output.put_line( t_stmt );
    ...
    loop
      ...
      case type_code
        ...
        when dbms_types.typecode_number        then dbms_sql.define_column( sctx.cur, i, to_number(NULL));
        when dbms_types.typecode_date          then dbms_sql.define_column( sctx.cur, i, to_date(NULL));
        when dbms_types.typecode_timestamp     then dbms_sql.define_column( sctx.cur, i, to_timestamp(NULL) );
        when dbms_types.typecode_timestamp_tz  then dbms_sql.define_column( sctx.cur, i, to_timestamp_tz(NULL) );
        when dbms_types.typecode_timestamp_ltz then dbms_sql.define_column( sctx.cur, i, to_timestamp_tz(NULL) );
