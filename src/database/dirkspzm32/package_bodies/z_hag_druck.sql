  ...
  */
  ...
  function vda_etikett(in_sid         in isi_sid.sid%type,
    ...
    CURSOR c_lte_all_lam is
      select nvl(lpad(to_char(op.auftrag),7, ' '), '         ') || '   ' || 
             nvl(lpad(to_char(op.best_nr_kunde),6, ' '), '      ') || '   ' ||
       ...
       where  lte.sid = lam.sid (+)
        ...
        group by op.li_nr, a.artikel, op.auftrag, op.best_nr_kunde, a.bezeichnung1
        order by op.li_nr, a.artikel;
  ...
  begin
    ...
    if lvs_p_lte_lhm.lvs_lte_lhm_ref(in_sid, in_firma_nr, in_id) = C.Basis_Lte then
      ...
      OPEN c_pos_lte_all;
      FETCH c_pos_lte_all into v_best_nr_kunde, v_SeqVon, v_SeqBis, v_HAGTurmBehaelterNr, v_HAGTurmBarcode, v_lam_order_pos_auf_id;
      ...
      OPEN c_pos_lhm_all;
      FETCH c_pos_lhm_all into v_best_nr_kunde, v_SeqVon, v_SeqBis, v_HAGTurmBehaelterNr, v_HAGTurmBarcode, v_lam_order_pos_auf_id;
    ...
    if v_found then
      ...
      elsif v_order_pos_auf_id is not NULL
      then
        OPEN c_fa;
        FETCH c_fa into v_fa_auftrag;
        if c_fa%FOUND
        then
          v_Vdasachnrlieferant := v_Vdasachnrlieferant || '-' || nvl(to_char(v_fa_auftrag.leitzahl), 'X') || '/' || to_char(v_fa_auftrag.fa_ag);
        end if;
        CLOSE c_fa;
      end if;
      ...
      if not v_found then
        ...
        v_HAGTurmBarcode := v_adr.kundennrlief || '-' || v_HAGTurmBehaelterNr || '-' || v_HAGTurmBarcode || '-000'; 
      else
        OPEN c_kopf;
   ...
   v_print_daten :=
    ...
    'SeqNrVon='                                           || v_SeqVon             || CHR(13) || CHR(10) ||
    'SeqNrBis='                                           || v_SeqBis             || CHR(13) || CHR(10) ||
    'HAGTurmBehaelterNr='                                 || v_HAGTurmBehaelterNr || CHR(13) || CHR(10) ||
    'HAGTurmBarcode='                                     || v_HAGTurmBarcode     || CHR(13) || CHR(10) ||
