create or replace package body dirkspzm32.isi_p_license is
  -- Initialisierung
  ...
  -------------------------------------------------------------------------------------------------------
    ...
    function check_license_key_valid (
        in_isi_firma_cfg isi_firma_cfg%rowtype
    ) return varchar2 is
        v_result     varchar2(1);
    ...
    begin
        ...
        if v_key = in_isi_firma_cfg.license_key then
            v_result := c.c_true;
        else
    ...
    procedure c_update_system_info (
        ...
        in_license_ticket_id in number
    ) is
    ...
    begin
        ...
        if c_system_info%found then
            update isi_system_info si
            set
                si.last_license_ticket_ts = in_license_ts,
    ...
    begin
        if in_license_type not in ( c_license_type_app_count ) then
        ...
        if c_isi_firma_cfg%found then
            if check_license_key_valid(v_isi_firma_cfg) = c.c_true then
                if in_license_type = c_license_type_app_count then
                    ...
                    loop
                        ...
                        if v_value is not null then
                            if lower(v_param) = 'int-max-app-count' then
                                v_max_app_count := to_number ( v_value );
                            elsif lower(v_param) = 'int-lic-valid-sek' then
                    ...
                    if v_max_app_count is null then
            -- Keine Lizenzierung installiert
                    ...
                    if v_license_valid then
                        select
                            seq_license_ticket_id.nextval
                        into out_license_ticket_id
                        from
                            dual;

                        out_license_ts := sysdate;
                        c_update_system_info(in_sid, in_firma_nr, in_app_exe_filename, in_hostname, in_os_username,
                                             out_license_ts, out_license_valid_sek, out_license_ticket_id);
...
-- sqlcl_snapshot {"hash":"d43aef6b9da9b8f7c4b4ff9ce671dafbe4b62590","type":"PACKAGE_BODY","name":"ISI_P_LICENSE","schemaName":"DIRKSPZM32","sxml":""}