create or replace function dirkspzm32.bde_pruefe_rohstoffe
/*
...
*/ (
    ...
    from
        bde_fa_auftrag
    where
            bde_fa_auftrag.sid = in_fa_auftrag.sid
        ...
        and ( ( ( bde_fa_auftrag.fa_ag < in_fa_auftrag.fa_ag
            ...
            group by
              ...
              or ( bde_fa_auftrag.fa_ag = in_fa_auftrag.fa_ag
                   and bde_fa_auftrag.satzart = 'MA' ) );
    ...
    if in_lhm_id is not null then
        ...
        loop
            ...
            if v_found then
                open c_fa_artikel;
                loop
                    fetch c_fa_artikel into
                        v_artikel_id,
                        v_leitzahl,
                        v_fa_ag;
                    v_found := c_fa_artikel%found;
                    if
                        nvl(v_artikel_id, v_lam.artikel_id) -- Dummy MA im Auftrag, dann kann alles verwendet werden
                         = v_lam.artikel_id
                        and (
                            nvl(v_leitzahl, -1) = nvl(v_lam.leitzahl, -1)
...
-- sqlcl_snapshot {"hash":"0589b400fcb8bd4552eb16221a2c948ac0bc2bf1","type":"FUNCTION","name":"BDE_PRUEFE_ROHSTOFFE","schemaName":"DIRKSPZM32","sxml":""}