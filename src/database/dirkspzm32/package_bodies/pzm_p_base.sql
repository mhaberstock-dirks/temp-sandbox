create or replace package body dirkspzm32.pzm_p_base is
  ...
  -------------------------------------------------------------------------------------------------------
    function get_schicht_modell (
        ...
        where
            ...
            and sm.sm_name = nvl(
                nvl(p.pers_sm_name, k.kst_standard_sm_name),
                a.abt_standard_sm_name
            );
  ...
  --
  -------------------------------------------------------------------------------------------------------
    function get_schichtart (
        in_sa_name     in pzm_schichtarten.sa_name%type,
  ...
  --
  -------------------------------------------------------------------------------------------------------
    function get_schichtart_by_uix (
  ...
  --
  -------------------------------------------------------------------------------------------------------
    function get_lohnart_by_alternative_lz_id (
        in_lz_id    in pzm_lohnarten.lz_id%type,
        out_lohnart out pzm_lohnarten%rowtype
    ) return boolean is
        v_result boolean;
        ...
        from
            pzm_lohnarten t
        where
            t.lz_alternativ_loa_id = in_lz_id;
  ...
  -------------------------------------------------------------------------------------------------------
    ...
    function get_allg_parameter_mandant (
        ...
        select
            ap_value
        from
            pzm_allg_parameter p
        where
                lower(p.ap_name) = lower(in_param_name)
            and nvl(p.ap_pb_id, in_pb_id) = in_pb_id
        order by
            decode(p.ap_pb_id, null, 1, 0); -- Der korrekte produktionsbereich / mandant zuerst
    begin
        open c_allgparam;
...
-- sqlcl_snapshot {"hash":"bf247d1eda870b57647d71e90b8accb0bf3c8a29","type":"PACKAGE_BODY","name":"PZM_P_BASE","schemaName":"DIRKSPZM32","sxml":""}