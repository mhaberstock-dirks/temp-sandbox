create or replace function dirkspzm32.is_schichtart_in_time
/*
  Modul: PZM
  Kommentare:
  ...
  --v_ToleranzMax := 1;
    ...
    if c_schichtart%found then
      ...
      -- Rahmenzeit bei Gleitzeit berücksichhtigen (Kernzeit wird nicht für Berechnungen genutzt)
            ...
            if p_stempel_zeit > p_sa_beginn then
      ...
      -- auf den Tag von p_schicht_datum umrechnen
            ...
            p_sa_std := v_schichtart.sa_std_pro_tag;
            if p_sa_ende <= p_sa_beginn then -- wenn das Ende vor dem Beginn liegt
                p_sa_ende := p_sa_ende + 1; -- nächster Tag
            elsif p_sa_beginn < trunc(p_sa_beginn) + 4 / 24 then
        ...
        -- aber immer noch für den aktuellen Schichttag
            ...
            if p_stempel_zeit between ( p_sa_beginn - ( v_toleranzmin / 24 ) ) and ( p_sa_ende - ( ( v_toleranzmin * 2 ) / 24 ) ) then
                v_result := 1; -- gefunden!!
            end if;

            if p_stempel_zeit = trunc(p_stempel_zeit) then
    ...
    close c_schichtart;
    if
        nvl(p_sm_d_arb_std_tag, 0) > 0
        and p_sa_kurzname != pzm_utils.get_standard_schicht_by_calc_basis(v_schichtart.calc_basis)
        and nvl(p_sa_std, 1) != 0
    then
...
-- sqlcl_snapshot {"hash":"83d7815c69ced8d7338bd1d0ee0b41e1c1222b50","type":"FUNCTION","name":"IS_SCHICHTART_IN_TIME","schemaName":"DIRKSPZM32","sxml":""}