create or replace package body dirkspzm32.tms_p_loading is
  ...
  */
   ...
   * public functions
   ******************************************************************************************************/
    procedure c_activate_loading (
        in_vorgang_id in tms_loading_points.order_vorgang_id%type,
        ...
        from
            lvs_lte t
        where
            t.order_vorgang_id = in_vorgang_id
        order by
            (
                select
                    max(tr.transp_id) -- die letzte transport id für diese LTE ist maßgeblich für die transportreihenfolge zum tor
                from
                    isi_transport tr
                where
                    tr.lte_id = t.lte_id
        ...
        order by
            t.transp_id desc; -- immer den letzten Transport holen
    begin
        if not tms_p_base.get_loading_point(in_lgr_platz, in_firma_nr, in_sid, v_loading_point) then
            raise_isi_error(10,
                            lc.ec_p1('O_TP1_TMS_LP_NA', in_lgr_platz));
        end if;

        if v_loading_point.status = 'FR' then
            raise_isi_error(20,
                            lc.ec_p1('O_TP1_TMS_LP_VEHIC_N_ASSGN', in_lgr_platz));
        end if;

        if v_loading_point.status = 'CL' then
            raise_isi_error(30,
                            lc.ec_p1('O_TP1_TMS_LP_IS_CLOSED', in_lgr_platz));
        end if;

        if v_loading_point.status != 'RES' then
            raise_isi_error(40,
                            lc.ec_p1('O_TP1_TMS_LP_IS_OCCUPIED', in_lgr_platz));
        end if;
        ...
        loop
            ...
            if not isi_p_order_base.get_order_pos(in_sid, v_lte.order_auf_id, v_order_p) then
                v_order_p := null;
            end if;

            if not isi_p_order_base.get_order_kopf(v_order_p.vorgang_id, v_order_p.vorgang_typ, v_order_p.li_nr, v_order_p.firma_nr, v_order_p.sid
            ,
                                                   v_order_k) then
                v_order_k := null;
            end if;
            ...
            if c_transp_by_lte%notfound then
                v_transport := null;
            end if;
            close c_transp_by_lte;
            if
                nvl(v_transport.lgr_platz_ziel, v_lte.lgr_platz) is not null
        ...
        -- wegen umgekehrter Verladereihenfolge mit -1 multiplizieren
                ...
                v_transp_result := lvs_transport.lvs_transp_lte(in_sid,
                                                                ...
                                                                0,
                                                                0,
                                                                0, -- in_progr_nr, in_quelle_leer_progr_nr, in_ziel_voll_progr_nr,
                                                                ...
                                                                nvl(v_transport.kunden_nr,
                                                                    nvl(v_order_k.adress_id, 0)), -- in_kunde_nr,
                ...
                if not v_transport.transp_id is null then
          -- es ist noch ein Transport aktiv, also Staffeltransport sperren
                    v_transp_result := lvs_transport.transport_sperren(in_sid, in_firma_nr, in_login_id, v_trans_id);
                end if;
  ...
  --------------------------------------------------------------------------------
  --******************************************************************************
    procedure c_dup_order_kopf_by_auf_id (
    ...
    begin
        v_err_nr := null;
        v_found := isi_p_order_base.get_order_pos(
            in_sid       => in_sid,
            in_auf_id    => in_auf_id,
      ...
      -- v_err_text := 'Ordernummer ' || nvl(to_char(v_transport.auf_id), '(NULL)') || ' konnte nicht gefunden werden.';
        ...
        tms_p_loading.c_dup_order_kopf(
            in_sid                => in_sid,
  ...
  --******************************************************************************
    ...
    begin
        v_err_nr := null;
        v_found := isi_p_order_base.get_order_kopf(
      ...
      --v_err_text := 'Fehler: Kopfdaten für Tour: ' || v_pos.vorgang_id || ' Lieferschein ' || v_pos.li_nr || ' nicht vorhanden!';
        ...
        if in_out_new_vorgang_id is null then
            select
                seq_tms_order_vorgang_id.nextval
            into in_out_new_vorgang_id
            from
                dual;

        end if;
        if in_out_new_li_nr is null then
            select
                seq_tms_order_li_nr.nextval
            into in_out_new_li_nr
            from
                dual;
        ...
        if in_out_new_vorgang_id is not null then
            begin
  ...
  --******************************************************************************
    ...
    begin
        v_err_nr := null;
        v_found := isi_p_order_base.get_order_kopf(
            in_vorgang_id  => in_new_vorgang_id,
            in_vorgang_typ => null,
      ...
      -- v_err_text := 'Ordernummer ' || nvl(to_char(v_transport.auf_id), '(NULL)') || ' konnte nicht gefunden werden.';
        ...
        from
            isi_order_pos x
        where
                x.auftrag = v_order_pos_row.auftrag
        ...
        from
            isi_order_pos x
        where
            x.vorgang_id = in_new_vorgang_id;
        ...
        insert into isi_order_pos
            select
                ...
                op.artikel_id,
                decode(v_order_kopf_row.status, 'N', 'F', 'V', 'F',
                       'T'), -- ware_disponiert
                in_user_id,
                op.arbeitsplatz_id,
                op.leitzahl,
  ...
  */
  --******************************************************************************
    function chk_order_kopf_by_li_nr_vor_id (
        in_sid        in isi_sid.sid%type,
        in_firma_nr   in isi_firma.firma_nr%type,
        in_vorgang_id in isi_order_kopf.vorgang_id%type,
        in_li_nr      in isi_order_kopf.li_nr%type
        ...
        v_order_kopf_row isi_order_kopf%rowtype;
    begin
        return isi_p_order_base.get_order_kopf(
            in_vorgang_id  => in_vorgang_id,
            in_vorgang_typ => null,
            in_li_nr       => in_li_nr,
            in_firma_nr    => in_firma_nr,
  ...
  */
  --******************************************************************************
    function chk_order_pos_by_li_nr_artikel (
        in_sid        in isi_sid.sid%type,
        in_firma_nr   in isi_firma.firma_nr%type,
        ...
        v_order_pos_row isi_order_pos%rowtype;
    begin
        return isi_p_order_base.get_order_pos_by_li_nr_artikel(
            in_sid            => in_sid,
            in_firma_nr       => in_firma_nr,
...
-- sqlcl_snapshot {"hash":"586fe02b17415bcb7294bb0ac8438f248dbe3329","type":"PACKAGE_BODY","name":"TMS_P_LOADING","schemaName":"DIRKSPZM32","sxml":""}