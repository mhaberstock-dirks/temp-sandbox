...
package body DIRKSPZM32.bde_p_base is
  ...
  --
  -------------------------------------------------------------------------------------------------------
  function get_fa_kopf(in_sid          in  bde_fa_kopf.sid%type,
                       in_firma_nr     in  bde_fa_kopf.firma_nr%type,
                       in_fa_nr        in  bde_fa_kopf.fa_nr%type,
                       out_bde_fa_kopf out bde_fa_kopf%rowtype) return boolean is
    v_found boolean;

    cursor c_bde_fa_kopf is
      select t.*
        from bde_fa_kopf t
       where t.fa_nr = in_fa_nr;
  begin
    open c_bde_fa_kopf;
  ...
  --
  -------------------------------------------------------------------------------------------------------
  function get_fa_ag(in_sid        in  bde_fa_auftrag.sid%type,
                     ...
                     out_bde_fa_ag out bde_fa_auftrag%rowtype) return boolean is
    v_found boolean;

    cursor c_bde_fa_ag is
      select t.*
        from bde_fa_auftrag t
       where t.leitzahl = in_fa_nr
  ...
  --
  -------------------------------------------------------------------------------------------------------
  function get_fa_by_auf_id(in_sid        in  bde_fa_auftrag.sid%type,
                            in_firma_nr   in  bde_fa_auftrag.firma_nr%type,
  ...
  --
  -------------------------------------------------------------------------------------------------------
  function get_pd_kopf(in_sid            in  bde_pd_kopf.sid%type,
  ...
  --
  -------------------------------------------------------------------------------------------------------
  function get_pd_pers_zeit_kst(in_sid                     in  bde_pd_pers_zeit_kst.sid%type,
  ...
  procedure c_set_fa_ag_mg(in_sid               in  bde_fa_auftrag.sid%type,
                        in_firma_nr           in  bde_fa_auftrag.firma_nr%type,
                        in_fa_nr              in  bde_fa_auftrag.leitzahl%type,
                        in_fa_ag              in  bde_fa_auftrag.fa_ag%type,
                        in_fa_upos            in  bde_fa_auftrag.fa_upos%type,
  ...
  begin
    if get_fa_ag(in_sid, in_firma_nr, in_fa_nr, in_fa_ag, in_fa_upos, v_bde_fa_auftrag)
    then
      update bde_pd_prod t
       set t.menge_a = t.menge_a + nvl((in_ag_ist_mg - v_bde_fa_auftrag.ag_ist_mg), t.menge_a),
      ...
      if nvl(in_ag_ist_mg, v_bde_fa_auftrag.ag_ist_mg) >= v_bde_fa_auftrag.ag_soll_mg
      then
      ...
      update bde_fa_auftrag t
         set t.ag_ist_mg          = nvl(in_ag_ist_mg, t.ag_ist_mg),
      ...
      RAISE_APPLICATION_ERROR(-20010,
                               lc.ec_p3(lc.o_tp3_fa_auftrg_fehlt,
                                        to_char(in_fa_nr),
                                        to_char(in_fa_ag),
                                        to_char(in_fa_upos)), true);
    end if;
    commit;
  end;

  function get_bde_pd_prozess_data(in_sid                          in  bde_pd_prozess_data.sid%type,
  ...
  procedure c_bde_pd_set_aktiv_ag (in_sid         in isi_sid.sid%type,
  ...
  begin
    ...
    if (v_res.typ = 'MS'
       ...
       or v_res.typ = 'MPG')
    then
      if v_fa_akt.satzart = 'MA'
      then
        v_err_nr   := 10;
        v_err_text := 'Auf der Res. ' || v_res.res_ext_name || ' kann FA Auftrag:' ||
                      in_leitzahl || '/' || in_fa_ag || '/' ||
                      nvl(in_fa_upos, 0) || ' nicht aktiviert werden. Bitte Auftrag korrekt anmelden';
        raise v_error;
      end if;
    end if;
    if  v_res.typ != 'MS'
    ...
    then
      ...
      then
        ...
        v_err_text := 'Auf der Res. ' || v_res.res_ext_name || ' kann FA Auftrag:' ||
                      in_leitzahl || '/' || in_fa_ag || '/' ||
                      nvl(in_fa_upos, 0) || ' nicht aktiviert werden, der der Arbeitgang keine Materialanforderung ist. Bitte Auftrag korrekt anmelden';
        raise v_error;
      end if;
    end if;
  ...
  procedure c_bde_pd_set_deaktiv_ag (in_sid         in isi_sid.sid%type,
                                     in_firma_nr    in isi_firma.firma_nr%type,
                                     in_res_id      in isi_resource.res_id%type,
                                     in_akt_term    in isi_arbeitsplatz.ip_name%type,
                                     in_ls_login_id in isi_user.login_id%type)
  ...
  begin
    ...
    then
      v_err_nr   := 10;
      v_err_text := 'Auf der Res. ' || v_res.res_ext_name || ' kann nicht deaktiviert werden. Bitte Auftrag korrekt anmelden';
      raise v_error;
    end if;
  ...
  procedure c_bde_pd_set_reset_ag (in_sid         in isi_sid.sid%type,
                                   in_firma_nr    in isi_firma.firma_nr%type,
                                   in_leitzahl    in bde_fa_auftrag.leitzahl%type,
                                   in_fa_ag       in bde_fa_auftrag.fa_ag%type,
                                   in_fa_upos     in bde_fa_auftrag.fa_upos%type,
    ...
    CURSOR c_resource is
      select *
        from isi_resource t
       where t.res_id = nvl(v_fa_akt.res_id, in_res_id);
  ...
  begin
    ...
    then
      ...
      then
        ...
        v_err_text := 'Auf der Res. ' || v_res.res_ext_name || ' kann FA Auftrag:' ||
                      in_leitzahl || '/' || in_fa_ag || '/' ||
                      nvl(in_fa_upos, 0) || ' nicht deaktiviert werden. Bitte Auftrag korrekt anmelden';
        raise v_error;
      end if;
    end if;
    if  v_res.typ != 'MS'
    ...
    then
      ...
      then
        ...
        v_err_text := 'Auf der Res. ' || v_res.res_ext_name || ' kann FA Auftrag:' ||
                      in_leitzahl || '/' || in_fa_ag || '/' ||
                      nvl(in_fa_upos, 0) || ' nicht deaktiviert werden, der der Arbeitgang keine Materialanforderung ist. Bitte Auftrag korrekt anmelden';
        raise v_error;
      end if;
    end if;
  ...
  function bde_pd_check_aktiv_ag(in_sid         in isi_sid.sid%type,
                                 in_firma_nr    in isi_firma.firma_nr%type,
                                 in_leitzahl    in bde_fa_auftrag.leitzahl%type,
                                 in_fa_ag       in bde_fa_auftrag.fa_ag%type,
                                 in_fa_upos     in bde_fa_auftrag.fa_upos%type)
  ...
  --
  -------------------------------------------------------------------------------------------------------
  function get_pd_prod_by_lam_id(in_sid          in  bde_pd_prod.sid%type,
  ...
  -------------------------------------------------------------------------------------------------------
  function get_leitzahl_by_lte_id (in_sid                 in lvs_lte.sid%type,
  ...
  begin
    ...
    or not lvs_p_base.get_lam_by_lte_id(in_sid, in_firma_nr, in_lte_id, v_lam)
    then
    ...
    if not bde_p_base.get_pd_prod_by_lam_id(in_sid, in_firma_nr, v_lam.lam_id, v_bde_pd_prod)
    then
      if bde_p_base.get_fa_by_auf_id (in_sid, in_firma_nr, v_lam.order_pos_auf_id, v_bde_fa)
      then
