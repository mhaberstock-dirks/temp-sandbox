...
package body DIRKSPZM32.res_schicht is
 ...
 -------------------------------------------------------------------------------------------------------
  ...
  function check_schicht_vorhanden(in_start_date in date,
                                   in_end_date in date,
                                   in_isi_res_id in isi_resource.res_id%type) return boolean is
  ...
  cursor c_schicht  is
      ...
      from
        isi_Res_schicht t
      where
        ((t.start_date_time between in_start_date and  in_end_date) or
         (t.ende_date_time between  in_start_date and in_end_date) or
         (in_end_date between t.start_date_time and t.ende_date_time) or
         (in_start_date between t.start_date_time and t.ende_date_time)) and
        t.ende_date_time <> in_start_date and
        t.start_date_time <> in_end_date and
        t.res_id = in_isi_res_id;
  begin
    OPEN c_schicht;
  ...
  procedure erzeuge_isi_res_schicht(in_start_date in date,
                                   in_end_date in date,
                                   ...
                                   in_schicht_ende_variabel in isi_res_schicht_modell.schicht_ende_variabel%type) is
  ...
  cursor c_schicht_modell_schicht is
    select t.*
      from isi_res_schicht_modell t
     where t.schicht_modell_id = v_schicht_modell_id
           and t.schicht_modell_type = 'S';-- 'S'chicht,

  cursor c_schicht_modell_pause is
    select t.*
      from isi_res_schicht_modell t
     where t.parent_id = v_schicht_modell_id
...
begin
  ...
  if not v_found_schicht then
    v_err_nr := 10;
    v_err_text := 'Fehler Schichtmodel <' || in_schicht_modell_id || '> nicht gefunden.';
    RAISE v_error;
  end if;
  ...
  --end if;
  for i in to_number(to_char(in_start_date ,'j')) .. to_number(to_char(in_end_Date ,'j'))
    loop
      v_curr_date := to_date(to_char(i),'j');
      ----------------------------
      -- 1. Passt der Tag zu den gï¿½ltigen Wochentagen ?
      ----------------------------
      v_wochentag  := substr(to_char(v_curr_date, 'FMday', 'NLS_DATE_LANGUAGE = german'), 1, 2); -- mo, di,mi,do,fr,sa,so
      if InStr(lower(v_modell_schicht.wochen_tage), v_wochentag) > 0 then

        v_soll_takte_pro_schicht := 1; -- ToDo: Berechnen!!!
        v_soll_stk_pro_schicht   := 1; -- ToDo: Berechnen!!!
        v_Start_Date := trunc(v_curr_date) +
                        EXTRACT( HOUR FROM CAST(v_modell_schicht.start_time AS TIMESTAMP ) ) / 24 +
                        EXTRACT( MINUTE FROM CAST(v_modell_schicht.start_time AS TIMESTAMP ) ) / 24 / 60 +
                        EXTRACT( SECOND FROM CAST(v_modell_schicht.start_time AS TIMESTAMP ) ) / 24 / 60 / 60;
        v_Ende_Date := trunc(v_curr_date) +
                        EXTRACT( HOUR FROM CAST(v_modell_schicht.Ende_time AS TIMESTAMP ) ) / 24 +
                        EXTRACT( MINUTE FROM CAST(v_modell_schicht.Ende_time AS TIMESTAMP ) ) / 24 / 60 +
                        EXTRACT( SECOND FROM CAST(v_modell_schicht.Ende_time AS TIMESTAMP ) ) / 24 / 60 / 60;
        if EXTRACT( HOUR FROM CAST(v_modell_schicht.Start_time AS TIMESTAMP ) ) >
           EXTRACT( HOUR FROM CAST(v_modell_schicht.Ende_time AS TIMESTAMP ) ) then
        ...
        if check_schicht_vorhanden(v_Start_Date, v_Ende_Date, in_isi_res_id) then
          v_err_nr := 11;
          v_err_text := 'Fehler: Schicht schon vorhanden am Datum <' || to_char(v_Start_Date,'dd.mm.yyyy hh24:mi')|| '>.';
        ...
        insert into isi_Res_schicht(schicht_id,
                                  parent_id,
                                  sid,
                                  firma_nr,
                                  schicht_type,
        ...
        loop
          ...
          if v_found_pause then
            v_Start_Date := trunc(v_curr_date) +
                         EXTRACT( HOUR FROM CAST(v_modell_pause.start_time AS TIMESTAMP ) ) / 24 +
                         EXTRACT( MINUTE FROM CAST(v_modell_pause.start_time AS TIMESTAMP ) ) / 24 / 60 +
                         EXTRACT( SECOND FROM CAST(v_modell_pause.start_time AS TIMESTAMP ) ) / 24 / 60 / 60;

            v_Ende_Date := trunc(v_curr_date) +
                         EXTRACT( HOUR FROM CAST(v_modell_pause.Ende_time AS TIMESTAMP ) ) / 24 +
                         EXTRACT( MINUTE FROM CAST(v_modell_pause.Ende_time AS TIMESTAMP ) ) / 24 / 60 +
                         EXTRACT( SECOND FROM CAST(v_modell_pause.Ende_time AS TIMESTAMP ) ) / 24 / 60 / 60;
            if EXTRACT( HOUR FROM CAST(v_modell_schicht.Start_time AS TIMESTAMP ) ) >
               EXTRACT( HOUR FROM CAST(v_modell_pause.start_time AS TIMESTAMP ) ) then
              v_start_Date := v_start_Date + 1;   -- start der pause ist am Folge Tag
            end if;
            if EXTRACT( HOUR FROM CAST(v_modell_schicht.Start_time AS TIMESTAMP ) ) >
               EXTRACT( HOUR FROM CAST(v_modell_pause.Ende_time AS TIMESTAMP ) ) then
...
procedure c_erzeuge_isi_res_schicht(in_start_date in date,
                                                   in_end_date in date,
                                                   in_isi_res_id in isi_resource.res_id%type,
                                                   in_schicht_modell_id in isi_res_schicht_modell.schicht_modell_id%type,
                                                   in_takt_zeit in isi_res_schicht_modell.takt_zeit_sek%type,
...
begin
  erzeuge_isi_res_schicht(in_start_date,
                         in_end_date,
                         in_isi_res_id,
...
procedure loesche_isi_res_schicht(in_schicht_id in isi_res_schicht.schicht_id%type) is
begin
...
procedure c_loesche_isi_res_schicht(in_schicht_id in isi_res_schicht.schicht_id%type) is
begin
  loesche_isi_res_schicht(in_schicht_id);
  commit;
end;

function differenz_nur_zeit_in_sekunden(in_start_zeit date,
                                        in_ende_zeit date)  return integer is
v_start number;
v_ende number;
...
procedure c_setze_schicht_status(in_schicht_id in isi_res_schicht.schicht_id%type,
                               in_status in isi_res_schicht.status%type) is
...
procedure schicht_modell_zeiten(in_schicht_modell_id in isi_res_schicht_modell.schicht_modell_id%type,
                                    out_schichtzeit_sekunden out integer,
                                    out_schicht_pausen_sekunden out integer) is
...
cursor c_schicht_modell_zeiten is
  ...
  (select
    ...
    where
      t.schicht_modell_id = in_schicht_modell_id and
      t.schicht_modell_type = 'S'
  ...
  (select
     ...
     from
       isi_Res_schicht_modell t
    where
      t.Parent_id> = in_schicht_modell_id and
      t.schicht_modell_type = 'P'
    group by
      schicht_modell_type
...
begin
  ...
  fetch c_schicht_modell_zeiten into out_schichtzeit_sekunden, out_schicht_pausen_sekunden;
