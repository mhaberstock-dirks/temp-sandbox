...
function ccg_etikett(in_lte_id      in lvs_lte.lte_id%type,
  ...
  cursor c_et_daten is
    select
       ...
       '(02)' || lpad(nvl(art.lhm_ean, '0'),14,'0') || '(37)' || lpad(lam.menge, 6, '0') || '(20)' || substr(art.artikel, length(art.artikel) - 2 + 1) as "BC5_Text",
       '(15)' || to_char(last_day(lam.lam_mhd_ausgabe), 'YYMMDD')  || '(10)' || cha.charge_bez  as "BC6_Text",
       ...
       '02' || lpad(nvl(art.lhm_ean, '0'),14,'0') || CHR(29) || '37' || lpad(lam.menge, 6, '0') || CHR(29) || '20' || substr(art.artikel, length(art.artikel) - 2 + 1) as "BC5",
...
-- MMa: Projekt 70340-2017 Etikettenaenderung
       '15' || to_char(last_day(lam.lam_mhd_ausgabe), 'YYMMDD') || CHR(29)  || '10' || cha.charge_bez  as "BC6_Text"
...
begin
  ...
  str_out := str_out || '-' || substr(str_code, 2, 2);
  str_out := str_out || '-' || substr(str_code, 4, 6);
  return(str_out);
end;
