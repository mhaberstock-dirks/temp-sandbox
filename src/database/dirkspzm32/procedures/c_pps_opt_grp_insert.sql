...
procedure DIRKSPZM32.C_PPS_OPT_GRP_INSERT is
  ...
  cursor c_auftr_pos is
   select s.*
     from s_rcv_kunden_auftr_pos s
    where s.lam_sel1 is not null
       or s.lam_sel3 is not null;

  cursor c_opt_grp_lam_sel1 is
   select *
     from pps_ruestmatrix_opt_grp p
    where p.opt_grp = v_auftr_pos.lam_sel1;

  cursor c_opt_grp_lam_sel3 is
   select *
     from pps_ruestmatrix_opt_grp p
    where p.opt_grp = v_auftr_pos.lam_sel3;

  cursor c_opt_grp_von is
   select distinct (p.opt_grp)
     from pps_ruestmatrix_opt_grp p
    where p.opt_name = 'Rezepte';
...
begin
  ...
  loop
    ...
    if not v_found and v_auftr_pos.lam_sel1 is not null then
    ...
    if not v_found and v_auftr_pos.lam_sel3 is not null then
      insert into pps_ruestmatrix_opt_grp
      values
        ('01',
        1,
  ...
  loop
    ...
    loop
      ...
      if v_opt_grp_von != v_opt_grp_nach then
        ...
        if not v_found then
          insert into pps_ruestmatrixelement
          values
            ('01',
            1,
            'Rezepte',
