create or replace procedure dirkspzm32.bde_pd_prod_p_pa_u
/*
...
*/ (
    ...
    in_akt_abfuell in isi_resource_zust_akt.abfuell_ist%type
) is
  ...
  -- Erst mal kein Fehler
    ...
    if v_found then                       -- Daten gefunden !!
     ...
     -- 26.02.2017   3.5.10.1    (-AG-)   Bei automatischer Rückmeldung und zwei Linien auf der gleichen RES in einer SEK fuehr das zu Problemen    
        ...
        set
            pd.menge_a = nvl(v_prod_pa.menge_a, 0) + in_menge,
            pd.abfuell_ist = nvl(pd.abfuell_ist, 0) + in_akt_abfuell
        where
                pd.sid = in_sid
            and pd.vorg_typ = 'PA'
            and pd.firma_nr = in_firma_nr
            and pd.leitzahl = in_leitzahl
            and pd.fa_ag = in_fa_ag
            and nvl(pd.fa_upos, 0) = nvl(in_fa_upos, 0)
            and pd.res_id = in_res_id
            and
            -- pd.prod_beginn <= in_sysdate and -- 26.02.2017   3.5.10.1    (-AG-) 
             pd.prod_beginn = v_prod_pa.prod_beginn
            and -- 26.02.2017   3.5.10.1    (-AG-) 
             ( ( pd.prod_ende is null )
                  or ( pd.prod_ende >= in_sysdate ) );
...
-- sqlcl_snapshot {"hash":"795923664dd178ffca9dc85530bb27958273cbec","type":"PROCEDURE","name":"BDE_PD_PROD_P_PA_U","schemaName":"DIRKSPZM32","sxml":""}