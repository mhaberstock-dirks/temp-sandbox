create or replace editionable trigger dirkspzm32.tr_s_erp_rcv_adr_biu before
    insert or update or delete on dirkspzm32.s_erp_rcv_adr
    for each row
declare
    v_found    boolean;
    ...
    where
        ...
        and t.adr_liefer = nvl(:new.adr_liefer,
                               0);
    ...
    from
        isi_adressen t
    where
            t.firma_nr = :new.firma_nr
        and t.adr_art = 'W'                       -- Warenempfänger
        and t.adr_nr = nvl(:new.adr_liefer,
                           0)
        and t.adr_liefer = nvl(:new.adr_liefer,
                               0);
...
begin
    ...
    */
        ...
        if
            :new.adr_art = 'K'
            and nvl(:new.adr_liefer,
                    0) != '0'                  -- Lieferadresse dann Warenempfaenger lesen
        then
            open c_adr;
      ...
      -- Adressenangaben fehler
        ...
        close c_s_adr;
        if not v_found then
            if :new.adr_nr != nvl(:new.adr_liefer,
                                  0)
            or :new.adr_art = 'W'                      -- Warenempfänger
             then
                insert into s_rcv_adr values ( :new.firma_nr,
                                               :new.adr_art,
                                               :new.adr_nr,
                                               nvl(:new.adr_liefer,
                                                   0),
        ...
        where
            ...
            and a.adr_liefer = nvl(:new.adr_liefer,
                                   0);
        ...
        if :new.adr_art = 'W'                      -- Warenempfänger
         ...
         then
            :new.adr_art := 'K';
            v_adr_nr := :new.adr_nr;
            v_ende_pos := instr(:new.adr_nr,
                                '-',
                                1,
                                1) - 1;

            if v_ende_pos > 0 then
                :new.adr_nr := substr(:new.adr_nr,
                                      1,
                                      v_ende_pos);
            end if;
            ...
            set                                -- SID                VARCHAR2(2) not null,
                ...
                adr.lte_etiketten_name = :new.lte_etiketten_name,
                                            -- LTE_ETIKETTEN_NAME VARCHAR2(50),
                adr.lhm_etiketten_name = :new.lhm_etiketten_name,
                ...
                adr.lte_etiketten_typ = :new.lte_etiketten_typ,
                                            -- LTE_ETIKETTEN_TYP          VARCHAR2(50),
                                            -- LTE_ETIKETTEN_SPEZ_BARCODE VARCHAR2(50),
                adr.lhm_etiketten_typ = :new.lhm_etiketten_typ,
                ...
                adr.lhm_etiketten_layout = :new.lhm_etiketten_layout,
                adr.lte_etiketten_layout = :new.lte_etiketten_layout,
  ...
  --                           :new.ecc_hersteller_id, -- ECC_HERSTELLER_ID          VARCHAR2(10),
                ...
                adr.adress_id_h = :new.adress_id_h,       -- ADRESS_ID_H                NUMBER,
                adr.edi_gln_r = :new.edi_gln_r,         -- EDI_GLN_R                  VARCHAR2(20),
                adr.edi_enabled = nvl(:new.edi_enabled,
                                      'F'), -- EDI_ENABLED                VARCHAR2(1) default 'F' not null,
                adr.avis = nvl(:new.avis,
                               adr.avis),   -- AVIS                       VARCHAR2(10)
                adr.waehrung = nvl(:new.waehrung,
                                   adr.waehrung), -- WAEHRUNG  N VARCHAR2(5) Y     Währung EUR, USD, ...
                adr.kundennrlief = nvl(:new.kundennrlief,
                                       adr.kundennrlief), -- KUNDENNRLIEF N VARCHAR2(20)  Y     Meine Kundennummer bei Lieferant / meine Lieferantennummer beim Kunden
                adr.lief_eink_rabatt_faktor = nvl(:new.lief_eink_rabatt_faktor,
                                                  adr.lief_eink_rabatt_faktor), -- LIEF_EINK_RABATT_FAKTOR N NUMBER  Y     Vorgabe Eink_Rabatt_Faktor für Berechnung Eink_Preis
                adr.eink_preis_berechnen = nvl(:new.eink_preis_berechnen,
                                               adr.eink_preis_berechnen), -- EINK_PREIS_BERECHNEN N VARCHAR2(1) Y 'F'   Vorgabe Artikel_Lieferant.EINK_PREIS Berechnen oder Eingeben.!
                adr.lieferbedingungen = nvl(:new.lieferbedingungen,
                                            adr.lieferbedingungen), -- LIEFERBEDINGUNGEN N VARCHAR2(500) Y     Lieferbedingungen
                adr.zahlungsbedingungen = nvl(:new.zahlungsbedingungen,
                                              adr.zahlungsbedingungen),  -- ZAHLUNGSBEDINGUNGEN  N VARCHAR2(500) Y     Zahlungsbedingungen
                adr.ust_id_nummer = nvl(:new.ust_id_nummer,
                                        adr.ust_id_nummer), -- UST_ID_NUMMER N VARCHAR2(20)  Y     Umsatzsteuer ID Nummer
                adr.rg_mwst_text = nvl(:new.rg_mwst_text,
                                       adr.rg_mwst_text) -- RG_MWST_TEXT  N VARCHAR2(500) Y     Text für Erklärung der MWST Ausland / Inlanf und Europa
            where
                    adr.firma_nr = :new.firma_nr
                and adr.adr_art = :new.adr_art
                and adr.adr_nr = :new.adr_nr
                and adr.adr_liefer = nvl(:new.adr_liefer,
                                         0);
    ...
    ---  :old.firma_nr := 1;  ----- Achtung
        ...
        where
            ...
            and a.adr_liefer = nvl(:old.adr_liefer,
                                   0);
...
-- sqlcl_snapshot {"hash":"6500ab7e74a195223b55cc6552576e752597eed1","type":"TRIGGER","name":"TR_S_ERP_RCV_ADR_BIU","schemaName":"DIRKSPZM32","sxml":""}