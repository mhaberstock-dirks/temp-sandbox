create or replace package body dirkspzm32.lvs_platz is
...
procedure lvs_transp_suche_einl_platz (
    ...
    -- Benoetigt fuer den Fall das auf der LTe der Zielplatz nicht mehr gesetzt ist
      ...
      --               offen sind. Ansonsten die Einlager Dispo gleich der noch offenen Transporte setzen
            if ( v_lgr_dispo_einl > 0 ) then
                ...
                if ( v_chk_amount_transp = 0 ) then
                    v_lgr_dispo_einl := 0;
                else
                    if ( v_chk_amount_transp != v_lgr_dispo_einl ) then
  ...
  */
    ...
    begin
        v_dispo_faktor := lvs_platz.lvs_platz_bewerten_db31(
            in_sid                 => in_sid,
  ...
  */
  ...
  -------------------------------------------------------------------------
    ...
    begin
        ...
        lvs_platz.lvs_suche_einl_platz(v_lte, v_basis_lte_name, v_lte_cfg.flaechen_stellplatz_erf, in_lgr_orte, -- Müssen immer 3-Stellig mit ; getrennt sein auch hinter dem letzten
         in_fahrzeuge_ids,
                                       v_lgr);
  ...
  */
    function get_calc_is_prev_storage (
        in_lgr_platz in lvs_lgr.lgr_platz%type,
        in_lte_id    in lvs_lte.lte_id%type
    ) return number is
    ...
    begin
        ...
        if ( v_found ) then
            ...
            if
                ( v_found )
                and ( v_chk_storage_grp = v_in_storage_grp )
            then
                v_ret_value := 0;
            end if;
  ...
  -------------------------------------------------------------------------
  */
    function lvs_platz_bewerten_db31 (
        in_sid                 in lvs_lgr.sid%type,
        in_firma_nr            in lvs_lgr.firma_nr%type,
        in_lgr_ort_typ         in lvs_lgr.lgr_typ%type,
        in_res_string          in lvs_lte.res_string%type,
    ...
    -- CMe 20220317 (P71141-94): Prüfen in der CFG ob die Disponioerung eines Platzes berücksichtigt wird
        ...
        v_check_is_prev_storage_grp := isi_allg.get_firma_cfg_param(in_sid, in_firma_nr, 'LVS_PLATZ_BEWERTEND', 'LVS', 'LVS_PLATZ_BEWERTEN_CHK_PREV_STOR_GRP'
        ,
                                                                    'LVS', 'CFG', 'T', 'BOOLEAN');
    ...
    -- CMe 20220317 (P71141-94): Neue Berechnung bei Kanalägern und ausgeschalteter DISPO Berücksichtigung
      ...
      --CMe 20220802: Berechnung ob der Lagerplatz vorher verwendet wurde
            if ( v_check_is_prev_storage_grp = 'T' ) then
                v_check_is_prev_result := lvs_platz.get_calc_is_prev_storage(
                    in_lgr_platz => in_lgr_platz,
                    in_lte_id    => in_lte_id
                );
                if ( v_check_is_prev_result > 0 ) then
...
-- sqlcl_snapshot {"hash":"a84544c52a4a5012ecfea6e628ef1c77b23728a2","type":"PACKAGE_BODY","name":"LVS_PLATZ","schemaName":"DIRKSPZM32","sxml":""}