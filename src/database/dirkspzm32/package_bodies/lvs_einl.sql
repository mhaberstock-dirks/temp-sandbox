create or replace package body dirkspzm32.lvs_einl is
  /*
  ...
  */
  ...
  -------------------------------------------------------------------------------------------------------
    ...
    begin
        return lvs_lam_zugang_owner(in_artikel_id, in_lte_id, in_lhm_id, in_charge_id, in_serie_id,
                                    in_leitzahl, in_fa_ag, in_fa_upos, in_abnr, in_best_nr,
                                    in_best_pos, in_res_id, in_prod_datum, in_zug_datum, in_ls_login_id,
                                    in_out_menge, in_lhm_name, in_kunde_nr, in_kd_art_nr, in_mhd,
                                    in_zeichnung, in_zeichnung_i, in_bew_id, in_lieferant, in_li_nr_lief,
                                    in_lte_id_lief, in_sonst_lief_id, in_menge_basis, in_mengeneinheit_basis, in_labor_status,
                                    in_lam_p1, in_lam_p2, in_lam_p3, in_lam_p4, in_lam_p5,
                                    in_lam_p6, in_lam_p7, in_lam_p8, in_lam_p9, in_lam_p10,
                                    in_lhm_eti_druck_status, in_lam_text, in_labor_text, in_fae_id, in_fae_id_position,
                                    in_lam_id, in_qs_status, in_lhm_lfd_nr, null -- no owner_adress_id > our product / material
                                    );
    end;
  ...
  -------------------------------------------------------------------------------------------------------
    ...
    begin
        return lvs_lam_zugang_owner_size(in_artikel_id, in_lte_id, in_lhm_id, in_charge_id, in_serie_id,
                                         in_leitzahl, in_fa_ag, in_fa_upos, in_abnr, in_best_nr,
                                         in_best_pos, in_res_id, in_prod_datum, in_zug_datum, in_ls_login_id,
                                         in_out_menge, in_lhm_name, in_kunde_nr, in_kd_art_nr, in_mhd,
                                         in_zeichnung, in_zeichnung_i, in_bew_id, in_lieferant, in_li_nr_lief,
                                         ...
                                         in_lam_p6, in_lam_p7, in_lam_p8, in_lam_p9, in_lam_p10,
                                         in_lhm_eti_druck_status, in_lam_text, in_labor_text, in_fae_id, in_fae_id_position,
                                         in_lam_id, in_qs_status, in_lhm_lfd_nr, in_owner_address_id, -- no owner_adress_id > our product / material,
    ...
    begin
        return lvs_lam_zugang_db31(in_artikel_id, in_lte_id, in_lhm_id, in_charge_id, in_serie_id,
                                   in_leitzahl, in_fa_ag, in_fa_upos, in_abnr, in_best_nr,
                                   in_best_pos, in_res_id, in_prod_datum, in_zug_datum, in_ls_login_id,
                                   in_out_menge, in_lhm_name, in_kunde_nr, in_kd_art_nr, in_mhd,
                                   in_zeichnung, in_zeichnung_i, in_bew_id, in_lieferant, in_li_nr_lief,
    ...
    begin
     ...
     -- -AG- Prüfen ob die LHM-ID Unique seien soll
        if isi_allg.get_global_param('lvs', 'LVS_UNIQUE_LHM_ID', 'F') = c.c_true then
            open c_lam_bh;
    ...
    -------------------------------------------------------------------------------------------------------
     ...
     -- -AG- 2008-04-24 BugFix: Lesen des Gewichts aus den Behaelter, der im Artikel hinterlegt ist (Um Nettogewicht zu rechnen)
        if not lvs_p_base.get_lhm_cfg('01', v_art.lhm_name, v_lhm_cfg_art) then
        ...
        if v_found then               -- KundenArtikel Daten gefunden
            ...
            if in_out_menge is null then
                if nvl(in_menge_basis, v_art.menge_basis) = c.basis_lte then
                    in_out_menge := 1;
                elsif nvl(in_menge_basis, v_art.menge_basis) = c.basis_lhm then
    ...
    -------------------------------------------------------------------------------------------------------
     ...
     -- Jetzt wird erst mal die LHM geprüft, und evtl neu angelegt !!!!
        ...
        if
            in_hersteller_kuerzel_liste is not null
            and in_hersteller_kuerzel_liste != ';'
            and isi_p_base.get_artikel_ctrl_typ('01',
                                                in_artikel_id,
                                                substr(in_hersteller_kuerzel_liste,
                                                       1,
                                                       length(in_hersteller_kuerzel_liste) - 1),
                                                v_art_ctrl)
        then
        ...
        if not v_found then
         ...
         -- -MM- Erweiterung 28.07.2014 Kundenspezifische LHM und LTE IDs
                in_lhm_id := lvs_p_lte.lvs_lte_lhm_next_id_v35('01', 1, c.basis_lhm, v_charge.charge_bez, in_artikel_id,
                                                               v_art.artikel_fuer_kunde_etikett, v_typ, v_h_tag);
            else
                if v_firma.lhm_barcode_type = c.lte_barcode_spez then
                    isi_utils.spez_barcode_lfdn('01', 1, in_lhm_id, c.basis_lhm, v_barcode_id);
                end if;
            end if;
     ...
     -- Stammdaten für den Lagerbestand eintragen
        if in_lam_id is null then
            ...
            v_mhd_berechnen := nvl(v_art.mhd_berechnung,
                                   nvl(v_firma.mhd_berechnung, 'TA'));

       -- -AG- BugFix:  20.08.2009 Fehler beim Berechnen des MHD Datums
            if in_mhd is null then
                if v_art.mhd_festes_datum is not null then
                    v_lam_mhd := v_art.mhd_festes_datum;
                else
                    v_lam_mhd := lvs_p_lte_lhm.lvs_mhd_berechne(
                        trunc(nvl(in_prod_datum, sysdate)),
                        v_art.mhd_tage,
                        v_mhd_berechnen
                    );
                end if;
     ...
     -- 23.02.2008 -AG- In den Abgangsbuchungen werden jetzt auch Lieferscheinnummer und Pos. gespeichert
        insert into lvs_lam_bh values ( '01',
                                        1,
                                        v_vorg_id,
                                        c.lam_bh_zugagng,
                                        v_lam_bh_id,
                                        ...
                                        decode(in_owner_address_id,
                                               ...
                                               c.lam_bh_bus_zug,
                                               decode(
                                            nvl(in_leitzahl, 0),  -- Mit Leitzahl ist Produktion dann BUS Zugang
                                            0,
                                            c.lam_bh_bus_zug_konsi,    -- Wenn owner gesetzt, dann Zugang KONSI
                                            c.lam_bh_bus_zug
                                        )),
                                        in_zug_datum,
                                        in_ls_login_id,
  ...
  -------------------------------------------------------------------------------------------------------
    ...
    -- lagerplatz aus lvs_lgr lesen
       ...
       --v_lte.lte_status != C.LTE_ET_STAT and
             v_lte.lte_status != c.lte_af_stat
        ...
        v_err_text := lvs_platz.lvs_platz_einl_pruef_err_t_r30(v_lte, v_basis_lte_name, v_lte_cfg.flaechen_stellplatz_erf, v_lgr, 'E'
        ,
                                                               in_fahrzeuge_ids);

        if v_err_text != null then
        ...
        if ( v_lgr.sid is not null ) then
            ...
            v_result := lvs_transport.lvs_transp_lte(v_lte.sid, v_lte.firma_nr, in_modul_erzeuger, in_modul_bearbeiter, 'F',
                                                     ...
                                                     in_progr_nr, in_quelle_leer_progr_nr, in_ziel_voll_progr_nr, v_lgr_platz, v_lgr.lgr_platz
                                                     ,
 ...
 *******************************************************************************/
  ...
  -------------------------------------------------------------------------------------------------------
    ...
    begin
        ...
        if not lvs_p_base.get_lgr_platz(in_ziel_lgr_platz, v_lgr) then
            v_err_nr := 20;
            v_err_text := lc.ec_p1(lc.o_tp1_lagerplatz_fehlt, in_ziel_lgr_platz);
            raise v_error;
        end if;

        if v_linie.lte_name = c.keinelte then
      -- wenn LTE Typ = "-KeineLTE", dann bei mehreren LHMs einzelne LTEs erzeugen
        ...
        -- damit immer nur eine LTE pro LHM erzeugt wird
                if ( v_menge_erz_ges + v_linie_waren.menge_je_karton ) > v_linie_waren.menge then
          -- Restmenge
        ...
        -- Mengen so setzen, dass immer nur ein LHM und eine LTE erzeugt wird
                ...
                set
                    t.menge = v_linie_waren.menge_je_karton,
                    t.menge_je_karton = v_linie_waren.menge_je_karton
                where
                        t.sid = v_linie_waren.sid
        ...
        -- lte erzeugen für jede LHM
                v_lte_id := lvs_p_lte.lvs_lte_erzeugen(in_sid, in_firma_nr, in_linie, null, in_login_id,
                                                       null, null);

                if not lvs_p_base.get_lte(v_lte_id, v_lte) then
                    v_err_nr := 40;
                    v_err_text := lc.ec(lc.o_txt_lte_anleg_err);
                    raise v_error;
                end if;

                if not lvs_p_base.get_lte_cfg(in_sid, v_lte.lte_name, v_lte_cfg) then
                    v_err_nr := 45;
                    v_err_text := lc.ec_p1(lc.o_tp1_lte_cfg_fehlt, v_lte.lte_name);
                    raise v_error;
                end if;

                lvs_platz.lvs_platz_einl_pruefen_r30(v_lte, v_lte_cfg.basis_lte_name, v_lte_cfg.flaechen_stellplatz_erf, v_lgr, 'E',
                                                     null); -- keine Fahrzeuge
                v_transport_gruppe := 0;
                v_result := lvs_transport.lvs_transp_lte(in_sid, in_firma_nr, in_modul_erzeuger, null, c.c_false,
                                                         'E', in_login_id, null, null, in_prio,
                                                         0, 0, 0, in_we_lgr_platz, in_ziel_lgr_platz,
                                                         v_lte_id, null, c.c_false, null, null,
 ...
 *******************************************************************************/
  ...
  -------------------------------------------------------------------------------------------------------
    ...
    -- damit sie sofort ausgelagert werden kann
        ...
        v_result := lvs_ausl.lvs_lte_reservieren(in_sid, in_firma_nr, v_order_pos.vorgang_id, in_order_auf_id, v_lte_id,
                                                 null, null, v_order_pos.artikel_id);

        update isi_order_pos op
        set
            op.arbeitsplatz_id = in_arbeitsplatz_id,
            op.status = 'A',
        ...
        v_transport_gruppe := 0;
        v_result := lvs_transport.lvs_transp_lte(in_sid, in_firma_nr, in_modul_erzeuger, null, c.c_false,
                                                 'E', in_login_id, in_order_auf_id, null, in_prio,
                                                 0, 0, 0, in_we_lgr_platz, v_order_pos.ziel,
                                                 v_lte_id, null, c.c_false, null, null,
                                                 v_order_pos.vorgang_id, null,             -- in_fahrzeuge_IDs Transport soll in jedem Fall erzeugt werden, auch wenn das Fahrzeug gestört ist
                                                  null, v_transport_gruppe, v_transp_id);
    ...
    begin
        ...
        if c_lte%found then
            ...
            if v_lte.order_vorgang_id is not null then
                ...
                if
                    v_found
                    and v_anz_order_pos > 0
                then
          -- Es gibt noch andere LTEs, die für diese Order reserviert sind
                    v_result := lvs_ausl.lvs_lte_res_rueck(in_sid, in_firma_nr, v_lte.order_vorgang_id, v_lte.order_auf_id, v_lte.lte_id
                    ,
                                                           v_lte.order_vorgang_id, v_lte.lgr_platz, c.c_true);
                else
          -- Die gesamte Order zurücksetzen
                    if isi_p_order_base.get_order_pos(in_sid, v_lte.order_auf_id, v_order_pos) then
                        isi_p_order.abbruch_transporte(in_sid, in_firma_nr, v_order_pos.vorgang_id, v_order_pos.auf_id, v_order_pos.vorgang_typ
                        ,
                                                       v_order_pos.satzart, in_login_id, c.c_true);
            -- Die Bindung an den Arbeitsplatz freigeben
                        update isi_order_pos op
                        set
                            op.arbeitsplatz_id = null
                        where
                                op.sid = in_sid
 ...
 *******************************************************************************/
    ...
    begin
        ...
        if not lvs_p_base.get_lgr_platz(in_ziel_lgr_platz, v_lgr) then
            raise_isi_error(20,
                            lc.ec_p1(lc.o_tp1_lagerplatz_fehlt, in_ziel_lgr_platz));
        end if;
        ...
        if not lvs_p_base.get_lte(v_lte_id, v_lte) then
            raise_isi_error(50,
                            lc.ec(lc.o_txt_lte_anleg_err));
        end if;

        if not lvs_p_base.get_lte_cfg(in_sid, v_lte.lte_name, v_lte_cfg) then
            raise_isi_error(55,
                            lc.ec_p1(lc.o_tp1_lte_cfg_fehlt, v_lte.lte_name));
        end if;
        ...
        if nvl(in_lte_drucker_name, in_lhm_drucker_name) is not null then
      -- automatischer Etkettendruck bei mitgeliefertem Druckername
            ...
            if v_lte.lte_eti_druck_status = c.eti_status_soll_drucken then
                v_result := lvs_p_lte.lvs_lte_drucken(v_lte.lte_id,
                                                      null,
                                                      nvl(in_lte_drucker_name, in_lhm_drucker_name));
            end if;
    ...
    -- Im Fehlerfall is v_err_nr bereits gesetzt
        when v_error then  -- Update 2011 show Exception Source Line
            if c_lhm_soll_druck%isopen then
                close c_lhm_soll_druck;
            end if;
            rollback;
            v_err_text := v_err_text
    ...
    function check_lte_name_einl (
        ...
        where
            ...
            and ( nvl(lgr.lte_namen_cfg,
                      nvl(in_lte_name, 'XXXX')) like '%;'
                                                     || nvl(in_lte_name, 'XXXX')
                                                     || ';%'
                  or nvl(lgr.lte_namen_cfg,
                         nvl(in_lte_name, 'XXXX')) like nvl(in_lte_name, 'XXXX')
                                                        || ';%' )
    ...
    begin
        ...
        close c_lgr_check;
        if lvs_p_base.get_lgr_ort_fuellg_chk_by_p(in_sid, in_firma_nr, in_lgr_ort, in_lgr_gruppe_id, in_lte_name,
                                                  v_lvs_lgr_ort_fuellg_check) then
...
-- sqlcl_snapshot {"hash":"ba0cae628bbbf530e4174cf0ad92e5b4894a6cab","type":"PACKAGE_BODY","name":"LVS_EINL","schemaName":"DIRKSPZM32","sxml":""}