create or replace package body dirkspzm32.bde_p_pers_zeit_kst is
/*
...
*/
    procedure bde_c_pd_pers_zeit_berech (
        ...
        type t_pers_zeit_kst is
            table of bde_pd_pers_zeit_kst%rowtype index by binary_integer;
        type t_pers_zeit_event is
        ...
        where
            ...
            and ( ( t.pd_pers_beginn > in_pd_pres_beginn
                  ...
                  or ( t.pd_pers_beginn < in_pd_pres_beginn
        ...
        loop
            ...
            or (
                ...
                loop
                    ...
                    fetch c_pd_pers_zeit_event into
                        v_pers_zeit_kst_event
                    (idx_event);
                ...
                anz_event := 0;
                idx_start := 1;
                while ( idx_event <= max_event ) loop
                    if idx_event > 1 then
                        idx := idx_start;
                        anz_change_event := 0;
                        while ( idx <= max_idx ) loop
                            if v_pers_zeit_kst_tab(idx_start).pd_pers_ende <= v_pers_zeit_kst_event(idx_event) then
                                idx_start := idx_start + 1;
                            end if;

                            if
                                v_pers_zeit_kst_tab(idx).pd_pers_beginn <= v_pers_zeit_kst_event(idx_event - 1)
                            ...
                            if v_pers_zeit_kst_tab(idx).pd_pers_beginn = v_pers_zeit_kst_event(idx_event) then
                                anz_event := anz_event + 1;
                            end if;

                            if v_pers_zeit_kst_tab(idx).pd_pers_ende = v_pers_zeit_kst_event(idx_event) then
                ...
                while ( idx <= max_idx ) loop
                    ...
                    set
                        t.pd_pers_zeit_anteil_min = v_pers_zeit_kst_tab(idx).pd_pers_zeit_anteil_min
                    where
                            t.sid = v_pers_zeit_kst_tab(idx).sid
...
-- sqlcl_snapshot {"hash":"703700ff284ba9001546c1b2d0f6b11f1e043efa","type":"PACKAGE_BODY","name":"BDE_P_PERS_ZEIT_KST","schemaName":"DIRKSPZM32","sxml":""}