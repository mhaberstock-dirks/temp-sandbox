create or replace package body dirkspzm32.pzm_p_pers_zeit_kst is
  ...
  -- Function and procedure implementations
    procedure pzm_c_pd_pers_zeit_berech (
        ...
        select
            ...
            sum(t.ts_day_arb_std + t.ts_day_flex_std + t.ts_day_ueb_std + nvl(
                get_sum_loa_std(t.ts_pers_nr, t.ts_datum, '0111'),
                0
            ))                                                       std,
        ...
        from
            pzm_ze_tagessatz t
        where
                t.ts_datum >= trunc(in_datum, 'MONTH')
        ...
        select
            t.pd_res_kst,
            sum(t.pd_pers_zeit_anteil_min / 60) std
        from
        ...
        select
            t.pers_nr,
            sum(t.kst_std) kst_std
        from
            pzm_ze_pers_kst_monat_ab t
        where
                t.sid = in_sid
            and t.firma_nr = in_firma_nr
    ...
    begin
        ...
        where
            t.datum = last_day(in_datum);

        open c_pzm_ze_tagessatz;
        loop
            fetch c_pzm_ze_tagessatz into
                v_pers_nr,
                v_kst,
                v_std,
                v_std_pause,
      ...
      --dbms_output.put_line('Ab ' || v_pers_nr || 'LOA' || in_loa || 'KST' || v_kst);
        ...
        fetch c_pzm_ze_pers_kst_monat_ab_sum into
            v_pers_nr,
            v_pers_nr_std;
        while c_pzm_ze_pers_kst_monat_ab_sum%found loop
            update pzm_ze_pers_kst_monat_ab t
            set
                t.kst_proz = decode(v_pers_nr_std, 0, 100, t.kst_std / v_pers_nr_std * 100)
            where
                    t.sid = in_sid
                and t.firma_nr = in_firma_nr
                and t.datum = last_day(in_datum)
...
-- sqlcl_snapshot {"hash":"43c8260c690acf0ee8280b37fa32887b8293754f","type":"PACKAGE_BODY","name":"PZM_P_PERS_ZEIT_KST","schemaName":"DIRKSPZM32","sxml":""}