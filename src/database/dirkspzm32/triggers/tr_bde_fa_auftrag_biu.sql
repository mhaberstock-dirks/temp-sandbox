create or replace editionable trigger dirkspzm32.tr_bde_fa_auftrag_biu before
    insert or update on dirkspzm32.bde_fa_auftrag
    for each row
declare
  -------------------------------------------------------------------------------------------------------
  ...
  --v_kenz_letzt_ag_old                                bde_fa_auftrag.kenz_letzt_ag%type;
    ...
    from
        lvs_lte lte
    where
        lte.order_vorgang_id = :new.leitzahl;
...
begin
  ...
  -- AG Bugfix aufgegallen bei Euscher. Evtl durch geänderte SAP-Aufträge
    :new.ag_ist_mg := nvl(:new.ag_ist_mg,
                          0);
  ...
  --v_kenz_letzt_ag_old               := :old.kenz_letzt_ag;
    ...
    if
        :new.ag_opt_grp is not null
        and not bde_p_pps.get_pps_ruestmatrix_opt_grp(:new.sid,
                                                      :new.firma_nr,
                                                      :new.ag_opt_grp,
                                                      v_opt_grp)
    then
        insert into pps_ruestmatrix_opt_grp values ( :new.sid,
                                                     :new.firma_nr,
                                                     :new.ag_opt_grp,
                                                     :new.ag_opt_grp,
                                                     sysdate,
    ...
    :new.prod_zeit_erf := nvl(:new.prod_zeit_erf,
                              0);
    :new.ruest_zeit_erf := nvl(:new.ruest_zeit_erf,
                               0);
    :new.rcv_prod_zeit_erf := nvl(:new.rcv_prod_zeit_erf,
                                  0);
    :new.rcv_ruest_zeit_erf := nvl(:new.rcv_ruest_zeit_erf,
                                   0);
    if inserting then
    ...
    if
        isi_allg.get_firma_cfg_param(:new.sid,
                                     :new.firma_nr,
                                     'BDE',
                                     null,
                                     'BDE_FA_GEN_LTE_ID',
    ...
    then
        ...
        if not v_found then
            ...
            insert into bde_fa_auftrag_lte_pool values ( :new.sid,
                                                         :new.firma_nr,
                                                         v_lte_id,
    ...
    if updating then
        ...
        if
            :new.nr_pruefung is not null
            and :old.nr_pruefung is null
        then
            update lvs_lam l
            set
                l.nr_pruefung = :new.nr_pruefung
            where
                    l.sid = :new.sid
        ...
        then
            ...
            set
                aps.fixiert = 0
            where
                    aps.fakopfnr = 'FA' || to_char(:new.leitzahl)
        ...
        if v_ag_artikel_id_old <> v_ag_artikel_id_new then
            begin
                update isi_resource_lam_akt res
                set
                    res.artikel_id = v_ag_artikel_id_new
                where
                        res.sid = :new.sid
            ...
            exception
                when others then
                    null; -- Kann denn Satz nicht ändern (Rohstoff bereits in Liste
            end;
        end if;
...
--    or nvl(v_kenz_letzt_ag_new, 0) != nvl(v_kenz_letzt_ag_old, 0)
      ...
      -- AG 20170308 Reservierung löschen wenn aus Dialog der Auftrag als Fertig gesetzt wurde
            if :new.freig_status = 'XF' then
                if :new.ma_reserviert = c.c_true then
        ...
        -- Auswertung der Produktionszeit ermitteln
                :new.prod_zeit_ist := bde_funktionen.get_fa_zeiten_upos(:new.sid,
                                                                        :new.firma_nr,
                                                                        :new.leitzahl,
                                                                        :new.fa_ag,
                                                                        :new.fa_upos,
        ...
        -- -AG- 20190917 - Einbau mitarbeiteranmeldung an Maschine zu FA mit Arbeitszeiterfassung
                ...
                v_bew_id := s_schnittstelle.write_host_prod_bew(v_fa_auftrag.sid, v_fa_auftrag.firma_nr, v_fa_auftrag, null, null,
                                                                null, null, 'S_FA', 'UE');
         ...
         -- -AG- Auftrag Status ist Fertig und Mengen und zeit sin zum Host übergeben
            ...
            then
                ...
                loop
                    ...
                    v_res_result := lvs_ausl.lvs_lte_res_rueck(v_lte.sid, v_lte.firma_nr, v_lte.order_vorgang_id, v_lte.order_auf_id,
                    v_lte.lte_id,
...
-- sqlcl_snapshot {"hash":"e623f79c6c2fc9b69f2ec96090a8a8a8fb3fb5cc","type":"TRIGGER","name":"TR_BDE_FA_AUFTRAG_BIU","schemaName":"DIRKSPZM32","sxml":""}