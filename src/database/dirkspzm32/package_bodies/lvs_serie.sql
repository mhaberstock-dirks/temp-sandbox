create or replace 
package body DIRKSPZM32.lvs_serie is
  ...
  *******************************************************************************/
  procedure LVS_SERIE_ID_KOPF_ERZEUGEN (in_sid                in lvs_serie_id_kopf.sid%type,
                                        ...
                                        out_serie_id          out lvs_serie_id_kopf.serie_id%type) is
  ...
  cursor c_get_bde_fa is
    select bde.*
     ...
     where (bde.leitzahl = in_leitzahl or
            bde.abnr = in_leitzahl_extern)
  ...
  begin
    ...
    if in_id_maske is null
    then
      v_serie_maske := isi_allg.c_get_firma_cfg_param (in_sid,
                                                       in_firma_nr,
    ...
    select seq_serie_id.nextval into out_serie_id from dual;
    insert into lvs_serie_id_kopf
    values
      (in_sid,                       --SID                   VARCHAR2(2)
  ...
  *******************************************************************************/
  procedure LVS_SERIE_ID_POS_ERZEUGEN (in_sid      in lvs_serie_id_pos.sid%type,
                                       ...
                                       in_serie_id in lvs_serie_id_pos.serie_id%type)is
  ...
  cursor c_get_serie_id_kopf is
    select sik.*
      from lvs_serie_id_kopf sik
     where sik.serie_id = in_serie_id;
  ...
  begin
    ...
    v_replace_char_serie_nr := isi_allg.c_get_firma_cfg_param (in_sid,
                                                           in_firma_nr,
                                                           'CFG',                         -- in_kategorie          in isi_firma_cfg.kategorie%type
                                                           NULL,                          -- in_kategorie_ix       in isi_firma_cfg.kategorie_ix%type
                                                           ...
                                                           '@',                           -- in_default_param_wert in isi_firma_cfg.parameter_wert%type
                                                           'STRING');                     -- in_default_param_typ  in isi_firma_cfg.parameter_typ%type
    v_replace_char_serie_ext_nr := isi_allg.c_get_firma_cfg_param (in_sid,
                                                                   in_firma_nr,
                                                                   'CFG',                              -- in_kategorie          in isi_firma_cfg.kategorie%type
                                                                   NULL,                               -- in_kategorie_ix       in isi_firma_cfg.kategorie_ix%type
    ...
    for i in 1..v_max_anzahl loop
      if v_lvs_serie_id_kopf.serie_gen_richtung = 'AU'
      then
      ...
      then
        ...
        select length(v_lvs_serie_id_kopf.serie_maske)- nvl(length(replace(v_lvs_serie_id_kopf.serie_maske, v_replace_char_serie_nr)), 0) into v_laenge from dual;
        ...
        v_serie_nr := v_prefix || lpad(to_char(v_first_id + (i - 1)), v_laenge, '0') || v_suffix;
        if v_lvs_serie_id_kopf.serie_extern_maske is not null
        then
          if v_lvs_serie_id_kopf.serie_extern_maske = v_lvs_serie_id_kopf.serie_maske
          then
            v_serie_nr_extern := v_serie_nr;
          else
            --Berechnet die Anzahl an Zeichen die ersetzt werden sollen
            select length(v_lvs_serie_id_kopf.serie_extern_maske)- nvl(length(replace(v_lvs_serie_id_kopf.serie_extern_maske, v_replace_char_serie_ext_nr)), 0) into v_laenge from dual;
            -- Hole die erste Position des vorkommens
            ...
            v_serie_nr_extern := v_prefix || lpad(to_char(v_first_id_ext + (i - 1)), v_laenge, '0') || v_suffix;
          end if;
        end if;
      end if;
      insert into lvs_serie_id_pos
      values
        (in_sid,                       --SID                   VARCHAR2(2)
         in_firma_nr,                  --FIRMA_NR              NUMBER(2)
  ...
  *******************************************************************************/
  procedure LVS_C_SERIE_ERZEUGEN (in_sid              in lvs_serie_id_kopf.sid%type,
                                  ...
                                  in_start_id         in lvs_serie_id_kopf.serie_start_id%type,
                                  in_start_externe_id in lvs_serie_id_kopf.serie_extern_start_id%type,
                                  in_serie_gen_richtung in lvs_serie_id_kopf.serie_gen_richtung%type) is
  ...
  cursor c_chk_serie is
    select sidk.serie_id
      from lvs_serie_id_kopf sidk
      join lvs_serie_id_pos sidp on sidp.serie_id = sidk.serie_id
     where sidk.leitzahl = in_leitzahl
        or sidk.abnr = in_leitzahl_extern
     group by sidk.serie_id;
  ...
  begin
    ...
    if not v_found
    then
      LVS_SERIE_ID_KOPF_ERZEUGEN (in_sid,
                                 in_firma_nr,
                                 in_leitzahl,
                                 in_leitzahl_extern,
                                 in_id_maske,
                                 ...
                                 in_start_id,
                                 in_start_externe_id,
                                 in_serie_gen_richtung,
                                 v_serie_id);

      LVS_SERIE_ID_POS_ERZEUGEN (in_sid,
                                 in_firma_nr,
                                 v_serie_id);
  ...
  *******************************************************************************/
  function GET_NEXT_SERIE_NR_BY_FA (in_sid              in lvs_serie_id_kopf.sid%type,
                                    ...
                                    in_login_id         in lvs_serie_id_pos.last_change_login_id%type) return varchar2 is
  ...
  cursor chk_lam_on_serie is
    select sidp.serie_nr
      from lvs_serie_id_pos sidp
     where sidp.sid = in_sid
  ...
  cursor get_next_serien_nr is
    select sidp.serie_nr
     ...
     where sidp.sid = in_sid
       ...
       and sidp.serie_pos_lfdn = (select min(x.serie_pos_lfdn)
                                    from lvs_serie_id_pos x
                                   where x.sid = in_sid
  ...
  begin
    ...
    if v_found
    then
      raise_isi_error(20, 'LAM Id' || in_lam_id || ' wird bereits fuer Serie ' || v_result || ' verwendet');
    end if;
    open get_serie_id;
    ...
    if not v_found
    then
      raise_isi_error(20, 'Serie Kopf fuer Leitzahl ' || in_leitzahl || ' nicht gefunden');
    end if;
    ...
    if not v_found
    then
      raise_isi_error(30, 'Keine freie Serie fuer Leitzahl ' || in_leitzahl || '[' || v_serie_id || '] nicht gefunden');
    end if;
    update lvs_serie_id_pos sidp
       set sidp.lam_id = in_lam_id,
           sidp.last_change_login_id = in_login_id
     where sidp.serie_id = v_serie_id
  ...
  *******************************************************************************/
  function GET_SERIE_NR_EXT_BY_SERIE_NR (in_sid              in lvs_serie_id_kopf.sid%type,
                                         in_firma_nr         in lvs_serie_id_kopf.firma_nr%type,
                                         in_serie_nr         in lvs_serie_id_pos.serie_nr%type) return varchar2 is

  v_result lvs_serie_id_pos.serie_nr%type;
  v_found  boolean;

  cursor c_get_serie_nr_ext is
    select sidp.serie_nr_extern
      from lvs_serie_id_pos sidp
     where sidp.sid = in_sid
       and sidp.firma_nr = in_firma_nr
  ...
  begin
    ...
    if not v_found
    then
      raise_isi_error(10, 'Serien Nr ' || in_serie_nr || ' nicht gefunden');
    end if;
    return(v_result);
  exception
    -- Im Fehlerfall is der Fehlertext bereits gesetzt.
  ...
  *******************************************************************************/
  procedure RESET_SERIE_NR (in_sid              in lvs_serie_id_kopf.sid%type,
                            ...
                            in_login_id         in lvs_serie_id_pos.last_change_login_id%type) is
  begin
    reset_isi_error;
    update lvs_serie_id_pos sidp
       set sidp.lam_id = null,
           sidp.last_change_login_id = in_login_id
     where sidp.sid = in_sid
       and sidp.firma_nr = in_firma_nr
       and sidp.serie_nr = in_serie_nr;
...
-- sqlcl_snapshot {"hash":"523133efaccc4c9dfc92fc6d78002a07e0e9f63a","type":"PACKAGE_BODY","name":"LVS_SERIE","schemaName":"DIRKSPZM32","sxml":""}