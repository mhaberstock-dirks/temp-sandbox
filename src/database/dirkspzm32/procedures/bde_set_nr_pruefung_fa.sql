create or replace procedure dirkspzm32.bde_set_nr_pruefung_fa (
    in_sid           isi_sid.sid%type,
...
begin
    v_found := ( isi_p_base.get_resource_by_name(
        in_res_name  => in_res_name,
    ...
    if ( in_leitzahl is not null ) then
      ...
      -- Ohne Leitzahl und Resource kann der Auftrag nicht gefunden werden
        ...
        if ( isi_p_base.get_resource_zust_akt(
            in_sid                => in_sid,
    ...
    -- Wenn Arbeitsgang angegeben das für alle 'V'errichten einträge die Pruefplannummer setzen
        if ( in_up_verrichten = 'T' ) then
            update bde_fa_auftrag bde
            set
                bde.nr_pruefung = in_nr_pruefung
            where
                    bde.sid = in_sid
                and bde.firma_nr = in_firma_nr
                and bde.leitzahl = v_leitzahl
            ...
            where
                ...
                and bde.firma_nr = in_firma_nr
                and bde.leitzahl = v_leitzahl
                and ( bde.fa_ag = v_fa_ag
                      or ( bde.res_id = v_isi_resource.res_id
                           and bde.satzart = 'V' ) );

        end if;

        if ( in_up_prim = 'T' ) then
            update bde_fa_auftrag bde
            set
                bde.nr_pruefung = in_nr_pruefung
            where
...
-- sqlcl_snapshot {"hash":"75ab8863d9e06315060712bb6ca09bf849f3932c","type":"PROCEDURE","name":"BDE_SET_NR_PRUEFUNG_FA","schemaName":"DIRKSPZM32","sxml":""}