create or replace package body dirkspzm32.lvs_uml is
  ...
  -------------------------------------------------------------------------------------------------------
    ...
    begin
      ...
      -- raise_isi_error(10, 'LTE ID ' || nvl(in_lte_id, '(null)') || ' konnte nicht gefunden werden.');
        ...
        v_result := lvs_ausl.lvs_lte_reserv_user_ziel(in_sid, in_firma_nr, -2, -- vorgang_id (-2 = Konstante für Umlgaerungsreservierung)

         -2, -- auf_id

         in_lte_id,
                                                      null, in_login_id, in_ziel_lgr_platz);
        ...
        v_result := lvs_transport.lvs_gesperrter_transp_lte(in_sid, in_firma_nr, in_modul_erzeuger, null, c.c_false,
                                                            'U', in_login_id, null, null, in_prio,
                                                            0, 0, 0, v_lte.lgr_platz, in_ziel_lgr_platz,
                                                            in_lte_id, null, c.c_false, null, null,
                                                            null, null, null, v_transport_gruppe, v_transp_id);
    ...
    procedure c_res_lhm_erz_transp_gesp (
        ...
        v_transp_id               isi_transport.transp_id%type;
    begin
        if not lvs_p_base.get_lhm(in_lhm_id, v_lhm) then
            raise_isi_error(10,
      ...
      -- Prüfen, ob von der Quelle etwas abkommissioniert werden kann
            if not lvs_p_base.get_lgr_komm_info(v_q_lte.lgr_platz, v_komm_direkt_moegl, v_komm_ausweich_lgr_platz, v_komm_picken_moegl
            , v_komm_neu_lte_lgr_platz) then
                raise_isi_error(30,
                                lc.ec_p1(lc.o_tp1_lgr_platz_o_komm,
                                         nvl(v_q_lte.lgr_platz, '(null)')));
        ...
        --                    ' konnten keine Kommissionierdaten gefunden werden.');
            ...
            then
                raise_isi_error(32,
                                lc.ec_p1(lc.o_tp1_lgr_ort_o_komm,
                                         nvl(v_q_lte.lgr_ort, '(null)')));
        ...
        --                    ' ist kein Kommissionierlagerplatz eingerichtet.');
            ...
            then
                raise_isi_error(32,
                                lc.ec_p1(lc.o_tp1_lgr_ort_o_k_ausw_pl,
                                         nvl(v_q_lte.lgr_ort, '(null)')));
        ...
        --                    ' ist kein Ausweichlagerplatz definiert, obwohl keine Kommissionierung möglich ist.');
            ...
            if not lvs_p_base.get_lhm_cfg(in_sid,
                                          nvl(in_neu_lhm_name, v_lhm.lhm_name),
                                          v_lhm_cfg) then
                raise_isi_error(36,
                                lc.ec_p1(lc.o_tp1_lhm_cfg_fehlt,
                                         nvl(
                          nvl(in_neu_lhm_name, v_lhm.lhm_name),
                          '(null)'
                      )));
        ...
        --                    ' konnten keine Daten gefunden werden.');
            ...
            if not lvs_p_base.get_lam_by_lhm_id(v_lhm.sid, v_lhm.firma_nr, in_lhm_id, v_lam) then
                raise_isi_error(37,
        ...
        -- raise_isi_error(37, 'Für die LHM ID ' || nvl(to_char(in_lhm_id), '(null)') || ' konnten keine Materialdaten gefunden werden.');
            ...
            if not lvs_p_base.get_charge(v_lam.charge_id, v_charge) then
                raise_isi_error(38,
                                lc.ec_p1(lc.o_tp1_charge_id_fehlt,
                                         nvl(
                          to_char(v_lam.charge_id),
                          '(null)'
                      )));
      ...
      -- Neue Ziel-LTE für die Umlagerung (Abkommissionierung) einer LHM anlegen
            v_komm_neu_lte_id := lvs_p_lte.lvs_lte_insert_v358(in_sid, in_firma_nr, c.keinelte, null, in_login_id,
                                                               null, null, c.lte_pf_stat, null, null,
                                                               v_charge.charge_id, v_charge.charge_bez, v_lam.artikel_id, null, -- -AG- In diesem Fall kann kein Packschema vorgegeben werden
      ...
      -- Die Quell-LTE wird für das Abpacken reserviert
            v_result := lvs_ausl.lvs_lte_reserv_user_ziel(in_sid, in_firma_nr, -2, -- vorgang_id (-2 = Konstante für Umlagerungsreservierung)
             -2, -- auf_id
             v_q_lte.lte_id,
                                                          null, in_login_id, null);
      ...
      -- LHM wird für die Ziel LTE reserviert
            update lvs_lam t
            set
                t.res_ziel_lte_id = v_komm_neu_lte_id
            where
                    t.sid = in_sid
      ...
      -- vor dem Transport noch den neuen LHM Typ speichern
            if in_neu_lhm_name is not null then
                update lvs_lhm t
                set
                    t.komm_neu_lhm_name = in_neu_lhm_name
                where
            ...
            v_result := lvs_transport.lvs_gesperrter_transp_lte(in_sid, in_firma_nr, in_modul_erzeuger, null, c.c_false,
                                                                'U', in_login_id, null, null, in_prio,
                                                                0, 0, 0, v_komm_neu_lte_lgr_platz, in_ziel_lgr_platz,
                                                                v_komm_neu_lte_id, null, c.c_false, null, null,
                                                                null, null, null, v_transport_gruppe, v_transp_id);
    ...
    procedure c_uml_gesp_transp_delete (
        ...
        from
            ...
            lvs_lhm t
        where
                lam.res_ziel_lte_id = v_lte.lte_id
    ...
    begin
      ...
      --                    ' konnten nicht gefunden werden.');
        ...
        if not lvs_p_base.get_lte(v_transport.lte_id, v_lte) then
            raise_isi_error(20,
                            lc.ec_p1(lc.o_tp1_lte_id_fehlt,
                                     nvl(v_transport.lte_id, '(null)')));
      ...
      --                    ' konnten nicht gefunden werden.');
        ...
        if v_t_result != 0 then
            raise_isi_error(30,
                            c.decode_function_fehler(v_t_result));
        end if;

    -- Reservierung auf der Transp-LTE zurücknehmen
        v_res_result := lvs_ausl.lvs_lte_res_rueck(in_sid, in_firma_nr, -2, -- vorgang_id (-2 = Konstante für Umlgaerungsreservierung)
         -2, -- auf_id
         v_lte.lte_id,
                                                   v_lte.order_vorgang_id, v_lte.lgr_platz, c.c_true);

        if v_res_result != 0 then
            raise_isi_error(40,
                            c.decode_function_fehler(v_res_result));
        end if;
      ...
      -- Einzel LHM
        ...
        -- "pseudo" LTE für Kommissionierung
                ...
                if not v_found then
                    raise_isi_error(50,
                                    lc.ec_p1(lc.o_tp1_lte_id_o_reservierung,
        ...
        -- Reservierung auf der Quell-LTE zurücknehmen
                v_res_result := lvs_ausl.lvs_lte_res_rueck(in_sid, in_firma_nr, -2, -- vorgang_id (-2 = Konstante für Umlgaerungsreservierung)
                 -2, -- auf_id
                 v_q_lte.lte_id,
                                                           v_q_lte.order_vorgang_id, v_q_lte.lgr_platz, c.c_true);
        ...
        -- Einzel LHM Daten holen
                if not lvs_p_base.get_lhm_by_lte_id(v_lte.lte_id, null, v_lhm) then
                    raise_isi_error(70,
          ...
          --                     ' konnte keine LHM gefunden werden.');
            ...
            if v_lhm.komm_neu_lhm_name is not null then
                update lvs_lhm t
                set
    ...
    -- Transport freigeben
      ...
      -- Einzelne LHM wird transportiert, evtl. hat sich der LHM Typ geändert
        ...
        -- erstmal mit altem LHM typ von der Quelle abpacken
                lvs_p_lte_lhm.lvs_lhm_umpacken(in_sid, in_firma_nr, in_login_id, null, v_lhm.lhm_id,
                                               v_lte.lte_id);
        ...
        -- Die Volumenwerte von der LHM auf die neue LTE anwenden (da der LTE Typ "-Keine LTE" ist)
                update lvs_lte t
                set
                    t.lte_vol_hoehe = v_lhm.lhm_vol_hoehe,
        ...
        -- Reservierung auf der Quell-LTE zurücknehmen
                ...
                if v_q_lte.lte_akt_lhm = 1 then
          -- letzte LHM wurde abgepackt
                    lvs_p_lte.lvs_korr_te_ausbuchen(in_sid, in_firma_nr, v_q_lte.lte_id, v_q_lte.lte_status, in_sid,
                                                    in_firma_nr, v_q_lte.lgr_ort, v_q_lte.lgr_platz, in_login_id);
                end if;

                if
                    in_eti_druck_status is not null
                    and v_q_lte.lte_akt_lhm > 1
                then
          -- für die Quell LTE Etikett drucken
                    update lvs_lte t
                    set
                        t.lte_eti_druck_status = decode(t.lte_eti_druck_status, null, c.eti_status_soll_drucken, c.eti_status_neu_drucken
                        ),
                        t.res_login_id = in_login_id
                    where
                        t.lte_id = v_q_lte.lte_id;
        ...
        -- neuen Volumendimensionen nicht auf dem alten Lagerplatz angewendet werden
                if v_lhm.komm_neu_lhm_name is not null then
                    v_t_result := lvs_transport.lvs_transp_transport(in_sid, in_firma_nr, in_login_id, in_transp_id, v_lte.lte_id,
                                                                     null, v_lam_bh_vorg_id);
        ...
        -- Der LHM Typ ändert sich
                if not lvs_p_base.get_lhm_cfg(in_sid, v_lhm.komm_neu_lhm_name, v_lhm_cfg) then
                    raise_isi_error(70,
                                    lc.ec_p1(lc.o_tp1_lhm_typ_fehlt, v_lhm.komm_neu_lhm_name));
          -- raise_isi_error(70, 'Die Konfigurationsdaten für LHM Typ ' || v_lhm.komm_neu_lhm_name ||
          --                    ' konnten nicht gefunden werden.');
                end if;

                if not lvs_p_base.get_lam_by_lhm_id(in_sid, in_firma_nr, v_lhm.lhm_id, v_lam) then
                    raise_isi_error(80,
                                    lc.ec_p1(lc.o_tp1_lhm_id_ohne_bestand, v_lhm.lhm_id));
        ...
        -- Auf dem Ziellagerplatz ist bereits die neue Höhe der LTE reserviert worden -> kein Update am Lagerplatz
            ...
            if in_eti_druck_status is not null then
                update lvs_lhm t
                set
                    t.lhm_eti_druck_status = decode(t.lhm_eti_druck_status, null, c.eti_status_soll_drucken, c.eti_status_neu_drucken
                    )
                where
                    t.lhm_id = v_lhm.lhm_id;
      ...
      -- Ganze (echte) LTE wird unverändert transportiert
        ...
        -- LTE bekommt ein neues Etikett
                update lvs_lte t
                set
                    t.lte_eti_druck_status = decode(v_lte.lte_eti_druck_status, null, c.eti_status_soll_drucken, c.eti_status_neu_drucken
                    )
                where
                    t.lte_id = v_lte.lte_id;
        ...
        -- LHMs auf der LTE, die ein Etikett haben, bekommen ein neues Etikett
                update lvs_lhm t
                set
                    t.lhm_eti_druck_status = c.eti_status_neu_drucken
                where
                        t.lte_id = v_lte.lte_id
...
-- sqlcl_snapshot {"hash":"4fb10c2f3b5835cb72d03dc9244faa919180098f","type":"PACKAGE_BODY","name":"LVS_UML","schemaName":"DIRKSPZM32","sxml":""}