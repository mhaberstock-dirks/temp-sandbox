  ...
  CREATE OR REPLACE EDITIONABLE TRIGGER "TR_PZM_KONTEN_BH_BI" 
  before insert on pzm_konten_bh
  for each row
declare
  -- Lokale Variablen
...
begin
  ...
  if v_pzm_konten.buch_einheit != :new.einheit
  then
    raise_application_error(-20000, 'Die Buchungseinheit etnspricht nicht der Buchungseinheit in den Kontoeinstellungen.');
  end if;

  if :new.bus = 1 or :new.bus = 2
  then
    if :new.typ is null
    then
      :new.typ := 'B'; -- normale Buchung
    end if;
  elsif :new.bus = 3 or :new.bus = 4
  then
    if :new.typ is null
    then
      :new.typ := 'S'; -- Stornobuchung
    end if;
    if :new.storno_konten_bh_id is null
    then
      raise_application_error(-20001, 'Es ist keine Buchungsnummer für Stornierung angegeben worden.');
    end if;
  end if;

  if :new.buch_datum is null
  then
    :new.buch_datum := sysdate;
  end if;

  :new.wert := round(:new.wert, 3);
  v_pzm_konten.letzte_buchung := :new.buch_datum;
  :new.wert := nvl(:new.wert, 0);

  if :new.bus = 1 or :new.bus = 4           -- normaler Zugang / Abgang storniert
  then
    v_pzm_konten.saldo := v_pzm_konten.saldo + :new.wert;
  elsif :new.bus = 2 or :new.bus = 3        -- normaler Abgang / Zugang storniert
  then
    v_pzm_konten.saldo := v_pzm_konten.saldo - :new.wert;
  end if;

  select seq_konten_bh_id.nextval into :new.konten_bh_id from dual;

  update pzm_konten t
     set t.saldo = v_pzm_konten.saldo,
...
/
ALTER TRIGGER "TR_PZM_KONTEN_BH_BI" ENABLE;


-- sqlcl_snapshot {"hash":"d7b16dc17ec9e44e0ade50c110230a9ca9070f3c","type":"TRIGGER","name":"TR_PZM_KONTEN_BH_BI","schemaName":"DIRKSPZM32","sxml":""}