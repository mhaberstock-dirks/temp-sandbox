create or replace procedure dirkspzm32.hm_update_pers_ze_tag (
    p_pers_nr  in number,
    p_datum    in date,
    p_result   out number,
...
*/
  ...
  -- Konstanten
    ...
    subtype t_zetag is c_zetag%rowtype;
    c_abwesenheitsmeldungen t_commoncursorref;
    v_abwesenheitsart       pzm_abwesenheitsarten%rowtype;
    v_lohnart               pzm_lohnarten%rowtype;
    v_zetag                 t_zetag;
  -- v_ID               number;
  --   v_Start            date;                                      -- Start-Zeit aus c_ZETag.ze_ist_start (nur Fetch)
  -- v_Ende             date;                                      -- Ende-Zeit aus c_ZETag.ze_ist_ende (nur Fetch)
  -- v_Status           number;                                    -- Status aus c_ZETag.Status (nur in Cursor-Loop)
  --                                                               -- Auswertung auf STATUS_-Konstanten zur Berechnung von 
  --                                                               -- v_DayReisePassivStd, v_DayAbwStd, v_DayAaStatus, v_DayAnwStd,
  --                                                               -- v_DayCalcAnwStart, v_DayCalcAnwEnde, v_DayPauseStempelStd + 
  --                                                               -- v_PauseStd
  -- v_aa_status        number;                                    -- Abwesenheitstatus aus c_ZETag (nur in Cursor-Loop)
  -- v_CalcStart        date;                                      -- Start-Zeit aus c_ZETag.ze_calc_ist_start (nur in Cursor-Loop)
  -- v_CalcEnde         date;                                      -- Ende-Zeit aus c_ZETag.ze_calc_ist_ende (nur in Cursor-Loop)
  -- v_SAKurzname       pzm_zeiterfassung.ze_sa_kurzname%type;     -- Schichtart-Kurzname aus c_ZETag. (nur in Cursor-Loop)
    v_std                   number;                                    -- Zeitdauer aus Cursor c_ZETag. (in und außerhalb Cursor-Loop!)
  -- v_ZETyp            pzm_zeiterfassung.ze_typ%type;             -- Zeit-Einheiten-Typ aus c_ZETag. (nur in Fetch)
  -- v_zeWorkLocation   pzm_zeiterfassung.ze_work_location%type;   -- Worklocation aus c_ZETag. (nur in Cursor-Loop); Abfrage auf Literale!
    v_pausestd              number;
  ...
  -- Schichtdaten (bereits umgerechnet auf den Schichttag)
    v_safound               boolean;
    v_sabeginn              date;                              -- Uhrzeit für den Schichtbeginn (Schichtdatum als Basis); Quelle get_schicht_Daten()
    v_saende                date;                              -- Uhrzeit für das Schichtende (Schichtdatum als Basis); Quelle get_schicht_Daten()
    v_sastdprotag           number;                            -- Gesamtstunden der Schicht (berücksichtigt die durchschn. Arbeitsstd. p. Woche); Quelle get_schicht_Daten();
  ...
  --v_DayAnwEnde              date;
    v_dayanwstd             number;                               -- berechnete Anwesenheitszeit in Stunden
    v_dayarbstd             number;                               -- berechnete Arbeitszeit (Anwesenheit - Pausen)
    v_dayflexstd            number;                               -- berechnete Arbeitszeit für Flex-Stundenkonto
    v_dayuebstd             number;
    v_dayabwstd             number;
    v_daysakurzname         pzm_zeiterfassung.ze_sa_kurzname%type; -- Kurzname der Schichtart (PZM_SCHICHTARTEN, UK);
                                                                     -- ermittelt in pzm_p_schicht_tag.c_schicht_tag_validieren()
    v_dayaastatus           number;                                -- Abwesenheitsstatus
    v_daycalcanwstart       date;                                  -- berechnete Startzeit Anwesenheit
    v_daycalcanwende        date;                                  -- berechnete Endzeit Anwesenheit
    v_daypausestd           number;                                -- kumulierte Pausenzeit
...
begin
  ...
  -- MHa 28.04.2026 --Optimierung für Cursor-Loop auf c_CETag_all 
  -->
    update pzm_ze_tagessatz t
    set
        t.ts_day_kst_id = (
            select
                min(ze_kst_id) keep(dense_rank first order by ze_calc_ist_start, ze_id)
            from
                pzm_zeiterfassung
            where
                    ze_pers_nr = p_pers_nr
                and trunc(ze_schicht_tag) = p_datum
        )
    where
            t.ts_pers_nr = p_pers_nr
        and trunc(t.ts_datum) = p_datum
    returning ts_day_kst_id into v_kst_id;
  -- <Optimierung Ende
  /*
  ...
  */
    ...
    */
    ...
    -- if pzm_p_base.get_abwesenheitsart(v_aa_status, v_abwes_art) is null           -- MWe Add P70460-15
        if pzm_p_base.get_abwesenheitsart(v_zetag.ze_aa_status, v_abwes_art) is null     -- MWe Add P70460-15
         then                                                                             -- MWe Add P70460-15
       --v_aa_status := null;                                                        -- MWe Add P70460-15
            v_zetag.ze_aa_status := null;
        else
            if v_abwes_art.kennz_urlaub = c.c_true then
                v_kenz_urlaub := true;
        -- v_GesamtTagStdUrlaub := v_GesamtTagStdUrlaub + v_Std;
                v_gesamttagstdurlaub := v_gesamttagstdurlaub + v_zetag.ze_std;
            end if;
          ...
          -- v_Std := 0;
          -- v_PauseStd := get_pause_time_day(v_DaySAKurzname, v_CalcStart, v_CalcEnde, v_DayAnwStd, v_DayPauseStd, v_pb_id, v_DayPauseBezStd);
                    v_dayanwstd := v_dayanwstd + v_zetag.ze_std; -- Anpassung, da ansonsten bspw. Arztbesuchzeit die Arbeitszeit ueberschreibt. (ABa W24120-465)
                    v_zetag.ze_std := 0;
                    v_pausestd := get_pause_time_day(v_daysakurzname, v_zetag.ze_calc_ist_start, v_zetag.ze_calc_ist_ende, v_dayanwstd
                    , v_daypausestd,
    ...
    -- then
        if
            v_zetag.ze_status = status_dienstgang
            and v_zetag.ze_work_location in ( 52, 53 ) -- Reise assiv oder Default
        then
      -- v_DayReisePassivStd := v_DayReisePassivStd + v_Std;
            v_dayreisepassivstd := v_dayreisepassivstd + v_zetag.ze_std;
        end if;
      ...
      --      v_DayCalcEnde := v_CalcEnde;
            v_daycalcstart := v_zetag.ze_calc_ist_start;
            v_daycalcende := v_zetag.ze_calc_ist_ende;

      -- if v_Status = STATUS_ABWESEND
            if v_zetag.ze_status = status_abwesend then
        -- if v_aa_status is not NULL
                if v_zetag.ze_aa_status is not null then -- Abweseheit begruendet?
          -- v_DayAaStatus := v_aa_status;
                    v_dayaastatus := v_zetag.ze_aa_status;
                end if;
        -- v_DayAbwStd := v_Std;
                v_dayabwstd := v_zetag.ze_std;
        
      -- elsif v_status = STATUS_FEIERTAG
            elsif v_zetag.ze_status = status_feiertag then
        --v_DayAaStatus := v_aa_status;
        --v_DayAbwStd := v_Std;
                v_dayaastatus := v_zetag.ze_status;
                v_dayabwstd := v_zetag.ze_std;

      -- elsif v_status = STATUS_ANWESEND
      --       or v_status = STATUS_DIENSTGANG
      --       or v_status = STATUS_PAUSE
            elsif v_zetag.ze_status = status_anwesend
            or v_zetag.ze_status = status_dienstgang
            or v_zetag.ze_status = status_pause then
        -- v_DayAnwStd := v_Std;
        -- v_DayCalcAnwStart := v_CalcStart;
        -- v_DayCalcAnwEnde := v_CalcEnde;
                v_dayanwstd := v_zetag.ze_std;
                v_daycalcanwstart := v_zetag.ze_calc_ist_start;
                v_daycalcanwende := v_zetag.ze_calc_ist_ende;

        -- anwesenheitsdauer basierte pausen berechnen
        -- if v_status = STATUS_PAUSE
                if v_zetag.ze_status = status_pause then
          -- v_DayPauseStempelStd := v_DayPauseStempelStd + v_Std; -- gestempelte pause
                    v_daypausestempelstd := v_daypausestempelstd + v_zetag.ze_std; -- gestempelte pause
                else
          -- v_PauseStd := get_pause_time(v_DaySAKurzname, v_CalcStart, v_CalcEnde, v_pb_id);
                    v_pausestd := get_pause_time(v_daysakurzname, v_zetag.ze_calc_ist_start, v_zetag.ze_calc_ist_ende, v_pb_id);
                    if v_pausestd > 0 then
      ...
      -- if v_Status = STATUS_ABWESEND
        ...
        -- v_DayAbwStd := v_DayAbwStd + v_Std;
                v_dayabwstd := v_dayabwstd + v_zetag.ze_std;
      ...
      --       or v_status = STATUS_PAUSE
        ...
        -- v_DayAnwStd := v_DayAnwStd + v_Std;
                v_dayanwstd := v_dayanwstd + v_zetag.ze_std;
        ...
        -- if v_CalcEnde is not NULL
                if v_zetag.ze_calc_ist_ende is not null then
    ...
    -- Schicht fuer den ganzen Tag ermitteln und keinen offenen Eintrag fuer diesen Tag
      ...
      -- Pausenzeiten unter Beruecksichtigung der gestempelten Pausen berechnen
        ...
        -- wenn keine Anwesenheit, dann auch keine Pause
            ...
            then
        -- Einheitliche Berechnung unabhängig von ganzen oder halben Urlaubstagen!
                v_dayflexstd := v_dayflexstd + v_dayarbstd + v_gesamttagstdurlaub - v_sastdprotag;
                v_dayarbstd := v_dayarbstd - v_dayflexstd;
        
        /* Aenderung M. Haberstock 04.05.2026
         * während Urlaub geleistete Arbeitszeit ins Aufbaukonto; 
         * 
         * 
         ...
         *   
         *   -- Aenderung M.Haberstock, W24120-554 21.04.2026 
         *   v_DayArbStd := 0;
         *   -- Aenderung Ende
         * else
            ...
            -- Abweseneheitsdifferenz bei Gleitzeit hinten dran schreiben
                        ...
                        , v_daycalcende + v_dayabwstddiff / 24,
                            ...
                            if nvl(p_zaehler, 0) <= 1 then
                                hm_update_pers_ze_tag(p_pers_nr,
                                                      v_schicht_datum,
                                                      p_result,
                                                      p_res_info,
                                                      nvl(p_zaehler, 0) + 1);
    ...
    commit;
end hm_update_pers_ze_tag;
/


-- sqlcl_snapshot {"hash":"02a29a88a6ac55ddf5752abebeb414285e323ed0","type":"PROCEDURE","name":"HM_UPDATE_PERS_ZE_TAG","schemaName":"DIRKSPZM32","sxml":""}