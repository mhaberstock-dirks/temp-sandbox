...
begin
  ...
  if inserting
  then
    if :new.zeaw_korr_datum is not null
    then
  ...
  then
    ...
    then
      ...
      if :new.zeaw_korr_datum is NULL OR :new.zeaw_korr_datum = :old.zeaw_korr_datum
      then
    ...
    if :new.zeaw_lz_lohnart <> :old.zeaw_lz_lohnart
    then
      ...
      then
        ...
        if pzm_kontoverwaltung.is_konto_vorhanden(v_sid, v_firma_nr, :old.zeaw_pers_nr,
                                                  v_lohnarten.lz_konto_name_kurz, 'ZK', v_konto)
        then
          ...
          then
            ...
            pzm_kontoverwaltung.zk_zugang_buchen(v_sid, v_firma_nr, v_konto.konto_nr, v_PersNr, get_pers_kst_id(v_PersNr),
                                                 v_wert, 'STORNIERT: Abbuchung nach LOA ' || :old.zeaw_lz_lohnart,
                                                 :old.zeaw_datum, :old.aa_id, get_pers_abt_id(v_PersNr), v_konten_bh_id);
          elsif v_lohnarten.lz_konto_bus = c_kto_bus_zugang
          then
            --LOA stunden wieder vom Konto abbuchen
            pzm_kontoverwaltung.zk_abgang_buchen(v_sid, v_firma_nr, v_konto.konto_nr, v_PersNr, get_pers_kst_id(v_PersNr),
                                                 v_wert, 'STORNIERT: Zubuchung nach LOA ' || :old.zeaw_lz_lohnart,
                                                 :old.zeaw_datum, :old.aa_id, get_pers_abt_id(v_PersNr), v_konten_bh_id);
          end if;
        end if;
      end if;
      ...
      then
        ...
        if pzm_kontoverwaltung.is_konto_vorhanden(v_sid, v_firma_nr, :new.zeaw_pers_nr,
                                                  v_lohnarten.lz_konto_name_kurz, 'ZK', v_konto)
        then
          v_wert := :new.zeaw_lz_loa_std;
          if v_konto.buch_einheit = 'DD'
  ...
  then
    ...
    then
      ...
      then
        ...
        if v_lohnarten.lz_konto_bus = c_kto_bus_abgang
        then
          select max(x.konten_bh_id) into v_k_bh_id
            from pzm_konten_bh x 
           where x.konto_nr = v_konto.konto_nr
             and x.zk_start = :old.zeaw_datum
             and x.bus = c_kto_bus_abgang
             and x.wert = v_wert
             and x.typ = 'B'
             and x.info = 'Abbuchung nach LOA ' || :old.zeaw_lz_lohnart;
          if v_k_bh_id is not NULL
          then
            delete pzm_konten_bh x
             where x.konten_bh_id = v_k_bh_id;  -- Jetzt den passenden Zatz löschen
          else
            --LOA stunden wieder auf das Konto buchen
            pzm_kontoverwaltung.zk_zugang_buchen(v_sid, v_firma_nr, v_konto.konto_nr, v_PersNr, get_pers_kst_id(v_PersNr),
                                                 v_wert, 'STORNIERT: Abbuchung nach LOA ' || :old.zeaw_lz_lohnart,
        ...
        then
          ...
          then
            delete pzm_konten_bh x             -- Jetzt den passenden Zatz löschen
             where x.konten_bh_id = v_k_bh_id;
          else
            --LOA stunden wieder vom Konto abbuchen
            pzm_kontoverwaltung.zk_abgang_buchen(v_sid, v_firma_nr, v_konto.konto_nr, v_PersNr, get_pers_kst_id(v_PersNr),
                                                 v_wert, 'STORNIERT: Zubuchung nach LOA ' || :old.zeaw_lz_lohnart,
