...
package body DIRKSPZM32.lvs_p_lgr_grp_fahrzeuge is
  ...
  function check_fahrzeug(in_fahrzeug         in lvs_fahrzeuge%rowtype,
  ...
  begin
    ...
    if in_fahrzeug.max_trans_lte = 0
    then
    ...
    then
      ...
      then
        ...
        or v_fahrzeug_max_trans_lte <  in_fahrzeug.akt_trans_lte + 1
        then
      ...
      if v_fahrzeug_max_trans_lte <  in_fahrzeug.akt_trans_lte + 1
  ...
  -------------------------------------------------------------------------*/
  function chk_lgr_leit_zugriff_ok(in_lgr              in   lvs_lgr%rowtype,
                                   in_check_endgueltig in   boolean,
                                   in_fah_text         in   varchar2
                                  )
                                   return varchar2 is
    ...
    CURSOR c_fahrzeug is
      select *
        from lvs_fahrzeuge fah
       where nvl(fah.stapler_ls_id, nvl(v_ort.stapler_ls_id, -1)) = nvl(v_ort.stapler_ls_id, -1)
          or exists (select * from lvs_fahrzeuge_ls_id x where x.res_id = fah.res_id and x.stapler_ls_id = nvl(v_ort.stapler_ls_id, -1));

    CURSOR c_ort is
      select *
        from lvs_lgr_ort ort
       where ort.sid = in_lgr.sid
    ...
    CURSOR c_fahrzeug_ls_id is
      select ls.stapler_ls_id
        from lvs_fahrzeuge_ls_id ls
       where ls.res_id = v_fahrzeug.res_id and ls.stapler_ls_id = nvl(v_ort.stapler_ls_id, -1);
  ...
  begin
    ...
    if  in_lgr.lgr_gruppe_id is NULL
  ...
  -- Call the function
    ...
    then
      v_fah_text := lvs_p_lgr_grp_fahrzeuge.chk_lgr_grp_zugriff_ok(v_lgr,
                                                                   v_lte,
                                                                   v_fahrzeuge_ids);
    end if;
  ...
  function chk_lgr_grp_zugriff_ok(in_lgr            in   lvs_lgr%rowtype,
    ...
    CURSOR c_fahrzeug is
      select *
        from lvs_fahrzeuge fah
       where fah.res_id = v_lgr_grp_fahrzeug.res_id;

    CURSOR c_lgr_grp_fahrzeug is
      select *
        from lvs_lgr_grp_fahrzeug grf
       where grf.lgr_gruppe_id = in_lgr.lgr_gruppe_id
         and grf.lgr_ort = in_lgr.lgr_ort
         and nvl(in_fahrzeuge_ids,';' || grf.res_id || ';')  like ('%;' || grf.res_id || ';%');

  begin
    -- Erst mal kein Fehler
    v_fah_text := nvl (v_g_fah_text, LC.ec_p1(LC.O_TP1_LGR_PLATZ_KEIN_FAHRZ_ERF, in_lgr.lgr_platz));
    ...
    or  (in_fahrzeuge_IDs is not NULL           -- Liste der Fahrzeuge ist mitgegeben
     and in_lte.lgr_platz is not NULL)          -- und Transport für diese Palette existiert schon, dann endgültig Prüfen
    then
  ...
  CURSOR c_lgr_gruppe_haupt_grp is
    select grp.lgr_gruppe_haupt_grp
      from lvs_lgr_grp grp
     where grp.lgr_ort = v_lgr_ort
       and grp.lgr_gruppe_id = v_lgr_gruppe_id;

  CURSOR c_lgr_haupt_grp_lte_res is
    select nvl(sum(lgr.lgr_akt_te), 0) + nvl(sum(lgr.lgr_dispo_einl_te), 0) lgr_res
      from lvs_lgr lgr
     where lgr.sid = in_sid
       and lgr.firma_nr = in_firma_nr
       and lgr.lgr_ort = in_lgr_ort
       and lgr.res_string = in_res_string
       and lgr.lgr_gruppe_id in (select grp.lgr_gruppe_id
                                   from lvs_lgr_grp grp
                                  where grp.lgr_ort = in_lgr_ort
  ...
  CURSOR c_lvs_lgr_haupt_grp is
    select sum(grp.lgr_gruppe_max_lte)
      from lvs_lgr_grp grp
     where grp.lgr_ort = in_lgr_ort
       and grp.lgr_gruppe_haupt_grp = v_gruppe_haupt_grp
     group by grp.lgr_gruppe_haupt_grp;
  ...
  begin
    update lvs_fahrzeuge f
       set f.fahrzeug_ok = in_status_OK,
           f.anz_test_lte = nvl(in_anz_test_lte, 0),
           f.fahrzeug_ausl_ok = in_ausl_status_OK
     where f.res_id = in_res_id;
     commit;
  end c_set_fahrzeug_status;


  procedure lgr_grp_kanal_kontrolle(in_sid                 in isi_sid.sid%type,
                                    in_firma_nr            in isi_firma.firma_nr%type,
                                    in_lgr_ort             in lvs_lgr_ort.lgr_ort%type,
                                    in_lgr_gruppe          in lvs_lgr.gruppe%type)  is
  ...
  begin
    ...
    LOOP
      ...
      CLOSE c_lte;
      -- Call the procedure
      lvs_lager_opt.lvs_kanal_kontrolle(v_lte,
                                        v_lgr);
  ...
  procedure c_set_lgr_grp_to_platz_grp(in_sid                 in isi_sid.sid%type,
                                       in_firma_nr            in isi_firma.firma_nr%type,
                                       in_lgr_ort             in lvs_lgr_ort.lgr_ort%type,
                                       ...
                                       in_anz_plaetze_gruppe  in number)  is
    ...
    CURSOR c_lgr_gruppe_rest is
      select *
       ...
       order by t.lgr_dim_t;

    CURSOR c_lte is
      select *
        from lvs_lte t
       where t.lgr_platz = v_lgr.lgr_platz;
  begin
    ...
    if v_anz_i_first + v_anz_i > in_anz_plaetze
    ...
    if  v_anz_i_first + in_anz_plaetze > in_anz_plaetze_gruppe - v_anz_i_first
    and v_anz_i > 0
    then
      v_anz_i := v_anz_i + 1;
    end if;

    if v_anz_i > v_anz_i_geteilt
    then
      v_anz_i := v_anz_i_geteilt;
    end if;

    if (v_anz_i_first >= in_anz_plaetze
    ...
    then
      v_err_nr   := 2;
      v_err_text := LC.ec_p1(LC.O_TP1_RES_ERROR, 'set > ' || to_char(v_anz_i_first)
                                          || ' or set < ' || to_char(in_anz_plaetze_gruppe - v_anz_i_first)
                                          || ' or set = ' || to_char(0)
                                          || ' or set = ' || to_char(in_anz_plaetze_gruppe)
                                          );
      raise v_error;
    end if;

    update lvs_lgr t
       set t.lgr_gruppe_id = in_lgr_gruppe_rest
       where t.sid = in_sid
         and t.firma_nr = in_firma_nr
         and t.lgr_ort = in_lgr_ort
         and t.lgr_dim_g = in_lgr_dim_g
    ...
    LOOP
      ...
      EXIT when c_lgr_gruppe%NOTFOUND or v_dim_t >= in_anz_plaetze;

      if v_lgr_platz_grp_set is NULL
      then
      ...
      update lvs_lgr t
         set t.lgr_gruppe_id = in_lgr_gruppe_set,
    ...
    LOOP
      ...
      then
        ...
        then
          ...
          update lvs_lgr t
             set t.lte_namen = v_lgr.lte_namen
           where t.sid = in_sid
             and t.firma_nr = in_firma_nr
             and t.lgr_platz = v_lgr.lgr_platz;
        else
    ...
    LOOP
      ...
      update lvs_lgr t
         set t.lgr_platz_gruppe = v_lgr_platz_grp_set,
  ...
  exception
    ...
    when v_error then  -- Update 2011 show Exception Source Line
      -- rollback;
      if c_lgr_gruppe%ISOPEN
      then
        CLOSE c_lgr_gruppe;
      end if;
      if c_lgr_gruppe_rest%ISOPEN
      then
        CLOSE c_lgr_gruppe_rest;
      end if;
      v_err_text := v_err_text  || CHR(13) || CHR(10) || DBMS_UTILITY.format_error_backtrace;
  ...
  procedure c_set_lgr_grp_to_fahrzeug(in_sid                 in isi_sid.sid%type,
    ...
    CURSOR c_lgr_gruppe is
      select t.gruppe, min(t.lgr_platz), max(t.lgr_platz), count(t.lgr_platz)
        from lvs_lgr t
       where t.sid = in_sid
         and t.firma_nr = in_firma_nr
         and t.lgr_ort = in_lgr_ort
  ...
  begin
    OPEN c_lgr_gruppe;
    LOOP
      FETCH c_lgr_gruppe into v_lgr_gruppe, v_lgr_platz_min, v_lgr_platz_max, v_anz_plaetze_gruppe;
      EXIT when c_lgr_gruppe%NOTFOUND;

      -- dbms_output.put_line(v_lgr_gruppe || ' ' || v_lgr_platz_min || ' ' || v_lgr_platz_max);
      c_set_lgr_grp_to_platz_grp(in_sid,
                                 in_firma_nr,
                                 in_lgr_ort,
                                 in_lgr_gruppe_set,
                                 in_lgr_gruppe_rest,
       ...
       lgr_grp_Kanal_kontrolle(in_sid,
                               in_firma_nr,
                               in_lgr_ort,
                               v_lgr_gruppe);
  ...
  CURSOR c_fahrz_def_status is
    select *
      from lvs_fahrz_defekt_dispo_st t
     where t.sid = in_sid
       and t.firma_nr = in_firma_nr
  ...
  CURSOR c_transport is
    select t.*
     ...
     where t.res_id = v_lvs_fahrz_defekt_dispo_st.res_id
       and t.transp_typ = 'A'
    ...
    LOOP
      ...
      begin
        update lvs_fahrz_defekt_dispo_st t
           set t.status = 'B'
         where t.sid = v_lvs_fahrz_defekt_dispo_st.sid
        ...
        LOOP
          ...
          begin
            v_ret := lvs_p_lte.lvs_neue_lte_res_nio_transp (v_transport.transp_id,
                                                            -1,
                                                            v_transport.lte_id);
            if v_ret is not NULL
            then
              if v_ret != LC.ec('O_TP1_NO_ACTION_TAKEN')
              then
          ...
          exception
            when others then v_status := 'R'; -- Im Fehler konnte die LTE nicht getauscht werden EGAL mit der nächsten weiter
                                              -- Und merken, das eine Rest da ist, der nicht verteilte werden komnnte
          end;
        end LOOP;
        CLOSE c_transport;
    ...
    LOOP
      ...
      begin
        ...
        LOOP
          ...
          begin
            v_ret := lvs_p_lte.lvs_neue_lte_res_nio_dispo (-1,
                                                           v_lte_id);
            if v_ret is not NULL
            then
              if v_ret != LC.ec('O_TP1_NO_ACTION_TAKEN')
          ...
          exception
            when others then v_status := 'B'; -- Im Fehler konnte die LTE nicht getauscht werden EGAL mit der nächsten weiter
                                              -- Und merken, das eine Rest da ist, der nicht verteilte werden komnnte
