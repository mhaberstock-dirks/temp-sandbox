...
procedure DIRKSPZM32.z_dir_pzm_delete_pdler_Werksvertrag is
  ...
  cursor c_personal is
    select * from pzm_personal t
      where t.pers_nr >= 80000
        and t.pers_nr <= 89999
      order by t.pers_nr;
...
begin
  ...
  loop
    ...
    select max(t.ze_bde_day_ist_ende) into v_del_date
      from  pzm_ze_bde_zeiten t
     where t.ze_bde_pers_nr = v_personal.pers_nr
       and t.ze_bde_day_ist_ende <= trunc(sysdate)
       and t.ze_bde_verbucht_status = 'UE';
    if v_del_date is NULL
    then
      select max(t.ze_bde_day_ist_ende) into v_del_date
        from  pzm_ze_bde_zeiten t
       where t.ze_bde_pers_nr = v_personal.pers_nr;
    ...
    then
      ...
      delete pzm_zeiterfassung t
       where t.ze_pers_nr = v_personal.pers_nr
      ...
      update pzm_ze_loa_ausw t
         set t.aa_id = NULL
       where t.zeaw_pers_nr = v_personal.pers_nr
      ...
      delete pzm_ze_tagessatz t
       where t.ts_pers_nr = v_personal.pers_nr
