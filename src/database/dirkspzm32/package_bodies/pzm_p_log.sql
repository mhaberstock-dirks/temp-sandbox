...
package body DIRKSPZM32.PZM_P_LOG is
  ...
  procedure set_log_level(p_level in number) is
  begin
  ...
  begin
    ...
    if p_level >= g_table_log_level then
      write_to_table(
        p_level, p_message, p_category, p_module, p_error_code, p_stacktrace,
        p_pers_nr, p_ze_rfid, p_ze_id, p_schicht_tag, p_terminal_id, p_aktion, p_quelle
      );
    end if;

    -- In Session-Buffer schreiben (Level-Filterung erfolgt in global_p_session_log)
    write_to_buffer(
      p_level, p_message, p_category, p_module, p_error_code,
      p_pers_nr, p_ze_rfid, p_ze_id, p_schicht_tag, p_terminal_id, p_aktion, p_quelle
    );
  end;
  ...
  begin
    ...
    -- Log Start
    info(
      'Cleanup gestartet: retention_days=' || p_retention_days || 
      ', cutoff=' || to_char(v_cutoff_date, 'DD.MM.YYYY HH24:MI:SS') ||
    ...
    loop
      ...
      if v_iterations > v_max_iterations then
        p_result_message := 'ABBRUCH: Max. Iterationen erreicht (' || v_max_iterations || ')';
        exit;
      end if;
      ...
      if v_elapsed_seconds >= p_max_runtime_sec then
        p_result_message := 'TIMEOUT: Max. Laufzeit erreicht (' || p_max_runtime_sec || 's)';
        exit;
      end if;
    ...
    info(
      'Cleanup beendet: ' || v_total_deleted || ' Eintraege geloescht' ||
      ', Dauer=' || round(v_elapsed_seconds, 1) || 's' ||
      ', Iterationen=' || v_iterations ||
      ', Status=' || p_result_message,
      CAT_SYSTEM,
      'cleanup_old_logs'
    );
  ...
  exception
    when others then
      ...
      error(
        'Cleanup fehlgeschlagen: ' || sqlerrm,
        CAT_SYSTEM,
        'cleanup_old_logs'
      );
  end;
