create or replace editionable trigger dirkspzm32.tr_isi_scanner_cfg_buid before
    insert or update or delete on dirkspzm32.isi_scanner_cfg
    for each row
declare
...
begin
    ...
    if inserting then
        if :new.com_name is not null then
            ...
            set
                cs.com_geraet_name = :new.scanner_name,
                cs.com_geraet_typ = 'SCANNER',
                cs.com_computername = nvl(:new.computername,
                                          cs.com_computername)
            where
                    cs.sid = :new.sid
        ...
        if not v_found then
            insert into isi_scanner_funk_cfg values ( :new.sid,                   -- SID                       VARCHAR2(2) not null,
            ...
            set
                t.scanner_funk_delimiter = decode(:new.scanner_typ,
                                                  'DRAGON',
                                                  '<EOT>',
                                                  null),
                ...
                t.scanner_funk_enabled = decode(:new.scanner_typ,
                                                'DRAGON',
                                                t.scanner_funk_enabled,
                                                :new.scanner_enabled)
            where
                    t.sid = :new.sid                   -- SID                       VARCHAR2(2) not null,
        ...
        if not v_found then
            ...
            update isi_scanner_funk_cfg t
            set
                t.scanner_funk_delimiter = decode(t.scanner_funk_delimiter,
                                                  null,
                                                  decode(:new.scanner_typ,
                                                         'DRAGON',
                                                         '<EOT>',
                                                         null),
                                                  null),
                t.scanner_funk_prae = nvl(:new.scanner_prae,
                                          t.scanner_funk_prae),
                t.scanner_funk_post = nvl(:new.scanner_post,
                                          t.scanner_funk_post),
                t.scanner_funk_typ = nvl(:new.scanner_typ,
                                         t.scanner_funk_typ),
                t.com_name = nvl(:new.com_name,
                                 t.com_name),
                t.appli_modul = decode(:old.appli_modul,
                                       null,
                                       nvl(:new.appli_modul,
                                           t.appli_modul),
                                       :new.appli_modul),
                t.appli_service = nvl(:new.appli_service,
                                      t.appli_service),
                t.scanner_funk_enabled = decode(:new.scanner_typ,
                                                'DRAGON',
                                                t.scanner_funk_enabled,
        ...
        if :new.ls_login_id is null then
            isi_message_board.c_send_raw_message('scanner_abmelden',
                                                 :old.ls_login_id);
      --dbms_alert.signal('scanner_abmelden', :old.ls_login_id);
        else
            isi_message_board.c_send_raw_message('scanner_anmelden',
                                                 :new.ls_login_id);
      --dbms_alert.signal('scanner_anmelden', :new.ls_login_id);
        end if;

    else
        if :old.scanner_typ != 'DRAGON' then
            delete isi_scanner_funk_cfg t
            where
                    t.sid = :old.sid                   -- SID                       VARCHAR2(2) not null,
...
-- sqlcl_snapshot {"hash":"c8dcd546489e68bba80e881c20a10b170b27ef3f","type":"TRIGGER","name":"TR_ISI_SCANNER_CFG_BUID","schemaName":"DIRKSPZM32","sxml":""}