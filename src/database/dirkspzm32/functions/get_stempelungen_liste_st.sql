create or replace function dirkspzm32.get_stempelungen_liste_st (
    p_pers_nr       in number,
    p_datum         in date,
    ...
    where
        ...
        and ( xze.ze_aa_status is null
              or (
            select
                p.pers_kennz_zeiterf
            from
                pzm_personal p
            where
                p.pers_nr = p_pers_nr
...
-- sqlcl_snapshot {"hash":"90f40648f6130d0330aab6edb01d377b52320cd4","type":"FUNCTION","name":"GET_STEMPELUNGEN_LISTE_ST","schemaName":"DIRKSPZM32","sxml":""}